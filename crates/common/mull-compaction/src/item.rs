...
//! harness implements [`CompactionItem`] for its `MullTurn`;
//! mull-build implements it for `mull_sampling_types::ConversationItem`.
//!
...
//! `<mull_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `MullRole` (Mull chat) and the
/// `ConversationItem` variants (mull-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Mull chat) — maps to System on harnesses without a
...
/// extraction (`<mull_file id=".." name=".." />` lines in the
/// `<mull_user_queries>` preamble).
...
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Mull chat's
    /// `MullTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Mull chat: a
    ...
    /// strips their `<mull_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<mull_file>` lines in the
    /// `<mull_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Mull chat: a `Developer` turn with category
    ...
    /// requests/results (Mull chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up mull-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Mull chat's
/// `MullTurn`; adding these constructors to it as required methods would break
/// that impl. They are also mull-build-specific (Mull chat's tail-keep path
...
///
/// The mull-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Mull chat stores turns as `Arc<MullTurn>`).
