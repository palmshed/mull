...
//! stay in each product host (for example `mull-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `mull-sampling-types`. It is decoupled from both Mull chat and
//! mull-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — mull-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Mull chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Mull chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Mull chat and
//! mull-build; hosts still own snapshotting and host-only sections).
...
// mull-build's full-replace subsystem now lives under `code_compaction`;
