...
// The `Shared` summarizer reuses mull-build's full-replace summarization core
...
/// `FullReplace` strategy (default): mull-build's full-replace — summarize the
...
/// `<mull_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching mull-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through mull-build's shared core.
    ...
    //     Shared with Mull chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<mull_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<mull_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<mull_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): mull-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<mull_user_queries>` preamble so
...
/// — mull-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and mull-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // mull-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // mull-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Mull chat's
...
/// the literal error messages produced by the Mull chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Mull chat sampler when the
    ...
    /// — guards the forwarding that Mull chat (`Arc<MullTurn>`) relies on.
    #[test]
