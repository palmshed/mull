...
///
/// - `FullReplace` (default): mull-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from mull-shell's
...
/// time (token threshold only, matching mull-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as mull-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<mull_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "openrouter/free";
