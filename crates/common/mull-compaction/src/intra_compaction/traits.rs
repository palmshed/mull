...
pub enum CompactionTarget {
    ...
    /// with a single summary — mull-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by Mull chat's
/// `StreamProcessor` (`Item = Arc<MullTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<MullTurn>`) already satisfies it.
