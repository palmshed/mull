//! Observability seam for the full-replace (mull-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (mull-build:
...
//!
//! Emission points are part of the behavior contract: the mull-build observer
