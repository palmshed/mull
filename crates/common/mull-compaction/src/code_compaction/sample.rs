...
//!
//! The canonical `sample → classify → retry` loop, used by **both** mull-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Mull chat's intra `Shared` summarizer
...
//! the winning summary (mull-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the mull-build host uses to step down its input size).
