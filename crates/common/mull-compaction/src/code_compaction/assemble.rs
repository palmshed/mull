//! Compacted-history assembly (mull-build's rebuild structure, generic).
//!
//! Moved from `mull-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! mull-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // mull-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn mull_build_order_recent_before_summary() {
