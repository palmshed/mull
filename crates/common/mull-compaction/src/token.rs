...
//!
//! - Mull chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `MullTurn::get_num_tokens`.
//! - mull-build estimates with `bytes / 4`.
//!
...
//! (Mull chat: `ItemTokenCounter<Arc<MullTurn>>`), so exactly one mechanism
//! is in play.
