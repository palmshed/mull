...
//!
//! Wire-level [`mull_tool_protocol::ToolErrorWire`] variants and JSON-RPC
...
use mull_tool_protocol::{IdError, JsonRpcError, ToolCallId, ToolErrorWire};
use thiserror::Error;
use url::Url;
...
pub enum ClientError {
    ...
    /// [`mull_tool_runtime::ToolCallContext::default`], which does so)
...
mod tests {
    ...
    fn sdk_reexported_recognizer_matches_decoded_error() {
        ...
        let err = mull_computer_hub_core::error_from_envelope(workspace_gone_envelope());
    ...
    fn sdk_reexported_recognizer_rejects_unrelated_custom_error() {
        ...
        let err = mull_computer_hub_core::error_from_envelope(env);
        assert!(!crate::is_workspace_unavailable(&err));
    }
}
