//! Leader-mode (`mull agent --leader stdio`) test harness.
//!
...
use mull_acp_lib::LineBufferedRead;
use tokio_util::compat::{TokioAsyncReadCompatExt, TokioAsyncWriteCompatExt};

use crate::env::mull_binary;
...
/// (version-skew) test. Falls back to [`mull_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "MULL_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`mull_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "MULL_BINARY_CLIENT";
...
/// (`MULL_BINARY_LEADER`, else the shared [`mull_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`MULL_BINARY_CLIENT`,
/// else the shared [`mull_binary`] resolution).
...
impl acp::Client for LeaderAcpClient {
    ...
    async fn ext_notification(&self, args: acp::ExtNotification) -> acp::Result<()> {
        if &*args.method == "palmshed.ai/leader_reconnected" {
            self.capture
                .reconnected_count
                .fetch_add(1, Ordering::SeqCst);
        }
...
/// A `mull agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&mull_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export MULL_* vars
            // (e.g. MULL_LEADER_SOCKET pointing at a REAL leader on this
            // machine). env_clear + explicit allowlist guarantees the test
            // can never touch a leader outside its sandbox home.
            .env_clear()
            .env("PATH", std::env::var("PATH").unwrap_or_default())
            .env("HOME", home)
            .env("MULL_HOME", home.join(".mull"))
            ...
            .env("MULL_LEADER_SOCKET", home.join(".mull").join("leader.sock"))
            .env("MULL_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("MULL_PALMSHED_API_BASE_URL", server.url())
            .env("PALMSHED_API_KEY", "test-key-for-ci")
            .env("MULL_TELEMETRY_ENABLED", "false")
            .env("MULL_FEEDBACK_ENABLED", "false")
            .env("MULL_TRACE_UPLOAD", "false")
            .env("MULL_INSTRUMENTATION", "disabled")
            // Inherited by the spawned leader, whose stderr goes to
            // ~/.mull/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "mull_shell=debug");
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init
            .auth_methods
            .iter()
            .find(|m| &*m.id().0 == "api_key")
            .expect("api_key auth method");
        self.conn
            .authenticate(
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".mull").join("leader.lock")
}
...
///
/// The `palmshed.ai/leader_reconnected` ext notification is dropped by the typed
/// `ClientSideConnection` (bare `palmshed.ai/*` methods are rejected by the ACP
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".mull").join("leader.log")).unwrap_or_default()
}
