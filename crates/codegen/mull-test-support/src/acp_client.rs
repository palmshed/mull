//! ACP stdio clients for testing mull sessions end-to-end: the typed
//! [`MullStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
/// kill-list so a test can still set e.g. `MULL_DEBUG_LOG=1` explicitly.
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init_resp
            ...
            .unwrap_or_else(|| {
                ...
                panic!(
                    "expected auth method 'api_key' but got: {ids:?}\n\
...
/// Drives `mull agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`MullStdioClient`] (`ClientSideConnection`,
