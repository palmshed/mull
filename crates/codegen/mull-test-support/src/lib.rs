...
//! Shared test utilities for mull-build crates: mock inference server, SSE
...
//! - [`MullStdioClient`] — ACP client that drives `mull agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `mull agent --leader stdio` (unix)
//! - [`run_headless`] — Run `mull -p` against the mock server and capture output
//! - [`git_workdir`] — Create a temp directory with git repo (forces libgit2 init)
//! - [`mull_binary`] — Resolve the mull binary path (MULL_BINARY env or cargo_bin)
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{MullStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, git_workdir, mull_binary};
pub use headless::{
