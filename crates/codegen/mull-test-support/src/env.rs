...
fn local_mull_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("mull{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_mull_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = Command::new(&cargo)
        ...
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build mull: {e}"));

    assert!(
        output.status.success(),
        "failed to build mull for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
...
/// Resolve mull binary: `MULL_BINARY` env (CI) or a locally built `mull` binary.
pub fn mull_binary() -> PathBuf {
    if let Ok(path) = std::env::var("MULL_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "MULL_BINARY does not exist: {}", p.display());
        return p;
    }
    ...
    let binary = local_mull_binary_path();
    ensure_local_mull_binary(&binary);
    binary
}
...
/// Point mull at the mock server with a fake API key and telemetry disabled.
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        // HOME alone does not sandbox mull on Windows: the product resolves
        // `~` via `USERPROFILE`/Known Folders (`std::env::home_dir()`), so
        // without an explicit MULL_HOME every spawned child shares the real
        // `%USERPROFILE%.mull` — test 1's models_cache.json (which embeds
        ...
        .env("MULL_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("MULL_PALMSHED_API_BASE_URL", mock_url)
        ...
        .env("MULL_DISABLE_AUTOUPDATER", "1");
}
