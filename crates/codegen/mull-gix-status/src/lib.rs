...
const ENV_THREADS: &str = "MULL_GIX_STATUS_THREADS";
...
/// Production budget (`n >= 1`). Honours `MULL_GIX_STATUS_THREADS=N` for `N >= 1`
...
mod tests {
    use super::*;
    use mull_test_utils::git::run_git;
...
mod nproc_tests {
    ...
    const CHILD_ENV: &str = "PALMSHED_GIX_STATUS_NPROC_CHILD";
    const REPO_ENV: &str = "PALMSHED_GIX_STATUS_NPROC_REPO";
