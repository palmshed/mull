//! Memory-system configuration value types, extracted from mull-shell
...
//! `mull-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.mull/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.mull/memory/` are scanned:
