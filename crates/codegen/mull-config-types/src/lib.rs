...
pub use permission::*;
mod pool;
use mull_announcements::RemoteAnnouncement;
pub use pool::*;
...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-mull-doom-loop-check` header and parse the reported
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `mull_build_settings`
    ...
    /// OAuth2 provider issuer URL (e.g., "https://auth.palmshed.ai"). When present
    ...
    /// default (on). Sits below env `MULL_FOLDER_TRUST`, user
    ...
    /// remote settings `mull_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `mull_build_settings` remote settings key) to enable it. See
    ...
    /// `mull_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `MULL_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `mull_build_settings`).
    ...
    /// remote settings gate for first-run auto-registration of the official Palmshed
    ...
    /// ghost text), from the `mull_build_settings` remote settings flag. Sits below
    /// env (`MULL_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `mull-0.1` default. The effective model is catalog-guarded: when
    ...
    /// (e.g. `mull-imagine-image`) instead of the default quality model
    /// (`mull-imagine-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// Overridable locally via `MULL_VOICE_MODE`. Free-tier SuperMull upsell
    ...
    /// User's subscription tier from remote settings `mull_build_access_gate`.
    /// E.g. "free", "premium", "supermull", "supermull_heavy".
    ...
    /// Whether the user is allowed to use Mull. Set by remote settings
    /// `mull_build_access_gate` targeting rules. `None` = no server response
    ...
    /// (e.g. "SuperMull", "X Premium+", "Free", "API Key"). Set by CCP
    ...
    /// Server-controlled via the remote settings `mull_build_usage_redirect_url`
    ...
    /// `mull_build_settings`. Per-model override on `ModelInfo`
    /// (`mull_build_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`MULL_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`MULL_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`MULL_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`MULL_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `mull workspace` CLI command (Computer Hub
    /// workspace exposure), from `mull_build_settings.workspace_command_enabled`.
...
/// only guaranteed to work with a compatible harness (cursor vs mull-build).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoalRoleModel {
    /// Model id, e.g. "openrouter/free". Resolved against available models at
    /// spawn time; unknown/unauthorized ⇒ fail-open to current model.
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "mull-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-mull-build toolset), applied REGARDLESS of the
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "mull"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("mull"));
    ...
    fn remote_settings_prompt_suggestion_model_round_trip() {
        let json = r#"{"prompt_suggestion_model": "mull-0.1"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.prompt_suggestion_model.as_deref(), Some("mull-0.1"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
