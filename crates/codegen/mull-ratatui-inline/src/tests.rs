...
mod links {
    ...
    fn emits_osc8_around_linked_cells() {
        let mut t = term(20, 3);
        let out = frame(&mut t, "AB", &[span(0, 2, "https://palmshed.ai", None)]);
        assert!(
            out.contains("\x1b]8;;https://palmshed.ai\x07"),
            "missing open: {out:?}"
        );
    ...
    fn link_removed_next_frame_rewrites_cells_without_osc8() {
        let mut t = term(20, 3);
        let _ = frame(&mut t, "AB", &[span(0, 2, "https://palmshed.ai", None)]);
    ...
    fn emit_id_param_included() {
        let mut t = term(20, 3);
        let out = frame(&mut t, "AB", &[span(0, 2, "https://palmshed.ai", Some(7))]);
        assert!(
            out.contains("\x1b]8;id=7;https://palmshed.ai\x07"),
    ...
    fn wide_char_under_link_wraps_lead_cell_only() {
        ...
        // the OSC 8 wraps it.
        let out = frame(&mut t, "世", &[span(0, 2, "https://palmshed.ai", None)]);
        assert!(
            out.contains("\x1b]8;;https://palmshed.ai\x07世\x1b]8;;\x07"),
            "wide-char run: {out:?}"
        );
    }
    ...
    fn nonzero_origin_viewport_maps_links() {
        ...
        t.set_frame_links(&[LinkSpan {
            ...
            col_start: 2,
            col_end: 4,
            url: "https://palmshed.ai".into(),
            id: None,
        }]);
        t.flush_with_links().unwrap();
        let out = String::from_utf8(t.backend().buf.clone()).unwrap();
        assert!(
            out.contains("\x1b]8;;https://palmshed.ai\x07AB\x1b]8;;\x07"),
