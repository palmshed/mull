...
# are the single source of truth for the `palmshed.ai/announcements/update` payload.
# Pipeline:
...
DESKTOP_DIR="$REPO_ROOT/frontend/apps/mull-desktop"
...
for f in "${bindings[@]}"; do
  {
    ...
    echo "// Regenerate via generate.sh in the mull-announcements package."
    cat "$f"
  } >"$DST/$(basename "$f")"
done
