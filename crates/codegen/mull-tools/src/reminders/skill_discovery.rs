...
//! after every tool call to check for SKILL.md files in `.mull/skills/`,
...
/// Directories that contain skill definitions (`.mull/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".mull", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.mull/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.mull/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.mull/) are outside the git root so the walk breaks early.
