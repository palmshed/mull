...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"MullBuild:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"MullBuild:run_terminal_cmd"`).
...
/// between namespaces (e.g. `MullBuild:run_terminal_cmd` vs.
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("MullBuild:run_terminal_cmd", "legacy-0.4.10"),
            ("MullBuild:read_file", "legacy-0.4.10"),
            ("MullBuild:search_replace", "legacy-0.4.10"),
            ("MullBuild:get_task_output", "legacy-0.4.10"),
            ("MullBuild:kill_task", "legacy-0.4.10"),
            ("MullBuild:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("MullBuild:run_terminal_cmd", "current"),
            ("MullBuild:read_file", "current"),
            ("MullBuild:search_replace", "current"),
            ("MullBuild:list_dir", "current"),
            ("MullBuild:grep", "current"),
            ("MullBuild:kill_task", "current"),
            ("MullBuild:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "MullBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "MullBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "MullBuild:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn unmanaged_tool_returns_none() {
        let v = resolve_version("current", "MullBuild:web_search", None).unwrap();
        assert_eq!(v, None);
    }
    ...
    fn override_on_unmanaged_tool_errors() {
        let err =
            resolve_version("current", "MullBuild:web_search", Some("legacy-0.4.10")).unwrap_err();
        assert!(err.contains("unmanaged tool"));
    }

    #[test]
    fn unknown_preset_errors() {
        let err = resolve_version("nonexistent", "MullBuild:run_terminal_cmd", None).unwrap_err();
    ...
    fn unknown_override_version_errors() {
        let err =
            resolve_version("current", "MullBuild:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn concise_namespace_not_managed() {
        // MullBuildConcise tools should NOT be version-managed.
        assert!(!is_version_managed("MullBuildConcise:run_terminal_cmd"));
        let v = resolve_version("current", "MullBuildConcise:run_terminal_cmd", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("MullBuild:run_terminal_cmd"));
        assert!(is_version_managed("MullBuild:read_file"));
        assert!(is_version_managed("MullBuild:search_replace"));
        assert!(is_version_managed("MullBuild:list_dir"));
        assert!(is_version_managed("MullBuild:grep"));
        assert!(is_version_managed("MullBuild:kill_task"));
        // Not managed:
        assert!(!is_version_managed("MullBuild:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res =
            resolve_version_with_warnings("current", "MullBuild:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "MullBuild:run_terminal_cmd", None)
                .unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let (version, warnings) =
            validate_and_resolve("MullBuild:run_terminal_cmd", "current").unwrap();
        ...
        let err = validate_and_resolve("MullBuild:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unmanaged_returns_none_no_warnings() {
        let res = resolve_version_with_warnings("current", "MullBuild:web_search", None).unwrap();
    ...
    fn resolve_with_warnings_unsupported_version_errors_with_list() {
        let err = resolve_version_with_warnings("current", "MullBuild:grep", Some("legacy-0.4.10"))
            .unwrap_err();
        assert!(
            err.contains("is not supported for tool"),
