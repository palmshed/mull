...
///
/// Serialized over the wire in the `palmshed.ai/task/kill` ext response
/// (`mull-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (mull-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in mull-tools, spawns processes)
/// - `AcpTerminalBackend` (in mull-shell, calls ACP protocol)
#[async_trait::async_trait]
