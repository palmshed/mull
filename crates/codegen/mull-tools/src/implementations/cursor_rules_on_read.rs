...
mod tests {
    ...
    use crate::implementations::mull_build::read_file::{
        ReadFileInput, ReadFileParams, ReadFileTool as MullReadFileTool,
    };
    ...
    fn resources_with_mull_rules_on_read(root: &Path) -> SharedResources {
        let mut resources = Resources::new();
        resources.insert(Cwd(root.to_path_buf()));
        resources.insert(FileSystem(Arc::new(LocalFs)));
        resources.insert(NotificationHandle(ToolNotificationHandle::noop()));
    ...
    #[tokio::test]
    async fn mull_read_file_output_includes_matching_rule_once() {
        let temp = TempDir::new().unwrap();
        let root = temp.path();
        ...
        let shared = resources_with_mull_rules_on_read(root);
        ...
        let first = mull_tool_runtime::Tool::run(&MullReadFileTool, test_ctx(shared), input)
            .await
            .unwrap();
