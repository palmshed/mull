...
impl mull_tool_runtime::Tool for WebSearchTool {
    type Args = WebSearchInput;
    type Output = WebSearchOutput;

    fn id(&self) -> mull_tool_protocol::ToolId {
        mull_tool_protocol::ToolId::new("web_search").expect("valid tool id")
    }
    ...
    ) -> Result<WebSearchOutput, mull_tool_runtime::ToolError> {
        ...
        let (content, citations) = client
            ...
            .map_err(|e| {
                mull_tool_runtime::ToolError::execution(
                    mull_tool_protocol::ToolId::new("web_search").expect("valid"),
                    e.to_string(),
                )
            })?;
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(mull_tool_runtime::Tool::id(&tool).as_str(), "web_search");
        assert!(
