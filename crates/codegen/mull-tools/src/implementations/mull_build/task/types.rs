...
//! (in `mull-tools`) and the subagent coordinator (in `mull-shell`).
//!
//! ## Resource types
//!
...
use educe::Educe;
use mull_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"mull-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`mull_tool_types::is_not_sentinel`] for existing call sites.
pub use mull_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `mull-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "MullBuild:run_terminal_cmd" | "MullBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `mull-shell` relies on
...
register_resource!("mull_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-mull-build
...
pub type MonitorEventBuffer = mull_interjection_core::EventQueue<MonitorEventNotification>;

register_resource!("mull_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// mull-shell produces it, the channel protocol carries it, and the
/// compaction pipeline in mull-chat-state (via `RunningSubagentSummary`)
...
register_resource!("mull_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("mull_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("mull_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by mull-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by mull-shell at turn start.
...
register_resource!("mull_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("MullBuild:run_terminal_cmd", ToolKind::Execute),
                tc("MullBuild:read_file", ToolKind::Read),
                tc("MullBuild:list_dir", ToolKind::List),
                tc("MullBuild:grep", ToolKind::Search),
                tc("MullBuild:kill_task", ToolKind::KillTaskAction),
                tc("MullBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("MullBuild:kill_task", ToolKind::KillTaskAction),
                tc("MullBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("MullBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("MullBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
