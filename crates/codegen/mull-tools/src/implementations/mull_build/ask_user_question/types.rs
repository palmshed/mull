...
//!
//! - **`mull-tools`** — tool blocks on a oneshot, formats the result.
//! - **`mull-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`mull-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `mull-tools`.
...
register_resource!("mull_build", "UserQuestionSender", UserQuestionSender);
