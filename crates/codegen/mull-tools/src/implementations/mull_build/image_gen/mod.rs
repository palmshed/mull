//! `image_gen` tool — generates images via the Palmshed Imagine API and saves
...
const PALMSHED_IMAGINE_MODEL: &str = "mull-imagine-image-quality";
// Some Imagine models (e.g. `mull-imagine-image`, selectable via `model_override`)
...
/// SuperMull upsell modal instead; this covers the natural-language path.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a SuperMull feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperMull: https://palmshed.ai/supermull?referrer=mull-build. Do not retry this tool.";

/// HTTP client for Palmshed Imagine API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
pub struct ImageGenClient {
    ...
    /// [`PALMSHED_IMAGINE_MODEL`]). `image_edit` uses its own model and is
    /// unaffected.
    model: String,
    ...
    /// HTTP call and return the SuperMull upsell prose instead. See
...
impl ImageGenClient {
    pub fn new(
        ...
        let model = model_override
            ...
            .unwrap_or_else(|| PALMSHED_IMAGINE_MODEL.to_owned());
    ...
    /// with the SuperMull upsell instead of issuing a doomed request.
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// ([`PALMSHED_IMAGINE_MODEL`]). Driven by the remote
        ...
        /// SuperMull upsell prose instead of a doomed request. Set by the
...
impl ImageGenConfig {
    ...
    /// default quality model ([`PALMSHED_IMAGINE_MODEL`]) is used.
...
impl mull_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> mull_tool_protocol::ToolId {
        mull_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(mull_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
        assert!(
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("mull-imagine-image".into()),
            tier_restricted: false,
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("mull-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("mull-imagine-image")), None)
                .unwrap()
                .model,
            "mull-imagine-image"
        );
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        // A free / X Basic user's image_gen call returns the SuperMull upsell
        ...
        match result {
            ToolOutput::Text(t) => {
                assert!(t.text.contains("SuperMull"), "got: {}", t.text);
                assert!(t.text.contains("supermull?referrer=mull-build"));
            }
