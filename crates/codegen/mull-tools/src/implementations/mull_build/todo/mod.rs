...
crate::register_resource!("mull_build", "Todo", TodoState);
...
impl mull_tool_runtime::Tool for TodoWriteTool {
    type Args = TodoWriteInput;
    type Output = TodoWriteOutput;

    fn id(&self) -> mull_tool_protocol::ToolId {
        mull_tool_protocol::ToolId::new("todo_write").expect("valid tool id")
    }
    ...
    ) -> Result<TodoWriteOutput, mull_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn name_and_description() {
        ...
        assert_eq!(mull_tool_runtime::Tool::id(&tool).as_str(), "todo_write");
    ...
    async fn replace_mode_creates_items() {
        ...
        let output = expect_success(
            mull_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
                .await
                .unwrap(),
        );
    ...
    async fn replace_clears_previous_state() {
        ...
        mull_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input1)
            .await
            .unwrap();
        ...
        let output = expect_success(
            mull_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input2)
                .await
                .unwrap(),
        );
    ...
    async fn state_serialization_roundtrip() {
        ...
        assert!(
            state_map.get("mull_build.Todo").is_some(),
            "TodoState should serialize under 'mull_build.Todo'"
        );
