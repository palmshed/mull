...
//! `LspTool` (Tool trait impl) under the `MullBuild` namespace.
...
impl mull_tool_runtime::ToolOutput for LspToolOutput {}
...
impl mull_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> mull_tool_protocol::ToolId {
        mull_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, mull_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    mull_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.mull/lsp.json or <cwd>/.mull/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
