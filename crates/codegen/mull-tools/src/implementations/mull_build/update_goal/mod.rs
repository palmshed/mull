...
pub use mull_tools_api::slash_commands::UPDATE_GOAL_TOOL_NAME;
...
impl mull_tool_runtime::ToolOutput for UpdateGoalOutput {}
...
impl mull_tool_runtime::Tool for UpdateGoalTool {
    type Args = UpdateGoalInput;
    type Output = UpdateGoalOutput;

    fn id(&self) -> mull_tool_protocol::ToolId {
        mull_tool_protocol::ToolId::new(UPDATE_GOAL_TOOL_NAME).expect("valid tool id")
    }
    ...
    ) -> Result<UpdateGoalOutput, mull_tool_runtime::ToolError> {
...
pub fn render_ack_into_output(
    ...
    match ack {
        ...
        UpdateGoalAck::Rejected { reason, detail } => Err(mull_tool_runtime::ToolError::custom(
            reason.error_code(),
            detail,
        )),
    }
