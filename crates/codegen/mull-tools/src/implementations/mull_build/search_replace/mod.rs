...
register_resource!("mull_build", "SearchReplace", SearchReplaceParams);
...
) -> Result<SearchReplaceOutput, mull_tool_runtime::ToolError> {
            ...
            let output = match e.io_error_kind() {
                ...
                _ => {
                    return Err(mull_tool_runtime::ToolError::execution(
                        mull_tool_protocol::ToolId::new("search_replace").expect("valid"),
                        e.to_string(),
                    ));
                }
            };
...
impl mull_tool_runtime::Tool for SearchReplaceTool {
    type Args = SearchReplaceInput;
    type Output = SearchReplaceOutput;
    fn id(&self) -> mull_tool_protocol::ToolId {
        mull_tool_protocol::ToolId::new("search_replace").expect("valid tool id")
    }
    fn description(
        &self,
...
mod tests {
    ...
    async fn consecutive_edits_succeed_without_prior_read() {
        ...
        let result1 = mull_tool_runtime::Tool::run(
            &tool,
        ...
        let result2 = mull_tool_runtime::Tool::run(
            &tool,
            test_ctx(shared),
