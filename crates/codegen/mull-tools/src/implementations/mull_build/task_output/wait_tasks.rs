...
use crate::implementations::mull_build::task_output::{
...
use mull_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for WaitTasksTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            mull_tool_types::build_wait_tasks_description(&mull_tool_types::WaitTasksToolNaming {
...
/// [`mull_tool_types::build_wait_tasks_description`] builder so the CLI and
...
impl mull_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> mull_tool_protocol::ToolId {
        mull_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let timeout =
            crate::implementations::mull_build::task_output::capped_wait_timeout(input.timeout_ms);
