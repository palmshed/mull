...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.mull/skills/ or .mull/skills/)
${%- endif %}
</available_skills>"#;
...
impl mull_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> mull_tool_protocol::ToolId {
        mull_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, mull_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(mull_tool_runtime::Tool::id(&tool).as_str(), "skill");
