...
impl mull_tool_runtime::ToolOutput for GlobOutput {}
...
impl mull_tool_runtime::Tool for GlobTool {
    type Args = GlobInput;
    type Output = GlobOutput;

    fn id(&self) -> mull_tool_protocol::ToolId {
        mull_tool_protocol::ToolId::new("glob").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: GlobInput,
    ) -> Result<GlobOutput, mull_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = GlobTool;
        assert_eq!(mull_tool_runtime::Tool::id(&tool).as_str(), "glob");
    ...
    async fn gitignore_respected() {
        ...
        mull_test_utils::git::ensure_hermetic_git_on_path();
