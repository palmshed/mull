...
use crate::implementations::mull_build::search_replace::CONTEXT_LINES;
use crate::implementations::mull_build::search_replace::helpers::{
...
impl mull_tool_runtime::Tool for EditTool {
    type Args = EditInput;
    type Output = SearchReplaceOutput;

    fn id(&self) -> mull_tool_protocol::ToolId {
        mull_tool_protocol::ToolId::new("edit").expect("valid tool id")
    }
...
async fn ensure_parent_dirs(path: &std::path::Path) -> Result<(), mull_tool_runtime::ToolError> {
    if let Some(parent) = path.parent()
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            mull_tool_runtime::ToolError::execution(
                mull_tool_protocol::ToolId::new("edit").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
// are imported from `mull_build::search_replace::helpers` — shared across
// both the mull_build and opencode edit tools.
...
mod tests {
    ...
    fn tool_id_and_kind() {
        ...
        let tool = EditTool;
        assert_eq!(mull_tool_runtime::Tool::id(&tool).as_str(), "edit");
