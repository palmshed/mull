...
///
/// `skills` is the standard layout (`.mull/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Mull sessions.
...
/// name (e.g. `~/.mull/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Mull.
...
/// `cwd` (exclusive). At each directory, checks for `.mull/skills/`,
...
/// visits known config roots (`.mull`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.mull` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".mull", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.mull is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.mull/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.mull/ → kept (user content).
        let mull_shell = tmp.path().join(".mull").join("skills").join("shell");
        std::fs::create_dir_all(&mull_shell).unwrap();
        std::fs::write(
            mull_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (mull_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.mull/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .mull skill in an intermediate dir.
        ...
        .unwrap();
        let mull_skill = sub.join(".mull").join("skills").join("mull-dyn");
        std::fs::create_dir_all(&mull_skill).unwrap();
        std::fs::write(mull_skill.join("SKILL.md"), "---\nname: mull-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"mull-dyn"),
            "mull-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only mull-dyn discovered.
        ...
        assert!(
            names_off.contains(&"mull-dyn"),
            "mull-dyn missing: {names_off:?}"
        );
        assert!(
