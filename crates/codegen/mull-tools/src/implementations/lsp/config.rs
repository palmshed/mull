//! LSP server configuration from `.mull/lsp.json`.
...
/// This is the canonical merge function — both session startup and `mull inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::mull_home::mull_home().join("lsp.json");
    let project_path = cwd.join(".mull").join("lsp.json");
...
/// Load LSP server configs from `~/.mull/lsp.json` and `<cwd>/.mull/lsp.json`.
