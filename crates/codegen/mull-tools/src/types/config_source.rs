...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.mull/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.mull/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.mull/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.mull/ or ~/.claude/.
    User { path: PathBuf },
