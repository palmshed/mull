...
///
/// Use with `#[schemars(with = "MullIntegerSchema")]` on `Option<usize>` fields.
pub struct MullIntegerSchema;
impl schemars::JsonSchema for MullIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "mull_integer_schema".into()
    }
...
/// Lenient boolean deserializers (shared via `mull-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use mull_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
