...
//!
//! `ToolSearchIndex` is defined in `mull-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `mull-shell`
...
/// in `mull-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `mull-shell` after MCP initialization.
#[derive(Clone)]
