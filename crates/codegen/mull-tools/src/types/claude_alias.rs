//! Canonical external-settings tool name ↔ Mull tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`mull-hooks`) needs the
//! Mull tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`mull-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `mull` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to Mull, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Mull [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Mull tool names this Claude tool maps to (empty when there is no direct
    /// Mull tool — the entry then only contributes a `kind`).
    mull: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, mull: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        mull,
...
const fn match_only(claude: &'static str, mull: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        mull,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (mull-build reads SKILL.md)
...
/// The Mull [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Mull tool names a Claude matcher term fires on.
pub fn mull_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.mull)
...
/// The Claude names that map to `mull_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(mull_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.mull.contains(&mull_name))
        .map(|t| t.claude)
}

/// Every distinct Mull name the table references, for the `mull-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn mull_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.mull.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Mull name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.mull.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
