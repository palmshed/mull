...
//!
//! The tool runtime contract (`Tool` trait) lives in `mull_tool_runtime`.
...
///
/// Serializes to snake_case (`mull_build`, `mcp`, …) for the
...
/// (e.g. `"MullBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "MullBuild")]
    MullBuild,
    #[serde(alias = "MullBuildConcise")]
    MullBuildConcise,
    #[serde(alias = "MullBuildHashline")]
    MullBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
