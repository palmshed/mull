...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (mull build harness) ─────────────
...
///
/// `"/path/.mull/skills/my-skill/SKILL.md"` -> `"/path/.mull/skills/"`
...
/// Used by the budgeted (mull build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Mull build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the mull
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // mull-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: mull build harness (budgeted XML) ───────────
