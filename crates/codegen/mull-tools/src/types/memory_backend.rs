...
//!
//! `MemoryBackend` is defined in `mull-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `mull-tools`.
/// Mirrors `mull_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "mull_memory";
