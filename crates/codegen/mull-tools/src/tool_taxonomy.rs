...
//! `ToolInput`, proto, or runtime). A future `mull-tool-taxonomy` leaf crate
...
/// object (mirroring `palmshed.ai/mcp_tool`). Consumers deserialize it into
/// [`CanonicalToolMeta`].
pub const TOOL_META_KEY: &str = "palmshed.ai/tool";
...
impl ToolKind {
    ...
    /// `palmshed.ai/tool`. Exhaustive, so a new `ToolKind` must add a label to compile.
...
///   should read `namespace` loosely (as a string) and, on any `palmshed.ai/tool`
...
impl CanonicalToolMeta {
    ...
    /// (`bash_mode`, `backend`, `palmshed.ai/mcp_tool`, …).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::MullBuild,
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::MullBuild => ("mull_build", "MullBuild"),
                ToolNamespace::MullBuildConcise => ("mull_build_concise", "MullBuildConcise"),
                ToolNamespace::MullBuildHashline => ("mull_build_hashline", "MullBuildHashline"),
