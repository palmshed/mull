use mull_config_types::DisplayRefreshSettings;
...
pub struct UiConfig {
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `MULL_HUNK_TRACKER` override
    ...
    /// Speech-to-text language preference for voice dictation. A Mull STT
    /// catalog code (`en`, `es`, `ja`, … — see Palmshed STT supported languages) or
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            fork_secondary_model: mull_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,
