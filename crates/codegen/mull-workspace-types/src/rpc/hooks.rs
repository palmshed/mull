...
//!
//! `mull_hooks` pulls in `git2`/`reqwest`/`mull-tools`, too heavy for
...
/// Wire mirror of `mull_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `mull_hooks::config::HookSpec`.
///
...
/// `mull-hooks`, so a server-side test (`mull-workspace`'s
...
/// Wire mirror of `mull_hooks::event::HookEventName`.
///
