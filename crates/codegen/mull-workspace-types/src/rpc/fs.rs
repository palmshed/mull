...
//! the shell's `palmshed.ai/fs/*` ACP methods.
...
// workspace server (`mull-workspace`) and the palmshed.ai backend
...
// `workspace.fs_*` ops above already serve the shell's `palmshed.ai/fs/*`
...
/// Filesystem node kind. Wire values match the shell's `palmshed.ai/fs/list`
/// node `type` strings.
...
/// `palmshed.ai/fs/list` plus `offset` pagination).
