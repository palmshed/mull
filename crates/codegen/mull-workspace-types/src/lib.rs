//! Wire types for the `mull-workspace` API.
//!
...
//! `mull-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `mull-workspace-types` (instead of `mull-workspace` or
/// `mull-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `mull_workspace::permission`
