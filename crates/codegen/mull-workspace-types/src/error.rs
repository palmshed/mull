...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] mull_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `mull-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `mull_tools::ToolError`) happens
