...
pub mod tips;
pub mod uname;
pub use mull_shared::clipboard;
pub use mull_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// True for first-party Palmshed endpoints (`*.palmshed.ai`, cli-chat-proxy, and optional
...
/// exempt. Safe against invalid URLs and suffix attacks (`evil-palmshed.ai.example`).
pub fn is_first_party_mull_url(url: &str) -> bool {
    ...
    reqwest::Url::parse(url)
        ...
        .is_some_and(|host| host == "palmshed.ai" || host.ends_with(".palmshed.ai"))
}
...
/// True if `pid` is a Mull (`mull`) process; pairs with [`kill_process_by_pid`]
...
pub fn is_mull_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        ...
        match std::fs::read(&cmdline_path) {
            Ok(data) => {
                ...
                s.contains("mull") || s.contains("mull")
            }
            Err(_) => false,
        }
    }
    #[cfg(windows)]
    {
        ...
        let s = String::from_utf16_lossy(&buf[..size as usize]).to_ascii_lowercase();
        s.contains("mull") || s.contains("mull")
    }
...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_rejects_public_api() {
        assert!(!is_cli_chat_proxy_url("https://api.palmshed.ai/v1"));
    }
    #[test]
    ...
    #[test]
    fn test_is_first_party_mull_url() {
        assert!(is_first_party_mull_url("https://api.palmshed.ai/v1"));
        assert!(is_first_party_mull_url(
            "https://api.palmshed.ai/v1/chat/completions"
        ));
        assert!(is_first_party_mull_url("https://palmshed.ai"));
        assert!(is_first_party_mull_url(
            "https://openrouter.ai/api/v1/chat/completions"
        ));
        assert!(!is_first_party_mull_url("https://api.openai.com/v1"));
        assert!(!is_first_party_mull_url("https://api.anthropic.com/v1"));
        assert!(!is_first_party_mull_url(
            "https://generativelanguage.googleapis.com"
        ));
        assert!(!is_first_party_mull_url(
            "https://api.palmshed.ai.evil.example/v1"
        ));
        assert!(!is_first_party_mull_url(
            "https://evil-palmshed.ai.attacker.com/v1"
        ));
        assert!(!is_first_party_mull_url("https://prefixpalmshed.ai/v1"));
        assert!(!is_first_party_mull_url("not-a-url"));
        assert!(!is_first_party_mull_url(""));
    }
    #[test]
    fn test_truncate() {
    ...
    #[test]
    fn is_mull_process_self_true_impossible_pid_false() {
        assert!(is_mull_process(std::process::id()));
        assert!(!is_mull_process(u32::MAX));
    }
}
