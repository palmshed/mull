...
//! are published per-version to the CDN at `palmshed.ai/cli/changelogs/`.
//!
...
const CHANGELOG_BASE: &str = "https://palmshed.ai/cli/changelogs";
...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$MULL_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `mull_home()` OnceLock). A seeded `$MULL_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("MULL_HOME")
            .map(std::path::PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .unwrap_or_else(crate::util::mull_home::mull_home);
        Self {
    ...
    ///
    /// When `MULL_CHANGELOG_OFFLINE` is set (PTY / integration tests), skip
    ...
    /// `$MULL_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`MULL_HOME` /
    /// `MULL_CHANGELOG_OFFLINE`), which races across the parallel test
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$MULL_HOME` even when offline mode was not
...
fn changelog_offline() -> bool {
    std::env::var_os("MULL_CHANGELOG_OFFLINE").is_some_and(|v| !v.is_empty() && v != "0")
}
...
mod tests {
    ...
    /// `$MULL_HOME` env so tests never race the parallel harness.
    ...
    fn cdn_miss_falls_back_to_env_home_disk_cache() {
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path().join("mull-home-fallback");
        std::fs::create_dir_all(&home).unwrap();
