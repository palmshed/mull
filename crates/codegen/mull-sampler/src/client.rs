//! HTTP client for the OpenRouter sampling APIs.
//!
...
use mull_sampling_types::error::{parse_error_bytes, try_parse_stream_error};
...
pub use mull_sampling_types::ApiBackend;

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "mull-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request headers (reserved for future use). Fields are no longer
/// applied to requests after the OpenRouter migration.
#[allow(dead_code)]
struct MullRequestHeaders<'a> {
    conv_id: &'a str,
    req_id: &'a str,
    session_id: &'a str,
...
impl MullRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        // OpenRouter migration: Palmshed proxy tracking headers stripped.
        // Model routing is handled by the "model" field in the JSON payload.
        // OpenRouter does not consume x-mull-* headers.
        builder
    }
}
...
/// Deserialize a Responses API SSE event, with a fallback for Palmshed-specific
...
/// when the API emits the Palmshed-specific `context_details` field.
    ...
    let mut event = match serde_json::from_str::<rs::ResponseStreamEvent>(data) {
        ...
        Err(first_err) => {
            ...
            if let Ok(mut value) = serde_json::from_str::<serde_json::Value>(data) {
                ...
                // (e.g., Palmshed-specific "x_search"). Instead of maintaining a
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = mull_sampling_types::reported_cost_ticks(
        value
...
pub(crate) const COST_USD_TICKS_METADATA_KEY: &str = "mull.cost_usd_ticks";
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-mull-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-mull-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<mull_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // OpenRouter migration: x-mull-client-version, x-mull-client-identifier,
        // x-mull-deployment-id, x-mull-user-id headers stripped.
        // OpenRouter does not consume these Palmshed proxy headers.
    ...
    fn format_request_headers(
        ...
        req_headers.push(Self::format_header("x-mull-conv-id", x_mull_conv_id));
        req_headers.push(Self::format_header("x-mull-req-id", x_mull_req_id));
        // x-mull-model-override stripped for OpenRouter migration;
        // model is sent in the JSON payload "model" field.
        if include_accept {
    ...
    pub async fn chat_completion(
        ...
        let x_mull_conv_id = &payload.x_mull_conv_id.clone().unwrap_or_default();
        let x_mull_req_id = &payload.x_mull_req_id.clone().unwrap_or_default();
        ...
        let mull_headers = MullRequestHeaders {
            conv_id: x_mull_conv_id,
            req_id: x_mull_req_id,
            session_id: payload.x_mull_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_mull_turn_idx.as_deref(),
            agent_id: payload.x_mull_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_mull_deployment_id.as_deref(),
            user_id: payload.x_mull_user_id.as_deref(),
        };
        let http_request = mull_headers
    ...
    pub async fn chat_completion_stream(
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_mull_conv_id, x_mull_req_id, &model_id, true);
    ...
    pub async fn create_response(
        ...
        let x_mull_conv_id = request.x_mull_conv_id.as_deref().unwrap_or_default();
        let x_mull_req_id = request.x_mull_req_id.as_deref().unwrap_or_default();
        ...
        let mull_headers = MullRequestHeaders {
            conv_id: x_mull_conv_id,
            req_id: x_mull_req_id,
            session_id: request.x_mull_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_mull_turn_idx.as_deref(),
            agent_id: request.x_mull_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_mull_deployment_id.as_deref(),
            user_id: request.x_mull_user_id.as_deref(),
        };
        ...
        mull_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = mull_headers
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_mull_conv_id, x_mull_req_id, &model_id, false);
            let server_message = parse_error_bytes(bytes.as_ref());
    ...
    /// gate that adds the opt-in `x-mull-doom-loop-check` request header, so
    ...
    pub async fn create_response_stream(
        ...
        // Inject Palmshed-specific fields not in async-openai's CreateResponse type.
        if self.defaults.stream_tool_calls {
            request_body["stream_tool_calls"] = serde_json::json!(true);
        }
        // Inject Palmshed-specific tools (e.g., x_search) that can't be expressed
        ...
        let mut http_request = mull_headers
            .apply(self.post(self.endpoint("responses")))
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_mull_conv_id = request.x_mull_conv_id.clone();
        let x_mull_req_id = request.x_mull_req_id.clone();
        let x_mull_session_id = request.x_mull_session_id.clone();
        let x_mull_turn_idx = request.x_mull_turn_idx.clone();
        let x_mull_agent_id = request.x_mull_agent_id.clone();

        // Collect Palmshed-specific tools that can't be expressed via rs::Tool
        // (e.g., x_search). These are injected as raw JSON after serialization.
        let extra_tools = mull_sampling_types::extra_raw_tools(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_mull_conv_id = x_mull_conv_id;
        wrapper.x_mull_req_id = x_mull_req_id;
        wrapper.x_mull_session_id = x_mull_session_id;
        wrapper.x_mull_turn_idx = x_mull_turn_idx;
        wrapper.x_mull_agent_id = x_mull_agent_id;
...
mod tests {
    ...
    use mull_sampling_types::types::ChatRequestMessage;
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            x_mull_conv_id: None,
            x_mull_req_id: None,
            x_mull_session_id: None,
            x_mull_turn_idx: None,
            x_mull_agent_id: None,
            x_mull_deployment_id: None,
            x_mull_user_id: None,
            trace: None,
        };
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-XAI-token-auth".to_string(), "mull-cli".to_string());
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the mull-shell agent product.
        assert!(ua.starts_with("my-client mull-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = mull_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "mull", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
