...
//! it" buckets.
//!
//! `mull-sampler` is intentionally decoupled from `mull-shell`
...
/// tools live in `mull-tools`
/// (`crates/codegen/mull-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `mull_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `mull_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `mull-shell/src/auth/manager.rs` to match, otherwise the
