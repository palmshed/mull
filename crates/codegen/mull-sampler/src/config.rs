...
//! `mull_sampling_types::SamplingConfig` so that the sampler crate
...
/// composing chat-state's `mull_sampling_types::SamplingConfig`
/// with `Credentials` (api key, client version).
///
/// URL-derived request headers are folded into [`Self::extra_headers`] by
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-mull-doom-loop-check` header on
