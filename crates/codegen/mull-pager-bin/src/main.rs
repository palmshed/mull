...
use mull_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use mull_pager::client_identity::PAGER_CLIENT_VERSION;
use mull_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use mull_shell::agent::config::Config as AgentConfig;
use mull_shell::leader::{
...
use mull_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};
use std::env;
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.mull_ws_origin {
        config.mull_com_config.mull_ws_origin = v.clone();
    }
    if let Some(v) = &args.mull_ws_url {
        config.mull_com_config.mull_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(agent_args: &mull_pager::app::AgentArgs, config: &mut AgentConfig) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.mull_api_base_url {
        config.endpoints.mull_api_base_url = v.clone();
    }
}
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Mull agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `mull -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    use mull_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(mull_telemetry::sampling_log::layer())
        .with(mull_telemetry::instrumentation::layer())
        .with(mull_telemetry::hooks_log::layer())
        .with(mull_telemetry::otel_layer::build_otel_layer(
            mull_telemetry::otel_layer::OtelClientInfo {
                client_name: "mull-pager",
                client_version: mull_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            mull_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    mull_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    mull_telemetry::external::init(mull_shell::agent::config::resolve_external_otel_config(
        mull_telemetry::external::config::ExternalClientInfo {
            service_version: env!("VERSION_WITH_COMMIT").to_owned(),
            client_version: mull_version::VERSION.to_owned(),
            app_entrypoint: app_entrypoint.to_owned(),
        },
    ));
}
/// `mull setup`: rendering + exit codes only; fetch logic lives in `mull_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use mull_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `mull login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export MULL_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:MULL_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  mull setup");
        eprintln!();
        eprintln!("Or add the key to ~/.mull/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's Mull administrator."
        );
        std::process::exit(1);
    }
    if json {
        match managed_config::fetch_setup_report().await {
            Ok(report) => {
                ...
                if !report.configured {
                    eprintln!(
                        "Your team doesn't have a managed configuration yet. A team admin can set one up at console.palmshed.ai."
                    );
                }
            }
            Err(e) => {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `mull setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = mull_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !mull_shell::util::is_mull_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not a mull process, removing stale lock");
        ...
        if let Err(e) = mull_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(mull_shell::env::MullBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, mull_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = mull_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = mull_shell::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "mull-pager-leader-cli",
        ClientMode::Stdio,
...
fn leader_info_json(
    ...
    info: Option<&mull_shell::leader::ControlPayload>,
...
/// Env override for the `mull workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "MULL_WORKSPACE_COMMAND";
/// Resolution of the `mull workspace` gate. `Unknown` is kept separate from
...
/// The `MULL_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&mull_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for mull on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<mull_shell::util::config::RemoteSettings> {
    join_early_prefetch(mull_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`mull workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `mull workspace`. Check your \
             network connection (run `mull login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.mull_com_config.mull_ws_url;
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a mull session, or run `mull workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = mull_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use mull_shell::auth::ensure_authenticated;
    mull_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = mull_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`mull workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.mull/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.mull_com_config,
        false,
        Some("No cached credentials found. Run `mull login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.mull_com_config);
...
struct StdioReplayState {
    ...
    /// `palmshed.ai/leader_reconnected` as the primary restored session.
...
fn cache_outgoing_acp_state(msg: &str, state: &std::sync::Mutex<StdioReplayState>) {
    ...
    match method {
        ...
        "palmshed.ai/session/close" | "_palmshed.ai/session/close" => {
...
const REPLAY_LOAD_REQUEST_ID: &str = "palmshed.ai/leader-replay/session-load";
...
/// `palmshed.ai/leader_reconnected` with empty params in that case, signalling the
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    mull_telemetry::sentry::flush_on_shutdown();
    mull_telemetry::otel_layer::shutdown_otel();
    mull_telemetry::debug_log::flush();
    std::process::exit(exit_code);
}
/// Emitted by both leader guards (server mode and leader-connect) so the two sites
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "mull: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<mull_pager::app::AgentArgs>,
    ...
    let _otel_guard = mull_telemetry::otel_layer::otel_guard();
    mull_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => mull_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = mull_shell::agent::models::start_early_prefetch(None);
    mull_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Mull (pager) - v{}",
            mull_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                mull_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(mull_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = mull_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("mull: {warning}");
    }
    agent_config.default_yolo_mode = launch_yolo.yolo;
    agent_config.default_auto_mode = mull_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&mull_shell::agent::config::RuntimeResolutionContext {
    ...
    let managed_install = is_managed_install(
        ...
        &mull_shell::util::mull_home::mull_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = mull_shell::leader::LeaderEnvUrls::from(&agent_config.mull_com_config);
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = mull_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
                            biased;
                            ...
                            maybe_line = stdin_lines.recv() => {
                                ...
                                {
                                    cache_outgoing_acp_state(&trimmed, &replay_state_stdin);
                                }
                                let send_deadline = tokio::time::Instant::now() + std::time::Duration::from_secs(300);
                                loop {
                                    {
                                        let tx = leader_tx_stdin.lock().await;
                                        if tx.send(trimmed.clone()).is_ok() {
                                            break;
                                        }
                                    }
                                    if cancel_stdin.is_cancelled() || tokio::time::Instant::now() >= send_deadline {
                                        tracing::error!(
                                    ...
                                    .await
                                {
                                    Ok((new_tx, mut new_rx, _disconnect_rx)) => {
                                        ...
                                        let notification = format!(
                                            r#"{{"jsonrpc":"2.0","method":"palmshed.ai/leader_reconnected","params":{params}}}"#
                                        );
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = mull_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            mull_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(mull_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = mull_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = mull_shell::managed_config::sync().await {
                                            tracing::warn!(
...
/// Sets `MULL_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `mull dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `mull dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `MULL_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !mull_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.mull/config.toml and \
             unsetting MULL_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("MULL_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<mull_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(mull_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<mull_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(mull_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    mull_shell::heap_profile::install(mull_shell::heap_profile::HeapProfileHooks {
...
fn main() {
    mull_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    mull_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        mull_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        mull_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = mull_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    mull_pager::memory_trace::start(mull_shell::util::mull_home::mull_home().join("memtrace"));
    raise_fd_limit();
    if let Err(e) = mull_config::validate_requirements() {
        eprintln!("Couldn't start Mull: {e}");
        eprintln!();
        eprintln!(
            "Update Mull to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = mull_telemetry::sentry::init(mull_telemetry::sentry::Config {
        client: "mull-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: mull_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    mull_pager::docs::extract_user_guide_docs(&mull_shell::util::mull_home::mull_home());
    mull_crash_handler::install_terminal_restore_only();
    if mull_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = mull_shell::util::mull_home::mull_home().join("crash");
        if let Some(report) = mull_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("Mull crashed during your last session.");
            eprintln!("  Signal:  {}", report.signal_name);
            eprintln!("  Version: {}", report.app_version);
            eprintln!("  Report:  {}", report.report_path.display());
            eprintln!();
        }
        if !mull_crash_handler::install(mull_crash_handler::CrashHandlerConfig {
    ...
    let crashed = mull_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    mull_telemetry::debug_log::flush();
    if let Err(e) = result {
        mull_tty_utils::restore_native_stderr();
        eprintln!("Error: {e:#}");
        drop(_sentry_guard);
        std::process::exit(1);
    }
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("MULL_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("MULL_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(mull_shell::agent::chat_modes::MULL_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(mull_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("MULL_DEBUG_LOG", path);
            std::env::remove_var("MULL_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("MULL_DEBUG_LOG", "1");
        set_if_unset("MULL_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        mull_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return mull_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        mull_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        mull_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    mull_shell::config::apply_sandbox(None, sandbox_profile_arg.as_deref(), args.cwd.as_deref());
    ...
    mull_shell::http::set_client_name(if is_interactive {
        mull_workspace::permission::ClientType::MullPager
    } else {
        mull_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        mull_update::channel_name().unwrap_or("unknown"), }
                    );
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    println!(
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `mull-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                mull_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return mull_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = mull_telemetry::otel_layer::otel_guard();
                return mull_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = mull_telemetry::otel_layer::otel_guard();
                let config = mull_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return mull_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return mull_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return mull_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return mull_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return mull_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return mull_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return mull_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                mull_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                mull_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = mull_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                mull_shell::auth::run_cli_logout(&config)?;
                mull_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = mull_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(mull_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == mull_pager::headless::OutputFormat::Plain {
                args.output_format = mull_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return mull_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            mull_pager::headless::HeadlessOptions {
    ...
    let result = mull_pager::app::run(args, bg_update_rx).await;
    mull_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `mull` to start.");
            } else {
                eprintln!("Update did not complete. Run `mull update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
///
/// Prefers awaiting the parked waiter for the background `mull update` child
...
/// fall back to a fresh blocking `mull update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    let environment = mull_shell::env::MullBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                mull_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("MULL_NPM_REGISTRY"))
        .ok()
        .or_else(mull_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = mull_shell::config::load_effective_config_disk_only()
        && let Some(ch) = mull_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("MULL_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<mull_home>/bin/mull` resolves to, the
...
/// npm shim hardcodes `~/.mull`, so a custom `MULL_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, mull_home: &std::path::Path) -> bool {
    if mull_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = mull_config::mull_application_in(mull_home);
...
/// Handle `mull-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `mull update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in mull_shell::leader::discover_leaders().await {
        if d.classification != mull_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match mull_shell::leader::LeaderClient::connect(
            socket_path,
            "mull-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(mull_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(mull_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            mull_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: mull_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/mull-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(mull_shell::heap_profile::stats().expect("shell stats after install"));
        if !require_opt_prof() {
            assert!(!mull_shell::heap_profile::prof_available());
            return;
        }
        assert!(mull_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(mull_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(mull_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        mull_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_mull_target() {
        let home =
            std::env::temp_dir().join(format!("mull-pager-managed-install-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
        std::fs::create_dir_all(home.join("bin")).unwrap();
        std::fs::create_dir_all(home.join("downloads")).unwrap();
        assert!(!is_managed_install(
            Some(home.join("bin").join("mull")),
            &home
        ));
        ...
        let target = home.join("downloads").join("mull-1.2.3");
        std::fs::write(&target, b"binary").unwrap();
        std::os::unix::fs::symlink(&target, home.join("bin").join("mull")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("mull")),
            &home
        ));
        assert!(is_managed_install(Some(target.clone()), &home));
        let pinned = home.join("bin").join("mull-9.9.9");
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("MULL_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("MULL_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `mull dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(MULL_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["mull", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(MULL_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("MULL_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("MULL_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("MULL_WORKSPACE_COMMAND", "off") };
    ...
    fn cache_session_close_stops_replaying_it() {
        ...
        cache_outgoing_acp_state(
            r#"{"jsonrpc":"2.0","id":3,"method":"_palmshed.ai/session/close","params":{"sessionId":"s1"}}"#,
            &state,
        );
        let s = state.lock().unwrap();
    ...
    async fn replay_waits_for_load_response_through_notifications() {
        ...
        let responder = tokio::spawn(async move {
            ...
            response_tx
                .send(
                    r#"{"jsonrpc":"2.0","method":"palmshed.ai/leader/version_mismatch","params":{}}"#
                        .to_string(),
                )
                .unwrap();
            response_tx
    ...
    /// `palmshed.ai/leader_reconnected` with empty params and the external client
