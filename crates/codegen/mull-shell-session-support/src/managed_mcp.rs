...
async fn get_authenticated_json<T: serde::de::DeserializeOwned>(
    ...
    let resp = match mull_http::shared_client()
        .get(url)
...
impl ManagedMcpState {
    ...
    /// connector re-authorized on palmshed.ai can retry. The reactive path must NOT
...
///
/// Servers with names starting with this prefix are managed by palmshed.ai —
...
///
/// Examples:
///   `mull_com_linear`  → managed by palmshed.ai
///   `mull_com_slack`   → managed by palmshed.ai
///   `my_company_api`   → user-managed (local)
///
/// Single source of truth lives in `mull-workspace` (which matches policy
/// `serverName`s against it); re-exported here so the two never drift.
pub use mull_workspace::permission::resolution::MANAGED_MCP_PREFIX;

/// `"Linear"` -> `"mull_com_linear"`. Shares normalization and the
...
pub fn to_managed_name(display_name: &str) -> String {
    ...
    mull_shell_base::util::truncate(&raw, MANAGED_MCP_NAME_MAX_CHARS).to_string()
}
...
/// 1. Server name starts with `MANAGED_MCP_PREFIX` ("mull_com_")
/// 2. Server URL matches a managed config endpoint
///
/// This prevents false injection if a user accidentally names a server `mull_com_*`
...
/// Inject managed OAuth headers into `mull_com_`-prefixed MCP servers.
///
...
mod tests {
    ...
    fn reauth_first_attempt_allowed_then_backs_off() {
        let mut state = ManagedMcpState::default();
        let now = Utc::now();
        assert!(state.reauth_allowed("mull_com_slack", now));
        assert!(!state.reauth_is_terminal("mull_com_slack"));

        state.record_reauth_failure("mull_com_slack", now);
        // 2^1 = 2s backoff: not eligible now, eligible after the window.
        assert!(!state.reauth_allowed("mull_com_slack", now));
        assert!(state.reauth_allowed("mull_com_slack", now + chrono::Duration::seconds(2)));
        assert!(!state.reauth_is_terminal("mull_com_slack"));
    }
    ...
    fn reauth_goes_terminal_after_max_attempts() {
        ...
        assert!(!state.reauth_allowed("mull_com_slack", now + chrono::Duration::seconds(3600)));
    }
    ...
    fn reauth_success_resets_cooldown() {
        ...
        state.record_reauth_success("mull_com_slack");
        assert!(state.reauth_allowed("mull_com_slack", now));
        assert!(!state.reauth_is_terminal("mull_com_slack"));
    }
    ...
    fn complete_fetch_preserves_cooldown_clear_resets_it() {
        ...
        let now = Utc::now();
        {
            ...
            for _ in 0..MAX_REACTIVE_REAUTH_ATTEMPTS {
                state.record_reauth_failure("mull_com_slack", now);
                state.record_reauth_failure("mull_com_linear", now);
            }
            assert!(state.reauth_is_terminal("mull_com_slack"));
            assert!(state.reauth_is_terminal("mull_com_linear"));
            ...
            assert!(state.reauth_allowed("mull_com_slack", now));
            assert!(state.reauth_allowed("mull_com_linear", now));
            assert!(!state.reauth_is_terminal("mull_com_slack"));
            assert!(!state.reauth_is_terminal("mull_com_linear"));
        }
    }
