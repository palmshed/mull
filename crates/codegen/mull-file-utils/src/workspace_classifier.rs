...
const EXCLUDED_DIR_NAMES: &[&str] = &[
    ".mull", ".cache", ".daemon", ".config", ".npm", ".cargo", ".rustup", ".vscode", ".gemini",
    ".hermes", ".claude",
];
...
fn has_excluded_component(path: &Path) -> bool {
    for component in path.components() {
        if let std::path::Component::Normal(name) = component {
            ...
            if name_lower.starts_with(".mull-") {
                return true;
            }
        }
    }
...
mod tests {
    ...
    mod config_and_cache {
        ...
        #[test]
        fn mull_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".mull")));
                assert!(!is_project_dir(&home.join(".mull/bin")));
            }
        }

        #[test]
        fn mull_prefixed_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".mull-proxy-work")));
            }
        }
