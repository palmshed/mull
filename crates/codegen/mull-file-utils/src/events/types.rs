...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `mull-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `mull-shell::session::events`.
    ...
    /// discriminator constants in `mull-shell::session::events`.
    ...
    /// `mull-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `mull-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
pub enum InterjectionSource {
    /// Direct `palmshed.ai/interject` while a turn was running (Ctrl+Enter).
    Direct,
...
pub enum RedirectKind {
    /// Mid-turn interjection — Ctrl+O / `palmshed.ai/interject`, or "Send now" on a
...
/// Telemetry mirror of `mull-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `mull-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `mull-shell`'s `GoalPauseReason`. The two types
