...
/// `[endpoints].mull_api_base_url` so enterprise proxies need no second knob.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(default)]
pub struct VoiceConfig {
    /// HTTPS API root (or bare host). Bases may end in `/v1`; the
    /// default STT path de-duplicates a leading `v1/` so it becomes `…/v1/stt`.
    pub api_base: String,
...
impl Default for VoiceConfig {
    fn default() -> Self {
        Self {
            api_base: "https://api.palmshed.ai".into(),
...
impl VoiceConfig {
    ...
    /// `api_base`: non-empty `[voice].api_base`, else `[endpoints].mull_api_base_url`
    /// from `root`, else `resolved_endpoints_base`, else `https://api.palmshed.ai`.
    ///
    ...
    pub fn from_config_table(root: &toml::Table, resolved_endpoints_base: Option<&str>) -> Self {
        ...
        // makes "unset" and an explicit `https://api.palmshed.ai` indistinguishable.
        ...
        .or_else(|| {
            non_empty_str(
                root.get("endpoints")
                    .and_then(|e| e.get("mull_api_base_url"))
                    .and_then(|v| v.as_str()),
            )
        })
...
fn ws_url(api_base: &str, path: &str) -> Result<String, VoiceError> {
    ...
    let rest = strip_scheme(base, "https://")
        ...
        .unwrap_or(base);
    // Default path `/v1/stt`; bases often end in `/v1`.
...
mod tests {
    ...
    fn scheme_less_and_wss_bases() {
        for base in [
            "api.palmshed.ai",
            "wss://api.palmshed.ai",
            "HTTPS://api.palmshed.ai",
        ] {
            let cfg = VoiceConfig {
                api_base: base.into(),
                ..VoiceConfig::default()
            };
            assert_eq!(cfg.stt_ws_url().unwrap(), "wss://api.palmshed.ai/v1/stt");
        }
    }
    ...
    #[test]
    fn mull_v1_base_preserves_prefix() {
        let cfg = VoiceConfig {
    ...
    fn inherits_endpoints_when_voice_api_base_unset() {
        let table: toml::Table = toml::from_str(
            r#"
[endpoints]
mull_api_base_url = "https://proxy.example.com/xai/v1"
"#,
        )
        .unwrap();
...
"#,
    ...
    fn table_endpoints_beat_resolved_endpoints_base() {
        let table: toml::Table = toml::from_str(
            r#"
[endpoints]
mull_api_base_url = "https://config.example.com"
"#,
        )
        .unwrap();
...
mull_api_base_url = "https://proxy.example.com/xai/v1"
[voice]
api_base = "https://api.palmshed.ai"
language = "es"
"#,
        )
        .unwrap();
        let cfg = VoiceConfig::from_config_table(&table, None);
        assert_eq!(cfg.api_base, "https://api.palmshed.ai");
