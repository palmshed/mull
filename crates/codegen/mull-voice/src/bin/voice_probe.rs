...
//!
//! ```bash
//! export PALMSHED_API_KEY=...
//! cargo run -p mull-voice --bin voice-probe -- --seconds 5
//! ```
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,mull_voice=debug".into()),
        )
        .init();

    let args = parse_args(std::env::args().skip(1).collect());

    let auth = std::env::var("PALMSHED_API_KEY")
        .ok()
        .and_then(StaticVoiceAuth::shared)
        .ok_or_else(|| {
            anyhow::anyhow!("set PALMSHED_API_KEY (standalone probe has no login session)")
        })?;
    ...
    if args.mic_only {
        #[cfg(feature = "audio")]
        {
            let (bytes, chunks) = mull_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("MULL_PALMSHED_API_BASE_URL").ok();
    ...
    if let Ok(home) = std::env::var("MULL_HOME")
...
Environment:
  PALMSHED_API_KEY     required
  RUST_LOG        optional (default info,mull_voice=debug)

Reads [voice] from ~/.mull/config.toml unless --config PATH is set.
"#
    );
}
