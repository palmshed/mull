...
/// Streaming STT over `wss://api.palmshed.ai/v1/stt`.
...
mod tests {
    ...
    fn optional_header_inserted_when_present_skipped_when_empty() {
        let mut req = "wss://api.palmshed.ai/v1/stt"
            .into_client_request()
            .unwrap();
        insert_optional_header(&mut req, "x-mull-client-identifier", "mull-shell");
        insert_optional_header(&mut req, "User-Agent", "");
        assert_eq!(
            req.headers().get("x-mull-client-identifier").unwrap(),
            "mull-shell"
        );
        assert!(
