...
//!
//! This crate defines the wire format for `palmshed.ai/hooks/*` and `palmshed.ai/plugins/*`
//! ACP extension methods. It is dependency-free (only `serde`) so both
//! `mull-shell` and `mull-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `mull-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `mull-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// Mull's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `mull-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.mull/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}

/// Response for `palmshed.ai/hooks/list`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response for `palmshed.ai/plugins/list`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response for `palmshed.ai/mcp/list` as consumed by the pager.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Request wrapper for `palmshed.ai/hooks/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Request wrapper for `palmshed.ai/plugins/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Shared action response for both `palmshed.ai/hooks/action` and `palmshed.ai/plugins/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.mull/hooks".into(),
        };
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.mull/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.mull/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserMull),
            conflict: None,
        };
...
// ---------------------------------------------------------------------------
// Marketplace types (wire format for palmshed.ai/marketplace/* ACP endpoints)
// ---------------------------------------------------------------------------

/// Response for `palmshed.ai/marketplace/list`.
...
/// Request wrapper for `palmshed.ai/marketplace/action`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
