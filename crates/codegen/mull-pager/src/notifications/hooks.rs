...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("MULL_EVENT", event_str)
        .env("MULL_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("MULL_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'MULL_EVENT=%s\\nMULL_MESSAGE=%s\\nMULL_SESSION_ID=%s\\n' \
             \"$MULL_EVENT\" \"$MULL_MESSAGE\" \"$MULL_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("MULL_EVENT=Turn complete"),
            "missing MULL_EVENT: {content}"
        );
        assert!(
            content.contains("MULL_MESSAGE=hello world"),
            "missing MULL_MESSAGE: {content}"
        );
        assert!(
            content.contains("MULL_SESSION_ID=sess-42"),
            "missing MULL_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("MULL_SESSION_ID"),
            "MULL_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("MULL_EVENT=Turn complete"));
        assert!(content.contains("MULL_MESSAGE=test body payload"));
        assert!(content.contains("MULL_SESSION_ID=test-session-123"));
    }
}
