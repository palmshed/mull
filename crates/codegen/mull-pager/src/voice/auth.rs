...
//!
//! voice-api accepts both API keys and OAuth2 tokens directly at `api.palmshed.ai`
...
use mull_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
///
/// Works for every auth method: OAuth / palmshed.ai / OIDC session tokens and
/// `PALMSHED_API_KEY` / per-model BYOK keys.
pub fn build_voice_auth(auth_manager: Arc<mull_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        mull_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
