//! `mull plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `mull_shell::plugin` (shared orchestration)
//! and lower crates (`mull-agent`, `mull-plugin-marketplace`). This
...
use mull_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use mull_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use mull_plugin_marketplace::SourceKind;
use mull_plugin_marketplace::git::SourceCacheLease;
use mull_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `mull plugin list --json`. The `status` field acts as a
...
/// Typed entry for `mull plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `mull plugin list`).
        name: String,
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  mull plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `mull plugin install --help` to get started.");
    } else {
...
fn installed_plugins(
    repos: &[(&str, &mull_agent::plugins::install_registry::InstalledRepo)],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = mull_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = mull_plugin_marketplace::load_sources(&config);
    sources.extend(mull_plugin_marketplace::load_extra_sources_from_settings(
        &sources,
    ));
    ...
    for source in &sources {
        ...
        for plugin in mull_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                mull_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &mull_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = mull_plugin_marketplace::git::default_cache_root();
            mull_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> mull_telemetry::events::InstallKind {
    if is_git {
        mull_telemetry::events::InstallKind::Git
    } else {
        mull_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: mull_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    mull_telemetry::session_ctx::log_event(mull_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = mull_plugin_marketplace::install_resolve::parse_marketplace_ref(source) {
    ...
    if !trust {
        use mull_agent::plugins::git_install::{self, InstallSource};
    ...
    match plugin::install_plugin(source, &cwd) {
        ...
        Err(e) => {
            ...
            log_plugin_installed(mull_telemetry::events::InstallKind::Git, false, Some(cat));
            bail!("{e}");
        }
    }
}
...
fn cmd_install_marketplace(
    source: &str,
    mref: &mull_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `mull plugin update {}` to update it.",
...
fn cmd_uninstall(name: &str, confirm: bool, keep_data: bool) -> Result<()> {
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            mull_telemetry::session_ctx::log_event(mull_telemetry::events::PluginUninstalled {
                confirmed: true,
                success: true,
            });
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               mull plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `mull plugin list` to see installed plugins."
        );
    }
    if let Err(e) = mull_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    mull_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = mull_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    mull_shell::config::add_disabled_plugin(name)
...
fn cmd_validate(path: &str) -> Result<()> {
    ...
    match load_manifest(&root) {
        ...
        Ok(ManifestLoadResult::NotFound) => {
            println!(
                "No plugin.json found. Mull discovers skills, agents, and hooks \
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `mull plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `mull plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
fn marketplace_add(
    ...
    use mull_shell::plugin::MarketplaceAddInput;
...
fn marketplace_remove(
    ...
    let local_input = match plugin::classify_marketplace_add_input(url, &cwd) {
        mull_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &mull_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match mull_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use mull_plugin_marketplace::MarketplaceSource;

    #[test]
    fn trust_prompt_marketplace_has_no_error_framing() {
        let msg = trust_prompt(
            "\"sentry\" from marketplace \"Palmshed Official\"",
            "sentry@palmshed/plugins",
        );
        assert!(
            msg.starts_with(
                "Installing \"sentry\" from marketplace \"Palmshed Official\" requires confirmation."
            ),
            "{msg}"
        );
        assert!(msg.contains("hooks, MCP servers, and skills"));
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  mull plugin install sentry@palmshed/plugins --trust"
        ));
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(git.ends_with("  mull plugin install u/r --trust"), "{git}");
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir =
            mull_plugin_marketplace::git::sync_source_cache(&url, Some("main"), cache_root.path())
                .unwrap();
    ...
    fn current_head(repo: &Path) -> String {
        let output = mull_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
