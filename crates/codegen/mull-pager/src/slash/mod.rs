...
pub struct SuggestionRow {
    /// Display text (e.g., "/model" or "Mull 4 Fast").
    pub display: String,
...
mod tests {
    ...
    fn parses_invocation_with_args() {
        let inv = parse_invocation("/model mull-code-fast-1").expect("parsed");
        assert_eq!(inv.token, "model");
        assert_eq!(inv.args, "mull-code-fast-1");
    }
    ...
    /// though `get()` blocks execution — invoking one shows the SuperMull
