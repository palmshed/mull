...
use mull_shell::auth::{AuthManager, try_ensure_fresh_auth};
use mull_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // deployments that only configure a deployment_key + custom mull_api_base_url.
    // If the user has previously run the interactive `mull` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.mull_com_config).await;
    ...
    let client = mull_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions =
                mull_shell::session::merge::fetch_merged(Some(&client), cwd.to_str(), None, limit)
                    .await;
            print_sessions_grouped(&sessions);
        }
        SessionsCommand::Search { query, limit } => {
            use mull_shell::session::merge::REMOTE_TIMEOUT;
            use mull_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = mull_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
