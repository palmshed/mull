...
use mull_shell::session::worktree::META_KEY_LABEL;
...
mod tests {
    ...
    fn test_abbreviate_home() {
        if let Ok(home) = std::env::var("HOME") {
            let path = std::path::PathBuf::from(format!("{home}/work/mull"));
            assert_eq!(abbreviate_home(&path), "~/work/mull");
        }
    }
    ...
    fn make_test_record(metadata: Option<serde_json::Value>) -> mull_fast_worktree::WorktreeRecord {
        use mull_fast_worktree::{WorktreeKind, WorktreeRecord, WorktreeStatus};
        WorktreeRecord {
            id: "test".into(),
            path: "/tmp/wt".into(),
