...
use clap::Subcommand;
use mull_fast_worktree::WorktreeRecord;
...
pub async fn run(args: WorktreeArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    let spawned = crate::acp::spawn::spawn_mull_shell(agent_config.clone(), &cancel, None).await?;
...
async fn dispatch(command: WorktreeCommand, tx: &mull_acp_lib::AcpAgentTx) -> Result<()> {
    match command {
...
async fn cmd_show(tx: &mull_acp_lib::AcpAgentTx, id_or_path: &str) -> Result<()> {
...
async fn cmd_db(tx: &mull_acp_lib::AcpAgentTx, command: WorktreeDbCommand) -> Result<()> {
    match command {
        WorktreeDbCommand::Stats => {
            let stats: DbStats = ext_call(tx, "palmshed.ai/git/worktree/db/stats", &()).await?;
        ...
        WorktreeDbCommand::Path => {
            ...
            let resp: PathResp = ext_call(tx, "palmshed.ai/git/worktree/db/path", &()).await?;
            println!("{}", resp.path);
            Ok(())
        }
        WorktreeDbCommand::Rebuild => {
            let report: RebuildReport =
                ext_call(tx, "palmshed.ai/git/worktree/db/rebuild", &()).await?;
...
mod tests {
    ...
    fn ext_request_builds_list_with_filters() {
        ...
        .unwrap();
        assert_eq!(req.method.as_ref(), "palmshed.ai/git/worktree/list");
        let params: serde_json::Value = serde_json::from_str(req.params.get()).unwrap();
        assert_eq!(params["repo"], "mull");
    ...
    fn ext_request_builds_db_stats_empty_params() {
        let req = ext_request("palmshed.ai/git/worktree/db/stats", &()).unwrap();
        assert_eq!(req.method.as_ref(), "palmshed.ai/git/worktree/db/stats");
    }
    ...
    fn ext_envelope_unwraps_success_result() {
        let json = r#"{"result": {"path": "/home/user/.mull/worktrees.db"}, "error": null}"#;
        #[derive(serde::Deserialize)]
        struct PathResp {
            path: String,
        }
        ...
        assert_eq!(inner.path, "/home/user/.mull/worktrees.db");
    }
