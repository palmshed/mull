//! Headless single-turn mode (`mull -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_mull_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use mull_acp_lib::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use mull_shell::agent::auth_method::AuthMethodKind;
use mull_shell::agent::config::Config as AgentConfig;
use mull_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
use mull_shell::sampling::error::{RATE_LIMITED_ERROR_CODE, rate_limited_user_message};
use mull_shell::sampling::types::{
    REASONING_EFFORT_META_KEY, parse_canonical_effort_token, reasoning_effort_meta_value,
};
use mull_shell::util::config as cli_config;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::spawn_mull_shell;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<mull_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use mull_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(json: &str) -> anyhow::Result<Vec<mull_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = mull_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut mull_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    mull_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `mull login` to authenticate \
         (or `mull login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         mull login --device-code\n\n\
         Alternatively, set the PALMSHED_API_KEY environment variable \
         or run `mull login` on a machine with a browser."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive =
                std::io::stdin().is_terminal() && !mull_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &mull_tools::types::compat::CompatConfig::default());
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'mull models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    mull_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = mull_shell::agent::config::AgentMode::Headless;
    ...
    // hub_url lives on `mull workspace` / WorkspaceStartArgs only).
    ...
    agent_config.cli_agent_overrides = mull_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        mull_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_mull_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("MULL_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = mull_shell::active_sessions::register(mull_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(mull_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(mull_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    // Pending background work: bash/monitor via palmshed.ai/task_backgrounded +
    // task_completed; background subagents via SubagentSpawned + SubagentFinished
    // on palmshed.ai/session_notification (prefixed `subagent:{id}` in pending_bg).
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.mull can't hang exit.
        let _ = mull_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = mull_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    ...
    if method == "palmshed.ai/task_completed" {
        #[derive(serde::Deserialize)]
    ...
    if method == "palmshed.ai/monitor_event" {
        return ExtEvent::MonitorEvent;
    }

    match method {
        "palmshed.ai/session_notification" | "palmshed.ai/session/update" => {}
    ...
    let Ok(mull_notif) = serde_json::from_str::<XaiNotif>(notif.request.params.get()) else {
        return ExtEvent::None;
    };

    match mull_notif.update {
...
mod tests {
    ...
    fn reap_request_for_task_kills_with_session_scope() {
        ...
        assert_eq!(request.method.as_ref(), "palmshed.ai/task/kill");
    ...
    fn reap_request_for_subagent_cancels_with_stripped_id() {
        ...
        assert_eq!(request.method.as_ref(), "palmshed.ai/subagent/cancel");
        let params: serde_json::Value = serde_json::from_str(request.params.get()).unwrap();
        assert_eq!(params["subagentId"], "sub-7");
    }

    use super::*;
    use mull_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            mull_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> mull_acp_lib::AcpArgsBox<acp::ExtNotification> {
    ...
    fn headless_task_backgrounded_parses_task_id() {
        ...
        // the inner update object (matching the real `palmshed.ai/task_backgrounded`
        // wire shape: `{ "update": { "sessionUpdate": ..., "task_id": ... } }`).
        let notif = make_ext_notif(
            "palmshed.ai/task_backgrounded",
            serde_json::json!({
    ...
    fn headless_session_update_unknown_method_is_none() {
        ...
        let raw = serde_json::value::to_raw_value(&payload).unwrap();
        let (tx, _rx) = tokio::sync::oneshot::channel();
        let notif = mull_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("palmshed.ai/other", raw.into()),
            response_tx: tx,
        }
        .boxed();
        assert!(matches!(
