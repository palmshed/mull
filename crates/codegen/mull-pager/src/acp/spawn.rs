...
//!
//! Simplified to only support MullShell (in-process) mode.
...
/// Spawn a MullShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_mull_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<mull_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    // skills to ~/.mull/skills/), and model catalog construction.
    let (agent_config, models_manager) =
        mull_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
