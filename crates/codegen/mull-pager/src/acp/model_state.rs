...
impl ModelState {
    ...
    /// correct today (all current Mull models accept images, so nothing is
...
mod tests {
    ...
    fn update_catalog_preserves_user_effort_when_model_unchanged() {
        let id = acp::ModelId::new(Arc::from("mull"));
        let mut state = ModelState::default();
        state
            .available
            .insert(id.clone(), model_with_effort("mull", "Mull", "high"));
        ...
        refreshed.insert(id.clone(), model_with_effort("mull", "Mull", "high"));
