...
use mull_shell::extensions::notification::RetryState;
use mull_shell::extensions::notification::SessionUpdate as MullSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> mull_shell::extensions::notification::ImageCompressedEntry {
    mull_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn interjection_broadcast(
    ...
    acp::ExtNotification::new(
        "palmshed.ai/session/interjection",
        std::sync::Arc::from(
...
pub(super) fn make_exit_plan_ext(
    ...
    mull_acp_lib::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<mull_acp_lib::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn make_exit_plan_ext_with_tool_call_id(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    let request = acp::ExtRequest::new("palmshed.ai/exit_plan_mode", raw.into());
    let (tx, rx) = tokio::sync::oneshot::channel();
    (
        mull_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        },
        rx,
...
pub(super) fn queue_changed_ext(session_id: &str, ids: &[&str]) -> acp::ExtNotification {
    ...
    acp::ExtNotification::new(
        "palmshed.ai/queue/changed",
        std::sync::Arc::from(serde_json::value::to_raw_value(&params).unwrap()),
    )
}
/// Build a `palmshed.ai/queue/changed` notification carrying `runningPromptId`.
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(mull_acp_lib::AcpArgs {
...
pub(super) fn make_inject_notif(payload: &serde_json::Value) -> acp::ExtNotification {
    ...
    acp::ExtNotification::new(
        "palmshed.ai/scheduled_task_inject_prompt",
...
pub(super) fn make_fired_notif(
    ...
    let notif = SessionNotification {
        ...
        update: MullSessionUpdate::ScheduledTaskFired {
            task_id: task_id.into(),
            prompt: prompt.into(),
    ...
    acp::ExtNotification::new("palmshed.ai/scheduled_task_fired", std::sync::Arc::from(raw))
}
...
pub(super) fn announcements_update_notif(
    r#gen: u64,
    announcements: &[mull_announcements::RemoteAnnouncement],
) -> acp::ExtNotification {
    acp::ExtNotification::new(
...
pub(super) fn make_created_ext_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::ScheduledTaskCreated {
            task_id: task_id.into(),
            prompt: prompt.into(),
            human_schedule: human_schedule.into(),
            next_fire_at: next_fire_at.map(str::to_string),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("palmshed.ai/scheduled_task_created", std::sync::Arc::from(raw))
}
pub(super) fn make_deleted_ext_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::ScheduledTaskDeleted {
            task_id: task_id.into(),
        },
        meta: None,
    };
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("palmshed.ai/scheduled_task_deleted", std::sync::Arc::from(raw))
}
...
pub(super) fn mull_model_switch_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::ModelAutoSwitched {
...
pub(super) fn mull_unhandled_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::MemoryFlushStarted,
        meta: Some(serde_json::json!({ "eventId" : event_id })),
    };
    acp::ExtNotification::new(
        "palmshed.ai/session/update",
...
/// Build an `palmshed.ai/session/prompt_complete` ext-notification for `session_id`.
pub(super) fn prompt_complete_ext(session_id: &str) -> acp::ExtNotification {
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("palmshed.ai/session/prompt_complete", std::sync::Arc::from(raw))
}
...
/// Build an `palmshed.ai/session/prompt_complete` ext-notification with an explicit
...
/// Build an `palmshed.ai/session/prompt_complete` ext-notification carrying a
...
/// Build a durable `TurnCompleted` update on the `palmshed.ai/session/update` rail,
...
pub(super) fn mull_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    is_replay: bool,
    ...
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::TurnCompleted {
...
pub(super) fn mull_wake_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
...
/// `palmshed.ai/session/update` rail, optionally stamped `isReplay`.
/// `prompt_id == None` models pre-attribution shells.
pub(super) fn mull_hook_execution_notif_for_prompt(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
) -> acp::ExtNotification {
    use mull_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::HookExecution {
...
pub(super) fn mull_hook_execution_notif(
    session_id: &str,
    event_name: &str,
    is_replay: bool,
) -> acp::ExtNotification {
    mull_hook_execution_notif_for_prompt(session_id, event_name, None, is_replay)
}
...
/// Build an `palmshed.ai/session/interjection` ext-notification (no id).
pub(super) fn interjection_ext(session_id: &str, text: &str) -> acp::ExtNotification {
    interjection_ext_with_id(session_id, text, None)
}
/// Build an `palmshed.ai/session/interjection` ext-notification with an optional
...
pub(super) fn interjection_ext_with_id(
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("palmshed.ai/session/interjection", std::sync::Arc::from(raw))
}
...
/// Build an `ExtNotification` envelope for `palmshed.ai/session_notification`.
pub(super) fn make_ext_session_notification(
    session_id: &str,
    update: MullSessionUpdate,
) -> AcpClientMessage {
...
/// Build an `ExtNotification` envelope with an explicit Palmshed session method.
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(mull_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
pub(super) fn test_subagent_spawned(
    ...
    MullSessionUpdate::SubagentSpawned {
...
pub(super) fn test_subagent_finished(child_sid: &str) -> MullSessionUpdate {
...
pub(super) fn test_subagent_finished_with_wake(
    ...
    MullSessionUpdate::SubagentFinished {
        subagent_id: child_sid.into(),
        child_session_id: child_sid.into(),
...
pub(super) fn test_subagent_progress(
    ...
    MullSessionUpdate::SubagentProgress {
        subagent_id: child_sid.into(),
        parent_session_id: parent_sid.into(),
        child_session_id: child_sid.into(),
        duration_ms: 100,
...
/// Shared temp `MULL_HOME` for disk-replay tests. `mull_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `MULL_HOME`
...
pub(super) fn replay_disk_test_home() -> &'static std::path::Path {
    ...
    HOME.get_or_init(|| {
            ...
            unsafe {
                std::env::set_var("MULL_HOME", tmp.path());
            }
            tmp
        })
        .path()
}
/// Runs `f` with a thread-local mull home override so disk replay tests do not
/// depend on process-wide `mull_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_mull_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_mull_home_for_tests(None);
    out
}
pub(super) fn write_child_updates_jsonl(
    mull_home: &std::path::Path,
    child_sid: &str,
    content: &str,
) {
    let sessions_dir = mull_home
        .join("sessions")
...
pub(super) fn dispatch_goal_update(
    ...
    handle(
        AcpClientMessage::ExtNotification(mull_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("palmshed.ai/session_notification", raw.into()),
            response_tx: tx,
        }),
        app,
    )
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(mull_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
}
/// Build an `palmshed.ai/session_notification` carrying
...
pub(super) fn interaction_resolved_ext(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::InteractionResolved {
            tool_call_id: tool_call_id.into(),
        },
        meta: None,
    };
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("palmshed.ai/session_notification", std::sync::Arc::from(raw))
}
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = mull_workspace::session::git::GitHeadChanged {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("palmshed.ai/git_head_changed", std::sync::Arc::from(raw))
}
pub(super) fn make_task_backgrounded_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::TaskBackgrounded {
            tool_call_id: tool_call_id.into(),
            task_id: task_id.into(),
            command: command.into(),
            cwd: "/tmp".into(),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("palmshed.ai/task_backgrounded", std::sync::Arc::from(raw))
}
...
pub(super) fn make_replayed_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("palmshed.ai/session/update", std::sync::Arc::from(raw))
}
...
pub(super) fn send_late_bg_detection(app: &mut AppView, tc_id: &str) {
    use serde_json::json;
    use mull_tools::types::output::{BashOutput, ToolOutput};
    let agent = app.agents.get_mut(&AgentId(0)).unwrap();
    let meta = crate::acp::meta::NotificationMeta::default();
    let bash = BashOutput {
...
pub(super) fn task_completed_notif(
    ...
    use mull_tools::types::TaskSnapshot;
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::TaskCompleted {
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("palmshed.ai/task_completed", std::sync::Arc::from(raw))
}
pub(super) fn make_monitor_event_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::MonitorEvent {
            task_id: task_id.into(),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("palmshed.ai/monitor_event", std::sync::Arc::from(raw))
}
...
pub(super) fn make_models_update_notif(
    ...
    acp::ExtNotification::new("palmshed.ai/models/update", std::sync::Arc::from(raw))
}
/// `palmshed.ai/models/update` carrying a single reasoning-capable model whose
...
pub(super) fn model_changed_ext(
    ...
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: MullSessionUpdate::ModelChanged {
...
/// Helper: build an `palmshed.ai/mcp/init_progress` notification.
pub(super) fn make_mcp_init_progress_notif(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("palmshed.ai/mcp/init_progress", std::sync::Arc::from(raw))
}
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: mull_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use mull_shell::extensions::mcp::{
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("palmshed.ai/mcp/server_status", std::sync::Arc::from(raw))
}
...
pub(super) fn make_servers_updated_notif() -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("palmshed.ai/mcp/servers_updated", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = mull_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "mull_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("palmshed.ai/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName" : "mull_com_linear", "tools" : [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("palmshed.ai/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_mcp_initialized_notif(session_id: &str) -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("palmshed.ai/mcp_initialized", std::sync::Arc::from(raw))
}
