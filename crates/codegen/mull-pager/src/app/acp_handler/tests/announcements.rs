#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.mull` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.mull` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.mull` reads).
    #[test]
