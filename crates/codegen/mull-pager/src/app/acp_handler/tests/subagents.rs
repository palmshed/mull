#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Regression: replay from `updates.jsonl` emits `palmshed.ai/session/update` (not
    ...
    fn ext_session_notification_and_update_equivalent_for_subagent_spawned() {
        ...
        let (spawn_notif, finish_notif) =
            run_subagent_lifecycle_via_method("palmshed.ai/session_notification", child_sid);
        let (spawn_update, finish_update) =
            run_subagent_lifecycle_via_method("palmshed.ai/session/update", child_sid);
    ...
    fn ext_session_update_malformed_params_returns_false() {
        ...
        let request = acp::ExtNotification::new("palmshed.ai/session/update", raw.into());
        let msg = AcpClientMessage::ExtNotification(mull_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        });
        ...
        assert!(
            !affected,
            "malformed palmshed.ai/session/update params must not redraw"
        );
        assert!(
    ...
    fn ext_session_notification_for_inactive_agent_updates_its_context_used() {
        // AutoCompactCompleted on the Palmshed ext path resets the context bar
