...
//! and Palmshed session extension notifications (`palmshed.ai/session_notification` and
//! replay-path `palmshed.ai/session/update`).
...
use mull_acp_lib::AcpClientMessage;

use super::actions::Effect;
use mull_shell::extensions::notification::{
    SessionNotification, SessionUpdate as MullSessionUpdate, is_reauthable_failure,
};
use mull_shell::tools::todo::todo_item_from_plan_entry;
use mull_workspace::permission::bash_command_splitting::BashCommandHighlights;
        ...
        AcpClientMessage::SessionNotification(notif) => {
            ...
            let affected = match find_session_match(app, &notif.request.session_id) {
                Some(SessionMatch::Root(id)) => {
                    ...
                    // time and drained FIFO. The Palmshed stream is direct-emitted
                    // and keeps a SEPARATE highwater (see the Palmshed dedup in
...
/// Handle an Palmshed extension notification.
///
/// Dispatches on method string:
/// - `palmshed.ai/session_notification` / `palmshed.ai/session/update` → per-agent session updates
fn handle_ext_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    match notif.method.as_ref() {
        ...
        "palmshed.ai/follow_ups" => handle_follow_ups(notif, app),
        "palmshed.ai/task_backgrounded" => handle_task_backgrounded(notif, app),
        "palmshed.ai/task_completed" => handle_task_completed(notif, app),
        "palmshed.ai/models/update" => handle_models_update(notif, app),
        "palmshed.ai/settings/update" => handle_settings_update(notif, app),
        "palmshed.ai/sessions/changed" => handle_sessions_changed(notif, app),
        "palmshed.ai/queue/changed" => handle_queue_changed(notif, app),
        ...
        "palmshed.ai/session/prompt_complete" => handle_prompt_complete(notif, app),
        "palmshed.ai/session/interjection" => handle_interjection(notif, app),
        "palmshed.ai/monitor_event" => handle_monitor_event(notif, app),
        "palmshed.ai/scheduled_task_created" => handle_scheduled_task_created(notif, app),
        "palmshed.ai/scheduled_task_fired" => handle_scheduled_task_fired(notif, app),
        "palmshed.ai/scheduled_task_deleted" => handle_scheduled_task_deleted(notif, app),
        "palmshed.ai/scheduled_task_inject_prompt" => {
            handle_scheduled_task_inject_prompt(notif, app)
        }
        "palmshed.ai/announcements/update" => handle_announcements_update(notif, app),
        "palmshed.ai/git_head_changed" => handle_git_head_changed(notif, app),
        "palmshed.ai/mcp/init_progress" => handle_mcp_init_progress(notif, app),
        "palmshed.ai/mcp/tools_changed" | "palmshed.ai/mcp_initialized" => {
            handle_mcp_tools_changed(notif, app)
        }
        "palmshed.ai/mcp/server_status" if push_server_status_enabled() => {
            handle_mcp_server_status(notif, app)
        }
        "palmshed.ai/mcp/servers_updated" => handle_mcp_servers_updated(notif, app),
        _ => false,
    }
}

/// Handle `palmshed.ai/session/interjection` — the leader broadcasts this
...
fn handle_interjection(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse palmshed.ai/session/interjection");
        return false;
    };
    ...
    // emits the queue-emptying `palmshed.ai/queue/changed` right after it).
...
fn handle_ext_method(ext: mull_acp_lib::AcpArgs<acp::ExtRequest>, app: &mut AppView) -> bool {
    match ext.request.method.as_ref() {
        "palmshed.ai/ask_user_question" => handle_ask_user_question(ext, app),
        "palmshed.ai/exit_plan_mode" => handle_exit_plan_mode(ext, app),
        unknown => {
