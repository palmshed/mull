use super::*;
use mull_shell::sampling::error::rate_limited_user_message;
...
/// Replay gate shared by the ACP and Palmshed session-update paths. Returns `true`
...
/// Handle `palmshed.ai/session_notification` and replay-path `palmshed.ai/session/update`.
///
...
pub(super) fn handle_session_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    match &session_notif.update {
        MullSessionUpdate::TaskBackgrounded { .. } => {
            return handle_task_backgrounded(notif, app);
        }
        MullSessionUpdate::TaskCompleted { .. } => {
            return handle_task_completed(notif, app);
        }
        MullSessionUpdate::ScheduledTaskCreated { .. } => {
            return handle_scheduled_task_created(notif, app);
        }
        MullSessionUpdate::ScheduledTaskDeleted { .. } => {
    ...
    let matched = match find_session_match(app, &session_notif.session_id) {
        ...
        None => {
            tracing::debug!(
                ...
                "load-race: palmshed.ai/session_notification DROPPED — no agent matches session_id"
            );
            return false;
        }
    };
        ...
        })
    {
        tracing::debug!(
            ...
            last_applied = agent.last_applied_mull_event_seq,
            "palmshed.ai/session update DROPPED by dedup highwater (event_seq <= last_applied)"
        );
        return false;
    }
    ...
    let changed = match session_notif.update {
        ref update @ (MullSessionUpdate::AutoCompactStarted { .. }
        | MullSessionUpdate::AutoCompactCompleted { .. }
        | MullSessionUpdate::AutoCompactFailed { .. }
        | MullSessionUpdate::AutoCompactCancelled { .. }
        | MullSessionUpdate::RetryState(_)
        | MullSessionUpdate::ImageDropped { .. }
        | MullSessionUpdate::MemoryFlushCompleted { .. }
        | MullSessionUpdate::MemoryDreamCompleted { .. }
        | MullSessionUpdate::MemorySessionSaved { .. }) => {
            ...
            if let MullSessionUpdate::AutoCompactCompleted { tokens_after, .. } = update {
                refresh_context_used(agent, *tokens_after);
                agent.todo.update_todos(Vec::new());
            }
            changed
        }
        MullSessionUpdate::ImageCompressed {
            ref images,
            ref message,
        } => apply_image_compressed(agent, images, message),
        MullSessionUpdate::TurnCompleted {
            prompt_id,
            stop_reason,
            agent_result,
            ..
        ...
        MullSessionUpdate::HookAnnotation { message } => {
        ...
        MullSessionUpdate::HookExecution {
            event_name,
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        mull_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        mull_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        mull_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
        ...
        MullSessionUpdate::HooksChanged {
            hooks,
            project_trusted,
            load_errors,
        } => {
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(mull_hooks_plugins_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
        ...
        MullSessionUpdate::PluginsChanged { plugins } => {
            if let Some(ref mut modal) = agent.extensions_modal {
                use crate::views::extensions_modal::TabDataState;
        ...
        MullSessionUpdate::SessionSummaryGenerated { session_summary } => {
            agent.generated_session_title =
                Some(crate::util::decode_html_entities(&session_summary).into_owned());
            true
        }
        MullSessionUpdate::SessionRecap { summary, auto } => {
        ...
        MullSessionUpdate::SessionRecapUnavailable => {
            if meta.is_replay {
                false
        ...
        MullSessionUpdate::ModelAutoSwitched {
            previous_model_id,
            new_model_id,
            reason,
        } => {
        ...
        MullSessionUpdate::ModelChanged {
            model_id,
            reasoning_effort,
        } => {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if mull_shell::agent::chat_modes::process_chat_mode_enabled() {
        ...
        MullSessionUpdate::MemoryFiles { files } => {
        ...
        MullSessionUpdate::InteractionResolved { tool_call_id } => {
    ...
    if let Some(agent) = app.agents.get_mut(&parent_id) {
        if let Some(seq) = meta.event_seq
            && !meta.is_replay
        {
            agent.last_applied_mull_event_seq = Some(seq);
        }
...
/// Handle an Palmshed session notification that targets a child (subagent) session.
///
...
pub(super) fn handle_child_session_notification(
    ...
    match update {
        MullSessionUpdate::AutoCompactStarted { .. }
        | MullSessionUpdate::AutoCompactCompleted { .. }
        | MullSessionUpdate::AutoCompactFailed { .. }
        | MullSessionUpdate::AutoCompactCancelled { .. }
        | MullSessionUpdate::RetryState(_) => {
            let compact_tokens = match &update {
                MullSessionUpdate::AutoCompactCompleted { tokens_after, .. } => Some(*tokens_after),
                _ => None,
            };
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct =
                        Some(mull_token_estimation::usage_percentage_u8(tokens_after, cw));
                }
            }
            changed
        }
        ref update @ (MullSessionUpdate::MemoryFlushCompleted { .. }
        | MullSessionUpdate::MemoryDreamCompleted { .. }
        | MullSessionUpdate::MemorySessionSaved { .. }) => {
            if let Some(child_view) = agent.subagent_views.get_mut(child_sid) {
                apply_session_event(
...
pub(super) fn apply_session_event(
    ...
    match update {
        MullSessionUpdate::AutoCompactStarted { percentage, .. } => {
        ...
        MullSessionUpdate::AutoCompactFailed { error } => {
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[mull_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &mull_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
    ...
    match retry {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::RateLimitHit {
                    model_id: session
                        .models
                        .current
                        .as_ref()
    ...
    if is_credit_limit {
        mull_telemetry::session_ctx::log_event(mull_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use mull_tools::types::SessionMode;
