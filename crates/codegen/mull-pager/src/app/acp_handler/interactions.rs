...
/// Handle `palmshed.ai/ask_user_question` ext-method.
///
...
pub(crate) fn handle_ask_user_question(
    ext: mull_acp_lib::AcpArgs<acp::ExtRequest>,
    app: &mut AppView,
) -> bool {
    ...
    if let Some(mut old_qv) = agent.question_view.take() {
        ...
        if let Some(ref kind) = old_qv.local_kind {
            ...
            let cmd = match kind {
                ...
                LocalQuestionKind::FreeUsageUpsell { .. } => "SuperMull upsell",
...
/// Handle an `palmshed.ai/exit_plan_mode` ext_method request.
///
