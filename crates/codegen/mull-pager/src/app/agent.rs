...
use mull_shell::extensions::notification::GoalClassifierVerdict;
...
pub enum AgentCommand {
    ...
    /// `palmshed.ai/session/fork` request is in flight.
    ForkSession,
}
impl AgentCommand {
...
pub struct AgentSession {
    ...
    /// (`palmshed.ai/prompt_history` scoped via `filter_session_id`). Most-recent-first.
    /// Fetched on session create/load; prompts sent in this session are
    /// additionally front-inserted locally on send.
    pub prompt_history: Vec<String>,
    /// True until the session's startup/load `palmshed.ai/prompt_history` fetch completes.
...
pub struct ChipElement {
    ...
    pub kind: mull_ratatui_textarea::ElementKind,
