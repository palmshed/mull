...
struct AgentLoadOutcome {
    ...
    success: bool,
    /// `palmshed.ai/runningPromptId` from the reload response: the turn another
...
/// [`decide`](mull_workspace::folder_trust::decide) precedence.
///
...
fn seed_trust_state(app: &mut AppView, remote: Option<&mull_shell::util::config::RemoteSettings>) {
    use mull_workspace::folder_trust::{
        TrustOutcome, decide, decide_inputs_with_interactive, feature_enabled,
    };
    use mull_workspace::trust::workspace_key;
...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("MULL_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = mull_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(mull_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = mull_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match mull_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = mull_config::env_bool("MULL_PLUGIN_CTA")
        .or_else(|| remote_settings.as_ref().and_then(|s| s.plugin_cta))
        .unwrap_or(false);
    // Voice is applied after auth_meta so API-key detection is accurate.
    app.session_picker_grouped = std::env::var("MULL_SESSION_PICKER_GROUPED")
        .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // --force-login: find the palmshed.ai method from the advertised list
            let mull_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "mull");
            if let Some(method) = mull_com {
                ...
                // No palmshed.ai method available, use the first method as fallback
        ...
        // fail-closed: do not invent palmshed.ai / auto-start OIDC.
        tracing::info!(
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    mull_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<mull_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == mull_shell::agent::auth_method::PALMSHED_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = mull_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = mull_shell::config::load_merged_requirements();
    let user_config = mull_shell::config::load_from_disk().ok();
    let managed_config = mull_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match mull_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = mull_shell::agent::config::resolve_compat_sessions_from_raw(
        effective_config.as_ref().ok_or(()),
        remote_settings.as_ref(),
    );
    app.foreign_session_compat = mull_workspace::foreign_sessions::EnabledForeignSessionSources {
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > MULL_PALMSHED_API_BASE_URL env > default).
            let endpoints_base = mull_shell::agent::config::EndpointsConfig::from_config_value(raw)
                .mull_api_base_url;
            app.voice_config =
                mull_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to mull-cli server-side (mirrors sampler / imagine). Done after
    ...
    app.zdr_access_enabled = mull_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        mull_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        mull_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        mull_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use mull_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = mull_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let mull_home = mull_tools::util::mull_home::mull_home();
            app.tip = mull_shell::util::tips::pick_and_advance(&app.tips, &mull_home);
        }
    }

    let hints = mull_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.mull/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        mull_shell::util::config::clamped_display_permission_mode(
            mull_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        mull_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = mull_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = mull_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // MULL_SCROLL_SPEED/_MODE/_LINES + MULL_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<mull_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`mull "fix the bug"`). When
    ...
    // `mull dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `MULL_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("MULL_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(mull_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice pipeline could not start — restart mull");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.mull/config.toml [ui]`
                // rather than `~/.mull/pager.toml`. The watcher only knows
...
pub(crate) fn load_initial_ui_config() -> mull_shell::agent::config::UiConfig {
    use mull_shell::agent::config::UiConfig;
    let Ok(root) = mull_shell::config::load_effective_config() else {
