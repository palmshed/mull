...
use mull_shell::sampling::types::ReasoningEffort;
...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: mull_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open palmshed.ai in the browser for SuperMull subscription upsell.
    OpenSupermullUrl,
    /// Re-check subscription status via the shell's `palmshed.ai/auth/check_subscription`.
    CheckSubscription,
    ...
    /// Open palmshed.ai managed connectors, appending session teamId when set.
    ...
    /// `prompt_id`. Routed to the agent as `palmshed.ai/queue/remove`;
    /// the resulting `palmshed.ai/queue/changed` rebroadcast is the source of truth.
    QueueRemoveShared {
        id: String,
        expected_version: u64,
    },
    /// Reorder the server-authoritative (shared) queued prompts to match
    /// `ordered_ids`. Routed as `palmshed.ai/queue/reorder`.
    QueueReorderShared {
        ordered_ids: Vec<String>,
    },
    /// Clear the caller's server-authoritative (shared) queued prompts.
    /// Routed as `palmshed.ai/queue/clear`.
    QueueClearShared,
    /// Replace the text of a server-authoritative (shared) queued prompt.
    /// Routed to the agent as `palmshed.ai/queue/edit`; the rebroadcast of
    /// `palmshed.ai/queue/changed` is the source of truth. Last write wins via the
    ...
    /// merges its text into the in-flight turn. Routed as `palmshed.ai/queue/interject`;
    /// the `palmshed.ai/session/interjection` + `palmshed.ai/queue/changed` rebroadcasts are
    ...
    OpenTranscriptPager,
    /// Minimal mode (`mull --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: mull_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(mull_hooks_plugins_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(mull_hooks_plugins_types::PluginsAction),
    /// Execute a marketplace management action from the modal.
    ExecuteMarketplaceAction(mull_hooks_plugins_types::MarketplaceAction),
    /// Add or update an MCP server via palmshed.ai/mcp/upsert.
    UpsertMcpServer {
        name: String,
        config: Box<mull_shell::util::config::McpServerConfig>,
    },
    /// Delete an MCP server via palmshed.ai/mcp/delete.
    DeleteMcpServer {
        server_name: String,
    },
    ...
    /// Toggle a skill enable/disable via palmshed.ai/skills/toggle.
    ToggleSkill {
        skill_name: String,
        enabled: bool,
    },
    ...
    DemoteToBackground,
    /// Request current bundle cache status via `palmshed.ai/bundle/status`.
    RequestBundleStatus,
    ...
    AnnouncementsOpenCta(mull_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"mullnight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `mull dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.mull/config.toml` and route ACP
/// `palmshed.ai/yolo_mode_changed` notifications. The ACP notification is
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: mull_workspace::foreign_sessions::EnabledForeignSessionSources,
        mull_home: std::path::PathBuf,
    ...
    FetchSessionList {
        /// Text search pushed down to `palmshed.ai/session/list` as `query` (chat
    ...
    /// `palmshed.ai/sessions/list`. Only issued in leader mode while the
    /// dashboard is open.
    FetchRoster,
    /// Fetch the local on-disk session list (dormant/idle sessions) for the
    /// dashboard via `palmshed.ai/session/list` — the non-leader fallback for the
    ...
    /// Cancel a subagent via `palmshed.ai/subagent/cancel`.
    KillSubagent {
        session_id: acp::SessionId,
        subagent_id: String,
    },
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.mull/config.toml`.
    ...
    /// Persist a typed setting to `~/.mull/config.toml`. On failure,
    ...
    /// `palmshed.ai/queue/remove`. The agent re-broadcasts the authoritative queue.
    QueueRemove {
        session_id: acp::SessionId,
        id: String,
        expected_version: u64,
    },
    /// Reorder server-owned queued prompts: fire-and-forget `palmshed.ai/queue/reorder`.
    QueueReorder {
        session_id: acp::SessionId,
        ordered_ids: Vec<String>,
    },
    /// Clear the caller's server-owned queued prompts: fire-and-forget
    /// `palmshed.ai/queue/clear`.
    QueueClear { session_id: acp::SessionId },
    /// Replace the text of a server-owned queued prompt in place: fire-and-forget
    /// `palmshed.ai/queue/edit`. The session actor's serialized mailbox makes this
    /// last-writer-wins for concurrent edits; the rebroadcast of
    /// `palmshed.ai/queue/changed` is the truth signal.
    QueueEdit {
        session_id: acp::SessionId,
        id: String,
        new_text: String,
    ...
    /// fire-and-forget `palmshed.ai/queue/interject`. The session actor atomically
    ...
    /// Resolve the running agent name for a session (`palmshed.ai/session/info`).
    ...
    /// Fetch MCP server list from the shell (palmshed.ai/mcp/list).
    FetchMcpsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
        cache: bool,
    },
    /// Trigger MCP OAuth for a server (palmshed.ai/mcp/auth_trigger).
    McpAuthTrigger {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    },
    /// Fetch hooks list from the shell (palmshed.ai/hooks/list).
    FetchHooksList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch plugins list from the shell (palmshed.ai/plugins/list).
    FetchPluginsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Fetch skills list from the shell (palmshed.ai/skills/list).
    FetchSkillsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Toggle a skill via palmshed.ai/skills/toggle (enable/disable without restart).
    ToggleSkill {
        agent_id: AgentId,
        session_id: acp::SessionId,
        skill_name: String,
    ...
    /// Install a plugin from the inline CTA via `palmshed.ai/marketplace/action`,
    ...
    /// Reload plugins after a CTA install via `palmshed.ai/plugins/action`
    ...
    /// Read the MCP server list after a CTA install via `palmshed.ai/mcp/list`,
    ...
    /// the same `palmshed.ai/mcp/list` fetch as `FetchPluginCtaMcps`, reported back via
    ...
    /// Upsert an MCP server via palmshed.ai/mcp/upsert.
    UpsertMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        name: String,
    ...
    /// Live-toggle an MCP server via palmshed.ai/mcp/toggle (no restart needed).
    ToggleMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Toggle a single MCP tool via palmshed.ai/mcp/toggle_tool.
    ToggleMcpTool {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Fetch and display session info via palmshed.ai/session/info.
    ShowSessionInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
        show_resolved_model: bool,
    },
    /// Fetch and display detailed context usage via palmshed.ai/session/info.
    ShowContextInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch current bundle cache status via `palmshed.ai/bundle/status`.
    FetchBundleStatus,
    /// Fetch a bundled entry's raw content via `palmshed.ai/bundle/entry/get`.
    ...
    /// Send raw note to palmshed.ai/memory/rewrite for LLM-powered reformatting.
    ...
    /// Fire a /btw side question via palmshed.ai/btw ext method.
    SendBtw {
        agent_id: AgentId,
        session_id: acp::SessionId,
        question: String,
    },
    /// Request a session recap via the palmshed.ai/recap ext method. Fire-and-forget:
    /// the recap arrives later as a `SessionRecap` notification.
    SendRecap {
        session_id: acp::SessionId,
        auto: bool,
    },
    /// Send a mid-turn interjection via palmshed.ai/interject ext method.
    SendInterject {
        agent_id: AgentId,
        session_id: acp::SessionId,
        text: String,
        /// Client-minted id echoed back on the `palmshed.ai/session/interjection`
    ...
    /// Log out via `palmshed.ai/auth/logout` (shell clears auth.json + in-memory state).
    Logout,
    /// Re-check subscription status via `palmshed.ai/auth/check_subscription`.
    ...
    /// registry (`~/.mull/active_sessions.json`).
    ...
    /// Call `palmshed.ai/session/fork` to create a peer session that resumes
    ...
    /// Fetch billing/credit usage from the agent's `palmshed.ai/billing` extension.
    ...
    /// Send an ACP `palmshed.ai/suggest` request to the shell. `agent_id` is echoed
    ...
    /// Send an ACP `palmshed.ai/suggestPrompt` request to the shell — predict the
    /// user's likely next prompt after a completed turn (tab autocomplete
    /// ghost text).
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`mull-0.1` when the
...
/// Outcome of an `palmshed.ai/subagent/cancel` request, telling dispatch whether the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<mull_workspace::session::git::RestoreDegree>,
    },
    ...
    SessionLoaded {
        ...
        /// `_meta["palmshed.ai/runningPromptId"]`), present only when the session was
    ...
    SessionListLoaded {
        ...
        /// Degraded conversations lane (`_meta["palmshed.ai/partial"]`), surfaced
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<mull_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    /// Leader session roster loaded via `palmshed.ai/sessions/list`.
    RosterLoaded {
    ...
    CancelComplete,
    /// Response to `palmshed.ai/subagent/cancel`; see [`SubagentKillOutcome`].
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<mull_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<mull_shell::util::changelog::ChangelogEntry>,
    },
    ...
    AuthUrlReady {
        ...
        external: bool,
        /// Presentation mode from `palmshed.ai/auth/get_url`; `None` on older agents.
        mode: Option<String>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<mull_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<mull_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<mull_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<mull_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<mull_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<mull_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    /// `palmshed.ai/recap` request acknowledged (fire-and-forget). The recap itself
    ...
    LogoutComplete,
    /// Shell responded to `palmshed.ai/auth/check_subscription`. `verify` echoes
    ...
    AuthCopiedTimeout,
    DeepSearchResults {
        results: Vec<mull_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    /// `palmshed.ai/session/fork` completed (no-worktree path). The pager adopts
    ...
    /// `palmshed.ai/session/fork` failed. The placeholder agent stays in
    ...
    GateRefreshed {
        settings: Option<mull_shell::util::config::RemoteSettings>,
    },
    ...
    /// Shell suggestions loaded from ACP `palmshed.ai/suggest`. `request_text` /
    ...
    /// Predicted next prompt loaded from ACP `palmshed.ai/suggestPrompt`.
