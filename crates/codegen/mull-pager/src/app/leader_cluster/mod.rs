...
//!
//! Env sandboxing follows this crate's `serial(MULL_HOME)` idiom; note
//! `mull_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_mull_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `PALMSHED_API_KEY`,
//! `MULL_LEADER_SOCKET`, `MULL_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `mull_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p mull-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The mull home the agent actually persisted under: `mull_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_mull_home() -> PathBuf {
    mull_config::mull_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same MULL_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(MULL_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = mull_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("MULL_HOME", mull_home.path()),
            crate::test_util::EnvVarGuard::set("MULL_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("MULL_PALMSHED_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("PALMSHED_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("MULL_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("MULL_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("MULL_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (mull_version), not
        ...
        // mull-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: mull-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same MULL_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `MULL_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    mull_ws_url: String::new(),
                    mull_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
