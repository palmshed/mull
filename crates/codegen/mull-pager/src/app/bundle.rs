...
//!
//! Pager-side cache of what `mull-shell` reports from
//! `palmshed.ai/bundle/status`. The shell now performs the actual bundle download in
...
///
/// Populated from `palmshed.ai/bundle/status` ACP responses.
...
/// Deserialized response from `palmshed.ai/bundle/status`.
...
/// Deserialized response from `palmshed.ai/bundle/entry/get`.
#[derive(Debug, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
