...
use super::*;
use mull_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
/// legacy `palmshed.ai/interject` wire shape stays byte-identical.
#[test]
...
fn picker_keeps_conversation_with_empty_cwd_and_missing_updated_at() {
    let payload = serde_json::json!(
        ...
        "Compare GPU vendors", "source" : "conversation", "_meta" : { "palmshed.ai/session" : {
        "kind" : "chat" } } }] }
    );
...
fn picker_keeps_old_conversation_past_cutoff() {
    let payload = serde_json::json!(
        ...
        "_meta" : { "palmshed.ai/session" : { "kind" : "chat" } } }] }
    );
    let entries = parse_session_picker_entries(&payload);
...
fn picker_drops_local_with_missing_updated_at() {
    let payload = serde_json::json!(
        { "sessions" : [{ "sessionId" : "local_no_ts", "cwd" : "/Users/me/mull", "summary"
        : "no timestamp", "source" : "local" }] }
    );
    let entries = parse_session_picker_entries(&payload);
    assert!(entries.is_empty(), "local rows still require a parseable updatedAt");
}
/// Untitled palmshed.ai chats must stay listed, rendered as "Untitled".
#[test]
...
fn session_list_partial_parses_reasons() {
    let payload = |reason: &str| {
        serde_json::json!(
            { "sessions" : [], "_meta" : { "palmshed.ai/partial" : { "conversations" : true,
            "reason" : reason } } }
        )
    };
    assert_eq!(
...
fn session_list_partial_absent_for_healthy_or_meta_less_responses() {
    let healthy = serde_json::json!(
        { "sessions" : [], "_meta" : { "palmshed.ai/partial" : { "conversations" : false } } }
    );
...
fn parse_kill_outcome_reads_result_envelope() {
    use mull_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use mull_workspace::session::git::RestoreDegree;
...
/// Spawn a fake ACP agent that counts `palmshed.ai/yolo_mode_changed`
/// notifications. Exits when the channel closes.
fn spawn_fake_acp_agent(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<mull_acp_lib::AcpAgentMessage>,
) -> Arc<AtomicUsize> {
    let counter = Arc::new(AtomicUsize::new(0));
    let counter_clone = counter.clone();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let mull_acp_lib::AcpAgentMessage::ExtNotification(args) = msg {
                if args.request.method.as_ref() == "palmshed.ai/yolo_mode_changed" {
...
/// Redirect `MULL_HOME` to a tempdir for test isolation.
fn setup_mull_home_in_tempdir() -> tempfile::TempDir {
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use mull_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        mull_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        mull_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_mull_home_in_tempdir();
    ...
    assert_eq!(
        ...
        "ACP `palmshed.ai/yolo_mode_changed` notification must fire exactly once \
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use mull_acp_lib::AcpAgentMessage;
    use mull_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "palmshed.ai/marketplace/action" => {
                        ...
                        let req: mull_hooks_plugins_types::MarketplaceActionRequest = serde_json::from_str(
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: mull_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            mull_home: PathBuf::from("/path/that/must/not/be-read"),
...
async fn fetch_session_list_pushes_query_and_echoes_seq() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "palmshed.ai/session/list");
...
/// Default flags produce no agent profile (uses mull-build default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (mull-build already has TaskTool).
#[test]
...
/// Neutralize `MULL_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(MULL_AGENT)]` (process-global env).
fn without_mull_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("MULL_AGENT", "")
}
...
#[serial_test::serial(MULL_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_mull_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "mull-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "mull-plan-no-subagents");
...
/// --ask-user alone selects the mull-ask-user profile.
#[serial_test::serial(MULL_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "mull-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `mull-build` already has it).
#[test]
...
/// All three flags on at the runtime default produce mull-build-plan
...
fn to_meta_chat_mode_stamps_kind_and_omits_agent_profile() {
    ...
    assert_eq!(meta["palmshed.ai/session"] ["kind"], "chat");
    assert!(
...
fn chat_load_meta_never_includes_workspace_bind_keys() {
    ...
    apply_chat_kind_meta(&mut meta);
    {
        ...
        obj.insert("palmshed.ai/cloud_server_id".into(), serde_json::json!("srv-poison"));
        obj.insert(
            "palmshed.ai/cloud_existing_workspace".into(),
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> mull_shell::session::SessionInfoResponse {
    use mull_shell::session::acp_types::{ContextInfo, SessionInfoData};
    mull_shell::session::SessionInfoResponse {
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("mull", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model Hash: abc123"));
