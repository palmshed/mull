...
/// Run the post-CTA-install `palmshed.ai/mcp/list` read (uncached, which also nudges
...
pub(super) fn format_acp_error(err: &acp::Error, is_api_key_auth: bool) -> String {
    ...
    if err.code == acp::ErrorCode::InvalidParams && let Some(data) = &err.data
        && let Some(msg) = mull_shell::sampling::error::error_detail_from_data(data)
        && !msg.is_empty()
    {
        return msg;
    }
...
) -> (bool, Option<String>, Option<mull_workspace::session::git::RestoreDegree>) {
...
/// CANONICAL wire parser for `LoadSessionResponse._meta["palmshed.ai/runningPromptId"]`.
///
...
pub(crate) fn parse_session_load_running_prompt_id(
    ...
    resp_meta
        .and_then(|m| m.get("palmshed.ai/runningPromptId"))
        .and_then(|v| v.as_str())
        .map(String::from)
}
...
pub(crate) fn sanitize_user_error(raw: &str) -> String {
    if raw.contains(mull_fast_worktree::OUT_OF_DISK_CONTEXT)
        || raw.contains(mull_fast_worktree::ENOSPC_OS_MESSAGE)
    {
...
/// |-------|-----------|----------|--------------------------------|--------------------|
/// | false | false     | false    | `mull-build` (default)         | `false`            |
/// | false | true      | false    | `mull-build` (default)         | `false`            |
/// | false | false     | true     | `mull-ask-user`          | omitted (shell gate) |
/// | false | true      | true     | `mull-ask-user`          | omitted (shell gate) |
/// | true  | false     | false    | `mull-build-plan-no-subagents` | `false`            |
/// | true  | true      | false    | `mull-build-plan`              | `false`            |
/// | true  | false     | true     | `mull-build-plan-no-subagents` | omitted (shell gate) |
/// | true  | true      | true     | `mull-build-plan`              | omitted (shell gate) |
///
/// When [`Self::chat_mode`] is set (gateway light-frontend / `--chat`), Build
/// `agentProfile` injection is omitted (K12) and `_meta["palmshed.ai/session"].kind`
...
pub(crate) struct SessionFlags {
    ...
    /// Injected as `palmshed.ai/restore_code` into `LoadSession` meta, or passed
...
impl SessionFlags {
    ...
    ///
    /// Returns `None` for the default `mull-build` profile (no `_meta`
    ...
    pub(super) fn agent_profile(&self) -> Option<&'static str> {
        ...
        match (self.plan_mode, self.subagents, self.ask_user) {
            (true, true, _) => Some("mull-plan"),
            (true, false, _) => Some("mull-plan-no-subagents"),
            (false, _, true) => Some("mull-ask-user"),
            (false, _, false) => None,
        }
    }
    ...
    /// `MULL_AGENT` — the env var chooses the *agent*, but the tool-strip is
    /// independent. Chat mode additionally stamps `palmshed.ai/session.kind`.
    pub(super) fn to_meta(&self) -> Option<acp::Meta> {
        let mut meta = serde_json::Map::new();
        if self.chat_mode {
            if self.plan_mode || self.agent_override.is_some()
                || std::env::var("MULL_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
            {
                tracing::warn!(
        ...
        } else if std::env::var("MULL_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
        {} else if let Some(profile) = self.agent_profile() {
            meta.insert("agentProfile".into(), serde_json::json!(profile));
        }
        if self.chat_mode {
            meta.insert("palmshed.ai/session".into(), serde_json::json!({ "kind" : "chat" }));
        }
        if !self.ask_user {
...
pub(super) const CHAT_FORBIDDEN_WORKSPACE_BIND_KEYS: &[&str] = &[
    "envId",
    "palmshed.ai/cloud_server_id",
    "palmshed.ai/cloud_existing_workspace",
];
/// Stamp `_meta["palmshed.ai/session"].kind = "chat"` and strip Build `agentProfile` (K12).
pub(super) fn apply_chat_kind_meta(meta: &mut Option<acp::Meta>) {
    let obj = meta.get_or_insert_with(acp::Meta::new);
    obj.insert("palmshed.ai/session".into(), serde_json::json!({ "kind" : "chat" }));
...
pub(super) fn extract_first_user_prompt(
    info: &mull_shell::session::info::Info,
) -> Option<String> {
    use std::io::BufRead;
    let history_path = mull_shell::session::persistence::session_dir(info)
...
pub(super) fn count_chat_history_stats(history_path: &Path) -> (usize, usize) {
    use std::io::BufRead;
    use mull_shell::sampling::{AssistantItem, ConversationItem, UserItem};
...
/// Degraded conversations lane on `palmshed.ai/session/list`, parsed from the
/// response's `_meta["palmshed.ai/partial"]` envelope.
...
/// Read `_meta["palmshed.ai/partial"]` from a session-list payload. `None` when the
...
pub(super) fn parse_session_list_partial(
    ...
    let partial = payload.get("_meta")?.get("palmshed.ai/partial")?;
...
/// Parse the `palmshed.ai/session/list` response payload (the unwrapped
    ...
    entries
        ...
        .filter_map(|v| {
            ...
            let is_conversation = v
                .get("_meta")
                .and_then(|m| m.get("palmshed.ai/session"))
            ...
            use mull_tools::implementations::skills::skill::extract_skill_display_text;
            let display = if let Some(ref fp) = first_prompt {
                ...
                let info_cwd = v
                    ...
                    .unwrap_or_default()
                    .to_string();
                let info = mull_shell::session::info::Info {
        ...
        .filter_map(|mut e| {
            ...
            if e.source == "remote"
                && mull_shell::session::resolve_local_session_any_cwd(&e.id)
                    .is_some()
            {
...
pub(super) async fn send_check_subscription(
    ...
    let req = acp::ExtRequest::new(
        "palmshed.ai/auth/check_subscription",
        serde_json::value::to_raw_value(&serde_json::json!({}))
...
pub(crate) async fn persist_setting(
    ...
    match key {
        "compact_mode" => {
            ...
            mull_shell::util::config::set_compact_mode(b)
                .await
        ...
        "show_timestamps" => {
            ...
            mull_shell::util::config::set_show_timestamps(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_timeline" => {
            ...
            mull_shell::util::config::set_show_timeline(b)
                .await
                .map_err(|e| e.to_string())
        }
        "simple_mode" => {
            ...
            mull_shell::util::config::set_simple_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.undo" => {
            ...
            mull_shell::util::config::set_contextual_hint_undo(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.plan_mode" => {
            ...
            mull_shell::util::config::set_contextual_hint_plan_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.image_input" => {
            ...
            mull_shell::util::config::set_contextual_hint_image_input(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.send_now" => {
            ...
            mull_shell::util::config::set_contextual_hint_send_now(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.small_screen" => {
            ...
            mull_shell::util::config::set_contextual_hint_small_screen(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.word_select" => {
            ...
            mull_shell::util::config::set_contextual_hint_word_select(b)
                .await
                .map_err(|e| e.to_string())
        }
        "theme" => {
            ...
            mull_shell::util::config::set_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_dark_theme" => {
            ...
            mull_shell::util::config::set_auto_dark_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_light_theme" => {
            ...
            mull_shell::util::config::set_auto_light_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "default_model" => {
            ...
            mull_shell::util::config::set_default_model(s)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_speed" => {
            ...
            mull_shell::util::config::set_scroll_speed(i)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_mode" => {
            ...
            mull_shell::util::config::set_scroll_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "invert_scroll" => {
            ...
            mull_shell::util::config::set_invert_scroll(b)
                .await
                .map_err(|e| e.to_string())
        }
        "display_refresh_auto_cadence" => {
            ...
            mull_shell::util::config::set_display_refresh_auto_cadence(b)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_lines" => {
            ...
            mull_shell::util::config::set_scroll_lines(i)
                .await
                .map_err(|e| e.to_string())
        }
        "default_selected_permission" => {
            ...
            mull_shell::util::config::set_default_selected_permission(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "cancel_subagents_on_turn_cancel" => {
            ...
            mull_shell::util::config::set_cancel_subagents_on_turn_cancel(
                    s.to_string(),
                )
                .await
                .map_err(|e| e.to_string())
        ...
        "vim_mode" => {
            ...
            mull_shell::util::config::set_vim_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "remember_tool_approvals" => {
            ...
            mull_shell::util::config::set_remember_tool_approvals(b)
                .await
                .map_err(|e| e.to_string())
        }
        "toolset.ask_user_question.timeout_enabled" => {
            ...
            mull_shell::util::config::set_ask_user_question_timeout_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_thinking_blocks" => {
            ...
            mull_shell::util::config::set_show_thinking_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "group_tool_verbs" => {
            ...
            mull_shell::util::config::set_group_tool_verbs(b)
                .await
                .map_err(|e| e.to_string())
        }
        "collapsed_edit_blocks" => {
            ...
            mull_shell::util::config::set_collapsed_edit_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "prompt_suggestions" => {
            ...
            mull_shell::util::config::set_prompt_suggestions(b)
                .await
                .map_err(|e| e.to_string())
        }
        "keep_text_selection" => {
            ...
            mull_shell::util::config::set_keep_text_selection(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        ...
        "render_mermaid" => {
            ...
            mull_shell::util::config::set_render_mermaid(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "hunk_tracker_mode" => {
            ...
            mull_shell::util::config::set_hunk_tracker_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "screen_mode" => {
            ...
            mull_shell::util::config::set_screen_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_capture_mode" => {
            ...
            mull_shell::util::config::set_voice_capture_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_stt_language" => {
            ...
            mull_shell::util::config::set_voice_stt_language(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "max_thoughts_width" => {
            ...
            mull_shell::util::config::set_max_thoughts_width(i)
                .await
                .map_err(|e| e.to_string())
        }
        "show_tips" => {
            ...
            mull_shell::util::config::set_show_tips(b)
                .await
                .map_err(|e| e.to_string())
        }
        "auto_update" => {
            ...
            mull_shell::util::config::set_auto_update(b)
                .await
                .map_err(|e| e.to_string())
        }
        "fork_secondary_model" => {
            ...
            mull_shell::util::config::set_fork_secondary_model(s)
                .await
                .map_err(|e| e.to_string())
        }
...
/// 2. Fire ACP `palmshed.ai/yolo_mode_changed` (gated on disk success for
...
pub(crate) async fn persist_permission_mode_and_notify(
    ...
    let disk_result = mull_shell::util::config::update_config(|cfg| {
...
/// Whether to fire the ACP `palmshed.ai/yolo_mode_changed` notification.
...
pub(super) fn marketplace_outcome_succeeded(
    outcome: &mull_hooks_plugins_types::ActionOutcome,
) -> bool {
    outcome.status == mull_hooks_plugins_types::OutcomeStatus::Success
}
/// Extract the typed kill outcome from an `palmshed.ai/task/kill` ext response.
///
...
/// serializes (`mull_shell::extensions::task::KillTaskResponse` +
/// `mull_shell::session::result::ExtMethodResult`) so the contract stays
...
pub(super) fn parse_kill_outcome(
    resp: &str,
) -> Option<mull_tools::types::KillOutcome> {
    use mull_shell::extensions::task::KillTaskResponse;
    use mull_shell::session::result::ExtMethodResult;
    serde_json::from_str::<ExtMethodResult<KillTaskResponse>>(resp)
        .ok()
        .and_then(|envelope| envelope.result)
        .map(|payload| payload.outcome)
}
/// Map an `palmshed.ai/subagent/cancel` response (payload under `result`) to a kill
...
pub(super) fn parse_subagent_kill_outcome(resp: &str) -> SubagentKillOutcome {
    use mull_shell::extensions::task::{
...
pub(super) fn credit_balance_from_config(
    c: mull_shell::extensions::billing::BillingConfig,
...
/// Fetch the user's auto top-up rule via the `palmshed.ai/auto-topup-rule` extension.
...
/// Map an `palmshed.ai/auto-topup-rule` payload to an [`AutoTopupFetch`]. A body that
...
pub(super) fn parse_auto_topup_response(
    ...
    use mull_shell::extensions::billing::GetAutoTopupRuleResponse;
...
/// A blocking flock on the shared, possibly-network `~/.mull` lock must never
...
pub(super) fn unregister_active_session_best_effort_in(
    ...
    match mull_shell::active_sessions::try_unregister_in(root, session_id) {
        Ok(true) => {}
        Ok(false) => {
            tracing::debug!(
