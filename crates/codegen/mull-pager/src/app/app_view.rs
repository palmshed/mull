...
/// [`mull_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<mull_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: mull_shell::util::config::WorktreeHintMode) -> Self {
        use mull_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        mull_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) = mull_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`mull_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `MULL_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `MULL_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("MULL_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`mull_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => mull_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: mull_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`mull_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.mull/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`MULL_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `MULL_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<mull_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<mull_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `MULL_PLUGIN_CTA`
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `mull dashboard`, the
    ...
    /// `palmshed.ai/sessions/list` polls and `palmshed.ai/sessions/changed` broadcasts.
    ...
    /// poll outside leader mode, so we fetch the same `palmshed.ai/session/list` the
    ...
    /// Reconciled from `palmshed.ai/queue/changed` broadcasts so
    ...
    /// confirming `palmshed.ai/queue/changed` broadcast has not yet arrived. Keyed by
    ...
    /// non-selectable headers. Gated by `MULL_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `MULL_CANCEL_REWIND` env /
    ...
    /// When false, the pager must not request recaps (zero `palmshed.ai/recap` traffic).
    ...
    /// empty and inert outside `--minimal`; the `mull-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<mull_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        mull_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `mull_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// Stamps `_meta["palmshed.ai/session"].kind = "chat"` and omits Build agent
    ...
    /// at startup and on settings toggles from `MULL_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: mull_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<mull_shell::util::config::ContextualHintsRemote>,
    ...
    /// Login button label from `AuthMethod.name` (e.g., "palmshed.ai", "Acme Corp").
    ...
    /// `mull_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, mull_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `mull_build_access_gate`. `Some` = blocked.
    pub gate: Option<mull_shell::auth::GateInfo>,
    /// User-friendly subscription tier name (e.g. "SuperMull", "Free").
    ...
    pub pending_gate_verification: Option<mull_shell::auth::GateInfo>,
    ...
    /// `~/.mull/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `MULL_VOICE_MODE=0` — the STT pipeline is not started and
    ...
    pub voice_config: mull_voice::VoiceConfig,
    /// Auth for STT (OAuth session via shell `AuthManager`, or `PALMSHED_API_KEY`).
    /// `None` until the pipeline is first started (lazy on `/voice`).
    pub voice_auth: Option<mull_voice::SharedVoiceAuth>,
    /// Commands into the voice pipeline (start/stop capture — toggle, not hold).
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<mull_voice::VoiceCommand>>,
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &mull_shell::util::config::RemoteSettings,
    ) -> Option<mull_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(mull_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &mull_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            mull_telemetry::session_ctx::log_event(mull_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: mull_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: mull_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: mull_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<mull_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && mull_voice::AUDIO_SUPPORTED
    }
    ...
    /// `palmshed.ai/settings/update` handler when the subscription tier changes, so
    ...
    fn voice_send(&self, cmd: mull_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(mull_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(mull_voice::VoiceCommand::PttRelease);
    ...
    /// `palmshed.ai/queue/changed` broadcast. The broadcast is
    ...
    /// `palmshed.ai/queue/changed` broadcast replaces it (matched by `id`) rather than
    ...
    /// Rows the dev `MULL_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results: &'a Option<Vec<mull_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    mull_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(rect) = ctx.gate_url_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::OpenSupermullUrl);
                }
                if let Some(rect) = ctx.upgrade_cta_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        mull_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
fn dispatch_access_gate_menu_action(index: usize) -> InputOutcome {
    match index {
        0 => InputOutcome::Action(Action::OpenSupermullUrl),
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                mull_shell::util::with_locked_stderr(|stderr| {
                    ...
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::SuperMullUpsellShown {
                                    source: mull_telemetry::events::SuperMullUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`mull_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use mull_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = mull_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                mull_telemetry::session_ctx::log_event(
                    mull_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: mull_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            mull_telemetry::session_ctx::log_event(mull_telemetry::events::ContextualTip {
                tip: mull_telemetry::events::ContextualTipKind::ImageInput,
                action: mull_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = mull_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![mull_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![mull_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: mull_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_hides_usage_for_team_users() {
        ...
        let meta = mull_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = mull_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_shows_usage_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&mull_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&mull_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_lifts_restrictions_for_paid_tiers_and_teams() {
        ...
        advertise_media_tools(&mut app);
        let meta = mull_shell::auth::AuthMeta {
            subscription_tier: Some("SuperMull".into()),
            ..Default::default()
        };
        app.apply_auth_meta(&meta);
        assert!(app.tier_restricted_commands.is_empty());
        ...
        app.apply_auth_meta(&mull_shell::auth::AuthMeta::default());
        assert!(!app.tier_restricted_commands.is_empty());
        app.subscription_tier = Some("SuperMull".into());
    ...
    fn is_restricted_tier_classification() {
        ...
        assert!(!is_restricted_tier(Some("SuperMull")));
        assert!(!is_restricted_tier(Some("SuperMull Heavy")));
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(mull_shell::auth::GateInfo {
            message: "Subscribe to use Mull".into(),
            url: Some("https://palmshed.ai/supergrok?referrer=mull-build".into()),
            label: None,
        });
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = mull_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(mull_workspace::foreign_sessions::RecentForeignSession {
                tool: mull_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![mull_workspace::file_system::FuzzyMatchResult {
