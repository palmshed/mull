...
//!
//! - Request/response `palmshed.ai/sessions/list` → [`RosterListResponse`].
//! - Broadcast notification `palmshed.ai/sessions/changed` → [`RosterChanged`].
//!
...
/// Response to `palmshed.ai/sessions/list`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Broadcast payload for `palmshed.ai/sessions/changed`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Parse an `palmshed.ai/sessions/list` ext-response body into a [`RosterListResponse`].
///
/// The agent serializes the response through
/// `ExtMethodResult::success(..).to_ext_response()` (see
/// `mull-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
/// `palmshed.ai/sessions/changed` broadcast. Mirrors how `Effect::FetchSessionList`
/// unwraps `result` for `palmshed.ai/session/list`.
...
mod tests {
    ...
    fn agent_entry() -> mull_shell::agent::roster::RosterEntry {
        use mull_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        // Exact wire bytes the agent emits for `palmshed.ai/sessions/list`.
    ...
    /// `palmshed.ai/sessions/changed` round-trip: serialize the agent's `RosterChanged`
