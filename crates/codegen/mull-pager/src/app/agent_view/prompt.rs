            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    mull_telemetry::session_ctx::log_event(
                        mull_telemetry::events::PromptSuggestion {
                            action: mull_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::PromptSuggestion {
                    action: mull_telemetry::events::PromptSuggestionAction::Dismissed,
                    chars,
                    words,
                });
                return InputOutcome::Changed;
...
mod prompt_suggestion_key_tests {
    ...
    /// right after a turn ends with `palmshed.ai/suggestPrompt` resolved. Pins the
