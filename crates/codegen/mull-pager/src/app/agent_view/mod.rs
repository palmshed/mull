...
impl McpInitProgress {
    ...
    ///   `palmshed.ai/mcp_initialized` clears the progress.
...
pub(crate) struct SessionReload {
    ...
    /// Live dedup highwaters (ACP + Palmshed) as of window open (same restore
    /// rationale).
    last_applied_event_seq: Option<u64>,
    last_applied_mull_event_seq: Option<u64>,
...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<mull_hooks_plugins_types::MarketplacePluginEntry>,
...
/// (`palmshed.ai/follow_ups`). Streaming-only: never persisted, does not survive a
...
pub struct AgentView {
    ...
    ///
    /// ACP stream only — the Palmshed stream keeps its own highwater
    /// ([`Self::last_applied_mull_event_seq`]) because the two streams are not
    /// delivered in one id order: ACP lines ride the agent's FIFO event
    /// pipeline while Palmshed lines are emitted direct-to-gateway, so a fresh Palmshed
    ...
    /// Palmshed-stream sibling of [`Self::last_applied_event_seq`] (see there for
    /// why the highwaters are split). Same drop rule, replay-exempt.
    pub last_applied_mull_event_seq: Option<u64>,
    /// Raw `eventId` of the most recent update APPLIED to this root session —
    /// replay or live, on both the ACP and Palmshed paths; dropped updates (dedup,
    ...
    pub context_state: Option<mull_shell::session::ContextInfo>,
    ...
    /// (optimistic echo). The shell broadcasts `palmshed.ai/session/interjection` to
    ...
    pub(crate) deferred_session_mode: Option<mull_tools::types::SessionMode>,
    ...
    /// MCP servers, cleared when `palmshed.ai/mcp_initialized` arrives.
    ...
    /// Running agent definition for this session (`palmshed.ai/session/info` `agentName`).
    ...
    /// Armed when an `palmshed.ai/session/prompt_complete` broadcast arrives for the
    ...
    /// `palmshed.ai/queue/changed` broadcast has confirmed the row yet. Inserted by
    ...
    /// optimistic echo. Firing `palmshed.ai/queue/interject` then would race the
    ...
    /// Parked here and fired from the confirming `palmshed.ai/queue/changed`
    ...
    /// Follow-up suggestion chips for the latest assistant response
    /// (`palmshed.ai/follow_ups`). `None` when no chips are shown. Set by
    ...
    /// Stamped `palmshed.ai/follow_ups` that arrived for a turn that is NOT yet the
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(mull_telemetry::events::CreditLimitChoice::PayAsYouGo);
            mull_telemetry::session_ctx::log_event(
                mull_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            mull_telemetry::session_ctx::log_event(
                mull_telemetry::events::SuperMullUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use mull_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        mull_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = mull_tools::util::mull_home::mull_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &mull_tools::implementations::mull_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(super) mod test_fixtures {
    ...
    /// FIX 4 (b): after adopting a NEW turn, a buffer-replayed `palmshed.ai/follow_ups`
    ...
    /// FINDING B (stamped path): a LATE FIRST-TIME (never-seen) `palmshed.ai/follow_ups`
    ...
    /// upstream by the `_meta["palmshed.ai/replayed"]` gate); it is pinned here so the
    ...
    /// FIX (buffer-before-adoption): a stamped `palmshed.ai/follow_ups` for a turn that
