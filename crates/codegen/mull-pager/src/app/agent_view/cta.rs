...
impl AgentView {
    ...
    pub(crate) fn log_prompt_suggestion_shown_if_visible(&mut self) {
        ...
        mull_telemetry::session_ctx::log_event(mull_telemetry::events::PromptSuggestion {
            action: mull_telemetry::events::PromptSuggestionAction::Shown,
            chars,
            words,
        });
    }
    ...
    /// Apply an `palmshed.ai/follow_ups` notification, keyed by `response_id`
    ...
    /// on each `palmshed.ai/follow_ups` notification (the same `promptId` it stamps on
    ...
    /// - A buffer-replayed `palmshed.ai/follow_ups` for a PRIOR turn's `response_id`
    ...
    /// Buffer a stamped `palmshed.ai/follow_ups` for a turn that is not yet current,
    ...
    /// Flush a buffered `palmshed.ai/follow_ups` for `prompt_id` (a turn that has just
    ...
    /// the running turn's `palmshed.ai/follow_ups` arrive on the ext channel DURING
    ...
    pub(in crate::app) fn connect_matched_plugin(&mut self) {
        ...
        mull_telemetry::session_ctx::log_event(mull_telemetry::events::PluginCtaConnectClicked {
        ...
        self.pending_effects
            .push(super::actions::Effect::InstallPluginFromCta {
                agent_id: self.session.id,
                session_id,
                source_url_or_path: mull_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
                plugin_relative_path,
            });
    }
}
...
mod plugin_cta_notify_tests {
    ...
    fn cta_entry(name: &str) -> mull_hooks_plugins_types::MarketplacePluginEntry {
