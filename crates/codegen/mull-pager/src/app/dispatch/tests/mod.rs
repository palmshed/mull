...
fn cta_entry(name: &str, status: &str) -> mull_hooks_plugins_types::MarketplacePluginEntry {
...
fn cta_outcome(
    status: mull_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
/// Plant a Build session under the process `mull_home()` (OnceLock-cached;
/// do not rely on setting `MULL_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = mull_shell::util::mull_home::mull_home();
    let encoded = mull_shell::util::mull_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
/// `palmshed.ai/ask_user_question` ext-method request. Returns the args
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode:
            mull_tools::implementations::mull_build::ask_user_question::AskUserQuestionMode::Default,
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    mull_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str = mull_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
fn push_synthetic_permission(
    ...
    let request = mull_acp_lib::AcpArgs {
