...
fn critical_announcement(id: &str) -> mull_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> mull_announcements::RemoteAnnouncement {
    mull_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(mull_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: Some(format!("https://palmshed.ai/{id}")),
            caption: None,
        }),
        ..Default::default()
    }
...
/// the promo url (observed via the `MULL_TEST_OPEN_URL_FILE` seam, from every
...
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    ...
    let url_file = std::env::temp_dir().join(format!("mull-cta-open-{}.txt", std::process::id()));
    ...
    for surface in [
        ...
        assert!(
            opened()
                .lines()
                .any(|l| l == "https://palmshed.ai/promo-open"),
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!(
        { "scope" : "user", "path" : "/home/user/.mull/skills/pick-best/SKILL.md", }
    );
