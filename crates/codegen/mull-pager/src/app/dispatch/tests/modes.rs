...
///    `palmshed.ai/yolo_mode_changed` ACP notification back to the shell.
...
fn enable_always_approve_sends_response_and_flips_yolo_and_persists() {
    ...
    //     ~/.mull/config.toml AND fires palmshed.ai/yolo_mode_changed.
    let persist = effects
        .iter()
        .find_map(|e| match e {
...
/// guard MUST precede the `mull_telemetry::log_event` call
