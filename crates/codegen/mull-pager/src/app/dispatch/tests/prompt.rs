...
/// Turn end with prompt suggestions enabled fires the `palmshed.ai/suggestPrompt`
...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `mull-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `mull-0.1`
...
/// `palmshed.ai/queue/changed` rebroadcast — not client-side prediction — updates
...
/// The always-on pipeline end to end, with `MULL_SUGGESTIONS` semantics
