...
pub(super) fn dispatch_send_feedback(app: &mut AppView, text: String) -> Vec<Effect> {
    ...
    agent.scrollback.push_block(RenderBlock::system(
        "Thanks for the feedback! The Mull team is on it.".to_string(),
    ));
...
/// Send a raw remember note for LLM-powered rewriting via `palmshed.ai/memory/rewrite`.
...
fn extract_session_context(agent: &AgentView) -> String {
    ...
    for i in (0..len).rev() {
        ...
        match &entry.block {
            RenderBlock::UserPrompt(prompt) if user_prompts.len() < 5 => {
...
/// agent is mid-turn. Fires the `palmshed.ai/recap` ext method; the recap arrives
...
pub(super) fn dispatch_send_recap(app: &mut AppView, auto: bool) -> Vec<Effect> {
    ...
    // entirely when the feature is off so we never hit `palmshed.ai/recap`.
            ...
            Ok(()) => {
                agent
                    ...
                    .push_block(crate::scrollback::block::RenderBlock::system(format!(
                        "Memory saved to {}",
                        crate::util::display_user_mull_path("memory/MEMORY.md")
                    )));
            }
            Err(error) => {
                agent
