...
//! `palmshed.ai/interject` effect, and prompt-history recording. Split out of
...
/// prompt, and fires the `palmshed.ai/interject` ext method carrying a client-minted
/// id.
///
/// The shell broadcasts `palmshed.ai/session/interjection` to every attached pane so
...
pub(super) fn dispatch_interject(
    ...
    // record its id so the broadcast echo (`palmshed.ai/session/interjection`) is
