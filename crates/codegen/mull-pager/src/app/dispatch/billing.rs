...
use mull_telemetry::events::{SuperMullUpsell, SuperMullUpsellClicked};
...
/// Whether the user is at the highest subscription tier (SuperMull Heavy).
///
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("supermull_heavy") and CCP display names ("SuperMull Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "supermull_heavy"
}

/// URL for upgrading the subscription tier.
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://mull.com/supermull?referrer=mull-build";

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) const UPSELL_URL_PAYG: &str = "https://mull.com?_s=usage";
...
pub(crate) fn acp_error_is_free_usage_exhausted(err: &agent_client_protocol::Error) -> bool {
    err.data
        .as_ref()
        .and_then(mull_shell::sampling::error::error_detail_from_data)
        .as_deref()
...
pub(crate) const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Mull usage limit for now. Get SuperMull for much higher limits, or try again later: https://mull.com/supermull?referrer=mull-build";
...
///
/// **`max_tier = true`** (positively identified as SuperMull Heavy):
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        mull_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using Mull",
            CreditLimitCardAction::PurchaseCredits,
            mull_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            mull_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(mull_telemetry::events::CreditLimitUpsellShown {
            surface: mull_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(mull_telemetry::events::CreditLimitUpsellShown {
        surface: mull_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    use mull_tools::implementations::mull_build::ask_user_question::{Question, QuestionOption};
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            mull_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
/// `SuperMullUpsellShown` funnel event.
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_supermull_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}

/// Open the SuperMull upsell for a tier-restricted slash command
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_supermull_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}

/// Which situation opened the SuperMull upsell modal. Controls the heading
...
fn open_supermull_upsell(
    ...
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit => (
            ...
            SuperMullUpsell::FreeUsagePaywall,
            "free-usage-upsell",
        ),
        UpsellReason::RestrictedCommand => (
            "Unlock all features with SuperMull.",
            SuperMullUpsell::RestrictedCommand,
            "restricted-command-upsell",
        ),
    };

    log_event(mull_telemetry::events::SuperMullUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        QuestionOption {
            label: "Upgrade to SuperMull".into(),
            description: "For everyday coding and productivity tasks".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
        QuestionOption {
            label: "Upgrade to SuperMull Heavy".into(),
            description: "Get the most out of Mull. Highest usage limits.".into(),
            preview: None,
            // No Heavy-specific URL exists; the /supermull page lists
...
/// `palmshed.ai/auth/check_subscription` completed. Meta is authoritative
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<mull_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<mull_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_supermull_url(app: &mut AppView) -> Vec<Effect> {
    log_event(SuperMullUpsellClicked {
        source: SuperMullUpsell::WelcomeScreen,
    ...
    let url = app
        ...
        .unwrap_or("https://mull.com/supermull?referrer=mull-build");
    // Funnel attribution: tag CLI-originated SuperMull upsell clicks
    // with `referrer=mull-build`, matching the OAuth consent flow and
    // palmshed.ai/cli marketing links. Applied even when the URL came from
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "mull");
