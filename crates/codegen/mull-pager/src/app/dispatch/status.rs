...
/// Show session info: fetch via palmshed.ai/session/info and display in scrollback.
///
...
pub(super) fn dispatch_show_privacy_info(app: &mut AppView) -> Vec<Effect> {
    ...
    if app.is_zdr {
        ...
        lines.push("  Usage and code data may be used by Palmshed to improve the product.");
        lines.push("");
        lines.push("  Use /privacy opt-out to enable privacy mode.");
    }

    lines.push("");
    lines.push("  Learn more: https://palmshed.ai/legal");
    let text = lines.join("\n");
...
/// (persists via ACP ext-request, NOT `~/.mull/config.toml`).
...
/// Show context info: fetch via palmshed.ai/session/info and display rich breakdown.
///
...
///
/// When the remote settings `mull_build_usage_redirect_url` flag is set (delivered via
