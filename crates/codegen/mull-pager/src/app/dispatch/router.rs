...
use super::billing::dispatch_open_supermull_url;
use super::ctx::{
...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, mull_telemetry::events::CreditLimitChoice)> = None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(mull_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::AnnouncementsOpenCta(surface) => {
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(mull_telemetry::events::AnnouncementCtaClicked {
                    id: promo_id,
                    source: surface,
                });
                open_url_or_show(app, &url);
        ...
        Action::OpenSupermullUrl => dispatch_open_supermull_url(app),
        Action::OpenUrl(url) => {
            ...
            OutcomeStatus::Success => {
                    ...
                    && modal.result_notice.is_none()
                {
                    let entry_index = match modal.last_plugins_action {
                        Some(mull_hooks_plugins_types::PluginsAction::Uninstall { .. }) => None,
            ...
            OutcomeStatus::ConfirmationRequired => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    let confirmed_action = modal.last_plugins_action.as_ref().map(|a| {
                        ...
                        if let mull_hooks_plugins_types::PluginsAction::Uninstall {
                            ref mut confirmed,
                            ..
                        } = action
                        {
