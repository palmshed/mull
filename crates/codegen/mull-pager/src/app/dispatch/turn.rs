...
fn effective_cancel_subagents_preference(
    ...
    ui: &mull_shell::agent::config::UiConfig,
) -> Option<bool> {
...
pub(super) fn do_cancel_turn(app: &mut AppView, cancel_subagents: bool) -> Vec<Effect> {
    ...
    // FRONT queued prompt as the next turn — its `palmshed.ai/queue/changed`
...
/// Grace window between a driver-side `palmshed.ai/session/prompt_complete`
...
/// Finish turns whose end was announced by `palmshed.ai/session/prompt_complete`
...
// TODO: Add dispatch_cancel_command() once mull-shell supports proper
