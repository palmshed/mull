...
/// only one normal component (e.g., `/mull`), returns that component alone.
/// Does not perform tilde expansion — callers provide absolute paths.
/// Returns `"unknown"` for empty input.
///
/// Examples: `/home/user/fw/1` → `"fw-1"`, `/mull` → `"mull"`, `/` → `"/"`.
///
...
impl SourceFilter {
    ...
    ///
    /// palmshed.ai conversations carry `source == "conversation"` and live remotely,
...
pub(crate) fn capture_picker_selection(
    ...
    content_results: Option<&[mull_shell::extensions::session_search::SearchSessionHit]>,
    state: &PickerState,
    query: &str,
    grouped: bool,
...
pub(crate) fn build_content_entry_data(
    hits: &[mull_shell::extensions::session_search::SearchSessionHit],
    entries_data: &[SessionPickerEntry],
    filtered_indices: &[usize],
    state: &PickerState,
    content_start: usize,
...
mod tests {
    ...
    fn repo_name_from_cwd_standard_path() {
        assert_eq!(repo_name_from_cwd("/home/user/mull"), "user-mull");
    }
    ...
    fn repo_name_from_cwd_single_dir() {
        assert_eq!(repo_name_from_cwd("/mull"), "mull");
    }
    ...
    fn source_filter_matches() {
        ...
        // palmshed.ai conversations are remote: visible under All + Remote, not Local.
