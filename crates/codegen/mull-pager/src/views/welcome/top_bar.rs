...
mod tests {
    ...
    fn format_cwd_plain_repo() {
        assert_eq!(format_cwd_parts("~/mull", None), "~/mull");
    }
    ...
    fn format_cwd_worktree_shows_main_repo() {
        assert_eq!(
            format_cwd_parts("~/wt/session-1", Some("~/mull")),
            "~/wt/session-1 (worktree of ~/mull)"
        );
    }
    ...
    fn format_cwd_display_shows_subdir_not_repo_root() {
        ...
        assert_eq!(
            format_cwd_display(Path::new("/work/mull/frontend/apps"), Some(&info)),
            "/work/mull/frontend/apps",
        );
    }
    ...
    fn format_cwd_display_cache_miss_shows_raw_cwd() {
        assert_eq!(
            format_cwd_display(Path::new("/work/mull/frontend/apps"), None),
            "/work/mull/frontend/apps",
        );
    }
}
