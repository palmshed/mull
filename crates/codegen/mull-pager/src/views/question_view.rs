...
use mull_acp_lib::AcpResult;
use mull_markdown::StreamingMarkdownRenderer;
pub use mull_tools::implementations::mull_build::ask_user_question::{
...
/// driven by an ACP `palmshed.ai/ask_user_question` request).
///
...
pub enum LocalQuestionKind {
    ...
    CreditLimitUpsell {
        choices: Vec<mull_telemetry::events::CreditLimitChoice>,
    },
    /// SuperMull upsell modal: the free-usage paywall (429 +
    ...
    /// `id`.
    FreeUsageUpsell {
        /// Telemetry source for `SuperMullUpsellClicked` — distinguishes
        /// the paywall from the restricted-command upsell.
        source: mull_telemetry::events::SuperMullUpsell,
    },
    ...
    AgentTypeMismatch {
        ...
        effort: Option<mull_shell::sampling::types::ReasoningEffort>,
    },
}
...
///
/// Created when an `palmshed.ai/ask_user_question` ext-method request arrives;
...
pub struct QuestionViewState {
    ...
    /// instead of by an ACP `palmshed.ai/ask_user_question` request. `None` for
...
impl QuestionViewState {
    ...
    ///
    /// Called by the `ExtMethod` handler when a blocking `palmshed.ai/ask_user_question`
...
impl QuestionViewState {
    ...
    /// questions (e.g. the SuperMull upsell) have no freeform row, so
...
impl QuestionViewState {
    ...
    pub fn build_accepted_response(
        &self,
    ) -> mull_tools::implementations::mull_build::ask_user_question::AskUserQuestionExtResponse
    {
    ...
    pub fn send_ext_response(
        &mut self,
        response: mull_tools::implementations::mull_build::ask_user_question::AskUserQuestionExtResponse,
    ) -> bool {
...
mod tests {
    ...
    /// `no_freeform` questions (e.g. the SuperMull upsell) have no "Other"
