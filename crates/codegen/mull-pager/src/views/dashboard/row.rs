...
pub struct DashboardRow {
    ...
    /// dim text (e.g. `"mull my-branch-2 worktree"`). `None` when the
...
mod tests {
    ...
    fn roster_rows_sort_by_real_timestamp() {
        ...
        append_roster_rows(
            ...
            &[
                ...
                roster_entry("sess-mullew", now_ms - 5_000),
            ],
            &agents,
            &pinned,
            None,
        ...
        assert_eq!(
            ...
            DashboardRowId::Roster {
                session_id: "sess-mullew".to_string()
            }
        );
        assert_eq!(
            rows[1].id,
    ...
    fn subtitle_worktree_shows_label_branch_and_marker() {
        ...
        agent.session.cwd = PathBuf::from("/home/me/.mull/worktrees/x/location-picker");
    ...
    fn subtitle_non_worktree_shows_branch_and_folder() {
        let mut agent = make_idle_agent_with_model(None);
        agent.session.cwd = PathBuf::from("/home/me/mull/crates/foo");
