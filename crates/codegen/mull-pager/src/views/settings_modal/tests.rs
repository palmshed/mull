...
fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
    ...
    let snapshot = PagerLocalSnapshot {
        available_models: vec![(
            "Mull Test".to_string(),
            acp::ModelId::new(Arc::from("mull-test")),
        )],
        ..PagerLocalSnapshot::default()
    };
...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("Mull Test", 4); // cursor between "Mull" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
fn picking_enum_esc_returns_to_browse() {
    ...
    match outcome {
        ...
        other => panic!("expected Action::PreviewTheme(\"mullnight\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode, SettingsModalMode::Browse));
}
...
fn picker_visual_smoke_debug() {
    let entries = vec![SettingMeta {
        ...
        description: "Controls whether Palmshed may retain and train on coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-out",
            choices: &[
                EnumChoice {
                    canonical: "opt-in",
                    display: "Opt in",
                    description: "Allow Palmshed to retain and use coding session data for training and product improvement.",
                },
                EnumChoice {
                    canonical: "opt-out",
                    display: "Opt out",
...
fn picker_multi_line_choice_hit_rect_spans_all_lines() {
    ...
    let entries = vec![SettingMeta {
        ...
        owner: SettingOwner::Shared,
        label: "Coding data sharing",
        description: "Controls whether Palmshed may retain coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-in",
            choices: &[
...
fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
    ...
    // (catalog has 'Mull 4 Fast'; "a" doesn't match) is
...
//    the "Tip · Ask Mull…" docs footer and the keybindings hints.
//
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT = "Tip · Ask Mull to change a setting" (34 cells);
    ...
    assert!(
        !row_short.contains("mullday"),
        "width=40 must NOT render LONG path (no `mullday`): {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask Mull") {
            tip_y = Some(y);
            break;
        }
    }
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    // `"mullnight"`. Tightened from the previous `Action(_) |
    ...
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"mullnight\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // Pick a different index. The default theme is `mullnight`
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `mullnight`; entering the picker
            // captures `original_value = current value = mullnight`,
...
/// misleading: on dark themes (MullNight, TokyoNight, RosePine
/// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
/// only on the MullDay light theme is title darker. The
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::MullNight => crate::theme::Theme::mullnight(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::MullDay => crate::theme::Theme::mullday(),
        ...
        crate::theme::ThemeKind::Auto => crate::theme::Theme::mullnight(),
    };
    assert_ne!(
