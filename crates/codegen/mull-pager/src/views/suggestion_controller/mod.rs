...
//! `palmshed.ai/suggest` request through the Effect pipeline. Stale responses
...
/// Parsed ghost suggestion from an ACP `palmshed.ai/suggest` response.
#[derive(Debug, Clone)]
...
/// A single completion item from an ACP `palmshed.ai/suggest` response.
...
// (e.g. mull-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
/// Parsed response from an ACP `palmshed.ai/suggest` request.
#[derive(Debug, Clone)]
...
impl SuggestResponseParsed {
    /// Parse a raw JSON value from an ACP `palmshed.ai/suggest` response.
...
/// Wire `limit` for `palmshed.ai/suggest` fetches. Matches the shell file
...
pub struct SuggestionController {
    ...
    /// `MULL_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `MULL_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `palmshed.ai/suggest` request.
    /// Resolved at construction from `MULL_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: mull_config::env_bool("MULL_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: mull_config::env_bool("MULL_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("MULL_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
    ...
    /// Called when an ACP `palmshed.ai/suggest` response arrives, with the text and
