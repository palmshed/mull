...
//! the generic [`CodeBlockSpan`](mull_markdown::CodeBlockSpan) API) and
...
use mull_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](mull_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](mull_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a mull_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `MullDay` is the only light theme; every other concrete theme (and the
/// `MullNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `mull_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::MullDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_mullday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::MullDay),
            "MullDay is the light theme"
        );
        for dark in [
            ThemeKind::MullNight,
