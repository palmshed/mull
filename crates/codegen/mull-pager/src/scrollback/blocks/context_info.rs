...
use mull_shell::session::{ContextInfo, count_detail};
...
mod tests {
    use super::*;
    use mull_shell::session::TokenUsageCategory;
    ...
    fn bar_and_legend_reconcile_used_with_overhead_excluding_tools() {
        ...
        let block = ContextInfoBlock::new(snap, "mull");
        let theme = test_theme();
        let lines = block.build_lines(&theme, BarLayout::WIDE);
