...
impl<'a> EntryRenderer<'a> {
    ...
    /// cache size off-screen entries on a bulk load (`mull -r`) without word-wrapping
...
impl Renderable for EntryRenderer<'_> {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        for line in output.lines.iter().skip(content_skip as usize) {
            ...
            // Paint the line background AFTER writing text so set_style does not
            // get overwritten by set_line_safe (which replaces the full cell style).
...
mod tests {
    ...
    fn gutter_keeps_code_block_background_on_no_background_block() {
        ...
        // carries no code background while this test's local `mullnight()`
    ...
    fn affordance_row_adds_one_row_per_diagram() {
        ...
        // invariant a bulk load (`mull -r`) relies on to avoid clipping.
