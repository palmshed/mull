#
# Mull CLI installer for PowerShell — https://palmshed.ai/cli/install.ps1
#
# Auth: MULL_DEPLOYMENT_KEY env var (takes precedence) or ~/.mull/auth.json from `mull login`.
# Env: MULL_CHANNEL (stable|alpha|enterprise, default: stable), MULL_BIN_DIR, MULL_PROXY_URL
#
# Usage:
#   irm https://palmshed.ai/cli/install.ps1 | iex                                       # latest stable
#   & ([scriptblock]::Create((irm https://palmshed.ai/cli/install.ps1))) -Version 0.1.42 # specific version
#   $env:MULL_VERSION="0.1.42"; irm https://palmshed.ai/cli/install.ps1 | iex           # specific version (alt)
#   $env:MULL_DEPLOYMENT_KEY="<key>"; irm https://palmshed.ai/cli/install.ps1 | iex
#
...
if (-not $Version -and $env:MULL_VERSION) {
    $Version = $env:MULL_VERSION
}

# This script is Windows-only. PS 5.1 has no Platform property and only runs on Windows.
if ($PSVersionTable.Platform -and $PSVersionTable.Platform -ne 'Win32NT') {
    Write-Error "This installer is for Windows. On macOS/Linux, use: curl -fsSL https://palmshed.ai/cli/install.sh | bash"
    exit 1
}

$MullDir = Join-Path $env:USERPROFILE '.mull'
...
function Read-MullToken([string]$Scope) {
    $authFile = Join-Path $MullDir 'auth.json'
...
$OidcScope = 'https://auth.palmshed.ai::b1a00492-073a-47ea-816f-4c329264a828'
$LegacyScope = 'https://accounts.palmshed.ai/sign-in'
$AuthSource = ''

if ($env:MULL_DEPLOYMENT_KEY) {
    $AuthSource = 'deployment key'
    Write-Host 'Auth: using deployment key.' -ForegroundColor DarkGray
} else {
    $oidcToken = Read-MullToken $OidcScope
    $legacyToken = Read-MullToken $LegacyScope
    if ($oidcToken) {
        $AuthSource = 'auth.json (oidc)'
        Write-Host 'Auth: using OIDC token from ~/.mull/auth.json.' -ForegroundColor DarkGray
    } elseif ($legacyToken) {
        $AuthSource = 'auth.json (legacy)'
        Write-Host 'Auth: using legacy token from ~/.mull/auth.json.' -ForegroundColor DarkGray
    }
}
...
$BaseUrlPrimary = 'https://palmshed.ai/cli'
$BaseUrlFallback = 'https://storage.googleapis.com/mull-build-public-artifacts/cli'
$DownloadDir = Join-Path $MullDir 'downloads'
$BinDir = if ($env:MULL_BIN_DIR) { $env:MULL_BIN_DIR } else { Join-Path $MullDir 'bin' }

New-Item -ItemType Directory -Path $DownloadDir -Force | Out-Null
New-Item -ItemType Directory -Path $BinDir -Force | Out-Null

$Channel = if ($env:MULL_CHANNEL) { $env:MULL_CHANNEL } else { 'stable' }

# Pick a working BaseUrl: try Cloudflare-fronted palmshed.ai first, fall back to
...
if ($AuthSource) {
    Write-Host "Installing Mull $resolvedVersion ($platform, $AuthSource)..." -ForegroundColor Cyan
} else {
    Write-Host "Installing Mull $resolvedVersion ($platform)..." -ForegroundColor Cyan
}

# --- Download binary ---

$binaryPath = Join-Path $DownloadDir "mull-$platform.exe"
$artifactBase = "$BaseUrl/mull-$resolvedVersion-$platform"
...
foreach ($binName in @('mull.exe', 'agent.exe')) {
...
Write-Host "  Installed to $BinDir\mull.exe and $BinDir\agent.exe." -ForegroundColor DarkGray

# --- Generate completions (best-effort) ---

$completionsDir = Join-Path (Join-Path $MullDir 'completions') 'powershell'
try {
    New-Item -ItemType Directory -Path $completionsDir -Force | Out-Null
    & (Join-Path $BinDir 'mull.exe') completions powershell 2>$null |
        Set-Content (Join-Path $completionsDir 'mull.ps1') -ErrorAction SilentlyContinue
} catch {}

# --- Persist installer config ---

$ConfigFile = Join-Path $MullDir 'config.toml'
...
if ($env:MULL_DEPLOYMENT_KEY) {
    $ProxyUrl = if ($env:MULL_PROXY_URL) { $env:MULL_PROXY_URL } else { 'https://openrouter.ai/api/v1' }
    Write-Host '  Fetching deployment config...' -ForegroundColor DarkGray
    try {
        $headers = @{ 'Authorization' = "Bearer $($env:MULL_DEPLOYMENT_KEY)" }
    ...
    if ($deployResponse) {
        ...
        $managedConfigPath = Join-Path $MullDir 'managed_config.toml'
        $requirementsPath = Join-Path $MullDir 'requirements.toml'
...
Write-Host "Mull $resolvedVersion installed to $BinDir\mull.exe" -ForegroundColor Green
...
if ($pathEntries -notcontains $BinDir) {
    ...
    # Update current session so mull works immediately.
...
Write-Host ''
Write-Host "Run 'mull' or 'agent' to get started!" -ForegroundColor Cyan
