#!/bin/bash
#
# Mull CLI installer — https://palmshed.ai/cli/install.sh
#
...
#
# Usage:
#   curl -fsSL https://palmshed.ai/cli/install.sh | bash            # latest stable
#   curl -fsSL https://palmshed.ai/cli/install.sh | bash -s 0.1.42  # specific version
#   MULL_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://palmshed.ai/cli/install.sh)
#
...
if [ "$os" = "windows" ]; then
    ...
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded mull failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.mull/bin and ~/.mull/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/mull"
