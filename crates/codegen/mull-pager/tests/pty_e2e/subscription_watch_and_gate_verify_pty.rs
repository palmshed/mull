...
const PAID_TIER: &str = "SuperMullPro";

/// Display name delivered via `/settings` `subscription_tier_display`.
const PAID_TIER_DISPLAY: &str = "SuperMull Pro";
...
///
/// Proto `prod_auth.SubscriptionTier`: 5 = `supermull_heavy` = live
/// `/user` string `SuperMullPro`. Must match
...
/// Like [`seed_fake_oauth`], but under the `MULL_LOCAL_AUTH` dev issuer
/// (`http://localhost:22255`). Two reasons: `is_mull_oauth2_issuer()` accepts
...
/// call to auth.palmshed.ai (hermetic, no CI-network flake). Pair with
/// `MULL_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let mull_home = content.home().join(".mull");
    std::fs::create_dir_all(&mull_home).expect("create temp .mull");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    env_refs.push(("MULL_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("MULL_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn startup_gate_shows_paywall_for_free_user_after_live_check() {
    ...
    content.server().set_settings(json!({
        ...
        "gate_url": "https://palmshed.ai/supergrok?referrer=mull-build",
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("MULL_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
