...
/// turn and the turn-end `palmshed.ai/suggestPrompt` call (the mock's fixed mode
...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`mull-0.1`, the shell's built-in default)
    ...
    env.retain(|(k, _)| k != "MULL_PROMPT_SUGGESTIONS");
    env.push(("MULL_PROMPT_SUGGESTIONS".into(), "true".into()));
