...
use mull_pager_pty_harness::PtyHarness;
pub(crate) use mull_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`mull-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `mull-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `MULL_SCROLL_SPEED`) appended to the controller's env.
