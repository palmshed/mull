...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // Trace uploads are gated on first-party Palmshed OAuth (`is_mull_auth()`:
    // AuthMode::Oidc + Palmshed issuer); the harness's PALMSHED_API_KEY is ApiKey mode
    ...
    env_refs.retain(|(k, _)| *k != "MULL_TRACE_UPLOAD");
    env_refs.push(("MULL_TRACE_UPLOAD", "true"));
    env_refs.push(("MULL_TELEMETRY_TRACE_UPLOAD", "true"));
    env_refs.push(("MULL_UPLOAD_QUEUE_AUTH_PROBE_SECS", "2"));
    ...
    //
    // Accounting (mull-file-utils upload queue):
    ...
    //   not fire inside this 3s window; with `MULL_UPLOAD_QUEUE_AUTH_PROBE_SECS=2`
