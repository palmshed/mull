...
//!
//! Uses `mull-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.mull/auth.json` from `MULL_AUTH_JSON` (path) or the
//! developer's `~/.mull/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p mull-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use mull_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.mull/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("MULL_AUTH_JSON") {
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    let _ = std::fs::create_dir_all(&mull);
    if let Some(src) = auth_json_source() {
        let dest = mull.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.mull/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("MULL_HOME".into(), mull.display().to_string()),
        ...
        ("TMUX".into(), "".into()),
        // Do not set PALMSHED_API_KEY — prefer OIDC entry in auth.json (pty_e2e pattern).
    ];
    // Pin the feature gate explicitly so the cycle is deterministic regardless
    // of the developer's shell. `MULL_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (mull_config::
    ...
    env.push((
        "MULL_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("MULL_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (mull-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "MULL_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
