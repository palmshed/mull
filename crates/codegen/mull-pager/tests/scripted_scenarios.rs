...
//!
//! These tests exercise the real `mull-pager` binary through a PTY using
...
/// enterprise deploy report: with `MULL_GOAL=1`, `/goal` must
...
/// explicitly off (`MULL_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `MULL_FOLDER_TRUST=1` and a git repo that
...
/// also pins `MULL_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
