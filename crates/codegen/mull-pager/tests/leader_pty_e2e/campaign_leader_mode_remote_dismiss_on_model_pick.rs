...
#[ignore = "PTY e2e; run with cargo test -p mull-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared MULL_HOME so every spawn elects/attaches to the
    // same leader (mirrors `LeaderCluster`).
    let mull_home = content.home().join(".mull");
    std::fs::create_dir_all(&mull_home).expect("create MULL_HOME");
    std::fs::write(
    ...
    // Session (OAuth) auth, not the harness's default PALMSHED_API_KEY: the
    ...
    let state_path = mull_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(mull_home.join("config.toml")).expect("read config.toml");
    assert!(
