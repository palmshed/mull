...
Mull reads configuration from local config files, environment variables, and
...
2. **Environment variables** (e.g., `PALMSHED_API_KEY`, `MULL_MEMORY`)
3. **config.toml** (`~/.mull/config.toml`)
...
Location: `~/.mull/config.toml`

If the file does not exist, Mull uses built-in defaults. Specify only the values you want to override.
...
**Disable vim input mode**); Mull writes your choice to `[ui] simple_mode` in
`config.toml`.
...
The setting can also be overridden with the `MULL_DEFAULT_SELECTED_PERMISSION`
...
(`/settings` → **Vim scrollback navigation**). Mull writes the change to
`[ui] vim_mode` in `~/.mull/config.toml` immediately and applies it to every
...
`mull` launches. Configure it from `/settings` → **Default screen mode**
...
`MULL_SCROLL_SPEED`, `MULL_SCROLL_MODE`, `MULL_INVERT_SCROLL`
(`1`/`true`/`0`/`false`), and `MULL_SCROLL_LINES`. Precedence: env var →
...
allowed_domains = ["docs.rs", "palmshed.ai"]           # override the built-in allowlist
```
...
(`MULL_ASK_USER_QUESTION_TIMEOUT_ENABLED` /
`MULL_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed →
...
env_key = "PALMSHED_API_KEY"               # env var(s) holding the API key; string or array (first set, non-empty wins)
...
Credential resolution: `api_key` > `env_key` > signed-in session token > `PALMSHED_API_KEY`.
...
MCP servers can also be configured per-project in `.mull/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.mull/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.mull/config.toml` (current dir) > `<repo-root>/.mull/config.toml` > `~/.mull/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `MULL_MEMORY=1`).
...
[subagents.models]
explore = "mull-build"               # route to different models
```
...
`mull inspect` reports cells that still need session-start resolution as
...
The `[hints]` table holds small persisted UI preferences — mostly "stop asking me" opt-outs. Mull writes these for you when you pick a "don't ask again" / "reset in config.toml" option in the TUI, but you can edit or remove them by hand. Deleting a key restores the default behavior.

`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.mull/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "mull"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `mull`. |
...
| Mull Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

When `method = "auto"`, Mull detects the terminal brand and selects the best
...
`$MULL_EVENT`, `$MULL_MESSAGE`, and `$MULL_SESSION_ID`.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'Mull' -message '$MULL_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$MULL_MESSAGE' ntfy.sh/my-mull-alerts"
...
fires, try `condition = "always"` as a fallback. Mull supports focus tracking
...
Set these only to point telemetry at your own infrastructure or to turn parts of it off. The built-in endpoint and credentials are managed by Mull; leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= MULL_EXTERNAL_OTEL)
...
|----------|-------------|
| `PALMSHED_API_KEY` | API key from console.palmshed.ai |
| `MULL_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `MULL_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `MULL_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `MULL_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `MULL_OIDC_ISSUER` | OIDC issuer URL |
| `MULL_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `MULL_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `MULL_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `MULL_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `MULL_AGENT` | Custom agent definition path or name |
| `MULL_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `MULL_LOG_FILE` | Write logs to this file path (the value is used verbatim as the path) |
| `RUST_LOG` | Log level filter (for example `debug`); controls the `MULL_LOG_FILE` log and headless stderr output |
...
| `MULL_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `MULL_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `MULL_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `MULL_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.mull/config.toml` | Main configuration file |
| `~/.mull/pager.toml` | TUI appearance configuration |
...
| `~/.mull/lsp.json` | LSP server configuration (user-scoped) |
| `~/.mull/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
| `.mull/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some configuration can be set per-project by placing files in `.mull/` within your repository:

| File | What it configures |
|------|--------------------|
| `.mull/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.mull/config.toml`) |
| `.mull/skills/` | Project-specific skills |
| `.mull/hooks/` | Project-specific lifecycle hooks |
| `.mull/agents/` | Project-specific agent definitions |
| `.mull/lsp.json` | LSP server configuration |
| `.mull/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.mull/lsp.json` | All projects |
| Project | `.mull/lsp.json` | Current repository |
...
1. **Project** -- `.mull/lsp.json`
2. **User** -- `~/.mull/lsp.json`
