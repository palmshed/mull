...
Headless mode runs Mull non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
Mull processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
On failure, Mull emits an error object (process exit non-zero). Prompt-level
...
Mull may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `mull -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
mull -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
mull -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
mull -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
mull -p "What's the secret number?" --resume abc123
```
...
mull -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
mull --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  mull -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
Mull's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(mull -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
mull -p "Format all files" --yolo

# Run tests and fix failures
mull -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| ------------------------------- | ------------------------------------------------------------- |
| `PALMSHED_API_KEY`        | API key for authentication (required when no browser login)   |
| `MULL_HOME`                    | Override config directory (default: `~/.mull`)                |
| `MULL_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
| `RUST_LOG`                     | Log level filter (e.g. `debug`). Headless logs to stderr.     |

For CI environments without browser access, set `PALMSHED_API_KEY` with an API key from [console.palmshed.ai](https://console.palmshed.ai):

```bash
export PALMSHED_API_KEY="palmshed-..."
mull -p "Run the test suite" --yolo
```
...
- **`PALMSHED_API_KEY`** — simplest for CI. See [Environment Variables](#environment-variables-for-headless) above.
- **`mull login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`mull login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `mull -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug mull -p "..." 2> debug.log`.

---

## Project Root Discovery

When Mull starts, it discovers the project root by walking upward from `--cwd`
...
Mull discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
Mull stores data in `~/.mull` (override with `MULL_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.mull` read-only:

- Pre-populate `auth.json` or use `PALMSHED_API_KEY`
- Session persistence fails silently (ephemeral)
- Update checks log a warning and skip

```bash
export PALMSHED_API_KEY="palmshed-..."
export MULL_DISABLE_AUTOUPDATER=1
mull -p "..." --no-auto-update
```
...
| `MULL_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`MULL_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `MULL_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$MULL_HOME/bin/mull`).
...
- Resume: `mull -p "continue" --resume "<id>"` or `mull -p "continue" --continue`
