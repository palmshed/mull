...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.mull/agents/` or `~/.mull/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.mull/personas/`; applied during subagent resolution |
...
| **Examples** | `mull-build`, `explore`, `plan` | `researcher`, `concise` |
...
Mull discovers file-based personas from these locations, in priority order:

- `.mull/personas/*.toml` (project)
- `~/.mull/personas/*.toml` (user)
...
> **Note:** Mull applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Mull resolves the effective model and reasoning effort in this order, highest priority first:
...
Mull manages worktrees through the `palmshed.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "mull-build"               # route explore to a specific model
```
...
# instructions_file = ".mull/personas/concise.md"  # or load from a file
```

Mull also discovers roles from `.mull/roles/*.toml` and personas from `.mull/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Mull shows running and finished work in side panes on the agent screen:
