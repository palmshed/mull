...
- **`mull dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.mull/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Mull · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Mull's sibling views (
`tasks_pane`):
...
Set `MULL_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `mull --bg`") would list
