...
Agent mode runs Mull as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`mull -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `mull agent` command and apply to every mode. Pass them before the mode name, for example `mull agent --model mull-build stdio`. The `stdio` subcommand itself takes no options.

| Flag                       | Description                                                       |
| -------------------------- | ---------------------------------------------------------------- |
| `-m, --model <MODEL>`      | Set the model ID (for example, `mull-build`).                    |
...
```bash
mull agent serve --bind 127.0.0.1:2419 --secret <token>
```

Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent generates a token and prints it at startup; you can also supply one through the `MULL_AGENT_SECRET` environment variable. The agent persists across reconnections, so a client can disconnect and later resume in-flight work.
...
Beyond the base ACP protocol, Mull defines extension methods under the `palmshed.ai/` prefix for Palmshed-specific functionality. These cover:

| Category                   | Prefix               | Examples                                         |
| -------------------------- | -------------------- | ------------------------------------------------ |
| **Filesystem**             | `palmshed.ai/fs/*`          | `list`, `exists`, `read_file`, `write_file`      |
| **Git**                    | `palmshed.ai/git/*`         | `status`, `stage`, `commit`, `diffs`, `discard`  |
| **Git Worktree**           | `palmshed.ai/git/worktree/*`| `create`, `remove`, `apply`, `list`, `gc`        |
| **Search**                 | `palmshed.ai/search/*`      | `fuzzy/open`, `fuzzy/change`, `content`          |
| **Terminal**               | `palmshed.ai/terminal/*`    | `create`, `kill`, `output`, `wait_for_exit`      |
| **Session Management**     | `palmshed.ai/session/*`     | `fork`, `resolve_local_for_worktree_resume`      |
| **Conversation & History** | `palmshed.ai/*`             | `prompt_history`, `rewind/*`, `compact_conversation` |
| **Authentication**         | `palmshed.ai/auth/*`        | `get_url`, `submit_code`                         |
| **Feedback & Telemetry**   | `palmshed.ai/*`             | `feedback`, `telemetry/*`                        |

The tables here show representative methods in each category. The `palmshed.ai/*` set is Palmshed-specific and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
...
| -------------------------- | ------------------------------------ |
| `palmshed.ai/search/fuzzy/status` | Fuzzy search results update          |
| `palmshed.ai/git/worktree/status` | Worktree creation progress           |
| `palmshed.ai/fs_notify`           | Filesystem change notification       |
| `palmshed.ai/fs/index`            | Full file index update               |
| `palmshed.ai/fs/index/delta`      | Incremental file index update        |
| `palmshed.ai/session_notification`| Session-specific updates (diff review, retry state, auto-compact) |
| `palmshed.ai/session/update`      | Session update (tool calls, content) |
