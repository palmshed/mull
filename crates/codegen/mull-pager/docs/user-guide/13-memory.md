...
Memory lets Mull recall facts, decisions, and patterns from earlier sessions. Mull indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each Mull session starts fresh: the model knows nothing about previous sessions. When you enable memory, Mull can:
...
3. `MULL_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
|----------|-------|-------------|
| `~/.mull/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.mull/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.mull/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |

Mull suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
When a session ends, Mull saves a structured metadata summary to that session's daily log. The summary contains:
...
Mull builds the summary from conversation metadata without an LLM call, without added latency. Mull skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask Mull to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.mull/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

Mull records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, Mull opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, Mull shows `Memory saved to ~/.mull/memory/MEMORY.md`.

### Forget

Ask Mull to forget something, and it finds and removes the matching entry:

```
> forget the snake_case convention
```

Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.mull/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.

### Recall

Ask what Mull remembers:

```
> what do you remember?
```

Mull searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.

### Direct Editing

You can edit memory files directly under `~/.mull/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
When you save a note with `/remember`, Mull confirms in the scrollback:

```
Memory saved to ~/.mull/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what Mull has stored.
...
Dream also runs automatically. By default, Mull checks the consolidation gates when a session ends and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, Mull automatically searches memory for content relevant to the current project and injects it as context. This means Mull starts with knowledge from previous sessions without a reminder.
...
Mull searches memory automatically, but you can also trigger searches manually in the chat:
...
The `mull memory` command manages memory from the shell. It has one subcommand, `clear`:
...
mull memory clear --workspace

# Clear the global MEMORY.md
mull memory clear --global

# Clear both workspace and global memory
mull memory clear --all

# Skip the confirmation prompt (-y is the short form)
mull memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.mull/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.mull/memory/` for external edits and reindex |
...
| `min_score` | unset | Score threshold for first-turn results. When unset, Mull applies no threshold, which is equivalent to `0.0`. |
...
| `flush_model` | unset | Model for the flush turn. When unset, Mull uses the session's primary model. |
...
When a session memory is old, Mull attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, Mull watches `~/.mull/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `mull inspect` output.
2. Check the flag: `mull --experimental-memory` or `MULL_MEMORY=1`.
3. Check for `--no-memory` or `MULL_MEMORY=0` overriding your config.
...
```bash
ls ~/.mull/memory/
cat ~/.mull/memory/MEMORY.md
$EDITOR ~/.mull/memory/MEMORY.md
```

### Debug Logging

```bash
RUST_LOG=debug MULL_LOG_FILE=/tmp/mull.log mull
grep "memory" /tmp/mull.log
```
