...
MCP (Model Context Protocol) servers extend Mull with external tool integrations. They let Mull interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to Mull over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside Mull's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.mull/config.toml` under `[mcp_servers.<name>]` sections.

### stdio Transport (Local Process)

Mull spawns a local process and communicates over stdin/stdout:
...
> `MULL_MCP_STARTUP_TIMEOUT_SECS` (seconds). A per-server `startup_timeout_sec`
...
>
> - env `MULL_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Mull-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.mull/config.toml`) **or repo-level**
>   (`.mull/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.mull/config.toml` >
...
mull mcp list
mull mcp list --json          # Machine-readable output

# Add a stdio server. Everything after -- is the server command, so flags
# like -y reach the server instead of being parsed by mull.
mull mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
mull mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
mull mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
mull mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
mull mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
mull mcp remove github

# Diagnose a server's configuration and connectivity
mull mcp doctor               # Check every configured server
mull mcp doctor github        # Check one server
mull mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `mull mcp add` writes to `~/.mull/config.toml` (`--scope user`). Use `--scope project` to write to `.mull/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `mull mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`mull mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.
...
MCP servers can be configured per-project by placing a `.mull/config.toml` in your repository:
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. Mull handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers Mull's own OAuth client with the provider.

Mull walks from the current directory up to the git repo root, loading `.mull/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.mull/config.toml` | All projects | Lowest |
| `<repo-root>/.mull/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Mull reads most other config sections only from `~/.mull/config.toml`.
...
You can enable or disable MCP servers during a session without restarting Mull.
...
Mull loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native Mull config | `~/.mull/config.toml`, `.mull/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.mull/config.toml` or the corresponding environment variable (`MULL_CURSOR_MCPS_ENABLED`, `MULL_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `mull inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, Mull handles the credential flow automatically. When an MCP server requests OAuth credentials, Mull opens a browser-based authorization flow and stores the resulting tokens for future use.
...
You must authenticate OAuth-based MCP servers before you can use them. Mull stores the resulting tokens under `~/.mull/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). Mull expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
For stdio servers, Mull captures the process's standard error to `~/.mull/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.mull/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `mull inspect` to see all loaded MCP servers and their sources:

```bash
mull inspect          # Human-readable
mull inspect --json   # Machine-readable
```
