...
Mull supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.mull/hooks/`
- **Project hooks** — from `.mull/hooks/` in your repo
...
Plugin hooks automatically receive `MULL_PLUGIN_ROOT` and `MULL_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.mull/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `mull plugin marketplace add <source>`)
...
Sources land in `~/.mull/config.toml`:
...
Or in `~/.mull/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../mull-hooks/examples/README.md) — ready-to-use sample hooks
