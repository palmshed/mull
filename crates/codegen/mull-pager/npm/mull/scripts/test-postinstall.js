...
// postinstall.js and the bin/mull trampoline.
//
...
function makeTmpDir() {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'mull-test-'));
}
...
// ─── Extracted logic (mirrors postinstall.js and bin/mull exactly) ─────

/** Semver-aware descending sort for "mull-X.Y.Z" filenames. */
...
function installVersionedBinary(vendoredBinPath, version, canonicalDir) {
    const canonicalPath = path.join(canonicalDir, 'mull');
    fs.mkdirSync(canonicalDir, { recursive: true });

    const versionedName = `mull-${version}`;
...
function cleanupOldVersions(canonicalDir, currentVersionedName) {
    ...
    const versionedBinaries = entries
        .filter(e => e.startsWith('mull-') && !e.includes('.tmp.') && !e.includes('.link.') && e !== currentVersionedName)
...
/** Bootstrap canonical from vendored (same as bin/mull trampoline). */
...
test('creates versioned binary and symlink on fresh install', () => {
    ...
    try {
        const vendored = path.join(dir, 'vendored-mull');
...
test('symlink swap is atomic (no intermediate missing state)', () => {
    ...
    try {
        const binDir = path.join(dir, 'bin');
        const canonicalPath = path.join(binDir, 'mull');
...
test('handles upgrade from old-style regular file to versioned symlink', () => {
    ...
    try {
        ...
        // Simulate old installation: mull is a regular file
...
test('three sequential upgrades: v1 -> v2 -> v3 all coexist', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mull')), 'grok-0.1.3');
...
test('cleanup with no old versions is a no-op', () => {
    ...
    try {
        ...
        const entries = fs.readdirSync(binDir).filter(e => e.startsWith('mull-'));
...
test('cleanup ignores .tmp. and .link. files', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(binDir, 'mull.link.12345'), 'crashed-link');
        ...
        assert.ok(fs.existsSync(path.join(binDir, 'mull.link.12345')), 'link file should not be touched');
    } finally {
        cleanup(dir);
    }
});
...
test('bootstrapCanonical creates versioned binary from vendored', () => {
    ...
    try {
        ...
        assert.strictEqual(result, path.join(binDir, 'mull'));
...
test('full lifecycle: install, upgrade, cleanup', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mull')), 'grok-0.1.142');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'mull'), 'utf8'), 'v3');
    } finally {
        cleanup(dir);
    }
});
...
test('downgrade: installing older version than current', () => {
    ...
    try {
        ...
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mull')), 'grok-0.1.140');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'mull'), 'utf8'), 'v1');
...
test('non-mull files in bin dir are not touched by cleanup', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });
        ...
        assert.ok(fs.existsSync(path.join(binDir, 'other-tool')), 'non-mull file should not be touched');
        assert.ok(fs.existsSync(path.join(binDir, 'README.md')), 'non-mull file should not be touched');
    } finally {
        cleanup(dir);
    }
});
...
// ═══════════════════════════════════════════════════════════════════════
// mull vs mull-pager Isolation Tests
// ═══════════════════════════════════════════════════════════════════════
 ...
 * Uses prefix + leading digit to avoid mull-* matching mull-pager-*.
 */
...
test('installing both mull and mull-pager creates independent symlinks', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored');
        fs.writeFileSync(vendored, 'mull-binary');
        const vendoredPager = path.join(dir, 'vendored-pager');
        fs.writeFileSync(vendoredPager, 'pager-binary');

        installNamedBinary(vendored, 'mull', '0.1.141', binDir);
        installNamedBinary(vendoredPager, 'mull-pager', '0.1.141', binDir);

        // Both symlinks exist and point to correct targets
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mull')), 'grok-0.1.141');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mull-pager')), 'mull-pager-0.1.141');

        // Both versioned files exist with correct content
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'grok-0.1.141'), 'utf8'), 'mull-binary');
        assert.strictEqual(fs.readFileSync(path.join(binDir, 'mull-pager-0.1.141'), 'utf8'), 'pager-binary');
    } finally {
        cleanup(dir);
    }
});

test('cleanup of mull-* does not remove mull-pager-*', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });
        ...
        // Current mull
        fs.writeFileSync(path.join(binDir, 'grok-0.1.141'), 'current-mull');

        // mull-pager versions (should not be touched)
        fs.writeFileSync(path.join(binDir, 'mull-pager-0.1.138'), 'old-pager-1');
        fs.writeFileSync(path.join(binDir, 'mull-pager-0.1.139'), 'old-pager-2');
        fs.writeFileSync(path.join(binDir, 'mull-pager-0.1.141'), 'current-pager');

        cleanupOldVersionsNamed(binDir, 'mull', '0.1.141');

        // mull cleanup: current + N-1 kept, older removed
        assert.ok(fs.existsSync(path.join(binDir, 'grok-0.1.141')), 'current mull should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'grok-0.1.140')), 'N-1 mull should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'grok-0.1.139')), 'N-2 mull should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'grok-0.1.138')), 'N-3 mull should be removed');

        // ALL mull-pager versions must be untouched
        assert.ok(fs.existsSync(path.join(binDir, 'mull-pager-0.1.138')), 'mull-pager-0.1.138 must survive mull cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'mull-pager-0.1.139')), 'mull-pager-0.1.139 must survive mull cleanup');
        assert.ok(fs.existsSync(path.join(binDir, 'mull-pager-0.1.141')), 'mull-pager-0.1.141 must survive mull cleanup');
    } finally {
        cleanup(dir);
    }
});

test('cleanup of mull-pager-* does not remove mull-*', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        fs.mkdirSync(binDir, { recursive: true });

        // mull versions (should not be touched)
        fs.writeFileSync(path.join(binDir, 'grok-0.1.138'), 'old-grok-1');
        fs.writeFileSync(path.join(binDir, 'grok-0.1.139'), 'old-grok-2');
        fs.writeFileSync(path.join(binDir, 'grok-0.1.141'), 'current-mull');

        // Old mull-pager versions
        fs.writeFileSync(path.join(binDir, 'mull-pager-0.1.138'), 'old-pager-1');
        fs.writeFileSync(path.join(binDir, 'mull-pager-0.1.139'), 'old-pager-2');
        fs.writeFileSync(path.join(binDir, 'mull-pager-0.1.140'), 'old-pager-3');
        // Current pager
        fs.writeFileSync(path.join(binDir, 'mull-pager-0.1.141'), 'current-pager');

        cleanupOldVersionsNamed(binDir, 'mull-pager', '0.1.141');

        // mull-pager cleanup: current + N-1 kept, older removed
        assert.ok(fs.existsSync(path.join(binDir, 'mull-pager-0.1.141')), 'current pager should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'mull-pager-0.1.140')), 'N-1 pager should be kept');
        assert.ok(!fs.existsSync(path.join(binDir, 'mull-pager-0.1.139')), 'N-2 pager should be removed');
        assert.ok(!fs.existsSync(path.join(binDir, 'mull-pager-0.1.138')), 'N-3 pager should be removed');

        // ALL mull versions must be untouched
...
test('full dual-binary lifecycle: install, upgrade, cleanup both', () => {
    ...
    try {
        ...
        // v1
        fs.writeFileSync(vendored, 'mull-v1');
        fs.writeFileSync(vendoredPager, 'pager-v1');
        installNamedBinary(vendored, 'mull', '0.1.140', binDir);
        installNamedBinary(vendoredPager, 'mull-pager', '0.1.140', binDir);

        // v2
        fs.writeFileSync(vendored, 'mull-v2');
        fs.writeFileSync(vendoredPager, 'pager-v2');
        installNamedBinary(vendored, 'mull', '0.1.141', binDir);
        installNamedBinary(vendoredPager, 'mull-pager', '0.1.141', binDir);

        // v3
        fs.writeFileSync(vendored, 'mull-v3');
        fs.writeFileSync(vendoredPager, 'pager-v3');
        installNamedBinary(vendored, 'mull', '0.1.142', binDir);
        installNamedBinary(vendoredPager, 'mull-pager', '0.1.142', binDir);

        // Cleanup both independently
        cleanupOldVersionsNamed(binDir, 'mull', '0.1.142');
        cleanupOldVersionsNamed(binDir, 'mull-pager', '0.1.142');
        ...
        assert.ok(fs.existsSync(path.join(binDir, 'mull-pager-0.1.142')));
        assert.ok(fs.existsSync(path.join(binDir, 'mull-pager-0.1.141')));
        assert.ok(!fs.existsSync(path.join(binDir, 'mull-pager-0.1.140')));

        // Symlinks correct
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mull')), 'grok-0.1.142');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mull-pager')), 'mull-pager-0.1.142');
    } finally {
        cleanup(dir);
    }
});
...
test('mull installs normally regardless of platform key', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored-mull');
        ...
        for (const platform of ['darwin-arm64', 'linux-x64', 'linux-arm64']) {
            const result = installNamedBinary(vendored, 'mull', '0.1.150', binDir);
            assert.ok(fs.existsSync(result.versionedPath), `mull should install for ${platform}`);
...
test('mull-pager installs when vendored binary exists (darwin-arm64 path)', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendoredPager = path.join(dir, 'vendored-pager');
        ...
        const result = installNamedBinary(vendoredPager, 'mull-pager', '0.1.150', binDir);
        assert.ok(fs.existsSync(result.versionedPath), 'pager versioned binary should exist');
        assert.strictEqual(fs.readlinkSync(result.canonicalPath), 'mull-pager-0.1.150');
...
test('Linux pager vendor files are not required for mull install', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        ...
        fs.writeFileSync(path.join(vendorBase, 'darwin-arm64', 'mull-pager'), 'mac-pager');
        ...
        assert.ok(!fs.existsSync(path.join(vendorBase, 'linux-x64', 'mull-pager')));
        assert.ok(!fs.existsSync(path.join(vendorBase, 'linux-arm64', 'mull-pager')));

        // mull install should succeed independently
        const mullVendored = path.join(dir, 'vendored-mull');
        fs.writeFileSync(mullVendored, 'mull-linux');
        const result = installNamedBinary(mullVendored, 'mull', '0.1.150', binDir);
        assert.ok(fs.existsSync(result.versionedPath), 'mull should install without Linux pager');
    } finally {
        cleanup(dir);
    }
});

test('skipping pager install on Linux does not affect mull cleanup', () => {
    const dir = makeTmpDir();
    try {
        const binDir = path.join(dir, 'bin');
        const vendored = path.join(dir, 'vendored');

        // Install mull across two versions
        fs.writeFileSync(vendored, 'mull-v1');
        installNamedBinary(vendored, 'mull', '0.1.149', binDir);
        fs.writeFileSync(vendored, 'mull-v2');
        installNamedBinary(vendored, 'mull', '0.1.150', binDir);

        // Simulate Linux: only run mull cleanup, skip pager entirely
        cleanupOldVersionsNamed(binDir, 'mull', '0.1.150');

        assert.ok(fs.existsSync(path.join(binDir, 'grok-0.1.150')), 'current mull should exist');
        assert.ok(fs.existsSync(path.join(binDir, 'grok-0.1.149')), 'N-1 mull should be kept');
        assert.strictEqual(fs.readlinkSync(path.join(binDir, 'mull')), 'grok-0.1.150');
...
test('canonical pager from non-npm install is preserved on Linux', () => {
    ...
    try {
        ...
        // Simulate pager installed by install-mull.sh (not npm)
        const pagerVersioned = path.join(binDir, 'mull-pager-0.1.150');
        fs.writeFileSync(pagerVersioned, 'installer-pager-binary');
        fs.chmodSync(pagerVersioned, 0o755);
        const pagerCanonical = path.join(binDir, 'mull-pager');
        fs.symlinkSync('mull-pager-0.1.150', pagerCanonical);

        // Run mull-only install + cleanup (simulating Linux postinstall)
        const vendored = path.join(dir, 'vendored');
        fs.writeFileSync(vendored, 'mull-binary');
        installNamedBinary(vendored, 'mull', '0.1.150', binDir);
        cleanupOldVersionsNamed(binDir, 'mull', '0.1.150');
        ...
        assert.strictEqual(fs.readlinkSync(pagerCanonical), 'mull-pager-0.1.150');
    } finally {
        cleanup(dir);
    }
});
