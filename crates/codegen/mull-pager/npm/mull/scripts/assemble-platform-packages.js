...
//   1. Brotli-compresses the built binary into `../mull-<platform>/bin/<bin>.br`
...
// (`@palmshed/mull`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw mull binary is
...
const palmshedRoot = process.env.PALMSHED_ROOT || path.resolve(__dirname, '..', '..', '..', '..', '..');
const npmRoot = path.resolve(__dirname, '..', '..');

const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'mull-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `mull-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] mull-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'mull',
            envVar: 'MULL_DARWIN_ARM64',
            defaultSource: path.join(palmshedRoot, 'target', 'release', 'mull-pager'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'mull',
            envVar: 'MULL_DARWIN_X64',
            defaultSource: path.join(palmshedRoot, 'target', 'x86_64-apple-darwin', 'release', 'mull-pager'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'mull',
            envVar: 'MULL_LINUX_X64',
            defaultSource: path.join(palmshedRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'mull-pager'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'mull',
            envVar: 'MULL_LINUX_ARM64',
            defaultSource: path.join(palmshedRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'mull-pager'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'mull.exe',
            envVar: 'MULL_WIN32_X64',
            defaultSource: path.join(palmshedRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'mull-pager.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'mull.exe',
            envVar: 'MULL_WIN32_ARM64',
            defaultSource: path.join(palmshedRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'mull-pager.exe'),
        },
    ];
