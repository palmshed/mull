#!/usr/bin/env node
// Runs once after npm install/update. Reads the mull binary from the
// matching per-platform optional dependency (@xai/mull-<platform>)
// and installs it to ~/.mull/bin/ using versioned filenames:
//
//   Unix:    mull-<version>  +  mull  (symlink)
//   Windows: mull-<version>.exe  +  mull.exe  (copy)
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.mull', 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@xai/mull: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@xai/mull-${key}`;
    try {
...
if (!version) {
    console.error('@xai/mull: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@xai/mull: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@xai/mull: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running mull processes and try again.');
                return false;
            }
        }
    } else {
...
// Uses an exact prefix match + hyphen + digit to avoid mull-* matching mull-pager-*.
...
if (!platformDir) {
    console.error(`@xai/mull: platform package @xai/mull-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @xai/mull');
    process.exit(0);
}

installBinary('mull', platformDir, `mull${EXE}`);
cleanupOldVersions('mull');
cleanupOldVersions('mull-pager');

// Write installer config
const configDir = path.join(os.homedir(), '.mull');
...
// Persist the npm registry so `mull update` and the trampoline use the same one.
const npmRegistry = process.env.MULL_NPM_REGISTRY
    || (() => {
        try {
            const resolved = execSync(
                'npm config get @palmshed-official:registry',
...
// Set MULL_INSTALL_COMPLETIONS=1 to auto-generate to ~/.mull/completions.
const MULL_PATH = path.join(CANONICAL_DIR, `mull${EXE}`);
if (process.env.MULL_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.mull', 'completions');
        const bashPath = path.join(completionsDir, 'bash', 'mull.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_mull');
        fs.mkdirSync(path.dirname(bashPath), { recursive: true });
        fs.mkdirSync(path.dirname(zshPath), { recursive: true });
        const bashRes = spawnSync(MULL_PATH, ['completions', 'bash'], { encoding: 'utf8' });
        if (bashRes.status === 0) fs.writeFileSync(bashPath, bashRes.stdout);
        const zshRes = spawnSync(MULL_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.mull/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
    console.log('Tip: mull completions bash > ~/.local/share/bash-completion/completions/mull');
    console.log('     mull completions zsh  > ~/.zsh/completions/_mull');
}
