...
pub enum TerminalName {
    ...
    JetBrains,
    /// Mull Desktop (Electron app).
    #[strum(to_string = "Mull Desktop")]
    MullDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` mull is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches mull, which
    ...
    pub fn telemetry_snapshot(&self) -> mull_telemetry::events::TerminalTelemetry {
        ...
        mull_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: mull_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> mull_shared::session::FeedbackTerminalInfo {
        use mull_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(mull_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `mull ssh` can
    // forward env vars.
...
/// `mull-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "mulldesktop" => Some(TerminalName::MullDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.mull/pager.toml` and
