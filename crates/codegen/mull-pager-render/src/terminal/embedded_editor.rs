//! Detects whether mull is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` mull is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `mull-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
