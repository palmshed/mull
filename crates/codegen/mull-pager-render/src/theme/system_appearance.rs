...
/// in `config.toml`. When `None`, defaults to `MullNight` / `MullDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::MullDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::MullNight),
    }
}
