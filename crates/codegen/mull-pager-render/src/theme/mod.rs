...
//! The default theme is MullNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! MullNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::MullNight, ThemeKind::MullDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::MullNight => "mullnight",
            Self::TokyoNight => "tokyonight",
            Self::MullDay => "mullday",
    ...
    /// when quantized to 256 or 16 colors. MullNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::MullNight => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "mullnight" | "mull-night" | "dark" => Some(Self::MullNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "mullday" | "mull-day" | "light" | "day" => Some(Self::MullDay),
...
/// `"mullnight"` → `"Mull Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "mullnight" => "Mull Night",
        "mullday" => "Mull Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.mull/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::MullNight => Self::mullnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::MullDay => Self::mullday(),
            ...
            // if reached, fall back to MullNight.
            ThemeKind::Auto => Self::mullnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `MULL_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on mullnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., MullNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // MullNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    mull_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::MullNight.is_auto());
        assert!(!ThemeKind::MullDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::mullday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::mullnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::mullday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. MullNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::MullNight => Theme::mullnight(),
                ThemeKind::MullDay => Theme::mullday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_mullnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark MullNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::mullnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("mullnight"),
            Some(ThemeKind::MullNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::MullNight));
        assert_eq!(ThemeKind::from_name("mullday"), Some(ThemeKind::MullDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::MullDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("mullnight", ThemeKind::MullNight),
            ("mull-night", ThemeKind::MullNight),
            ("dark", ThemeKind::MullNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("mullday", ThemeKind::MullDay),
            ("mull-day", ThemeKind::MullDay),
            ("light", ThemeKind::MullDay),
            ("day", ThemeKind::MullDay),
