...
pub use mull_config::mull_home;

/// Path to `$MULL_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    mull_home().join("pager.toml")
}

/// User-facing label for the user mull directory (``~/.mull`` or ``$MULL_HOME``).
///
/// Derived from resolved [`mull_home()`] vs `mull_config::default_mull_home()`,
/// not from whether `MULL_HOME` is set in the environment.
pub fn display_mull_home_prefix() -> String {
    if mull_home() == mull_config::default_mull_home() {
        "~/.mull".to_string()
    } else {
        "$MULL_HOME".to_string()
    }
...
/// User-facing path under [`mull_home()`], e.g. ``~/.mull/config.toml``.
pub fn display_user_mull_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_mull_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`mull_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let mull = mull_home();
    if let Ok(rest) = path_buf.strip_prefix(&mull) {
        let prefix = display_mull_home_prefix();
...
/// True when `path` is under user [`mull_home()`] (not project `{cwd}/.mull`).
pub fn is_under_user_mull_home(path: &Path) -> bool {
    path.starts_with(mull_home())
}
...
mod tests {
    ...
    #[test]
    fn display_mull_home_prefix_default_install() {
        if std::env::var("MULL_HOME").is_ok() {
            return;
        }
        assert_eq!(display_mull_home_prefix(), "~/.mull");
    }

    #[test]
    fn display_user_mull_path_joins_relative() {
        let path = display_user_mull_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".mull") || path.contains("$MULL_HOME"));
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_mull() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.mull/memory/MEMORY.md");
