...
//! frame diff (see `mull_ratatui_inline::Terminal::flush_with_links`).
...
/// One path segment without spaces (`main.rs`, `.mull`, `@scope`). Leading `.`
/// matches dot-directories and `%` matches percent-encoded segments — mull
/// session media lives under `~/.mull/sessions/%2F…/images/1.jpg`.
...
/// Relative file path (`images/1.png`, `.mull/x.txt`) — one or more `/`-joined
...
/// media lives at `~/.mull/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    fn local_link_ignores_web_anchor_and_unknown() {
        ...
        assert!(local_link_to_file_target("https://palmshed.ai", &media).is_none());
    ...
    fn scan_content_x_offset_applied() {
        let line = make_line("https://palmshed.ai");
        let mut overlay = LinkOverlay::new();
    ...
    #[test]
    fn scan_detects_mull_session_media_path() {
        // Dot-directory (`.mull`), percent-encoded session segment, and a
        // trailing sentence period — the shape of `image_gen` output prose.
        let line = make_line("Saved to /Users/alice/.mull/sessions/%2Fabc/00000000/images/1.jpg.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.mull/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let row0 =
            make_line("Image generated and saved to /Users/alice/.mull/sessions/%2FUsers%2Fali");
        ...
        let expected_url = "file:///Users/alice/.mull/sessions/%252FUsers%252Fali\
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.mull/sessions/%2FUsers%2Fali"
            ) as u16
        );
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.mull/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*resolve_link_target(&link.target)
                    .and_then(|resolved| resolved.osc8_url)
                    .expect("url"),
                "file:///Users/me/.mull/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_file_path_with_dots_and_hyphens() {
        let line = make_line("Reading /tmp/mull-impl-summary.md now.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/mull-pager-retention-findings-2026-06-06.md";
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/mull-pager/src/render` should
        // NOT produce a link for the `/mull-pager/src/render` substring.
        let line = make_line("find crates/codegen/mull-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
