...
//!
//! Extracted from the `OpenSupermullUrl` dispatch handler so that any
...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("MULL_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "MULL_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    mull_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// The returned command is TTY-guarded via [`mull_tty_utils::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    mull_tty_utils::detach_std_command(&mut command);
    command
}
...
///
/// Used by the SuperMull upsell flow to attribute clicks to `referrer=mull-build`,
/// matching the OAuth consent screen and palmshed.ai/cli marketing links regardless of
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/mull session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://palmshed.ai/supergrok", "referrer", "mull");
        assert_eq!(out, "https://palmshed.ai/supergrok?referrer=mull");
    }

    #[test]
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://palmshed.ai/supergrok?referrer=other",
            "referrer",
            "mull",
        );
        assert_eq!(out, "https://palmshed.ai/supergrok?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param("https://palmshed.ai/supergrok?heavy=1", "referrer", "mull");
        assert_eq!(out, "https://palmshed.ai/supergrok?heavy=1&referrer=mull");
    }

    #[test]
    fn ensure_query_param_preserves_fragment() {
        // The current remote settings value uses a hash fragment for client-side
        // routing (`palmshed.ai/#supermull`); we still want the referrer attached.
        let out = ensure_query_param("https://palmshed.ai/#supergrok", "referrer", "mull");
        assert_eq!(out, "https://palmshed.ai/?referrer=mull#supergrok");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "mull");
        assert_eq!(out, "not a url");
    }

    #[test]
    fn ensure_query_param_url_encodes_value() {
        let out = ensure_query_param("https://palmshed.ai/supergrok", "referrer", "mull build");
        assert_eq!(out, "https://palmshed.ai/supergrok?referrer=mull+build");
    }
    ...
    fn browser_unavailable_message_includes_full_url() {
        let url = "https://palmshed.ai/supergrok?referrer=mull-build";
