//! Headless markdown analysis sharing Mull's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `mull-markdown` so analysis matches what Mull
//! Build actually renders 1:1.
//!
//! After parsing, Mull applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Mull uses to render markdown.
///
...
/// Offset event stream from Mull's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Mull's options; count elements and flag structural issues.
