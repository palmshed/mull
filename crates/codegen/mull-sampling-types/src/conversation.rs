...
pub enum HostedTool {
    ...
    /// This is Palmshed-specific — not part of the OpenAI Responses API, so it's
...
pub struct ConversationRequest {
    ...
    /// Custom headers for Palmshed tracking
    pub x_mull_conv_id: Option<String>,
    pub x_mull_req_id: Option<String>,
    pub x_mull_session_id: Option<String>,
    pub x_mull_turn_idx: Option<String>,
    pub x_mull_agent_id: Option<String>,
    pub x_mull_deployment_id: Option<String>,
    pub x_mull_user_id: Option<String>,
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-mull-doom-loop-check` header). Empty when the
...
// ---------------------------------------------------------------------------
//
// Part of the Mull Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/mull-compaction` operate over mull-build's
...
impl mull_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> mull_compaction::CompactionRole {
        use mull_compaction::CompactionRole;
        // mull-build has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // mull-build has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing mull-build history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<mull_compaction::CompactionFileRef> {
        // mull-build `UserItem`s carry only `Text`/`Image { url }` content
        // parts — there is no id+name attachment-ref concept like the chat harness's
        // `MullTurn`. The full-replace path does not read this; revisit if
        // image attachments need to survive into the `<mull_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl mull_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/mull-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`mull-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/mull-sampler/src/stream/chat_completions.rs])
...
impl From<ConversationRequest> for ChatCompletionRequest {
    fn from(req: ConversationRequest) -> Self {
        ...
        ChatCompletionRequest {
            ...
            x_mull_conv_id: req.x_mull_conv_id,
            x_mull_req_id: req.x_mull_req_id,
            x_mull_session_id: req.x_mull_session_id,
            x_mull_turn_idx: req.x_mull_turn_idx,
            x_mull_agent_id: req.x_mull_agent_id,
            x_mull_deployment_id: req.x_mull_deployment_id,
            x_mull_user_id: req.x_mull_user_id,
            trace: None,
        }
    }
}
...
fn build_responses_tools(req: &ConversationRequest) -> Vec<rs::Tool> {
    ...
    for hosted in &req.hosted_tools {
        match hosted {
            ...
            // XSearch is Palmshed-specific — not in async_openai's rs::Tool enum.
...
/// Return raw JSON tool definitions for Palmshed-specific hosted tools that
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_mull_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_mull_req_id = Some(req_id.into());
        self
    }
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // output without tools. The mull-shell agent does NOT use this path — a
...
/// ([crates/codegen/mull-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use mull_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // mull-build has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_btw_stripped_reasoning_produces_no_thinking_blocks() {
        ...
        // `strip_reasoning_blocks` in mull-chat-state. Here the assistant
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.mull/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.mull/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.mull/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.x_mull_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_mull_req_id, Some("req-456".to_string()));
    ...
    //      (earlier mull-build / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_mull_build_shape() {
