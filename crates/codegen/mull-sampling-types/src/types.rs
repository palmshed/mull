...
// Re-export ToolDefinition and FunctionTool from mull-tools.
...
pub use mull_tools::types::definition::{FunctionTool, ToolDefinition};
...
pub struct Usage {
    ...
    /// Palmshed extension: request price in USD ticks (1 USD = 1e10 ticks).
...
/// custom header fields for Palmshed request tracking, similar to
...
pub struct CreateResponseWrapper {
    ...
    /// Palmshed-specific tool definitions that can't be expressed via
...
/// Wrapper around `MessagesRequest` that adds custom header fields for Palmshed
...
mod tests {
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(json!({ "agentType": "mull" }).as_object().unwrap()))
                .is_none()
        );
    }
