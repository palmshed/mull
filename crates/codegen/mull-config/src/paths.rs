//! Filesystem locations for mull config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static MULL_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user mull directory (`~/.mull`, canonicalized) used when
/// `MULL_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`mull_home()`] is the default without duplicating the computation.
///
...
/// `~/.mull/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `mull_fast_worktree::db::resolve_mull_home` (deliberately standalone crate).
pub fn default_mull_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".mull")
}

/// Per-user config directory: `$MULL_HOME` or `~/.mull`. Created if needed.
pub fn mull_home() -> PathBuf {
    MULL_HOME
        .get_or_init(|| {
            let mull_home = if let Ok(v) = std::env::var("MULL_HOME") {
                PathBuf::from(v)
            } else {
                default_mull_home()
            };
...
/// The user-global mull home, but only when one genuinely resolves: `Some` when
/// `$MULL_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`mull_home()`], this never falls back to a cwd-relative `.mull`, so callers
/// that *scan* user-global mull resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.mull` tree for the user-global one when no home resolves.
pub fn user_mull_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("MULL_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(mull_home)
}

/// Canonical mull application path: `$MULL_HOME/bin/mull` (Unix) or `mull.exe` (Windows).
pub fn mull_application() -> PathBuf {
    mull_application_in(&mull_home())
}

/// [`mull_application`] under an explicit home instead of `$MULL_HOME`.
pub fn mull_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "mull.exe" } else { "mull" };
    home.join("bin").join(name)
}

/// System-wide config directory: `/etc/mull/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/mull"))
    } else {
        None
    }
}
...
/// `mull_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    mull_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = mull_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_mull_home_has_no_verbatim_prefix() {
        ...
        let home = default_mull_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".mull"));
    }
