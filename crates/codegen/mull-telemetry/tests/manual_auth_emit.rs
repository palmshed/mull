...
//! `mull-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use mull_telemetry::client;
use mull_telemetry::config::{TelemetryConfig, TelemetryMode};
use mull_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn manual_auth_posts_to_events_endpoint_as_mull_shell_manual_auth() {
    ...
    mull_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "mull-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no mull-shell-manual_auth POST received"
        );
