...
use mull_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
fn external_stream_gates_on_end_to_end() {
    ...
    mull_telemetry::log_event(mull_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "mull_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.mull.mull_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("mull-cli"),
        "service.name=mull-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("mull_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt = col::find_event(&collected, "mull_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "mull_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "mull_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "mull_code.session.count");
