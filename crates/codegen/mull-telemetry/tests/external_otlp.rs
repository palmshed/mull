...
fn external_stream_end_to_end() {
    ...
    let mut cfg = mull_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "MULL_EXTERNAL_OTEL" => Some("1".into()),
    ...
    cfg.client = mull_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    mull_telemetry::external::init(Some(cfg));
    assert!(mull_telemetry::external::is_active());
    ...
    assert!(!mull_telemetry::is_enabled());
    mull_telemetry::log_event(mull_telemetry::events::SessionNew {
        session_id: "sess-int-1".into(),
        client_identifier: None,
        client_version: None,
        is_git_repo: true,
        permission_mode: mull_telemetry::enums::PermissionMode::Ask,
    });
    mull_telemetry::log_event(mull_telemetry::events::SessionHarness {
        session_id: "sess-int-1".into(),
    ...
    mull_telemetry::log_event(mull_telemetry::events::PromptSubmitted {
    ...
    mull_telemetry::log_event(mull_telemetry::events::ModelResponseReceived {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("mull-cli")),
        "service.name=mull-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "mull_code.session_start",
        "mull_code.user_prompt",
        "mull_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "mull_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "mull_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "mull_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "mull_code.token.usage"));
    assert_eq!(
    ...
    mull_telemetry::external::shutdown();
    assert!(
