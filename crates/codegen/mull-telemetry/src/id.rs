...
//!
//! Extracted from `mull-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$MULL_HOME` is resolved through `mull-config::mull_home`.
...
/// This function caches the result in `$MULL_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = mull_config::mull_home().join("agent_id");
...
/// Returns true when workspace marker env vars (`PALMSHED_ROOT` and `PALMSHED_USER`) are set.
///
...
pub fn has_workspace_env_markers() -> bool {
    std::env::var("PALMSHED_ROOT").is_ok() && std::env::var("PALMSHED_USER").is_ok()
}

/// Opt-in special-user gate for telemetry.
///
/// Enabled only when `MULL_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("MULL_TELEMETRY_SPECIAL_USER").as_deref(),
