...
//!
//! Extracted from `mull-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `mull-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `mull-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "mull-shell-",
            EmitterOrigin::Workspace => "mull-workspace-",
        }
    }
}
...
/// event-name prefix (`mull-shell-*` vs `mull-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "mull-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "mull-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("mull-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("mull-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "mull-workspace-turn");
    }
