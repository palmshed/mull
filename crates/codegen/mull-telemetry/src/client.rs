...
//!
//! Extracted from `mull-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`mull-shell-` /
/// `mull-workspace-`). Unprefixed names pass through unchanged. Kept in
...
/// format for Mixpanel. Handles both CCP display names ("SuperMull Heavy")
/// and JWT-derived keys ("supermull_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "SuperMull Heavy" | "supermull_heavy" => "supermull_heavy",
        "SuperMull" | "supermull" => "supermull",
        "SuperMull Lite" | "supermull_lite" => "supermull_lite",
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("Mull"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("mull-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("mull-shell-turn"), "turn");
        assert_eq!(
            event_value("mull-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("mull-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("mull-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("SuperMull"), "supermull");
        assert_eq!(normalize_tier("SuperMull Heavy"), "supermull_heavy");
        assert_eq!(normalize_tier("supermull_heavy"), "supermull_heavy");
        ...
        assert_eq!(normalize_tier("SuperMull Lite"), "supermull_lite");
    ...
    /// (e.g. a future `"mull-shell-ext-"` origin), an earlier `ALL` entry could
