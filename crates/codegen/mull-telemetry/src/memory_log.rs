...
//!
//! Provides a dedicated tracing target (`mull_memory`) with an optional
//! file logger that writes to `~/.mull/logs/memory.log`.
//!
//! ## When to use
//!
...
//! MULL_MEMORY_LOG=0 mull                # disable even when enabled
//! tail -f ~/.mull/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "mull_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "MULL_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.mull/logs/memory.log`. Filters to `mull_memory=trace`.
    /// Set `MULL_MEMORY_LOG=0` to disable, `MULL_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || mull_home().join("logs").join("memory.log");
