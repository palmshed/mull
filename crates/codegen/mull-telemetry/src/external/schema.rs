...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "mull_code.session_start",
            Self::SessionEnd => "mull_code.session_end",
            Self::UserPrompt => "mull_code.user_prompt",
            Self::TurnCompleted => "mull_code.turn_completed",
            Self::ApiRequest => "mull_code.api_request",
            Self::ApiError => "mull_code.api_error",
            Self::ToolResult => "mull_code.tool_result",
            Self::ToolDecision => "mull_code.tool_decision",
            Self::McpServerConnection => "mull_code.mcp_server_connection",
            Self::PermissionModeChanged => "mull_code.permission_mode_changed",
            Self::SkillActivated => "mull_code.skill_activated",
            Self::PluginLoaded => "mull_code.plugin_loaded",
            Self::Compaction => "mull_code.compaction",
            Self::Subagent => "mull_code.subagent",
            Self::Auth => "mull_code.auth",
            Self::InternalError => "mull_code.internal_error",
            Self::ModelSwitched => "mull_code.model_switched",
            Self::ContextualTip => "mull_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `mull_code.session.count`.
    SessionCount,
    /// `mull_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `mull_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `mull_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `mull_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `mull_code.error.count`.
    ErrorCount {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "mull_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "mull_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "mull_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "mull_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "mull_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "mull_code.error.count";
...
/// Built-in tool names (closed enum surface in `mull-tools`) that pass
...
fn tool_outcome_label(o: &mull_file_utils::events::types::ToolOutcome) -> &'static str {
    use mull_file_utils::events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `mull_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `mull_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `mull_code.session_end`.
...
/// `PromptSubmitted` → `mull_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `mull_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `mull_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `mull_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `mull_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `mull_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `mull_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `mull_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `mull_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `mull_code.permission_mode_changed`.
...
/// `ContextualTip` → `mull_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `mull_code.permission_mode_changed`.
...
/// `SkillDispatched` → `mull_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `mull_code.plugin_loaded`.
...
/// `PluginUsed` → `mull_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `mull_code.compaction`.
...
/// `CompactionTriggered` → `mull_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `mull_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `mull_code.subagent` (`phase=completed`).
...
/// `InternalError` → `mull_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `mull_code.model_switched`.
