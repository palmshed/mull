...
//!
//! Enterprise customers point the Mull CLI at *their own* OpenTelemetry
//! collector (standard `OTEL_*` env vars + the `MULL_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `MULL_EXTERNAL_OTEL` unset (or no exporter
...
//!   data-retention opt-outs (user-confirmed): those govern Palmshed-side
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &mull_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             MULL_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }
