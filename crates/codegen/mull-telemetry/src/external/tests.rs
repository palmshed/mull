...
fn event_names_are_pinned() {
    ...
    let expected: &[(E, &str)] = &[
        (E::SessionStart, "mull_code.session_start"),
        (E::SessionEnd, "mull_code.session_end"),
        (E::UserPrompt, "mull_code.user_prompt"),
        (E::TurnCompleted, "mull_code.turn_completed"),
        (E::ApiRequest, "mull_code.api_request"),
        (E::ApiError, "mull_code.api_error"),
        (E::ToolResult, "mull_code.tool_result"),
        (E::ToolDecision, "mull_code.tool_decision"),
        (E::McpServerConnection, "mull_code.mcp_server_connection"),
        (
            E::PermissionModeChanged,
            "mull_code.permission_mode_changed",
        ),
        (E::SkillActivated, "mull_code.skill_activated"),
        (E::PluginLoaded, "mull_code.plugin_loaded"),
        (E::Compaction, "mull_code.compaction"),
        (E::Subagent, "mull_code.subagent"),
        (E::Auth, "mull_code.auth"),
        (E::InternalError, "mull_code.internal_error"),
        (E::ModelSwitched, "mull_code.model_switched"),
        (E::ContextualTip, "mull_code.contextual_tip"),
    ];
...
fn client_identifier_allowlist_is_pinned() {
    ...
    assert_eq!(
        schema::sanitize_client_identifier("mull-pager"),
        "mull-pager"
    );
    assert_eq!(
...
fn sentinel_session_harness() -> events::SessionHarness {
    events::SessionHarness {
        ...
        client_identifier: Some("mull-pager".into()),
        model_id: "grok-4".into(),
        agent_name: "mull-plan".into(),
...
fn session_start_snapshot_counts_not_names() {
    ...
    let ev = &events[0];
    assert_eq!(ev.0, "mull_code.session_start");
...
fn session_new_increments_session_count_only() {
    ...
    assert_eq!(names, vec!["mull_code.session.count".to_owned()]);
}
...
fn api_request_snapshot_and_token_usage() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "mull_code.api_request");
...
fn one_failed_turn_increments_error_count_exactly_once() {
    ...
    let names: Vec<String> = exported_events(&stream)
        ...
        .collect();
    assert_eq!(
        names.iter().filter(|n| *n == "mull_code.api_error").count(),
        2
    );
    ...
    let total: u64 = stream
        ...
        .filter(|m| m.name() == "mull_code.error.count")
        .map(|m| match m.data() {
...
fn turn_error_increments_error_count() {
    ...
    names.sort();
    assert_eq!(names, vec!["mull_code.error.count", "mull_code.turn.count"]);
}
...
fn tool_result_gates_off_collapses_and_reduces() {
    ...
    emit_event_into(
        ...
        &events::ToolCallCompleted {
            ...
            outcome: mull_file_utils::events::types::ToolOutcome::Success,
            duration_ms: 42,
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "mull_code.tool_result");
...
fn user_prompt_gates_off_drops_text() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "mull_code.user_prompt");
...
fn mcp_connection_collapses_server_name_by_default() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "mull_code.mcp_server_connection");
...
fn tool_decision_snapshot() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "mull_code.tool_decision");
...
fn contextual_tip_maps_every_tip_and_action() {
    ...
    for (tip, action, tip_label, action_label) in cases {
        ...
        let events = exported_events(&stream);
        let ev = &events[0];
        assert_eq!(ev.0, "mull_code.contextual_tip");
...
/// mull-workspace sampler events, which live outside this crate and have
...
fn validating_metric_exporter_drops_export_on_bad_attr_key() {
    ...
    let rogue = meter.u64_counter("mull_code.session.count").build();
    rogue.add(
        1,
...
fn redacting_log_exporter_drops_record_with_closed_gate_key() {
    ...
    record.set_event_name("mull_code.user_prompt");
...
fn redacting_log_exporter_drops_record_with_unknown_key() {
    ...
    let logger = stream.ext.logger.as_ref().unwrap();
    let mut record = logger.create_log_record();
    record.set_event_name("mull_code.api_request");
...
fn metric_increments_pass_model_through_scrub_and_attach_session_id() {
    ...
    let exported = stream.metrics.get_finished_metrics().unwrap();
    let blob = format!("{exported:?}");
    assert!(blob.contains("mull_code.token.usage"));
    assert!(
