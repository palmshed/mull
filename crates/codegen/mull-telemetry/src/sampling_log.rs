//! Tracing layer for `target: "sampling_log"` → `~/.mull/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `MULL_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "MULL_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = mull_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
