...
//!
//! `mull-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "MULL_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `MullComConfig`).
///
/// Built by the binaries (`mull-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `mull_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-XAI-Token-Auth` header (typically `"mull-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`mull-pager`) -> `client.name`.
...
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL (e.g. `https://openrouter.ai/api/v1/traces`).
...
///
/// - `client_name`: binary name (e.g. `"mull-tui"`, `"mull-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-mull-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("mull-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid MULL_OTEL_FILTER '{}': {}. Using default '{}'.",
...
fn build_export_headers(
    ...
    snapshot: &mull_auth::CredentialSnapshot,
...
fn build_server_provider(client: OtelClientInfo, config: OtelLayerConfig) -> SdkTracerProvider {
    ...
    if config.exporter.enabled {
        ...
        static_headers.insert(
            "x-mull-client-version".to_string(),
...
mod tests {
    ...
    use mull_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    use std::sync::Mutex;
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("mull-cli"),
