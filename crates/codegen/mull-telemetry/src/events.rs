...
//!
//! These structs were extracted from `mull-shell` so they can be
...
/// A login method was chosen from the picker. `method` is "mull" or "api_key";
...
/// A login flow completed successfully. `method` is "mull" or "api_key";
...
impl CompactionScope {
    pub fn begin(
        ...
        let percentage = mull_token_estimation::usage_percentage_u8(tokens_used, context_window);
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: mull_file_utils::events::types::ToolOutcome,
    pub duration_ms: u64,
...
// ---------------------------------------------------------------------------
// Pager events (called from mull-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when Mull is launched from a non-project directory).
...
#[derive(Serialize)]
pub struct SuperMullUpsellShown {
    pub source: SuperMullUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}

#[derive(Serialize)]
pub struct SuperMullUpsellClicked {
    pub source: SuperMullUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}
...
/// Modeled on [`SuperMullUpsell`]; lets the funnel attribute the click to the
...
/// is lifted — i.e. they subscribed (externally on palmshed.ai) and came back.
/// This is the actual conversion signal for SuperMull Heavy subscriptions
/// attributed to Mull: the user saw the gate in Mull, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to Mull's upsell surface.
...
/// `mull-shell-manual_auth` (the `manual_auth` binding gets the `mull-shell-`
...
telemetry_event!(SuperMullUpsellShown, "supermull_upsell_shown");
telemetry_event!(SuperMullUpsellClicked, "supermull_upsell_clicked");
