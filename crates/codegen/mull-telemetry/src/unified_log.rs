...
//! over ACP (`palmshed.ai/log` notifications); shell receives them in
...
pub const LOG_METHOD: &str = "palmshed.ai/log";
...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "mull-pager")]
    #[serde(rename = "mull-pager")]
    MullPager,
    #[strum(serialize = "mull-desktop")]
    #[serde(rename = "mull-desktop")]
    MullDesktop,
}
...
/// Wire format for the `palmshed.ai/log` ACP notification params.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
fn log_path() -> PathBuf {
    mull_home().join(LOG_DIR).join(LOG_FILE)
}
...
///
/// Called by the `palmshed.ai/log` notification handler. Entries from
