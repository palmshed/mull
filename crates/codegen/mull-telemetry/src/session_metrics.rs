...
pub enum TraceUploadReason {
    ...
    FeatureOff,
    /// No palmshed.ai auth or deployment key.
    NoCredentials,
    /// Direct-to-bucket S3 upload.
    DirectS3,
    /// Proxy mode via palmshed.ai auth.
    Proxy,
...
impl TraceUploadReason {
    ...
    pub fn from_upload_method(method: &Option<mull_file_utils::UploadMethod>) -> Self {
        match method {
            Some(mull_file_utils::UploadMethod::Proxy { .. }) => Self::Proxy,
            Some(mull_file_utils::UploadMethod::S3 { .. }) => Self::DirectS3,
            Some(mull_file_utils::UploadMethod::Direct { .. }) => Self::DirectGcs,
...
#[cfg(test)]
mod tests {
    use mull_file_utils::UploadMethod;

    use super::TraceUploadReason;

    /// The `mull-shell-doom_loop_recovery` Mixpanel event's name and
