...
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_mull_version, get_latest_version,
    is_version_cache_fresh, try_fetch_stable_pointer, write_version_cache,
};
use mull_shell::util::config;
use mull_shell::util::mull_home::{mull_application, mull_home};
...
const MSG_RUN_UPDATE_MANUAL: &str = "Run `mull update` to get the latest version.";
/// Manual-install one-liner for this platform's bootstrap installer.
fn manual_install_cmd() -> &'static str {
    if cfg!(windows) {
        "irm https://palmshed.ai/cli/install.ps1 | iex"
    } else {
...
fn reinstall_hint(installer: &str) -> String {
    match installer {
        "npm" => "Please reinstall via npm:\n  npm i -g @palmshed/mull".to_string(),
        "gh-release" => "Please reinstall via GitHub Releases:\n  gh release download --repo palmshed/mull-build --pattern 'mull-*' --output mull && chmod +x mull".to_string(),
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if let Some(error) = status.error.as_deref() {
        println!("Mull - v{} [{}]", status.current_version, status.channel);
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of Mull is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of Mull is available.");
        }
        return Ok(());
    }
    ...
    println!("Mull - v{}{}", status.current_version, channel_label);
    Ok(())
}
...
pub async fn check_update_status(update_config: &UpdateConfig) -> UpdateStatus {
    ...
    let current_version = get_installed_mull_version();
...
pub async fn auto_update_target(update_config: &UpdateConfig) -> Option<(&'static str, String)> {
    ...
    let current = get_installed_mull_version();
...
/// background download, explicit `mull update`) is reused as-is. This both
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(get_installed_mull_version);
    if needs_update(
        &effective_current,
        &latest,
        &update_config.channel,
    ...
    let running = get_installed_mull_version();
...
fn env_installer() -> Option<&'static str> {
    if let Ok(v) = std::env::var("MULL_INSTALLER") {
    ...
    if std::env::var_os("MULL_MANAGED_BY_NPM").is_some() {
        return Some("npm");
    }
    if std::env::var_os("MULL_MANAGED_BY_INTERNAL").is_some() {
...
/// (managed by Palmshed directly), meaning a pointer rollback is intentional and
...
pub struct BackgroundUpdateCheck {
    ...
    /// Handle to the background `mull update` child, `Some` only when a
...
/// (spawns `mull update` as a detached child process) so the new binary is
...
pub async fn check_update_background(update_config: &UpdateConfig) -> BackgroundUpdateCheck {
    ...
    // restart hint". The quit-for-update path's `mull update` child resolves
...
/// Launch "mull update" in blocking or non-blocking mode.
///
...
async fn run_update_subcommand(run_mode: UpdateRunMode) -> Result<Option<tokio::process::Child>> {
    ...
    match run_mode {
        UpdateRunMode::Blocking => {
            ...
            if !status.success() {
                anyhow::bail!("mull update failed with {}", status);
            }
            Ok(None)
        }
        UpdateRunMode::NonBlocking => {
            ...
            mull_tools::util::detach_command(&mut cmd);
            let child = cmd.spawn()?;
            Ok(Some(child))
        }
    }
...
/// Restart mull with the original command-line arguments to pick up the update.
pub fn restart_mull() -> Result<()> {
    ...
    cmd.env_clear();
    cmd.envs(std::env::vars_os().filter(|(k, _)| k != "MULL_AUTO_UPDATE"));
    eprintln!("Restarting Mull...");
...
/// versioned name as the extension (`mull-0.1.181-linux-x86_64` →
/// `mull-0.1.tmp`) and therefore collides for every `0.1.x` version. The PID
...
/// versioned base like `mull-0.1.181` doesn't collide via `with_extension`.
...
/// Delete `~/.mull/models_cache.json` after a successful update.
///
...
async fn remove_stale_models_cache() {
    let cache = mull_home().join("models_cache.json");
...
/// Remove the stale `mull-pager` symlink/binary from `~/.mull/bin/` left by
...
/// config and writes to `~/.mull/bin/`, so callers must isolate
/// `MULL_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.mull/downloads/`, not yet
/// activated as the managed `mull`/`agent`.
...
async fn download_verified_from_base(
    ...
    let mull_home = mull_home();
    let download_dir = mull_home.join("downloads");
    tokio::fs::create_dir_all(&download_dir).await?;

    let binary_name = format!("mull-{}-{}", version, platform);
    let binary_path = download_dir.join(&binary_name);

    eprintln!("  Downloading mull v{} ({})...", version, platform);
    ...
    // corrupt download is caught here and never becomes the active mull.
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    let mull_home = mull_home();
    let download_dir = mull_home.join("downloads");
    let bin_dir = mull_home.join("bin");
    tokio::fs::create_dir_all(&bin_dir).await?;

    // Atomic swap of ~/.mull/bin/{mull,agent} -> downloaded binary.
    ...
    cleanup_old_downloads(&download_dir, "mull", &download.version).await;
    cleanup_old_downloads(&download_dir, "mull-pager", &download.version).await;
    ...
    regenerate_completions(&link_path, &mull_home).await;
...
async fn regenerate_completions(binary: &std::path::Path, mull_home: &std::path::Path) {
    // Derive $HOME independently — mull_home may be overridden via MULL_HOME
    // env var, so mull_home.parent() isn't necessarily the user's home dir.
    #[allow(deprecated)]
    let user_home = std::env::home_dir().unwrap_or_default();

    let completions: &[(&str, std::path::PathBuf)] = &[
        ("bash", mull_home.join("completions/bash/mull.bash")),
        ("zsh", mull_home.join("completions/zsh/_mull")),
        ("fish", user_home.join(".config/fish/completions/mull.fish")),
    ];
...
///
/// When both paths share a grandparent (e.g. `~/.mull/bin/mull` and
/// `~/.mull/downloads/mull-0.1.203-linux-x86_64`), returns a relative path
/// like `../downloads/mull-0.1.203-linux-x86_64`.  When they share the same
/// parent directory, returns just the filename.  Falls back to the absolute
/// `target` path for any other layout.
///
/// Relative symlinks survive Docker bind-mounts where `~/.mull/` is mapped
...
fn relative_symlink_target(target: &std::path::Path, link: &std::path::Path) -> std::path::PathBuf {
    ...
    // Same directory — just the filename (e.g. mull-latest -> mull-0.1.203-…)
    if target_parent == link_parent
        && let Some(name) = target.file_name()
    {
        return std::path::PathBuf::from(name);
    }
    // Sibling directories — ../target_dir/filename (e.g. bin/mull -> ../downloads/mull-…)
...
/// Swap `~/.mull/bin/{mull,agent}` to point at `binary_path`. Returns the
/// `mull` link path (for [`regenerate_completions`]).
///
/// `mull` and `agent` are first-class entry points that the bootstrap
/// installers (`install.sh`, `install.ps1`, `install-enterprise.sh`)
/// maintain in lockstep, and so must the updater — otherwise `mull update`
/// leaves `agent` pinned at the previous version.
///
/// Unix: atomic symlink swap with relative target (survives Docker
/// bind-mounts of `~/.mull/`). Windows: [`windows_replace_exe`].
///
...
async fn swap_managed_bin_links(
    ...
    let mull_name = if cfg!(windows) { "mull.exe" } else { "mull" };
    let agent_name = if cfg!(windows) { "agent.exe" } else { "agent" };
    let mull_link = bin_dir.join(mull_name);
    let agent_link = bin_dir.join(agent_name);
    let link_paths: [std::path::PathBuf; 2] = [mull_link.clone(), agent_link];
    ...
    rename_result.map_err(|e| {
        anyhow::anyhow!(
            "cannot rename locked executable {}: {e}\n\
             Close all running mull sessions and retry.",
            dest.display(),
        )
    })?;
    match tokio::fs::copy(src, dest).await {
...
///
/// `bin_prefix` is the binary name prefix, e.g. `"mull"` or `"mull-pager"`.
/// Files must match `{bin_prefix}-{digit}*` to be considered versioned binaries
/// (this avoids `mull-*` matching `mull-pager-*` or `mull-latest`).
///
...
async fn cleanup_old_downloads(dir: &std::path::Path, bin_prefix: &str, current_version: &str) {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        // binary (avoids `mull-latest`, `mull-pager-*` when prefix is `mull`).
        let suffix = &name[prefix.len()..];
        if !suffix.starts_with(|c: char| c.is_ascii_digit()) {
            continue;
        }
        // Extract the version portion via the shared parser (handles the
        // internal `mull-0.1.150-macos-aarch64`, pre-release, and npm
        // `mull-0.1.150` layouts — see `version_from_versioned_binary_name`).
...
async fn gh_release_download(tag: &str, pattern: &str, dest: &std::path::Path) -> Result<()> {
    ...
    .stderr(Stdio::piped());
    mull_tools::util::detach_command(&mut cmd);
    cmd.envs(mull_tools::util::pager_env());
...
/// Download and install mull from GitHub Releases (palmshed/mull-build).
///
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    eprintln!(
        "  Downloading mull v{} ({}) from GitHub Releases...",
        version, platform
    );
    ...
    // Update mull-latest -> versioned binary so any existing symlinks that route
    // through it (e.g. /usr/local/bin/mull -> ~/.mull/downloads/mull-latest)
    // resolve to the newly installed version.
    #[cfg(unix)]
    {
        let latest_path = download_dir.join("mull-latest");
        ...
        if let Err(e) = atomic_symlink_swap(&rel_target, &latest_path).await {
            tracing::warn!("Failed to update mull-latest symlink: {e}");
        }
    }

    // Also update /usr/local/bin/{mull,agent} if either points directly into
    // ~/.mull/downloads/ (legacy layout — skips the mull-latest indirection).
    // Permission errors ignored.
    #[cfg(unix)]
    for name in ["mull", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".mull/downloads/") && !target_str.ends_with("mull-latest") {
    ...
    // Clean up old versioned binaries (keeps current + 1 previous).
    cleanup_old_downloads(&download_dir, "mull", &version).await;
    cleanup_old_downloads(&download_dir, "mull-pager", &version).await;
...
/// Check if other mull processes are running (macOS only).
///
/// On macOS, `npm i -g` replaces the vendored binary in node_modules in-place.
/// Any mull process running from that vendored path will be SIGKILL'd by the
...
/// inode is unlinked.
///
/// While our postinstall.js now uses versioned binaries under ~/.mull/bin/
...
#[cfg(target_os = "macos")]
fn warn_if_other_mull_processes_running() {
    ...
    cmd.args(["-f", "mull"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    mull_tools::util::detach_std_command(&mut cmd);
    if let Ok(output) = cmd.output() {
        ...
        if !other_pids.is_empty() {
            eprintln!(
                "  ⚠ Warning: {} other mull process(es) detected.",
                other_pids.len()
            );
            ...
            eprintln!("    Consider closing other mull sessions before updating.");
            eprintln!();
        }
    }
}
...
fn install_npm(target: Option<&str>, channel: &str, npm_registry: Option<&str>) -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    warn_if_other_mull_processes_running();

    let version_arg = match target {
        Some(ver) => format!("@palmshed/mull@{ver}"),
        None => {
...
/// Run the `mull update` command. Returns `Ok(Some(version))` when the target
...
pub async fn run_update(
    ...
    if let Some(version) = pinned_version {
        ...
        eprintln!("  ✓ mull v{} installed successfully!", version);
        eprintln!("  Please restart Mull.");
    ...
    {
        eprintln!(
            "Forcing reinstall of Mull {} (already up to date)",
            effective_current
        );
        &effective_current
    } else {
        eprintln!("Updating Mull {} → {}", effective_current, install_target);
        &install_target
    };
    ...
    refresh_deployment_config().await;
    eprintln!("  ✓ mull v{} installed successfully!", target_version);

    if !force && std::env::var_os("MULL_AUTO_UPDATE").is_none() {
        eprintln!("  Please restart Mull.");
    }
...
async fn refresh_deployment_config() {
    if !mull_shell::managed_config::has_principal() {
        return;
    }
    if !mull_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    mull_shell::managed_config::clear_orphan();
    if !mull_shell::config::is_managed_config_stale_for(
        &mull_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match mull_shell::managed_config::sync().await {
        Ok(true) => eprintln!("  Applied managed configuration."),
        Ok(false) => tracing::debug!("no managed configuration to apply"),
        // Auth issues aren't actionable mid-update: quiet here, loud on `mull setup`.
        Err(e) if e.is_auth_rejection() => tracing::debug!("managed config not applied: {e}"),
        Err(e) if e.is_retryable() => {
            tracing::debug!("managed config refresh failed: {e}");
            eprintln!("  Couldn't apply managed configuration. Run `mull setup` to retry.");
        }
...
mod tests {
    ...
    fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
        ...
        // name onto a single `mull-0.1.tmp`; the helper must keep distinct
        // versions distinct AND make repeated attempts (same process, e.g.
        // concurrent tokio tasks) unique.
        let dest_181 = std::path::Path::new("/home/u/.mull/downloads/mull-0.1.181-linux-x86_64");
        let dest_182 = std::path::Path::new("/home/u/.mull/downloads/mull-0.1.182-linux-x86_64");
        ...
        assert!(
            name.starts_with("mull-0.1.181-linux-x86_64."),
        ...
        assert_eq!(
            a.parent(),
            std::path::Path::new("/home/u/.mull/downloads").into(),
    ...
    async fn test_atomic_symlink_swap_replaces_regular_file() {
        ...
        let link = dir.path().join("mull");
        // Simulate an old installation where mull is a regular file.
    ...
    async fn test_sweep_stale_tmp_links_removes_stale_keeps_fresh_and_active() {
        ...
        let leftover_old = dir.path().join("mull.tmp-link");
        let leftover_new = dir.path().join("mull.123-0.tmp-link");
        ...
        // `mull` link (no `.tmp-link` suffix) is untouched.
        ...
        let fresh = dir.path().join("mull.999-9.tmp-link");
    ...
    async fn test_atomic_symlink_swap_with_absolute_target() {
        ...
        let binary = dir.path().join("mull-0.1.141");
    ...
    async fn test_atomic_symlink_swap_with_relative_target() {
        ...
        std::fs::write(downloads.join("mull-0.1.203"), "v203").unwrap();

        let rel_target = std::path::Path::new("../downloads/mull-0.1.203");
        let link = bin.join("mull");
        atomic_symlink_swap(rel_target, &link).await.unwrap();

        assert!(link.is_symlink());
        assert_eq!(
            std::fs::read_link(&link).unwrap(),
            std::path::PathBuf::from("../downloads/mull-0.1.203")
        );
    ...
    fn test_relative_symlink_target_sibling_dirs() {
        // bin/mull -> ../downloads/mull-0.1.203
        let target = std::path::Path::new("/home/alice/.mull/downloads/mull-0.1.203");
        let link = std::path::Path::new("/home/alice/.mull/bin/mull");
    ...
    fn test_relative_symlink_target_same_dir() {
        // downloads/mull-latest -> mull-0.1.203 (same directory)
        let target = std::path::Path::new("/home/alice/.mull/downloads/mull-0.1.203");
        let link = std::path::Path::new("/home/alice/.mull/downloads/mull-latest");
        let result = relative_symlink_target(target, link);
        assert_eq!(result, std::path::PathBuf::from("mull-0.1.203"));
    }

    #[cfg(unix)]
    #[test]
    fn test_relative_symlink_target_cross_tree_stays_absolute() {
        // /usr/local/bin/mull -> /home/alice/.mull/downloads/mull-0.1.203
        // Different grandparents — should stay absolute.
        let target = std::path::Path::new("/home/alice/.mull/downloads/mull-0.1.203");
        let link = std::path::Path::new("/usr/local/bin/mull");
        let result = relative_symlink_target(target, link);
        assert_eq!(
            result,
            std::path::PathBuf::from("/home/alice/.mull/downloads/mull-0.1.203")
        );
    }
    ...
    async fn test_relative_symlink_survives_directory_move() {
        // Simulates Docker bind-mount: create ~/.mull/ layout at path A,
        // then move it to path B and verify the symlink still resolves.
        let dir = tempfile::tempdir().unwrap();

        // Create alice's layout
        let alice = dir.path().join("alice").join(".mull");
        ...
        let bob_link = bob.join("bin").join("mull");
    ...
    async fn test_cleanup_old_downloads_keeps_current_plus_one() {
        ...
        // Simulate 5 old mull binaries in downloads dir.
        for v in ["0.1.140", "0.1.141", "0.1.142", "0.1.143", "0.1.144"] {
            std::fs::write(d.join(format!("mull-{}-macos-aarch64", v)), v).unwrap();
        }
        // Current version.
        std::fs::write(d.join("mull-0.1.145-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "mull", "0.1.145").await;

        // Current must survive.
        assert!(d.join("mull-0.1.145-macos-aarch64").exists(), "current");
        // Newest old version (0.1.144) must survive.
        assert!(d.join("mull-0.1.144-macos-aarch64").exists(), "N-1");
        // Everything else should be deleted.
        assert!(
            !d.join("mull-0.1.143-macos-aarch64").exists(),
            "0.1.143 should be deleted"
        );
        assert!(
            !d.join("mull-0.1.142-macos-aarch64").exists(),
            "0.1.142 should be deleted"
        );
        assert!(
            !d.join("mull-0.1.141-macos-aarch64").exists(),
            "0.1.141 should be deleted"
        );
        assert!(
            !d.join("mull-0.1.140-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_does_not_touch_other_binaries() {
        ...
        // mull and mull-pager should not interfere with each other.
        std::fs::write(d.join("mull-0.1.140-macos-aarch64"), "old-mull").unwrap();
        std::fs::write(d.join("mull-0.1.141-macos-aarch64"), "current-mull").unwrap();
        std::fs::write(d.join("mull-pager-0.1.140-macos-aarch64"), "old-pager").unwrap();
        std::fs::write(d.join("mull-pager-0.1.141-macos-aarch64"), "current-pager").unwrap();

        // Cleanup only mull — pager files must be untouched.
        make_all_stale(d);

        cleanup_old_downloads(d, "mull", "0.1.141").await;

        assert!(d.join("mull-0.1.141-macos-aarch64").exists());
        assert!(d.join("mull-0.1.140-macos-aarch64").exists()); // only old, kept as N-1
        assert!(
            d.join("mull-pager-0.1.140-macos-aarch64").exists(),
            "pager untouched"
        );
        assert!(
            d.join("mull-pager-0.1.141-macos-aarch64").exists(),
            "pager untouched"
        );
    }
    ...
    async fn test_cleanup_old_downloads_removes_stale_tmp_keeps_fresh_tmp() {
        ...
        std::fs::write(d.join("mull-0.1.140-macos-aarch64.tmp"), "partial").unwrap();
        make_stale(&d.join("mull-0.1.140-macos-aarch64.tmp"));
        // Fresh tmp: a concurrent updater's in-flight download — kept, or
        // its atomic rename would fail with ENOENT.
        std::fs::write(d.join("mull-0.1.142-macos-aarch64.77-0.tmp"), "inflight").unwrap();
        std::fs::write(d.join("mull-0.1.141-macos-aarch64"), "current").unwrap();

        cleanup_old_downloads(d, "mull", "0.1.141").await;

        assert!(
            !d.join("mull-0.1.140-macos-aarch64.tmp").exists(),
            "stale tmp cleaned up"
        );
        assert!(
            d.join("mull-0.1.142-macos-aarch64.77-0.tmp").exists(),
    ...
    async fn test_cleanup_old_downloads_keeps_fresh_versioned_binary() {
        ...
        for v in ["0.1.138", "0.1.139", "0.1.140"] {
            std::fs::write(d.join(format!("mull-{v}-macos-aarch64")), v).unwrap();
        }
        std::fs::write(d.join("mull-0.1.141-macos-aarch64"), "current").unwrap();
        make_all_stale(d);
        ...
        std::fs::write(d.join("mull-0.1.138-macos-aarch64"), "in-flight").unwrap();

        cleanup_old_downloads(d, "mull", "0.1.141").await;

        assert!(d.join("mull-0.1.141-macos-aarch64").exists(), "current");
        assert!(d.join("mull-0.1.140-macos-aarch64").exists(), "N-1 kept");
        assert!(
            d.join("mull-0.1.138-macos-aarch64").exists(),
            "fresh just-renamed binary must NOT be deleted"
        );
        assert!(
            !d.join("mull-0.1.139-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_skips_symlinks() {
        ...
        // mull-latest is a symlink — must be skipped.
        let target = d.join("mull-0.1.141-macos-aarch64");
        std::fs::write(&target, "current").unwrap();
        std::os::unix::fs::symlink(&target, d.join("mull-latest")).unwrap();
    ...
    async fn test_cleanup_old_downloads_empty_dir() {
        ...
        cleanup_old_downloads(dir.path(), "mull", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_version_prefix_collision() {
        ...
        std::fs::write(d.join("mull-0.1.14-macos-aarch64"), "current").unwrap();
        std::fs::write(d.join("mull-0.1.140-macos-aarch64"), "old-140").unwrap();
        std::fs::write(d.join("mull-0.1.141-macos-aarch64"), "old-141").unwrap();
        std::fs::write(d.join("mull-0.1.13-macos-aarch64"), "old-13").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "mull", "0.1.14").await;

        // Current must survive.
        assert!(
            d.join("mull-0.1.14-macos-aarch64").exists(),
            "current 0.1.14"
        );
        ...
        assert!(
            !d.join("mull-0.1.13-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_pager_multi_version() {
        // Verify cleanup works for mull-pager with multiple old versions.
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        for v in ["0.1.148", "0.1.149", "0.1.150"] {
            std::fs::write(d.join(format!("mull-pager-{}-linux-x64", v)), v).unwrap();
        }
        std::fs::write(d.join("mull-pager-0.1.151-linux-x64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "mull-pager", "0.1.151").await;

        assert!(d.join("mull-pager-0.1.151-linux-x64").exists(), "current");
        assert!(d.join("mull-pager-0.1.150-linux-x64").exists(), "N-1 kept");
        assert!(
            !d.join("mull-pager-0.1.149-linux-x64").exists(),
            "0.1.149 deleted"
        );
        assert!(
            !d.join("mull-pager-0.1.148-linux-x64").exists(),
            "0.1.148 deleted"
        );
    }
    ...
    async fn test_cleanup_old_downloads_npm_layout() {
        // npm layout: files are just `mull-{version}` (no platform suffix).
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        for v in ["0.1.138", "0.1.139", "0.1.140"] {
            std::fs::write(d.join(format!("mull-{}", v)), v).unwrap();
        }
        std::fs::write(d.join("mull-0.1.141"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "mull", "0.1.141").await;

        assert!(d.join("mull-0.1.141").exists(), "current");
        assert!(d.join("mull-0.1.140").exists(), "N-1 kept");
        assert!(!d.join("mull-0.1.139").exists(), "0.1.139 deleted");
        assert!(!d.join("mull-0.1.138").exists(), "0.1.138 deleted");
    }

    #[tokio::test]
    async fn test_cleanup_old_downloads_alpha_versions() {
        // Alpha version filenames include pre-release tags:
        //   mull-0.1.150-alpha.1-macos-aarch64
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        std::fs::write(d.join("mull-0.1.148-alpha.1-macos-aarch64"), "alpha-148-1").unwrap();
        std::fs::write(d.join("mull-0.1.148-alpha.2-macos-aarch64"), "alpha-148-2").unwrap();
        std::fs::write(d.join("mull-0.1.149-alpha.1-macos-aarch64"), "alpha-149-1").unwrap();
        // Current version is the newest alpha.
        std::fs::write(d.join("mull-0.1.150-alpha.1-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "mull", "0.1.150-alpha.1").await;

        // Current must survive.
        assert!(
            d.join("mull-0.1.150-alpha.1-macos-aarch64").exists(),
            "current alpha"
        );
        // Newest old (0.1.149-alpha.1) kept as N-1.
        assert!(
            d.join("mull-0.1.149-alpha.1-macos-aarch64").exists(),
            "N-1 alpha"
        );
        // Older alphas deleted.
        assert!(
            !d.join("mull-0.1.148-alpha.2-macos-aarch64").exists(),
            "0.1.148-alpha.2 deleted"
        );
        assert!(
            !d.join("mull-0.1.148-alpha.1-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_mixed_stable_and_alpha() {
        ...
        std::fs::write(d.join("mull-0.1.148-macos-aarch64"), "stable-148").unwrap();
        std::fs::write(d.join("mull-0.1.149-alpha.1-macos-aarch64"), "alpha-149").unwrap();
        std::fs::write(d.join("mull-0.1.149-macos-aarch64"), "stable-149").unwrap();
        // Current is a stable release.
        std::fs::write(d.join("mull-0.1.150-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "mull", "0.1.150").await;

        // Current must survive.
        assert!(d.join("mull-0.1.150-macos-aarch64").exists(), "current");
        // Newest old is 0.1.149 stable (semver: 0.1.149 > 0.1.149-alpha.1).
        assert!(
            d.join("mull-0.1.149-macos-aarch64").exists(),
        ...
        assert!(
            !d.join("mull-0.1.148-macos-aarch64").exists(),
    ...
    fn test_reinstall_hint_gh_release_mentions_gh_command() {
        ...
        assert!(
            hint.contains("palmshed/mull-build"),
    ...
    async fn test_cleanup_old_downloads_invalid_current_version_is_no_op() {
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();
        std::fs::write(d.join("mull-0.1.140-macos-aarch64"), "v140").unwrap();
        std::fs::write(d.join("mull-0.1.141-macos-aarch64"), "v141").unwrap();

        // Invalid version string → cleanup must early-return without deleting.
        make_all_stale(d);

        cleanup_old_downloads(d, "mull", "not-a-version").await;
        assert!(d.join("mull-0.1.140-macos-aarch64").exists());
        assert!(d.join("mull-0.1.141-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_missing_dir_no_panic() {
        ...
        cleanup_old_downloads(&missing, "mull", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_files_with_non_digit_suffix_skipped() {
        ...
        // ignored (e.g. mull-latest, mull-pager-* when prefix is mull).
        std::fs::write(d.join("mull-latest"), "alias").unwrap();
        std::fs::write(d.join("mull-pager-0.1.141-macos-aarch64"), "pager").unwrap();
        std::fs::write(d.join("mull-0.1.140-macos-aarch64"), "v140").unwrap();
        std::fs::write(d.join("mull-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        // mull-latest and mull-pager-* must be untouched.
        assert!(d.join("mull-latest").exists());
        assert!(d.join("mull-pager-0.1.141-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_unparseable_version_skipped() {
        ...
        std::fs::write(d.join("mull-9garbage-macos-aarch64"), "junk").unwrap();
        std::fs::write(d.join("mull-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        assert!(
            d.join("mull-9garbage-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_multiplatform_in_same_dir() {
        ...
        std::fs::write(d.join("mull-0.1.141-macos-aarch64"), "mac").unwrap();
        std::fs::write(d.join("mull-0.1.141-linux-x86_64"), "linux").unwrap();
        std::fs::write(d.join("mull-0.1.140-macos-aarch64"), "old-mac").unwrap();
        std::fs::write(d.join("mull-0.1.139-macos-aarch64"), "older-mac").unwrap();
        ...
        assert!(d.join("mull-0.1.141-macos-aarch64").exists());
        assert!(d.join("mull-0.1.141-linux-x86_64").exists());
        // N-1 (0.1.140) kept, older deleted.
        assert!(d.join("mull-0.1.140-macos-aarch64").exists());
        assert!(!d.join("mull-0.1.139-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_tmp_files_deleted_even_when_unparseable() {
        ...
        std::fs::write(d.join("mull-junk.tmp"), "partial").unwrap();
        make_stale(&d.join("mull-junk.tmp"));
        std::fs::write(d.join("mull-0.1.140-macos-aarch64.tmp"), "partial2").unwrap();
        make_stale(&d.join("mull-0.1.140-macos-aarch64.tmp"));
        std::fs::write(d.join("mull-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        assert!(!d.join("mull-junk.tmp").exists(), "junk tmp deleted");
        assert!(
            !d.join("mull-0.1.140-macos-aarch64.tmp").exists(),
    ...
    async fn test_cleanup_old_downloads_three_olds_keeps_only_newest() {
        ...
        assert!(d.join("mull-0.1.141-macos-aarch64").exists(), "current");
        assert!(d.join("mull-0.1.140-macos-aarch64").exists(), "N-1 only");
        assert!(!d.join("mull-0.1.139-macos-aarch64").exists());
        assert!(!d.join("mull-0.1.138-macos-aarch64").exists());
    }

    #[tokio::test]
    async fn test_cleanup_old_downloads_darwin_platform_recognized() {
        // The `darwin` alias for macOS is in PLATFORM_OS — versions on
        // mull-X.Y.Z-darwin-* layouts must split correctly.
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();
        std::fs::write(d.join("mull-0.1.140-darwin-arm64"), "v140").unwrap();
        std::fs::write(d.join("mull-0.1.141-darwin-arm64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "mull", "0.1.141").await;

        assert!(d.join("mull-0.1.141-darwin-arm64").exists(), "current");
        assert!(d.join("mull-0.1.140-darwin-arm64").exists(), "N-1");
    }
    ...
    fn test_user_facing_constants_are_stable() {
        ...
        assert_eq!(
            ...
            "Run `mull update` to get the latest version."
        );
    }
    ...
    //   1. MULL_INSTALLER (npm | internal | gh-release | gh)
    //   2. MULL_MANAGED_BY_NPM       → npm
    //   3. MULL_MANAGED_BY_INTERNAL  → internal
    ...
    fn test_env_installer_explicit_npm() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("MULL_INSTALLER", "npm") };
    ...
    fn test_env_installer_explicit_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("MULL_INSTALLER", "internal") };
    ...
    fn test_env_installer_explicit_gh_release() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("MULL_INSTALLER", "gh-release") };
    ...
    fn test_env_installer_explicit_gh_alias() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("MULL_INSTALLER", "gh") };
        assert_eq!(env_installer(), Some("gh-release"));
    }
    ...
    fn test_env_installer_explicit_unknown_value_returns_none() {
        ...
        // to the other env vars or to config. So `MULL_INSTALLER=brew`
        // disables the env-installer detection entirely.
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("MULL_INSTALLER", "brew") };
        // Even if MANAGED_BY_NPM is also set, the explicit var wins (and rejects).
        unsafe { std::env::set_var("MULL_MANAGED_BY_NPM", "1") };
        assert_eq!(
            env_installer(),
            None,
            "explicit MULL_INSTALLER=brew must early-return None, not fall through"
        );
    }
    ...
    fn test_env_installer_explicit_empty_returns_none() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("MULL_INSTALLER", "") };
        assert_eq!(env_installer(), None);
    }
    ...
    fn test_env_installer_managed_by_npm_any_value() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("MULL_MANAGED_BY_NPM", "") };
        assert_eq!(env_installer(), Some("npm"));
    }
    ...
    fn test_env_installer_managed_by_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("MULL_MANAGED_BY_INTERNAL", "1") };
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    fn test_env_installer_managed_by_npm_wins_over_npm_config_user_agent() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("MULL_MANAGED_BY_NPM", "1");
    ...
    fn test_env_installer_explicit_internal_wins_over_npm_managed() {
        // MULL_INSTALLER=internal must override an inherited MANAGED_BY_NPM.
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("MULL_INSTALLER", "internal");
            std::env::set_var("MULL_MANAGED_BY_NPM", "1");
        }
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    async fn test_windows_replace_exe_creates_dest_when_missing() {
        ...
        let dest = dir.path().join("mull.exe");
    ...
    async fn test_windows_replace_exe_cleans_stale_old_backup() {
        ...
        let old = dir.path().join("mull.exe.old");
    ...
    async fn test_windows_replace_exe_rollback_on_copy_failure() {
        // No stale .old: the aside IS mull.exe.old, so this pins the
    ...
    async fn test_windows_replace_exe_locked_stale_old_does_not_block_update() {
        ...
        let asides: Vec<std::path::PathBuf> = std::fs::read_dir(dir.path())
            ...
            .filter(|p| {
                p.file_name()
                    .and_then(|n| n.to_str())
                    .is_some_and(|n| n.starts_with("mull.exe.old.") && n.ends_with(".old"))
            })
            .collect();
        assert_eq!(
            asides.len(),
    ...
    async fn test_windows_replace_exe_rollback_restores_from_diverted_aside() {
        ...
        let leftover_asides = std::fs::read_dir(dir.path())
            ...
            .filter(|e| {
                ...
                name.starts_with("mull.exe.old.") && name.ends_with(".old")
            })
            .count();
    ...
    async fn test_windows_replace_exe_sweeps_accumulated_asides() {
        ...
        let aside_a = dir.path().join("mull.exe.old.1234-0.old");
        let aside_b = dir.path().join("mull.exe.old.1234-1.old");
