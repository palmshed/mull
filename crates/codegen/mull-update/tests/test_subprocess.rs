...
use mull_update::auto_update::install_npm_for_test;
use mull_update::version::{
...
async fn fetch_npm_tag_passes_pkg_and_tag_to_npm() {
    ...
    assert!(args.contains("@palmshed/mull"), "args: {args}");
...
async fn fetch_npm_tag_alpha_appends_at_alpha_suffix() {
    ...
    let log = g.args_log();
    assert!(log[0].contains("@palmshed/mull@alpha"), "args: {}", log[0]);
}
...
async fn install_npm_calls_npm_with_version_arg() {
    ...
    assert!(args.contains("@palmshed/mull@0.1.181"), "args: {args}");
}
...
async fn install_npm_falls_back_to_dist_tag_on_no_target() {
    ...
    let log = g.args_log();
    assert!(
        log[0].contains("@palmshed/mull@latest"),
...
async fn install_npm_falls_back_to_alpha_dist_tag_on_alpha_channel() {
    ...
    let log = g.args_log();
    assert!(
        log[0].contains("@palmshed/mull@alpha"),
...
async fn fetch_gh_release_passes_repo_flag() {
    ...
    assert!(log[0].contains("palmshed/mull-build"), "args: {}", log[0]);
}
