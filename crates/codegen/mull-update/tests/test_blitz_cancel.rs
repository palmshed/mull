...
//!
//! > `~/.mull/bin/mull` resolves to a binary that passes the smoke-test, OR it
...
use mull_update::auto_update::install_internal_from_base;
...
/// (`mull` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `mull` should resolve to after an install attempt.
...
/// to both `mull` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p mull-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("MULL_BLITZ_ITERS")
        .ok()
