...
//!
//! Security: the subprocess is spawned with [`mull_tty_utils::detach_std_command`]
//! (TTY/session detach) + [`mull_tty_utils::pager_env`] + null stdio, source is
...
mod tests {
    ...
    fn fake_mmdc_nonzero_exit_maps_to_layout() {
        ...
        // timeout or a missing binary.  Use a generous timeout so the shell
        // startup on slow CI runners doesn't cause a spurious Timeout.
