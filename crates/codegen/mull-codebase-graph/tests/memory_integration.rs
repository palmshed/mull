...
fn test_build_batch_peak_rss_is_bounded() {
    ...
    // Allow 40 MB of jitter from concurrent allocator/OS activity.
    if peak_batched > 0.0 && peak_unbatched > 0.0 {
        assert!(
            peak_batched <= peak_unbatched + 40.0,
            "Batched build peaked {:.1}MB higher than unbounded (>40MB unexpected)",
...
fn test_compact_reduces_rss_vs_uncompacted() {
    ...
    if let (Some(unc), Some(cpt)) = (rss_uncompacted, rss_compacted) {
        ...
        // compact() must not materially increase RSS (RSS noise can exceed
        // the actual savings on small workloads; 10 MB tolerance absorbs
        // page-reclamation and allocator jitter).
        assert!(
            cpt <= unc + 10.0,
