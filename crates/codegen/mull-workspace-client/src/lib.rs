...
//! `mull-workspace`. Wire types live in
//! `mull_workspace_types::rpc`; this crate adds the connected-state
...
//! own their timeouts.
use mull_computer_hub_sdk::harness::ToolHarness;
use mull_tool_runtime::{ToolCallContext, ToolStreamItem, TypedToolOutput};
use mull_workspace_types::rpc::agents_md::{AgentConfigFile, DiscoverAgentsMdReq};
use mull_workspace_types::rpc::code_nav::{
...
use mull_workspace_types::rpc::fs::{
...
use mull_workspace_types::rpc::git::{
...
use mull_workspace_types::rpc::hunks::{
...
use mull_workspace_types::rpc::search::{
    ContentSearchData, ContentSearchRequest, FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq,
    FuzzyStatusReq,
};
use mull_workspace_types::rpc::session::{
    BeginPromptReq, EndPromptReq, FileRewindResponse, RewindToReq,
};
use mull_workspace_types::rpc::skills::{DiscoverPluginsReq, DiscoverSkillsReq, SkillInfo};
...
use mull_workspace_types::rpc::worktree::{
...
use mull_workspace_types::rpc::{RpcEnvelope, RpcError, WORKSPACE_RPC_TOOL_ID, WorkspaceRpc};
...
pub async fn consume_stream_terminal(
    stream: &mut mull_tool_runtime::ToolStream<TypedToolOutput>,
) -> Result<TypedToolOutput, mull_tool_runtime::ToolError> {
    loop {
...
/// Check whether a [`ToolError`](mull_tool_runtime::ToolError) indicates
...
pub fn is_transport_fatal(err: &mull_tool_runtime::ToolError) -> bool {
    match err.kind {
        mull_tool_runtime::ToolErrorKind::NetworkError => true,
        mull_tool_runtime::ToolErrorKind::Custom => err
            .details
            .as_ref()
...
impl WorkspaceClient {
    ...
    pub async fn rpc_raw(
        ...
        let tool_id = mull_tool_protocol::ToolId::new(WORKSPACE_RPC_TOOL_ID)
...
mod tests {
    use super::*;
    use mull_computer_hub_sdk::harness::LocalRegistry;
    use mull_tool_protocol::{SessionId, ToolId};
    use mull_tool_runtime::{Tool, ToolError};
    use mull_tool_types::ToolDescription;
    use mull_workspace_types::rpc::skills::SkillScope;
    ...
    impl mull_tool_runtime::ToolOutput for RawOut {}
    #[derive(Debug)]
    ...
    impl Tool for FakeWorkspaceRpc {
        ...
        fn description(&self, _ctx: &::mull_tool_runtime::ListToolsContext) -> ToolDescription {
        ...
        async fn run(&self, _ctx: ToolCallContext, args: Self::Args) -> Result<RawOut, ToolError> {
            ...
            match args.method.as_str() {
                ...
                "workspace.discover_skills" => ok(serde_json::json!(
                    ...
                    "path" : "/workspace/.mull/skills/my-skill/SKILL.md", "scope"
                    : "local", }]
                )),
    ...
    async fn consume_stream_terminal_returns_ok() {
        ...
        let mut stream = mull_tool_runtime::terminal_only(Ok(typed));
        assert_eq!(
    ...
    async fn consume_stream_terminal_returns_err() {
        let mut stream: mull_tool_runtime::ToolStream<TypedToolOutput> =
            mull_tool_runtime::terminal_only(Err(ToolError::network_error("oops")));
    ...
    async fn consume_stream_terminal_exhausted_stream_is_network_error() {
        ...
        let mut stream = mull_tool_runtime::terminal_only::<TypedToolOutput>(Ok(typed));
