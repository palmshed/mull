//! Pure resolution logic for `mull plugin install <name>` marketplace refs.
...
mod tests {
    ...
    fn parse_name_with_owner_repo_qualifier() {
        assert_eq!(
            parse_marketplace_ref("sentry@palmshed/plugins"),
            Some(MarketplaceRef {
                name: "sentry".into(),
                qualifier: Some("palmshed/plugins".into()),
            })
        );
    }
    ...
    fn parse_rejects_fragment() {
        ...
        assert_eq!(
            parse_marketplace_ref("sentry@palmshed/marketplace#sub"),
            None
        );
    }
    ...
    fn slugify_lowercases_and_hyphenates_spaces() {
        ...
        assert_eq!(slugify("Palmshed Official"), "palmshed-official");
    }

    #[test]
    fn addressable_qualifier_git_and_local() {
        assert_eq!(
            addressable_qualifier(&git_source("x", "https://github.com/palmshed/plugins.git")),
            "palmshed/plugins"
        );
        assert_eq!(
    ...
    fn resolve_qualifier_matches_git_owner_repo_across_url_forms() {
        for url in [
            ...
            "git@github.com:palmshed/plugins.git",
            "ssh://git@github.com/palmshed/plugins",
            "https://GitHub.com/palmshed/plugins",
        ] {
            let sources = [git_source("src", url)];
            assert_eq!(
                resolve_qualified_source("palmshed/plugins", &sources),
                Ok(0),
                "url: {url}"
            );
        }
    ...
    fn resolve_qualifier_normalizes_dot_git_in_qualifier() {
        let sources = [git_source("src", "https://github.com/palmshed/plugins")];
        assert_eq!(
            resolve_qualified_source("palmshed/plugins.git", &sources),
            Ok(0)
        );
    }
    ...
    fn resolve_qualifier_ambiguous_lists_all_matching_indices() {
        let sources = [
            git_source("Mirror A", "https://github.com/palmshed/plugins.git"),
    ...
    fn resolve_qualifier_name_vs_other_source_owner_repo_is_ambiguous() {
        let sources = [
            ...
            git_source("palmshed/plugins", "git@github.example.com:mirror/mull.git"),
        ];
        assert_eq!(
            resolve_qualified_source("palmshed/plugins", &sources),
            Err(QualifierResolveError::Ambiguous(vec![0, 1]))
    ...
    fn bare_name_ambiguous_when_more_than_one_official_match() {
        let pairs = [
            ...
            (
                git_source("Official Mirror B", "git@github.com:palmshed/plugins.git"),
                entry("sentry"),
            ),
        ];
        let scanned = scanned_entries(&pairs);
