...
/// Display name of the official Palmshed marketplace source.
pub const OFFICIAL_SOURCE_NAME: &str = "Palmshed Official";

/// Git URL of the official Palmshed marketplace source. Auto-registered on first run.
pub const OFFICIAL_SOURCE_GIT_URL: &str = "https://github.com/palmshed/plugins.git";

/// Whether `url` is the official Palmshed marketplace source, normalizing case, a
/// `www.` prefix, a trailing `/` or `.git`, and HTTPS/SSH forms before comparing.
pub fn is_official_source_url(url: &str) -> bool {
    canonical_github_owner_repo(url).as_deref() == Some("palmshed/plugins")
}
...
mod tests {
    ...
    fn is_official_matches_ssh_form() {
        ...
        assert!(is_official_source_url("git@github.com:palmshed/plugins"));
        assert!(is_official_source_url(
            "ssh://git@github.com/palmshed/plugins.git"
        ));
        assert!(is_official_source_url(
    ...
    fn is_official_rejects_unrelated_urls() {
        ...
        assert!(!is_official_source_url(
            "https://github.com/palmshed/some-other-repo.git"
        ));
        assert!(!is_official_source_url(""));
    }
    ...
    fn is_official_matches_noncanonical_forms() {
        ...
        assert!(is_official_source_url("http://github.com/palmshed/plugins"));
        assert!(is_official_source_url(
            "https://www.github.com/palmshed/plugins.git"
        ));
        assert!(is_official_source_url(
            "git@github.com:palmshed/plugins.git"
        ));
