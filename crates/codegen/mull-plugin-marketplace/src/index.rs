...
//!
//! Catalog file lookup, in order: `.mull-plugin/marketplace.json` (preferred),
//! `.mull-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.mull-plugin/marketplace.json` (preferred Palmshed convention)
/// 2. `.mull-plugin/plugin.json`
...
mod tests {
    ...
    fn load_index_valid_mull_dir() {
        ...
        assert_eq!(result.unwrap().name, "mull");
    }

    #[test]
    fn load_index_mull_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".mull-plugin", "mull"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "mull");
    }
