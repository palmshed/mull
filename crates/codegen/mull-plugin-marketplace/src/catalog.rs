...
//! `.mull-plugin/plugin-index.json` (preferred), then
...
use mull_hooks_plugins_types::PluginComponents;
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_mull_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".mull-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_mull_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".mull-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".mull-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
    ...
    fn load_catalog_ignores_unknown_fields() {
        ...
        write_catalog(
            ...
            ".mull-plugin",
            r#"{
                "$schema": "https://palmshed.ai/mull/plugin-index.schema.json",
                "version": 1,
