//! Parse marketplace sources from `~/.mull/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
//! name = "Palmshed Official"
//! git = "https://github.com/palmshed/mull-plugin-marketplace.git"
//!
//! [[marketplace.sources]]
//! name = "Local Dev"
//! path = "~/dev/my-plugins"
...
/// instead of `~/.mull`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn parse_git_source() {
        ...
        let sources = load_sources(&config);
        assert_eq!(sources.len(), 1);
        assert_eq!(sources[0].name, "Palmshed Official");
        assert!(
            matches!(&sources[0].kind, SourceKind::Git { url, branch } if url.contains("palmshed") && branch.as_deref() == Some("main"))
        );
    }
