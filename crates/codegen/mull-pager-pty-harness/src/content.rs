...
//! [`MockInferenceServer`] from `mull-test-support` and provides the
...
pub use mull_test_support::mock_server::LogEntry;
pub use mull_test_support::mock_server::MockModelEntry as MockModel;
pub use mull_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use mull_test_support::sse;
pub use mull_test_support::{ScriptedResponse, SseEvent};
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.mull
    ...
    ///
    /// Mirrors `mull_test_support::env::test_env_cmd_tokio`.
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit MULL_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // MULL_HOME is set in the test runner's env).
            ("MULL_HOME".into(), mull_home),
            ("MULL_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("MULL_PALMSHED_API_BASE_URL".into(), self.url()),
            ("PALMSHED_API_KEY".into(), "test-key-for-ci".into()),
            ...
            ("MULL_PROMPT_SUGGESTIONS".into(), "false".into()),
...
mod tests {
    ...
    /// on the SuperMull upsell screen and breaks every PTY test.
    #[tokio::test]
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("MULL_HOME").as_deref(),
            content.home().join(".mull").to_str()
        );
        assert_eq!(get("MULL_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("MULL_PALMSHED_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("PALMSHED_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("MULL_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("MULL_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("MULL_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("MULL_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("MULL_MAX_RETRIES").as_deref(), Some("0"));
