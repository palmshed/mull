//! Scripted TUI scenario runner for mull-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$MULL_HOME`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&mull_home)
                .with_context(|| format!("create scenario MULL_HOME {}", mull_home.display()))?;
            fs::write(mull_home.join("config.toml"), config_toml)
