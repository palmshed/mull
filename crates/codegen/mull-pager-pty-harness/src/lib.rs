//! Unified PTY harness for mull-pager.
//!
...
//!    `mull-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `MULL_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use mull_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through mull's stdout. Used to reproduce the doubled-line class of bugs
    /// where mull's diff renderer never re-asserts a region it didn't write
