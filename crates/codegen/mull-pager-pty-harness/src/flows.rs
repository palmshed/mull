...
/// Seed a fake Palmshed OAuth entry into the isolated home's `auth.json` so the
/// shell has session auth (the harness's `PALMSHED_API_KEY` is ApiKey/BYOK mode
...
pub fn seed_fake_oauth(content: &ContentController, user: &str) {
    ...
    std::fs::write(
        ...
        format!(
            r#"{{
  "https://auth.palmshed.ai::b1a00492-073a-47ea-816f-4c329264a828": {{
...
/// [`ContentController::env_for_pager`] minus `PALMSHED_API_KEY`, so the entry
...
pub fn oauth_env_for_pager(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.retain(|(k, _)| k != "PALMSHED_API_KEY");
    env
}
