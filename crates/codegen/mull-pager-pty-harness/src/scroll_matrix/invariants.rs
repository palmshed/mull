//! The invariant suite: predicates over grouped `MULL_SCROLL_LOG` streams.
//!
...
pub enum InvariantId {
    ...
    Cfg,
    /// I-MUX-NO-OVER: delivered total per stream ≤ `events × speed + 3` —
    /// the conservative remuxed profile (ept=1/wheel_lpt=1) prices at most
    /// one line per event; the +3 slack absorbs tmux mux batching jitter
    /// (inner ept=3 chunking can re-time partial lines). Attach only to
    /// accel-free gestures (>20ms spacing — exactly 20ms still interpolates
    /// to 1.6× in the accel band; G9's 55ms clears it): a fast trackpad-
    /// classified mux stream may legitimately exceed it via accel.
    MuxNoOver,
...
fn check_mux_no_over(expected: &ExpectedProfile, groups: &[StreamGroup<'_>]) -> InvariantResult {
    for group in groups {
        let Some(fin) = group.finalize else { continue };
        let bound = fin.events_total as f64 * f64::from(expected.speed) + 3.0;
        let got = delivered(fin).abs() as f64;
        if got > bound {
            return violated(format!(
...
mod tests {
    ...
    /// delivered lines than `events × speed + 3` (the mux jitter slack).
    #[test]
