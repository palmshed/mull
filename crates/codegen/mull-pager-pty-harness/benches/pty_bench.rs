//! `pty-bench` — PTY benchmark CLI for `mull-pager`.
//!
...
//! ```bash
//! cargo bench -p mull-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/mull-${VERSION}-linux-x86_64 \
//!   cargo bench -p mull-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for mull-pager",
    long_about = None,
)]
struct Cli {
