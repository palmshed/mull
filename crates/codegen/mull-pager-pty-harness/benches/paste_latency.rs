//! `paste-latency` — clipboard paste latency benchmark for `mull-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p mull-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p mull-pager-pty-harness --bench paste_latency -- \
//!   --binary ~/Downloads/mull-old --mode text --json /tmp/paste-old.json
//! ```
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for mull-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
