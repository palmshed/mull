//! mull-chat-state — Actor-based chat state management for Palmshed agents.
//!
//! This crate extracts conversation state management from `mull-shell`'s
//! `acp_session.rs` into a standalone actor. It follows the same actor pattern
//! as `mull-hunk-tracker`:
//!
//! ```text
//! ┌────────────────┐                  ┌──────────────────────────────────────┐
