...
/// emitter in `mull-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (mull login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
    /// From user config ([model.*] api_key, env_key, PALMSHED_API_KEY). Not refreshable.
    ApiKey,
}
...
/// `mull_sampling_types::SamplingConfig` (which is secret-free).
