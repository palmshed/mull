...
const PRODUCTION_ENDPOINTS: MullBuildEndpoints = MullBuildEndpoints {
    cli_chat_proxy_base_url: "https://openrouter.ai/api/v1",
    asset_server_url: "https://assets.palmshed.ai",
    relay_ws_url: "wss://code.palmshed.ai/ws/code-agent",
    gateway_ws_url: "wss://palmshed.ai/ws/gw/",
    ws_origin: "https://palmshed.ai",
};
...
pub enum MullBuildEnvironment {
    #[default]
    Production,
}
impl MullBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        MullBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            MullBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, MullBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            MullBuildEnvironment::Production => "MULL_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> MullBuildEndpoints {
        match self {
            MullBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// `MULL_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for MullBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MullBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            MullBuildEnvironment::Production.env_prefix(),
            "MULL_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "MULL_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    /// loop mistakenly connecting to `wss://palmshed.ai/ws/gw/`).
    #[test]
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            MullBuildEnvironment::Production.relay_ws_url(),
            MullBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            MullBuildEnvironment::from_flags(false, false),
            MullBuildEnvironment::Production
        );
    }
}
