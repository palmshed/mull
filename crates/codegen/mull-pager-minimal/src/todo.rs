...
use mull_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (mull_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    mull_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use mull_shell::tools::{TodoItem, TodoPriority};
