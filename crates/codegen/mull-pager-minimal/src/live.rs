...
//! show.
use mull_pager::app::PagerTerminal;
use mull_pager::app::app_view::{ActiveView, AppView};
use mull_pager::minimal_api;
use mull_pager::render::Renderable;
use mull_pager::scrollback::state::ScrollbackState;
use mull_pager::scrollback::wrappers::EntryRenderer;
use mull_pager::theme::Theme;
use mull_pager::views::prompt_widget::PromptStyle;
use mull_pager::views::turn_status;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &mull_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
pub(super) fn prompt_style(appearance: &mull_pager::appearance::AppearanceConfig) -> PromptStyle {
    PromptStyle {
        focused: true,
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    mull_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                mull_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(mull_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a mull_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &mull_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
) -> EntryRenderer<'a> {
...
fn minimal_watchers(agent: &mull_pager::app::agent_view::AgentView) -> turn_status::Watchers {
    turn_status::Watchers {
        monitors: agent
            .session
            .bg_tasks
            .values()
            .filter(|t| t.is_monitor && t.status == mull_pager::app::agent::BgTaskStatus::Running)
            .count(),
...
fn minimal_advance_phase_timer(
    agent: &mut mull_pager::app::agent_view::AgentView,
) -> Option<mull_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &mull_pager::app::agent_view::AgentView,
    activity: &Option<mull_pager::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = mull_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = mull_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use mull_pager::views::context_bar::fmt_tokens;
    ...
    if let (Some(used), Some(total)) = (used, total)
        && total > 0
    {
        let pct = mull_token_estimation::usage_percentage(used, total);
        segs.push((
...
fn minimal_pending_hint(
    pending: &Option<mull_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> mull_pager::app::agent_view::AgentView {
        minimal_api::test_agent_view(Some("s1"), std::path::PathBuf::from("/tmp"))
    }
    #[test]
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use mull_pager::app::agent::AgentState;
        use mull_pager::scrollback::RenderBlock;
        use mull_pager::scrollback::entry::ScrollbackEntry;
        use mull_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &mull_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use mull_pager::acp::tracker::TurnActivity;
        use mull_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        mull_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        ...
        mull_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            mull_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(mull_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &mull_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use mull_pager::app::app_view::PendingAction;
        use mull_pager::input::key::KeyShortcut;
