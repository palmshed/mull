//! Minimal (scrollback-native) render mode — `mull --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the
//! terminal's *native* scrollback (via `mull_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `mull-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`mull_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`mull-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `mull-pager`.
///
...
/// subsequent calls are ignored (see [`mull_pager::minimal_hook`]).
pub fn install() {
    mull_pager::minimal_hook::install(mull_pager::minimal_hook::MinimalHooks { draw });
}
