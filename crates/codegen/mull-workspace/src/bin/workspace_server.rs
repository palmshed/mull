...
//!
//! Reads OIDC credentials from `~/.mull/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use mull_workspace::config::WorkspaceServerMetadata;
use mull_workspace::daemonize;
use mull_workspace::diag_server;
use mull_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_mull_workspace";
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<mull_tool_protocol::ServerId>()
        .err()
        .map(|e| format!("{INVALID_SERVER_ID_MARKER} {id:?}: {e}"))
}
#[derive(Parser)]
#[command(name = "mull-workspace-server")]
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.palmshed.ai/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `MULL_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "MULL_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Confine `palmshed.ai/fs/*` resolution to the workspace root (reject `..`,
    ...
    /// Override with `MULL_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "MULL_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/mull-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = mull_computer_hub_sdk::DonatingLogLayer::new_inert();
    tracing_subscriber::registry()
        .with(env_filter)
        .with(tracing_subscriber::fmt::layer())
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("MULL_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match mull_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use mull_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("MULL_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized MULL_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if mull_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            tracing::info!(
                ...
                "Sandbox explicitly disabled via MULL_SANDBOX_PROFILE=off"
            );
        } else {
            ...
            sandbox.install();
            let active = mull_sandbox::is_active();
            ...
            tracing::info!(
                ...
                mull_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = mull_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("MULL_SESSION_ID").ok();
    ...
    mull_workspace::init_metrics();
    let ws_handle = mull_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = mull_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(grace_budget, mull_workspace::handle::DrainReason::Sigterm)
        .await;
    ...
    mull_computer_hub_sdk::flush_log_layer();
...
mod tests {
    ...
    fn capabilities_flag_parses_and_defaults_off() {
        let args = Args::try_parse_from(["mull-workspace-server"]).unwrap();
        assert!(!args.capabilities);
        let args = Args::try_parse_from(["mull-workspace-server", "--capabilities"]).unwrap();
        assert!(args.capabilities);
    }
    #[test]
    ...
    fn capabilities_probe_of_legacy_binary_exits_nonzero() {
        ...
        let err = LegacyArgs::try_parse_from(["mull-workspace-server", "--capabilities"])
    ...
    fn ready_file_is_accepted_as_a_deprecated_no_op() {
        let args = Args::try_parse_from(["mull-workspace-server", "--ready-file", "/tmp/x.ready"])
            .unwrap();
    ...
    fn argv_rejection_exit_code_is_distinct_from_server_id_exit_code() {
        let err = Args::try_parse_from(["mull-workspace-server", "--flag-from-the-future"])
            .err()
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        let args = Args::try_parse_from([
            ...
            "https://palmshed.ai/preview-auth",
        ...
        assert_eq!(
            ...
            Some("https://palmshed.ai/preview-auth")
        );
