...
//! access token from `~/.mull/auth.json` does (same identity).

use base64::Engine;
use clap::Parser;
use mull_computer_hub_sdk::pool::HubConnectionPool;
use mull_computer_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &mull_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<mull_computer_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "MullBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
            {"id": "MullBuild:read_file"},
        ],
    });
    let tools = harness
