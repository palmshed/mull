...
use mull_computer_hub_mcp_adapter::McpBridgeHandle;
use mull_hunk_tracker::HunkTrackerHandle;
use mull_mcp::servers::McpState;
use mull_tool_protocol::ToolId;
use mull_tool_runtime::WorkspaceViewerContext;
use mull_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use mull_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
...
pub struct WorkspaceSession {
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, mull_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<mull_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<mull_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<mull_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<mull_hooks::error::HookError>,
    ...
    /// Auth provider for Palmshed service calls.
    pub(crate) auth_provider: Option<mull_computer_hub_sdk::SharedAuthProvider>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<mull_tools::notification::types::ToolNotificationHandle>>,
    /// Sink for workspace-originated ext-notifications to the client (e.g.
    /// `palmshed.ai/search/fuzzy/status`). Mode-agnostic: the shell wires it to the
    ...
    pub(crate) local_registry: mull_computer_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn mull_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `MULL_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$MULL_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<mull_home>/workspace` by default). The upload queue spills here.
    pub(crate) workspace_home: std::path::PathBuf,
    pub(crate) upload_queue: Option<std::sync::Arc<mull_file_utils::queue::UploadQueue>>,
    ...
    /// (`MULL_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](mull_file_utils::events::EventWriter::noop) and
    ...
    /// enabled (`MULL_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, mull_file_utils::events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<mull_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<mull_file_utils::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](mull_file_utils::events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> mull_file_utils::events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<mull_file_utils::events::EventWriter> {
    ...
    /// Auth provider used for Palmshed service calls.
    pub fn auth_provider(&self) -> Option<&mull_computer_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](mull_computer_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<mull_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, mull_file_utils::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> mull_file_utils::events::EventWriter {
...
mod tests {
    ...
    use dashmap::DashMap;
    use mull_file_utils::events::{Event, EventWriter};
