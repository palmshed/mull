...
pub(crate) fn resolve_session_toolset(
    ...
    local_registry: Option<mull_computer_hub_sdk::LocalRegistry>,
    ...
    notification_handle: Option<mull_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<mull_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<mull_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn mull_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            mull_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`MULL_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("MULL_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = mull_file_utils::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
/// `MULL_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: mull_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<mull_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$MULL_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// (`$MULL_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(mull_tools::computer::local::LocalFs)
            as Arc<dyn mull_tools::computer::types::AsyncFileSystem>;
        let notification_handle = mull_tools::notification::ToolNotificationHandle::noop();
        let (image_gen_config, video_gen_config, web_search_config, app_builder_deployer_config) =
            if let (Some(auth), Some(url)) = (&self.auth, &self.api_base_url) {
                ...
                match cred {
                    mull_computer_hub_sdk::AuthCredential::Bearer { token, .. } => {
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = mull_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("mull-workspace/{version}"),
    );
    headers.insert("x-mull-client-version".to_string(), version.to_string());
...
/// `MULL_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> mull_tools::implementations::mull_build::web_fetch::WebFetchConfig {
    use mull_tools::implementations::mull_build::web_fetch::{WebFetchConfig, WebFetchParams};
    if std::env::var("MULL_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("MULL_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("MULL_WEB_SEARCH_MODEL").unwrap_or_else(|_| "openrouter/free".to_string())
}
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("MullBuild:search_replace", Some(ToolKind::Edit)),
                tc("MullBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let mut mcp_dup = test_support::tc("MullBuild:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("MullBuild:search_replace".to_owned(), ToolKind::Edit),
            ("MullBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("MullBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                test_support::tc("MullBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("MullBuild:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("MullBuild:read_file", None),
                test_support::tc("MullBuild:grep", None),
                test_support::tc("MullBuild:list_dir", None),
                test_support::tc("MullBuild:search_replace", None),
                test_support::tc("MullBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("MullBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"MullBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "MULL_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use mull_tools::types::resources::{State, WebCitationCounter};
