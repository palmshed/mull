...
///
/// ```bash
/// PALMSHED_API_KEY='palmshed-some-key' cargo run --bin mull-pager
/// ```
///
...
///   prefix: ["PALMSHED_API_KEY=palmshed-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "mull-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        assert_eq!(
            ...
            Some(BashCommandHighlights {
                prefix: vec!["PALMSHED_API_KEY=palmshed-some-key".to_owned()],
        ...
        let redirection_command = "cargo build --bin mull-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package mull-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
