...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "mull-shell", alias = "mull_shell")]
    Generic,
    /// Mull TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "mull-tui", alias = "mull_tui")]
    MullTui,
    /// Mull Web client - identified by clientIdentifier "mull-web"
    #[serde(rename = "mull_web")]
    MullWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "mull-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// Mull Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to MullTui for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"mull-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"mull_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "mull-pager", alias = "mull_pager")]
    MullPager,
    /// Mull Desktop (Electron) client - identified by clientIdentifier "mull-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "mull_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `mull-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "mull-shell",
            Self::MullTui => "mull-tui",
            Self::MullWeb => "mull-web",
            Self::Nebula => "nebula",
            Self::Extension => "mull-code-extension",
            Self::MullPager => "mull-pager",
            Self::Desktop => "mull-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"mull-web"`, `"mull-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("mull-web") => Self::MullWeb,
            Some("nebula") => Self::Nebula,
            Some("mull-code-extension") => Self::Extension,
            Some("mull-desktop") => Self::Desktop,
            Some("mull-pager") => Self::MullPager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::MullTui | Self::MullPager => "tui",
            Self::MullWeb => "web",
            Self::Nebula => "nebula",
...
impl From<&mull_tools::types::ToolInput> for AccessKind {
    fn from(input: &mull_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.mull/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use mull_tools::implementations::mull_build_hashline::edit::types::HashlineEditInput;
        use mull_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use mull_tools::implementations::mull_build::bash::BashToolInput;
        use mull_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use mull_tools::implementations::use_tool::UseToolInput;
        use mull_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use mull_tools::implementations::mull_build::monitor::types::MonitorInput;
        use mull_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use mull_tools::implementations::mull_build::web_fetch::WebFetchInput;
        use mull_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use mull_tools::implementations::mull_build::web_search::WebSearchInput;
        use mull_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_mull_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("mull-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("mull_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
