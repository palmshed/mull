...
use mull_hunk_tracker::{HunkTrackerActor, HunkTrackerHandle, TrackingMode};
use mull_tool_protocol::ToolServerStatusPayload;
use mull_tool_protocol::turn_hook::TurnHookOutcome;
use prometheus::{
...
/// `MULL_WORKSPACE_TERMINATION_GRACE_MS`. 45s fits under the K8s grace period.
const DEFAULT_TERMINATION_GRACE_MS: u64 = 45_000;
/// preStop-hook drain marker; override via `MULL_WORKSPACE_DRAINING_FILE`.
...
static DRAIN_STARTED_TOTAL: std::sync::LazyLock<IntCounterVec> = std::sync::LazyLock::new(|| {
    register_int_counter_vec!(
        "mull_workspace_drain_started_total",
...
static DRAIN_COMPLETED_TOTAL: std::sync::LazyLock<IntCounterVec> = std::sync::LazyLock::new(|| {
    register_int_counter_vec!(
        "mull_workspace_drain_completed_total",
...
static DRAIN_DURATION: std::sync::LazyLock<Histogram> = std::sync::LazyLock::new(|| {
    register_histogram!(
        "mull_workspace_drain_duration_seconds",
...
static DRAIN_LOST_ITEMS_TOTAL: std::sync::LazyLock<IntCounter> = std::sync::LazyLock::new(|| {
    register_int_counter!(
        "mull_workspace_drain_lost_items_total",
...
static PRODUCER_SPAWNED_AFTER_DRAIN_TOTAL: std::sync::LazyLock<IntCounter> =
    std::sync::LazyLock::new(|| {
        register_int_counter!(
            "mull_workspace_producer_spawned_after_drain_total",
...
static WORKSPACE_BIND_ZERO_TOOLS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "mull_workspace_bind_zero_tools_total",
            "session.bind resolutions advertising zero model-facing tools, by reason",
            &["reason"]
        )
        .unwrap()
...
static WORKSPACE_BIND_FAILED_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "mull_workspace_bind_failed_total",
...
static WORKSPACE_BIND_UNSERVED_TOOLS_TOTAL: std::sync::LazyLock<IntCounter> =
    std::sync::LazyLock::new(|| {
        register_int_counter!(
            "mull_workspace_bind_unserved_tools_total",
...
static WORKSPACE_BIND_ADVERTISED_TOOLS: std::sync::LazyLock<Histogram> =
    std::sync::LazyLock::new(|| {
        register_histogram!(
            "mull_workspace_bind_advertised_tools",
...
pub(crate) static WORKSPACE_TERMINAL_BACKEND_ORPHANED_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "mull_workspace_terminal_backend_orphaned_total",
...
static ENV_CAPTURE_PANIC_TOTAL: std::sync::LazyLock<IntCounter> = std::sync::LazyLock::new(|| {
    register_int_counter!(
        "mull_workspace_env_capture_panic_total",
...
use mull_file_utils::events::types::CancellationCategory;
use mull_file_utils::events::{Event, SessionRelationship, TurnOutcomeLabel};
use mull_file_utils::queue::EnqueueOutcome;
use mull_tool_protocol::turn_hook::{AfterTurnAckPayload, AfterTurnAckStatus};
...
pub(crate) static REWIND_CHECKPOINT_CAPTURE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "mull_workspace_rewind_checkpoint_capture_total",
...
pub(crate) static REWIND_CHECKPOINT_FINALIZE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "mull_workspace_rewind_checkpoint_finalize_total",
...
pub(crate) static REWIND_RESTORE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "mull_workspace_rewind_restore_total",
...
pub(crate) static REWIND_CHECKPOINT_DURATION: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "mull_workspace_rewind_checkpoint_duration_seconds",
...
pub(crate) static REWIND_NON_COMPLETED_FINALIZE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "mull_workspace_rewind_non_completed_finalize_total",
...
impl WorkspaceHandle {
    ...
    pub async fn trace_donation_reporter(
        ...
        mull_computer_hub_sdk::HubDonatingReporter,
        mull_computer_hub_sdk::TraceDonationPump,
    )> {
        self.shared
            .hub_handle
            .lock()
    ...
    /// teardown.
    ///
    /// [`LogDonationSender`]: mull_computer_hub_sdk::LogDonationSender
    pub async fn log_donation_layer(
        &self,
        service_name: &str,
    ) -> Option<(
        mull_computer_hub_sdk::LogDonationSender,
        mull_computer_hub_sdk::LogDonationPump,
    )> {
        self.shared
            .hub_handle
            .lock()
    ...
    pub async fn metric_donation_reporter(
        &self,
        service_name: &str,
    ) -> Option<mull_computer_hub_sdk::MetricDonationPump> {
        self.shared
            .hub_handle
            .lock()
            .await
    ...
    /// Construct a handle with an explicit `$MULL_WORKSPACE_HOME` and a
    /// pre-spawned [`UploadQueue`](mull_file_utils::queue::UploadQueue).
    ///
    ...
    pub(crate) fn new_with_data_collection(
        ...
        upload_queue: Arc<mull_file_utils::queue::UploadQueue>,
    ...
    fn build(
        config: WorkspaceConfig,
        workspace_home: std::path::PathBuf,
        upload_queue: Option<Arc<mull_file_utils::queue::UploadQueue>>,
        ...
        let local_registry = mull_computer_hub_sdk::LocalRegistry::new();
        ...
        let (hook_registry, hook_load_errors) = {
            use mull_hooks::discovery::{HookSource, load_hooks_from_sources};
        ...
        let lsp: Option<Arc<dyn mull_tools::implementations::lsp::LspBackend>> = {
            let sourced =
                mull_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
                    &config.root_cwd,
                    &[],
                    &[],
                    &[],
            ...
            let servers =
                mull_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
                    sourced,
            ...
            if servers.is_empty() {
                ...
                use mull_tools::implementations::lsp::{LspBackend, LspBackendAdapter, LspManager};
                let mgr = Arc::new(tokio::sync::Mutex::new(LspManager::new(
                    servers,
                    config.root_cwd.clone(),
                    true,
                    mull_tools::notification::ToolNotificationHandle::noop(),
                )));
        ...
        let session_event_writers: Arc<
            dashmap::DashMap<String, mull_file_utils::events::EventWriter>,
    ...
    /// The [`ToolServer`](mull_computer_hub_sdk::ToolServer) for this
    ...
    pub fn hub_server(&self) -> Option<mull_computer_hub_sdk::ToolServer> {
    ...
    pub async fn hub_server_blocking(&self) -> Option<mull_computer_hub_sdk::ToolServer> {
    ...
    pub fn create_session_with_config(
        ...
        session_id: impl Into<String>,
        cwd: Option<std::path::PathBuf>,
        tool_config: Option<mull_tools::registry::types::ToolServerConfig>,
        capability: CapabilityMode,
        viewer_ctx: Option<mull_tool_runtime::WorkspaceViewerContext>,
    ...
    pub fn create_session_with_tracker_and_viewer_ctx(
        ...
        let system_notify_channel = system_notifications
            .then(mull_tools::notification::types::ToolNotificationHandle::channel);
    ...
    pub(crate) async fn update_tool_config(
        ...
        session_id: &str,
        new_config: mull_tools::registry::types::ToolServerConfig,
    ...
    async fn resolve_and_swap_session_toolset_locked(
        ...
        let _ = self
            .shared
            .events
            .send(mull_workspace_types::WorkspaceEvent::ToolsChanged {
    ...
    pub(crate) async fn rebind_existing_hub_session(
        &self,
        session_id: &str,
        explicit_cfg: Option<mull_tools::registry::types::ToolServerConfig>,
    ...
    pub async fn on_before_turn(
        &self,
        session_id: &str,
        payload: &mull_tool_protocol::turn_hook::BeforeTurnPayload,
    ) {
    ...
    pub async fn on_after_turn(
        &self,
        session_id: &str,
        payload: &mull_tool_protocol::turn_hook::AfterTurnPayload,
    ) {
    ...
    pub async fn compute_turn_injections(
        &self,
        session_id: &str,
        request: &mull_tool_protocol::turn_hook::TurnHookRequest,
    ) -> mull_tool_protocol::turn_hook::HookReply {
        use mull_tool_protocol::turn_hook::{HookReply, TurnHookRequest};
        match request {
    ...
    /// `MULL_WORKSPACE_TOOL_STATE_ENABLED` is off, opted out,
    ...
    ///
    /// No-op when the `MULL_WORKSPACE_TOOL_DEFS_ENABLED` flag is off, no upload
    ...
    pub fn cancel_tool_call(&self, session_id: &str, call_id: &str) {
        self.shared.activity_tracker.tool_call_completed(
            call_id,
            Some(session_id),
            mull_file_utils::events::ToolOutcome::Cancelled,
        );
    ...
    pub fn hook_registry(&self) -> mull_hooks::discovery::HookRegistry {
    ...
    pub fn hook_load_errors(&self) -> &[mull_hooks::error::HookError] {
    ...
    /// polling / stop. Drives the `palmshed.ai/search/fuzzy/status` notification loop.
    ...
    /// Drive the `palmshed.ai/search/fuzzy/status` stream for an active search: poll
    ...
    pub async fn run_fuzzy_notifications(
        ...
        for _ in 0..max_polls {
            ...
            self.emit_client_ext("palmshed.ai/search/fuzzy/status".to_string(), params);
            if data.done {
                break;
            }
        }
    ...
    /// batch as `palmshed.ai/search/content/status` via the client sink, and returning
    ...
    pub async fn run_content_search(
        ...
        crate::file_system::content_search_streaming(&cwd, &params, cancel, move |batch| {
            ...
            handle.emit_client_ext("palmshed.ai/search/content/status".to_string(), params);
        })
        .await
    ...
    pub fn get_or_create_codebase_index(
        &self,
        cwd: std::path::PathBuf,
    ) -> (Arc<mull_codebase_graph::IndexManagerHandle>, bool) {
        self.shared.codebase_indexes.lock().get_or_create(cwd)
    }
    pub fn get_codebase_index(
        &self,
        cwd: &std::path::Path,
    ) -> Option<Arc<mull_codebase_graph::IndexManagerHandle>> {
    ...
    fn spawn_codebase_index_event_forwarder(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(mull_workspace_types::WorkspaceEvent::FsChanged { ref path, kind }) => {
                    ...
                    Ok(mull_workspace_types::WorkspaceEvent::GitHeadChanged { .. }) => {
    ...
    fn spawn_tool_definitions_event_forwarder(&self) -> Option<tokio::task::JoinHandle<()>> {
        ...
        Some(tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(mull_workspace_types::WorkspaceEvent::ToolsChanged { session_id }) => {
    ...
    async fn emit_environment_artifact(
    ...
    ) -> Option<mull_file_utils::queue::EnqueueOutcome> {
        ...
        let env = {
            ...
            .in_span(
                fastrace::Span::root(
                    ...
                    trace_parent.unwrap_or_else(mull_tracing::local_or_random_span_ctx),
                )
                .with_properties(|| {
                    [
                        ("session_id", session_id.to_owned()),
        ...
        let outcome = upload_queue
            ...
            .await;
        match &outcome {
            mull_file_utils::queue::EnqueueOutcome::Failed { reason: _ } => {
                dc_log!(
    ...
    pub async fn start_session_mcp_servers(
        ...
        use mull_computer_hub_mcp_adapter::McpBridge;
        use mull_computer_hub_sdk::ToolServerHandler as _;
        use mull_mcp::servers::MCP_TOOL_NAME_DELIMITER;
        use mull_tool_protocol::SessionId;
        let tool_server = {
        ...
        let mcp_results: Vec<Result<mull_mcp::servers::McpClient, mull_mcp::servers::McpError>> =
            tokio::task::spawn_blocking(move || {
                use mull_mcp::oauth_config::McpOAuthConfigMap;
                use mull_mcp::servers::{McpClientTimeoutOverrides, McpMetaConfigMap};
                ...
                rt_handle.block_on(mull_mcp::servers::start_mcp_servers(
                    configs,
                    ...
                    &oauth_config_map,
                    &event_writer,
                    mull_mcp::servers::OauthInteractivity::Interactive,
                ))
            })
            .await
        ...
        for result in mcp_results {
            match result {
                Ok(client) => {
                    ...
                    let transport: Arc<dyn mull_computer_hub_mcp_adapter::McpTransport> =
                        ...
                        Ok(handle) => {
                            for handler in handle.bridge.handlers() {
                                    ...
                                    .await
                                {
                                    ...
                                    mull_tool_protocol::ToolId::new(&qualified_name)
                                {
    ...
    pub async fn teardown_session_mcp(&self, session_id: &str) {
        ...
        let sid = match mull_tool_protocol::SessionId::new(session_id) {
            Ok(s) => s,
            Err(_) => return,
        };
        let mut tool_ids = session.mcp_tool_ids.lock().await;
    ...
    pub fn on_mcp_snapshot_changed(
        &self,
        new_snapshot: Vec<mull_tools::registry::types::ToolConfig>,
    ) -> usize {
    ...
    pub fn on_hub_tools_changed(
        &self,
        new_hub_tools: Vec<mull_tools::registry::types::ToolConfig>,
    ) -> usize {
        self.shared
            .hub_tools_snapshot
    ...
    pub(crate) fn session_bind_resolver(
        &self,
        catalog: Arc<Vec<Arc<dyn mull_computer_hub_sdk::ToolServerHandler>>>,
        rpc_tool_id: mull_tool_protocol::ToolId,
    ) -> mull_computer_hub_sdk::SessionHandlerResolver {
        let weak_shared = Arc::downgrade(&self.shared);
        Arc::new(
            move |sid: mull_tool_protocol::SessionId, params: Option<serde_json::Value>| {
                ...
                let bind_parent = params
                    ...
                    .unwrap_or_else(mull_tracing::local_or_random_span_ctx);
                ...
                async move {
                    let Some(shared) = weak_shared.upgrade() else {
                        WORKSPACE_BIND_FAILED_TOTAL
                            ...
                            .inc();
                        return Err(
                            mull_tool_runtime::ToolError::service_unavailable(
                    ...
                    let bind_config = params
                        ...
                        .unwrap_or_default();
                    let empty_toolset = || mull_tools::registry::types::ToolServerConfig {
                        tools: vec![],
                    ...
                    Ok(mull_computer_hub_sdk::ResolvedSessionHandlers {
                        handlers,
                        unserved_tool_ids,
                        resolve_error,
                    })
    ...
    pub async fn connect_hub(&self) -> WorkspaceResult<()> {
        ...
        let (template_handlers, rpc_tool_id) = {
            ...
            let tool_names: Vec<String> = handlers
                ...
                .map(|h| h.tool_id().as_str().to_owned())
                .collect();
            let rpc_handler: Arc<dyn mull_computer_hub_sdk::ToolServerHandler> =
        ...
        let catalog: Arc<Vec<Arc<dyn mull_computer_hub_sdk::ToolServerHandler>>> =
        ...
        let (activity_notify_handle, activity_notify_rx) =
            mull_tools::notification::types::ToolNotificationHandle::channel();
        ...
        let listener_task = tokio::spawn(async move {
            while let Some(notification) = notification_rx.recv().await {
                match notification {
                    mull_computer_hub_sdk::HubNotification::ToolsChanged {
                        added,
                        removed,
                        updated,
                        ..
            ...
            loop {
                match rx.recv().await {
                    Ok(event) => {
                        ...
                        let frame = mull_tool_protocol::ToolNotificationFrame::custom(
                            mull_tool_protocol::ToolId::new(
        ...
        let status_publisher_task = tokio::spawn(async move {
            ...
            async fn send_status(
                conn: &mull_computer_hub_sdk::HubConnection,
                ...
                let req = mull_tool_protocol::JsonRpcRequest {
                    jsonrpc: mull_tool_protocol::JsonRpcVersion,
                    id: mull_tool_protocol::JsonRpcId::from_request_id(&request_id),
                    session_id: None,
                    method: mull_tool_protocol::Method::ToolServerStatus
                        .as_wire_str()
                        .to_owned(),
                    params,
                };
...
fn build_session_routed_handlers(
    toolset: &mull_tools::registry::types::FinalizedToolset,
    ws: &WorkspaceHandle,
) -> Vec<Arc<dyn mull_computer_hub_sdk::ToolServerHandler>> {
    ...
    for def in toolset.tool_definitions() {
        ...
        let mut desc = mull_tool_types::ToolDescription::new(
        ...
        match crate::hub::SessionRoutedToolHandler::new(
            ...
            Ok(handler) => handlers
                .push(Arc::new(handler) as Arc<dyn mull_computer_hub_sdk::ToolServerHandler>),
            Err(e) => {
                tracing::warn!(
...
pub(crate) fn apply_background_task_notification(
    ...
    notification: &mull_tools::notification::types::ToolNotification,
) {
    use mull_tools::notification::types::ToolNotification;
    match notification {
...
pub(crate) async fn run_activity_feed(
    ...
    mut rx: tokio::sync::mpsc::UnboundedReceiver<mull_tools::notification::types::ToolNotification>,
) {
...
impl DrainReason {
    /// Stable `reason` label for `mull_workspace_drain_started_total`.
...
impl DrainOutcome {
    /// Stable `outcome` label for `mull_workspace_drain_completed_total`.
    pub fn as_str(self) -> &'static str {
        match self {
...
/// The SIGTERM drain budget from `MULL_WORKSPACE_TERMINATION_GRACE_MS`
...
pub fn termination_grace_from_env() -> std::time::Duration {
    grace_budget_from_raw(std::env::var("MULL_WORKSPACE_TERMINATION_GRACE_MS").ok())
}
...
/// Path of the preStop drain marker (`MULL_WORKSPACE_DRAINING_FILE` or
/// [`DEFAULT_DRAINING_FILE`]).
fn draining_file_path() -> std::path::PathBuf {
    std::env::var("MULL_WORKSPACE_DRAINING_FILE")
        .map(std::path::PathBuf::from)
...
///
/// `confine_fs_to_workspace_root` confines `palmshed.ai/fs/*` resolution to the root.
/// The standalone workspace server defaults it on (it always backs a remote
/// sandbox; override via `MULL_WORKSPACE_CONFINE_FS_TO_ROOT`); the CLI leader
/// passes `false`.
///
...
pub async fn connect_local_workspace(
    cwd: std::path::PathBuf,
    hub_url: url::Url,
    auth: mull_computer_hub_sdk::SharedAuthProvider,
    ...
    let api_base_url = std::env::var("MULL_CLI_CHAT_PROXY_BASE_URL")
        .unwrap_or_else(|_| "https://openrouter.ai/api/v1".to_string());
    let data_collection_disabled =
        std::env::var("MULL_WORKSPACE_DATA_COLLECTION_DISABLED").as_deref() != Ok("false");
    ...
    let tool_config = mull_agent::workspace_mull_build_toolset();
    ...
    if let Ok(dir) = std::env::var("MULL_WORKSPACE_SERVER_SKILLS_DIR")
        && !dir.is_empty()
    {
        ws_config.skills_config.server_skill_dirs = vec![dir];
    }
    if let Ok(dir) = std::env::var("MULL_WORKSPACE_BUNDLED_SKILLS_DIR")
        && !dir.is_empty()
    {
        let allowlist = std::env::var("MULL_WORKSPACE_BUNDLED_SKILLS_ALLOWLIST").ok();
        ws_config
            .skills_config
            .ignore
    ...
    let trace_source: Arc<dyn mull_file_utils::queue::TraceExportSource> = Arc::new(
        crate::upload::WorkspaceTraceExportSource::new(proxy_storage.clone()),
    );
    let upload_queue = Arc::new(mull_file_utils::queue::UploadQueue::spawn(
        &workspace_home,
        trace_source,
        mull_file_utils::queue::UploadRetryPolicy::default(),
    ));
    ...
    upload_queue.cleanup_orphans(mull_file_utils::queue::DEFAULT_MAX_AGE);
...
/// Resolve `$MULL_WORKSPACE_HOME` — the workspace-owned on-disk state root.
///
/// Precedence:
/// 1. `$MULL_WORKSPACE_HOME` (operator override).
/// 2. `<mull_home>/workspace`, where `<mull_home>` honours `$MULL_HOME` and
///    otherwise falls back to `~/.mull` (see [`mull_config::mull_home`]).
pub fn resolve_workspace_home() -> std::path::PathBuf {
    if let Ok(p) = std::env::var("MULL_WORKSPACE_HOME")
        && !p.trim().is_empty()
    {
        return std::path::PathBuf::from(p);
    }
    mull_config::mull_home().join("workspace")
}
...
/// (`MULL_WORKSPACE_EVENTS_ENABLED=true`). Any other value — including unset —
/// keeps the legacy behaviour: [`WorkspaceShared::session_event_writer`] hands
/// back [`EventWriter::noop()`](mull_file_utils::events::EventWriter::noop)
/// and no `events.jsonl` is ever opened.
fn events_enabled() -> bool {
    std::env::var("MULL_WORKSPACE_EVENTS_ENABLED").as_deref() == Ok("true")
}
...
/// `MULL_WORKSPACE_AFTER_TURN_WATCHDOG_MS` (malformed values fall back).
fn after_turn_watchdog() -> std::time::Duration {
    const DEFAULT_MS: u64 = 8_000;
    let ms = std::env::var("MULL_WORKSPACE_AFTER_TURN_WATCHDOG_MS")
        .ok()
...
/// (`MULL_WORKSPACE_TOOL_DEFS_ENABLED=true`; any other value keeps legacy
/// behaviour).
fn tool_defs_enabled() -> bool {
    std::env::var("MULL_WORKSPACE_TOOL_DEFS_ENABLED").as_deref() == Ok("true")
}
...
async fn enqueue_workspace_tool_definitions(
    upload_queue: &mull_file_utils::queue::UploadQueue,
    session_id: &str,
    object_path: &str,
    bytes: &[u8],
) -> mull_file_utils::queue::EnqueueOutcome {
    use mull_file_utils::queue::EnqueueOutcome;
    let outcome = upload_queue
        .enqueue_bytes_blocking(
            bytes,
...
fn turn_outcome_label(outcome: mull_tool_protocol::turn_hook::TurnHookOutcome) -> TurnOutcomeLabel {
    use mull_tool_protocol::turn_hook::TurnHookOutcome;
    match outcome {
...
/// backing upload queue (tests, local mode). Never the real mull home —
/// only [`connect_local_workspace`] resolves `$MULL_WORKSPACE_HOME` — so the
/// queue-less default path can never collide with a real workspace's state dir.
fn ephemeral_workspace_home() -> std::path::PathBuf {
    std::env::temp_dir().join(format!("mull-workspace-ephemeral-{}", std::process::id()))
}
/// Resolve `workspace_rewind_all_outcomes` from `MULL_WORKSPACE_REWIND_ALL_OUTCOMES` (default off).
fn rewind_all_outcomes_from_env() -> bool {
    mull_config::env_bool("MULL_WORKSPACE_REWIND_ALL_OUTCOMES").unwrap_or(false)
}
...
struct SessionToolHandle {
    tool_id: mull_tool_protocol::ToolId,
    desc: mull_tool_types::ToolDescription,
...
impl SessionToolHandle {
    fn new(
        ...
        workspace: WorkspaceHandle,
        session_id: String,
    ) -> Result<Self, mull_tool_protocol::IdError> {
        Ok(Self {
            tool_id: mull_tool_protocol::ToolId::new(tool_name)?,
            desc,
            workspace,
            session_id,
        })
...
impl mull_tool_runtime::ToolDyn for SessionToolHandle {
    fn id(&self) -> mull_tool_protocol::ToolId {
        self.tool_id.clone()
    }
    fn description(
        &self,
        _ctx: &::mull_tool_runtime::ListToolsContext,
    ) -> mull_tool_types::ToolDescription {
        self.desc.clone()
    }
    async fn execute(
        &self,
        ctx: mull_tool_runtime::ToolCallContext,
        args: serde_json::Value,
    ) -> mull_tool_runtime::ToolStream<mull_tool_runtime::TypedToolOutput> {
        use mull_tool_runtime::{ToolError, ToolErrorKind, ToolStreamItem, terminal_only};
...
impl WorkspaceHandle {
    ...
    pub fn create_local_harness(
        &self,
        session_id: &str,
    ) -> WorkspaceResult<mull_computer_hub_sdk::ToolHarness> {
        let session = self
            .session(session_id)
            .ok_or_else(|| WorkspaceError::SessionNotFound(session_id.to_string()))?;
        let toolset = session.toolset();
        let registry = mull_computer_hub_sdk::LocalRegistry::new();
        for def in toolset.tool_definitions() {
            let tool_name = def.function.name.clone();
            let desc = mull_tool_types::ToolDescription::new(
                tool_name.clone(),
                def.function.description.clone().unwrap_or_default(),
            );
            match SessionToolHandle::new(tool_name, desc, self.clone(), session_id.to_string()) {
                Ok(tool) => {
                    registry.register_dyn(Arc::new(tool) as Arc<dyn mull_tool_runtime::ToolDyn>);
                }
                Err(e) => {
                    tracing::warn!(
                        tool = % def.function.name, error = % e,
        ...
        let session_id = mull_tool_protocol::SessionId::new(session_id.to_string())
            .map_err(|e| WorkspaceError::HubError(format!("invalid session id: {e}")))?;
        Ok(mull_computer_hub_sdk::ToolHarness::local_only_with(
            registry,
            session_id,
            mull_tool_runtime::TypedExtensions::default(),
        ))
    }
}
impl WorkspaceHandle {
    ...
    pub fn new_minimal(
        ...
        let config = WorkspaceConfig {
            root_cwd: cwd,
            default_tool_config: mull_tools::registry::types::ToolServerConfig {
                tools: vec![],
                behavior_preset: None,
            },
...
pub(crate) mod tests {
    ...
    use mull_tools::registry::types::ToolServerConfig;
    use mull_tools::types::tool::ToolKind;
    use mull_workspace_types::WorkspaceEvent;
    use std::sync::Arc;
    ...
    fn make_handle_inner(
        ...
        let config = WorkspaceConfig {
            ...
            plugin_discovery_config: Default::default(),
            hub_config: None,
            auth_provider: Some(Arc::new(mull_computer_hub_sdk::AuthCredential::bearer(
                "test-token",
            ))),
            server_metadata: None,
            status_config,
    ...
    impl mull_tools::types::tool_metadata::ToolMetadata for BashCcoStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Execute
        }
        fn tool_namespace(&self) -> mull_tools::types::tool::ToolNamespace {
            mull_tools::types::tool::ToolNamespace::MCP
        }
        fn description_template(&self) -> &str {
            "bash cco stub"
        }
    }
    impl mull_tool_runtime::Tool for BashCcoStub {
        type Args = serde_json::Value;
        type Output = mull_tools::types::output::ToolOutput;
        fn id(&self) -> mull_tool_protocol::ToolId {
            mull_tool_protocol::ToolId::new(BASH_CCO_STUB_NAME).expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::mull_tool_runtime::ListToolsContext,
        ) -> mull_tool_types::ToolDescription {
            mull_tool_types::ToolDescription::new(BASH_CCO_STUB_NAME, "bash cco stub")
        }
        async fn run(
            &self,
            _ctx: mull_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> Result<mull_tools::types::output::ToolOutput, mull_tool_runtime::ToolError> {
            let output = BASH_CCO_STUB_STDOUT.as_bytes();
            Ok(mull_tools::types::output::ToolOutput::Bash(
                mull_tools::types::output::BashOutput {
                    output: output.to_vec(),
                    output_for_prompt:
                        mull_tools::types::output::BashOutput::make_output_for_prompt(
    ...
    pub(crate) fn assert_bash_cco_terminal(typed: &mull_tool_runtime::TypedToolOutput) {
        use mull_tool_runtime::ToolOutput as _;
        let resp = typed
    ...
    pub(crate) async fn drain_terminal_ok(
        mut stream: impl futures::Stream<
            Item = mull_tool_runtime::ToolStreamItem<mull_tool_runtime::TypedToolOutput>,
        > + Unpin,
    ) -> mull_tool_runtime::TypedToolOutput {
        use futures::StreamExt;
        use mull_tool_runtime::ToolStreamItem;
    ...
    async fn local_harness_preserves_bash_chat_completion_output() {
        use mull_tool_runtime::ToolCallContext;
        let handle = make_handle();
        register_bash_cco_stub(&handle);
        let harness = handle.create_local_harness("main").expect("local harness");
        let tool_id = mull_tool_protocol::ToolId::new(BASH_CCO_STUB_NAME).expect("valid tool id");
        let stream = harness
    ...
    async fn donation_entry_points_are_inert_without_a_hub() {
        ...
        assert!(
            handle
                .trace_donation_reporter("prod_mull_workspace")
                .await
                .is_none(),
        ...
        assert!(
            handle
                .log_donation_layer("prod_mull_workspace")
                .await
                .is_none(),
        ...
        assert!(
            handle
                .metric_donation_reporter("prod_mull_workspace")
                .await
                .is_none(),
    ...
    async fn build_session_routed_handlers_skips_invalid_client_name_without_panic() {
        let handle = make_handle();
        let mut renamed = tc("MullBuild:read_file", Some(ToolKind::Read));
        ...
        let session = handle
            .create_session_with_config(
                ...
                Some(ToolServerConfig {
                    tools: vec![renamed, tc("MullBuild:grep", Some(ToolKind::Read))],
                    behavior_preset: None,
                }),
                CapabilityMode::All,
                None,
    ...
    /// ABSENT from that (mull-build) catalog is still advertised. The old
    /// `catalog ∩ session-names` filter silently dropped exactly such tools
    /// (mull-build renames → 6/11).
    #[tokio::test]
    ...
    async fn create_fingerprint_write_does_not_clobber_concurrent_rebind() {
        ...
        let fp_a = serde_json::to_value(&ToolServerConfig {
            tools: vec![tc("MullBuild:list_dir", Some(ToolKind::ListDir))],
            behavior_preset: None,
        })
        .ok();
    ...
    async fn rebind_explicit_to_explicit_with_in_flight_call_defers_then_corrects() {
        use mull_file_utils::events::ToolOutcome;
    ...
    async fn toolset_terminal(
        toolset: &Arc<mull_tools::registry::types::FinalizedToolset>,
    ) -> Arc<dyn mull_tools::computer::types::TerminalBackend> {
        let res = toolset.resources.lock().await;
        res.get::<mull_tools::types::resources::Terminal>()
            .map(|t| t.0.clone())
    ...
    pub(crate) fn background_capable_cfg() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                tc("MullBuild:read_file", Some(ToolKind::Read)),
                tc("MullBuild:run_terminal_cmd", Some(ToolKind::Execute)),
                tc(
                    "MullBuild:get_task_output",
                    Some(ToolKind::BackgroundTaskAction),
                ),
                tc("MullBuild:kill_task", Some(ToolKind::KillTaskAction)),
            ],
            behavior_preset: None,
        }
    }
    ...
    ///
    /// [`TerminalRunRequest`]: mull_tools::computer::types::TerminalRunRequest
    pub(crate) fn terminal_run_request(
        command: &str,
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> mull_tools::computer::types::TerminalRunRequest {
        mull_tools::computer::types::TerminalRunRequest {
            ...
            notification_handle: mull_tools::notification::ToolNotificationHandle::noop(),
            tool_call_id: tool_call_id.to_string(),
            display_command: None,
            auto_background_on_timeout: false,
            foreground_block_budget: None,
            kind: mull_tools::computer::types::TaskKind::Bash,
    ...
    pub(crate) async fn start_background_sleep(
        ...
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> mull_tools::computer::types::BackgroundHandle {
        session
            .terminal_backend()
    ...
    impl crate::config::SessionContextFactory for PersistentShellFactory {
        fn build_session_context(
            ...
            backend: Arc<dyn mull_tools::computer::types::TerminalBackend>,
        ) -> mull_tools::registry::types::SessionContext {
            self.inner
        ...
        fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
            crate::config::SessionTerminalBackend::local(
                mull_tools::computer::local::LocalTerminalBackend::with_persistent_shell(),
            )
        }
        fn registry_builder(&self) -> mull_tools::registry::types::ToolRegistryBuilder {
    ...
    async fn restarted_workspace_recreates_session_and_reports_lost_task() {
        ...
        let mull_tools::types::output::ToolOutput::TaskOutput(
            mull_tool_types::TaskOutputOutput::TaskNotFound(msg),
        ) = &result.output
        else {
    ...
    async fn client_ext_sink_receives_emitted_notification() {
        ...
        handle.emit_client_ext("palmshed.ai/noop".to_string(), serde_json::json!({}));
        ...
        handle.emit_client_ext(
            "palmshed.ai/search/fuzzy/status".to_string(),
            serde_json::json!({ "a" : 1 }),
        );
        let got = captured.lock();
        assert_eq!(got.len(), 1);
        assert_eq!(got[0].0, "palmshed.ai/search/fuzzy/status");
    ...
    /// `palmshed.ai/search/fuzzy/status` is delivered through the sink with the match.
    #[tokio::test]
    async fn fuzzy_change_streams_status_through_sink() {
        ...
        let sink_captured = captured.clone();
        handle.set_client_ext_sink(Arc::new(move |method, params| {
            if method == "palmshed.ai/search/fuzzy/status" {
    ...
    async fn events_jsonl_captures_turn_tool_toggle_and_mcp_variants() {
        use mull_file_utils::events::ToolOutcome;
        use mull_tool_protocol::turn_hook::{AfterTurnPayload, BeforeTurnPayload, TurnHookOutcome};
        ...
        handle.shared().session_event_writer(sid).emit(
            mull_file_utils::events::Event::McpToolCallStarted {
    ...
    async fn before_turn_hooks_sync_session_yolo_mode() {
        use mull_tool_protocol::turn_hook::{BeforeTurnPayload, TurnHookRequest};
        let handle = make_handle();
        ...
        assert_eq!(
            reply,
            mull_tool_protocol::turn_hook::HookReply::default(),
    ...
    async fn before_turn_yolo_transition_emits_yolo_toggled_event() {
        use mull_tool_protocol::turn_hook::BeforeTurnPayload;
        let (handle, home) = make_handle_with_events();
        let sid = "sess-yolo";
        let _session = handle
    ...
    fn turn_outcome_label_maps_every_variant() {
        use mull_file_utils::events::TurnOutcomeLabel;
        use mull_tool_protocol::turn_hook::TurnHookOutcome;
        assert!(matches!(
    ...
    impl mull_file_utils::queue::TraceExportSource for UnreachableSource {
        fn resolve(&self) -> mull_file_utils::TraceExportConfig {
            mull_file_utils::TraceExportConfig {
                bucket_url: None,
                service_account_key: None,
                upload_method: mull_file_utils::UploadMethod::Proxy {
    ...
    fn spawn_test_queue(home: &std::path::Path) -> Arc<mull_file_utils::queue::UploadQueue> {
        let policy = mull_file_utils::queue::UploadRetryPolicy {
            initial_delay: std::time::Duration::from_secs(3600),
            ..Default::default()
        };
        Arc::new(mull_file_utils::queue::UploadQueue::spawn(
            home,
    ...
    /// `$MULL_WORKSPACE_HOME` — must NOT configure an upload queue, and must leave
    ...
    async fn new_defaults_to_ephemeral_home_and_inert_legacy_upload() {
        ...
        assert_ne!(
            ...
            "default construction must NOT use the real $MULL_WORKSPACE_HOME"
        );
        assert!(
    ...
    async fn tool_state_upload_is_noop_when_flag_off() {
        ...
        unsafe { std::env::remove_var("MULL_WORKSPACE_TOOL_STATE_ENABLED") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    async fn tool_state_upload_is_noop_when_data_collection_disabled() {
        ...
        let _env = crate::session::tool_config::TOOL_STATE_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("MULL_WORKSPACE_TOOL_STATE_ENABLED", "true") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    fn make_queue_backed_handle_with(
        ...
        let auth: mull_computer_hub_sdk::SharedAuthProvider = Arc::new(
            mull_computer_hub_sdk::auth::AuthCredential::bearer("test-token"),
        );
        ...
        let source: Arc<dyn mull_file_utils::queue::TraceExportSource> =
            Arc::new(crate::upload::WorkspaceTraceExportSource::new(proxy));
        let policy = mull_file_utils::queue::UploadRetryPolicy {
            max_attempts: 1,
            ..Default::default()
        };
        let queue = Arc::new(mull_file_utils::queue::UploadQueue::spawn(
            home.path(),
            source,
            policy,
        ));
    ...
    async fn environment_artifact_enqueued_when_queue_present() {
        ...
        assert!(
            matches!(
                outcome,
                Some(mull_file_utils::queue::EnqueueOutcome::Enqueued)
            ),
    ...
    async fn fork_session_inherits_parent_tool_config_when_none() {
        ...
        let new_parent_baseline = ToolServerConfig {
            tools: vec![tc("MullBuild:read_file", Some(ToolKind::Read))],
            behavior_preset: None,
        };
    ...
    async fn fork_session_uses_explicit_tool_config_when_provided() {
        ...
        let custom = ToolServerConfig {
            tools: vec![
                tc("MullBuild:read_file", Some(ToolKind::Read)),
                tc("MullBuild:list_dir", Some(ToolKind::ListDir)),
            ],
            behavior_preset: None,
        };
        let child = handle
    ...
    async fn fork_session_uses_main_session_when_parent_session_id_is_none() {
        ...
        let baseline_ids: Vec<String> = child
            ...
            .map(|t| t.id.clone())
            .collect();
        assert_eq!(baseline_ids, vec!["MullBuild:read_file".to_string()]);
    }
    #[tokio::test]
    ...
    async fn fork_session_cwd_override_used_when_set() {
        let handle = make_handle();
        let alt = std::env::temp_dir().join("mull-workspace-test-cwd-override");
    ...
    async fn on_mcp_snapshot_changed_emits_per_session_events_and_rebuilds() {
        ...
        handle
            ...
            .expect("subB ok");
        let mut rx = handle.shared.events.subscribe();
        let mcp_tool = tc("MullBuild:read_file", Some(ToolKind::Read));
    ...
    async fn hook_registry_snapshot_is_disconnected() {
        ...
        {
            let spec = mull_hooks::config::HookSpec {
                name: "injected".into(),
                event: mull_hooks::event::HookEventName::SessionStart,
    ...
    fn workspace_shared_auth_provider_uses_workspace_config() {
        ...
        let service_auth: mull_computer_hub_sdk::SharedAuthProvider = Arc::new(
            mull_computer_hub_sdk::auth::AuthCredential::bearer("palmshed-service-token"),
        );
        let hub_auth: mull_computer_hub_sdk::SharedAuthProvider = Arc::new(
            mull_computer_hub_sdk::auth::AuthCredential::bearer("hub-token"),
        );
        let hub_cfg = crate::hub::HubConfig {
    ...
    async fn fork_session_inherits_viewer_ctx_from_parent() {
        ...
        let parent = handle
            .create_session_with_tracker_and_viewer_ctx(
                ...
                mull_hunk_tracker::HunkTrackerHandle::noop(),
                None,
                CapabilityMode::All,
                Some(mull_tool_runtime::WorkspaceViewerContext {
    ...
    fn handler_names(resolved: &mull_computer_hub_sdk::ResolvedSessionHandlers) -> Vec<String> {
        resolved
            .handlers
            .iter()
            .map(|h| h.tool_id().as_str().to_owned())
    ...
    async fn strict_bind_without_explicit_toolset_fails_closed_end_to_end() {
        ...
        let resolved = resolver(
            mull_tool_protocol::SessionId::new("bind-e2e-strict").unwrap(),
            Some(serde_json::json!(
                { "metadata" : { "preset" : "mull-computer", "capability_mode" :
                "all" }, }
            )),
        )
        .await
        ...
        assert!(
            reason.contains(mull_version::VERSION),
    ...
    async fn strict_rpc_only_bind_fails_closed_with_resolve_error_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            mull_tool_protocol::SessionId::new("bind-e2e-rpc-only").unwrap(),
            Some(serde_json::json!(
    ...
    async fn strict_bind_with_explicit_toolset_serves_it_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            mull_tool_protocol::SessionId::new("bind-e2e-tools").unwrap(),
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "MullBuild:read_file" }] },
                }
            )),
        )
        .await
    ...
    async fn lax_bind_without_metadata_uses_default_catalog_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            mull_tool_protocol::SessionId::new("bind-e2e-lax").unwrap(),
            None,
        )
        .await
        .expect("bind must succeed");
    ...
    async fn rejected_rebind_config_keeps_resolve_error_end_to_end() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = mull_tool_protocol::SessionId::new("bind-e2e-rejected").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "MullBuild:read_file" }] },
    ...
    async fn explicit_empty_toolset_rebind_never_swaps_session_tools() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = mull_tool_protocol::SessionId::new("bind-e2e-rpc-only").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "MullBuild:read_file" }] },
    ...
    async fn strict_rebind_with_corrected_toolset_heals_end_to_end() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = mull_tool_protocol::SessionId::new("bind-e2e-heal").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!({ "metadata" : { "preset" : "mull-computer" } })),
        )
        .await
    ...
    fn owner_full_bind_metadata() -> serde_json::Value {
        serde_json::json!(
            ...
            "MullBuild:read_file" }, { "id" : "MullBuild:search_replace" }, { "id" :
            "MullBuild:grep" }, { "id" : "MullBuild:list_dir" },], }, }
        )
    }
    ...
    async fn owner_toolset_survives_concurrent_consumer_shaped_rebinds() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = mull_tool_protocol::SessionId::new("bind-e2e-consumer-storm").unwrap();
    ...
    async fn restored_server_first_bind_ordering_decides_capability_and_toolset() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = mull_tool_protocol::SessionId::new("bind-e2e-restore-read-first").unwrap();
        ...
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = mull_tool_protocol::SessionId::new("bind-e2e-restore-write-first").unwrap();
        resolver(
            sid.clone(),
            Some(serde_json::json!(
        ...
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = mull_tool_protocol::SessionId::new("bind-e2e-restore-owner-first").unwrap();
    ...
    async fn bind_flow_rebinds_keep_backend_and_task_alive_end_to_end() {
        ...
        let handle = make_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = mull_tool_protocol::SessionId::new("bind-e2e-bg").unwrap();
        let bg_metadata = serde_json::json!(
            { "metadata" : { "tools" : [{ "id" : "MullBuild:read_file" }, { "id" :
            "MullBuild:run_terminal_cmd" }, { "id" : "MullBuild:get_task_output" }, {
            "id" : "MullBuild:kill_task" },] }, }
        );
    ...
    async fn compute_turn_injections_after_returns_skipped_ack_without_queue() {
        use mull_tool_protocol::turn_hook::{AfterTurnPayload, TurnHookOutcome, TurnHookRequest};
        let handle = make_handle();
    ...
    async fn compute_turn_injections_before_runs_turn_start_and_replies_noop() {
        use mull_tool_protocol::turn_hook::{BeforeTurnPayload, HookReply, TurnHookRequest};
        let handle = make_handle();
        let reply = handle
            .compute_turn_injections(
                "main",
    ...
    /// Unique skill names: discovery also reads the dev machine's `~/.mull`.
    #[tokio::test]
    ...
    fn make_handle_with_queue_routing(
        ...
        WorkspaceHandle,
        Arc<mull_file_utils::queue::UploadQueue>,
        tempfile::TempDir,
    ) {
        use mull_computer_hub_sdk::auth::{AuthCredential, AuthProvider};
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let config = WorkspaceConfig {
            root_cwd: cwd,
    ...
    async fn wait_enqueued(queue: &mull_file_utils::queue::UploadQueue, want: u64) {
    ...
    async fn enqueue_workspace_tool_definitions_reports_enqueued_at_session_root() {
        ...
        assert_eq!(outcome, mull_file_utils::queue::EnqueueOutcome::Enqueued);
        assert_eq!(
            queue
                .stats()
                .enqueued
    ...
    async fn two_phase_drain_no_queue_marks_draining_and_returns_zero() {
        ...
        assert_eq!(
            snap.status,
            mull_tool_protocol::ToolServerLifecycleStatus::Draining
        );
        assert!(
