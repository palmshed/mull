...
use dashmap::DashMap;
use mull_file_utils::events::{Event, EventWriter, ToolOutcome};
use mull_file_utils::queue::UploadQueueStats;
use mull_tool_protocol::{ToolServerLifecycleStatus, ToolServerStatusPayload};
...
/// `MULL_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
...
pub struct ActivityTracker {
    ...
    /// `MULL_WORKSPACE_EVENTS_ENABLED` is off). Consumed by
...
impl ActivityTracker {
    ...
    /// idle-hold cap comes from `MULL_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`
    ...
    pub fn snapshot_session(&self, session_id: &str) -> ToolServerStatusPayload {
        ...
        ToolServerStatusPayload {
            status,
            session_id: mull_tool_protocol::SessionId::new(session_id).ok(),
            connection_id: None,
...
/// The durability idle-hold cap from `MULL_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
fn durability_idle_hold_max_from_env() -> u64 {
    durability_idle_hold_from_raw(std::env::var("MULL_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS").ok())
}
