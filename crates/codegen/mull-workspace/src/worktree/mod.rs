...
//!
//! Moved from `mull-shell/src/session/worktree.rs` into the workspace
...
use mull_fast_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};
...
// Canonical in mull-workspace-types; re-exported for existing paths.
...
const WORKTREE_LOG: &str = "mull_worktree";

/// Map a [`WorktreeType`] to the fast-worktree crate's `CreationMode`.
pub(crate) fn to_creation_mode(t: WorktreeType) -> mull_fast_worktree::CreationMode {
    match t {
        WorktreeType::Linked => mull_fast_worktree::CreationMode::Linked,
        WorktreeType::Standalone => mull_fast_worktree::CreationMode::Standalone,
        WorktreeType::Git => mull_fast_worktree::CreationMode::GitCheckout,
    }
}
...
/// Resolve the mull home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`mull_fast_worktree::resolve_mull_home`), so checkout dirs and
/// the metadata DB always live under the same `.mull` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `mull_config::mull_home()`,
/// so worktree paths also agree with trust/hooks and other mull-home paths.
fn mull_home() -> std::path::PathBuf {
    mull_fast_worktree::resolve_mull_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.mull/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    mull_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.mull/worktrees/<repo_name>`)
/// for a given source path, correctly handling mull-managed worktrees.
///
/// When `source_path` is already under `~/.mull/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the mull worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = mull_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.mull/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the mull-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.mull/worktrees/`.
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Map WorktreeCopyMode to mull_fast_worktree::WorkingTreeMode
    ...
    // Use mull-fast-worktree for high-performance worktree creation
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(mull_fast_worktree::WorktreeKind::Session)
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        mull_fast_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        mull_fast_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        mull_fast_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        mull_fast_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        mull_fast_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
///
/// When the source path is already inside `~/.mull/worktrees/<repo>/`, the
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        mull_fast_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
...
pub async fn create_worktree_from_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Use mull-fast-worktree -- it handles copying from any worktree path
    ...
    let builder_result = {
        ...
        tokio::task::spawn_blocking(move || {
            let mut builder = WorktreeBuilder::new(&source_worktree_path, &dest_path)
                ...
                .ignored_files_mode(IgnoredFilesMode::Skip)
                .creation_mode(to_creation_mode(creation_mode))
                .worktree_kind(mull_fast_worktree::WorktreeKind::Fork)
                .session_id(session_id_for_builder)
                .metadata(label_metadata);
...
///
/// ACP equivalent of `mull -w -r <session_id>` (optionally with `--ref`).
...
/// Response from `palmshed.ai/git/worktree/resume_session`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response from `palmshed.ai/session/rehydrate`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
use mull_fast_worktree::{
    DbStats, GcOptions, GcReport, ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord,
    gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_mull_home,
};
...
pub fn worktree_db_rebuild() -> Result<mull_fast_worktree::RebuildReport> {
    let home = resolve_mull_home()?;
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = mull_fast_worktree::ListFilter {
...
/// Scan `~/.mull/worktrees/<repo_name>/` for subdirectories not tracked
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[mull_fast_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/mull/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/mull/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/mull/subagents/standalone-1";
    ...
    /// Point `MULL_HOME` at an isolated tempdir (`resolve_mull_home` re-reads
    ...
    /// and restores `MULL_HOME` on drop (before releasing the lock), so the
    ...
    fn worktree_db_fixture(
        ...
        let env = LockedTestEnv::lock().set("MULL_HOME", &home);
        ...
        let record = WorktreeRecord {
            ...
            status: mull_fast_worktree::WorktreeStatus::Alive,
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (MullHomeFixture is not visible across crates).
