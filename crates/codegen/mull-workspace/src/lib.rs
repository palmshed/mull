...
pub use mull_hunk_tracker::HunkTrackerHandle;
pub use mull_workspace_client::WorkspaceClient;
pub use mull_workspace_types::WorkspaceEvent;
pub use permission::*;
...
/// environment (`MULL_HOME`, `HOME`, …). nextest isolates each test in its own
...
/// can clobber `MULL_HOME` mid-test. A single shared lock (used by every
...
mod init_metrics_tests {
    ...
    fn init_metrics_is_idempotent_and_registers_baselines() {
        ...
        assert!(has(
            "mull_workspace_toolset_swap_rejected_total",
            &[("reason", "turn_active"), ("trigger", "update_tool_config")]
        ));
        assert!(has(
            "mull_workspace_orphan_lost_total",
            &[("reason", "sha_mismatch")]
        ));
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "mull_workspace_env_capture_panic_total")
        );
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "mull_workspace_permission_timeout_total")
        );
    }
}
