...
use mull_computer_hub_sdk::ToolServerHandler;
use mull_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
...
use mull_tools::implementations::mull_build::scheduler::interval::interval_to_human;
use mull_tools::implementations::mull_build::scheduler::types::{
    SchedulerCommand, SchedulerHandle,
};
use mull_tools::registry::types::FinalizedToolset;
use mull_tools::types::resources::Terminal;
use mull_workspace_types::rpc::workspace::{
...
static WORKSPACE_RPC_CALLER_MISMATCH_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "mull_workspace_rpc_caller_mismatch_total",
...
static WORKSPACE_RPC_MUTATION_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "mull_workspace_rpc_mutation_total",
...
static WORKSPACE_RPC_REQUESTS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "mull_workspace_rpc_requests_total",
...
static WORKSPACE_RPC_DURATION_SECONDS: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "mull_workspace_rpc_duration_seconds",
...
) -> Vec<mull_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use mull_tools::computer::types::TaskKind;
    use mull_tools::types::resources::Terminal;
    use mull_tools::types::tool::ToolKind;
    use mull_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &mull_tools::registry::types::FinalizedToolset,
) -> Vec<mull_workspace_types::rpc::workspace::TodoSummaryWire> {
    use mull_tools::implementations::mull_build::todo::{TodoState, TodoStatus};
    use mull_tools::types::resources::State;
    use mull_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use mull_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use mull_workspace_types::rpc::search::FuzzyStatusReq;
        use mull_workspace_types::rpc::skills::DiscoverPluginsReq;
        use mull_workspace_types::rpc::workspace::{
        ...
        use mull_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<mull_tool_runtime::SessionContext>();
    ...
    async fn handle_hook(&self, session_id: SessionId, frame: HookFrame) {
        match frame.event {
            ...
            HookEvent::Custom { kind, payload } => {
                use mull_tool_protocol::turn_hook::{
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use mull_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    use mull_tool_protocol::turn_hook;
    use mull_tools::implementations::mull_build::scheduler::types::ScheduledTask;
    /// Helper: consume the first item from a ToolStream.
    async fn next_item(
        stream: &mut ToolStream<TypedToolOutput>,
    ) -> Option<mull_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use mull_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use mull_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
        ...
        let read_only = ToolServerConfig {
            tools: vec![tc(
                "MullBuild:read_file",
                Some(mull_tools::types::tool::ToolKind::Read),
            )],
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(mull_tool_runtime::SessionContext("main".to_owned()));
        ...
        match item {
            mull_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => {
                let ok_val = typed
                    .value
                    .get("ok")
    ...
    async fn handle_call_records_rpc_metrics_and_collapses_unknown_method() {
        ...
        let has_bogus_series = prometheus::gather()
            .iter()
            .filter(|mf| mf.name() == "mull_workspace_rpc_requests_total")
    ...
    async fn handle_hook_cancel_marks_call_completed() {
        use mull_tool_protocol::ToolCallId;
        let handle = make_handle();
        let handler = WorkspaceRpcHandler::new(handle.clone());
        let tracker = handle.activity_tracker();
