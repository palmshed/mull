...
use mull_computer_hub_sdk::ToolHarness;
use mull_tools::types::output::ToolRunResult;
use mull_workspace_client::{WorkspaceClient, is_transport_fatal};
pub use mull_workspace_types::rpc::WorkspaceRpc;
pub use mull_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use mull_workspace_types::rpc::code_nav::{
...
pub use mull_workspace_types::rpc::fs::{
...
pub use mull_workspace_types::rpc::git::{
...
pub use mull_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use mull_workspace_types::rpc::hunks::{
...
pub use mull_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use mull_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use mull_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use mull_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use mull_workspace_types::rpc::worktree::{
...
fn hunk_line_info_to_wire(info: &mull_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: mull_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use mull_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &mull_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: mull_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use mull_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: mull_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(entry: mull_hunk_tracker::FileContentEntry) -> FileContentEntryWire {
...
fn session_stats_to_wire(stats: &mull_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: mull_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: mull_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> mull_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => mull_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => mull_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<mull_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = mull_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                mull_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &mull_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<mull_hooks::discovery::HookRegistry> {
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<mull_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<mull_codebase_graph::QueryResult, mull_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<mull_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](mull_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: mull_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<mull_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](mull_tools::registry::types::FinalizedToolset)
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(mull_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = mull_tool_protocol::ToolId::new(name).map_err(|e| {
                    mull_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                        format!("invalid tool name: {e}"),
                    )
                })?;
                let mut ctx = mull_tool_runtime::ToolCallContext::default();
                ctx.call_id =
                    mull_tool_protocol::ToolCallId::new(call_id.to_owned()).unwrap_or(ctx.call_id);
...
mod tests {
    ...
    /// request types are defined in `mull-workspace-types` (and re-exported
    ...
    fn git_op_cwd_uses_explicit_git_root_per_window() {
        ...
        let window_a = std::path::PathBuf::from("/repos/palmshed-main");
        let window_b = std::path::PathBuf::from("/repos/palmshed-main-2");
        assert_eq!(
    ...
    fn hunk_to_wire_serializes_identically() {
        use mull_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use mull_hunk_tracker::FileContentEntry;
        use mull_hunk_tracker::types::FileContentView;
    ...
    fn session_summary_to_wire_serializes_identically() {
        use mull_hunk_tracker::SessionSummary;
        use mull_hunk_tracker::types::{Hunk, HunkSource, TurnSummary};
        use std::sync::Arc;
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = mull_hooks::config::HookSpec {
            ...
            event: mull_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.mull/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
        };
        let mut registry = mull_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use mull_hooks::event::HookEventName as E;
    ...
    /// can't depend on `mull-hooks`, and `hook_registry_to_wire` only couples
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use mull_hooks::config::HookSpec;
