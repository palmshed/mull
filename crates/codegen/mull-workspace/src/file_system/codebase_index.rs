...
use mull_codebase_graph::{IndexManager, IndexManagerConfig, IndexManagerHandle};

use mull_tools::util::mull_home::mull_home;

/// Get the cache path for a cwd's index.
///
/// Cache is stored in: `~/.mull/indexes/{url_encoded_cwd}/goto_index.bin`
...
mod tests {
    ...
    // which maps to `palmshed.ai/code/status` reporting `reason: notStarted`.
    ...
    /// In `palmshed.ai/code/status`, `resolve_index_handle()` calls
