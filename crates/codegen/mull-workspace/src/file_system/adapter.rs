//! AcpFsAdapter: implements `mull-tools::AsyncFileSystem` using ACP gateway calls.
//!
//! This adapter enables file tool execution over ACP (remote filesystem).
//! It translates mull-tools' `AsyncFileSystem` trait into ACP protocol calls:
...
use mull_tools::computer::types::{AsyncFileSystem, ComputerError};

/// Wraps mull-shell's ACP gateway to satisfy mull-tools' AsyncFileSystem.
///
