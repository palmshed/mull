...
//! for the shell's `palmshed.ai/fs/*` ACP extension methods.
//!
//! These mirror the pure functions that previously lived only in the
//! shell (`mull-shell/src/session/file_system.rs`) so that, in proxy
//! mode, a `palmshed.ai/fs/*` request executes on the *remote* workspace server
...
use mull_workspace_types::rpc::fs::FsReadEncoding;
