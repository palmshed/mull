...
const MCP_OAUTH_CLIENT_NAME: &str = "Mull";
...
// mode disabled, multiple `mull` invocations).
//
// Layer 1 (cross-process): filesystem lock at $MULL_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    mull_config::mull_home().join(format!("mcp_auth_{safe}.lock"))
}
