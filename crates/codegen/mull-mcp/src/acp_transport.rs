...
//!
//! In-process SDK MCP servers (the official `mull-agent-sdk`'s `@tool` /
...
//! reverse `palmshed.ai/mcp/sdk_call` request and feeding the response back. This module
...
/// channel (`palmshed.ai/mcp/sdk_call`) and returns its JSON-RPC response. The `Err` string is
...
/// `palmshed.ai/mcp/sdk_call` and writes the server→client response back. The pump exits when
...
async fn read_requests(
    ...
    loop {
        ...
        // `palmshed.ai/mcp/sdk_call`s without a JSON-RPC id, so id-less messages (e.g. rmcp's
