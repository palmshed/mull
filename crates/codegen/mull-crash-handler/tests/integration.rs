//! Integration tests for mull-crash-handler.
//!
...
fn subprocess_entry() {
    ...
    let config = mull_crash_handler::CrashHandlerConfig {
        app_version: "0.0.0-test".to_string(),
        crash_dir,
    };
    mull_crash_handler::install(config);
...
fn sigbus_produces_valid_crash_blob() {
    ...
    let blob =
        mull_crash_handler::format::CrashBlob::parse(&data).expect("crash blob should parse");
    ...
    let report = mull_crash_handler::check_previous_crash(tmp.path())
