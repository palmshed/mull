...
//!
//! Extracted from `mull-shell/src/agent/subagent/` `resolve_effective_overrides()`.

use std::collections::HashMap;
use std::path::Path;

use mull_tool_types::{SubagentCapabilityMode, SubagentIsolationMode};
use mull_tools::implementations::mull_build::task::types::SubagentRuntimeOverrides;
...
mod tests {
    use super::*;
    use mull_tools::implementations::mull_build::task::types::ModelOverrideProvenance;
    ...
    fn explicit_model_overrides_role() {
        let overrides = make_overrides(Some("mull-light"), None, None, None, None);
        ...
        assert_eq!(result.model.as_deref(), Some("mull-light"));
    }
