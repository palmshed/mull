...
//! `mull_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `MULL_POOL_*` /
//! `MULL_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `MULL_SAMPLER_SHARED_CLIENT=0` falls back to
...
// `OriginClientInfo` is owned by `mull-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `mull-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `mull-telemetry` consumes
// the same type via `mull_sampler::OriginClientInfo`. The shell-specific
...
/// `MULL_CLIENT_VERSION` env vars. Returns `None` when
/// `MULL_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`MULL_CLIENT_NAME` env var, default `"mull-shell"`).
pub fn process_client_identifier() -> String {
    std::env::var("MULL_CLIENT_NAME").unwrap_or_else(|_| "mull-shell".to_string())
}

/// Header telling cli-chat-proxy whether this process is a single-prompt
/// (`mull -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-mull-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `mull -p`, `run_headless_inner` for
/// `mull agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn mull_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(mull_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        ...
        assert_eq!(
            ...
            Some(OriginClientInfo {
                product: "mull-desktop".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("mull-desktop/1.2.3 mull-shell/"));
        ...
        assert!(without_version.starts_with("mull-web mull-shell/"));
        assert!(!without_version.starts_with("mull-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        ...
        assert_eq!(ua.render(), "mull-shell/0.1.171 (macos; aarch64)");
    }
}
