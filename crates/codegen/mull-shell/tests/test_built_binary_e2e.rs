//! Built-binary end-to-end tests for the mull (mull-pager) binary.
//!
//! These tests verify that the built mull binary works end-to-end against a mock
...
//!
//! The tests exercise:
//! - **Smoke** (`mull --version`): binary loads without crashing
//! - **ACP stdio** (`mull agent stdio`): full protocol lifecycle via ClientSideConnection
//!
//! Tests are `#[ignore]`d by default — they require a pre-built binary.
//!
//! Run locally (auto-builds the binary if not already present):
//! ```bash
//! cargo test -p mull-shell --test test_built_binary_e2e -- --ignored
//! ```
//!
//! In CI, set `MULL_BINARY` to point at the release artifact:
//! ```bash
//! MULL_BINARY=./artifacts/mull-0.1.159-linux-x86_64 \
//!   cargo test -p mull-shell --test test_built_binary_e2e -- --ignored
//! ```
...
use mull_test_support::env::test_env_cmd_tokio;
...
async fn mull_build_server() -> MockInferenceServer {
    MockInferenceServer::start_with_models(vec![
        MockModelEntry::with_agent_type("grok-4.5", "mull")
...
async fn test_version_with_crash_handler_exits_zero() {
    ...
    let output = Command::new(&binary)
        .arg("--version")
        .env("MULL_CRASH_HANDLER", "1")
        .output()
        .unwrap_or_else(|e| panic!("failed to run {}: {e}", binary.display()));

    assert!(
        output.status.success(),
        "mull --version with MULL_CRASH_HANDLER=1 failed (exit {:?}):\n{}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_headless_session_in_git_repo() {
    ...
    assert_headless_success(&result, "mull -p in git repo", Some(&server));
...
/// Verify mull works in a non-git directory (exercises the fallback codepath
...
async fn test_headless_session_in_non_git_dir() {
    ...
    assert_headless_success(&result, "mull -p in non-git dir", Some(&server));
    assert_no_crashes(&result.stderr);
}
...
async fn test_headless_tools_allowlist_keeps_enabled_web_tools() {
    let server = mull_build_server().await;
    ...
    assert_headless_success(&result, "mull -p --tools with web tools", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_tools_allowlist_does_not_fail_open_for_disabled_web_fetch() {
    ...
    assert_headless_success(
        &result,
        "mull -p --tools with disabled web_fetch",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
    let names = inference_tool_names(&server);
...
async fn test_headless_terminal_only_allowlist_is_foreground_only() {
    ...
    assert_headless_success(&result, "mull -p --tools run_terminal_cmd", Some(&server));
    assert_no_crashes(&result.stderr);
    let request = inference_request(&server);
...
async fn test_headless_free_usage_exhausted_prints_paywall_message() {
    ...
    assert!(
        combined.contains("reached your free Mull usage limit"),
...
async fn test_headless_streaming_json_output() {
    ...
    assert_headless_success(
        &result,
        "mull -p --output-format streaming-json",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_json_reports_server_cost() {
    use mull_test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        mull_test_support::scripted::ScriptedResponse::sse(vec![
    ...
    assert_headless_success(&result, "mull -p (scripted cost)", Some(&server));
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        mull_test_support::scripted::ScriptedResponse::sse(
            mull_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
async fn headless_json_schema_chat_completions_uses_response_format() {
    ...
    assert_headless_success(
        &result,
        "mull -p --json-schema (chat_completions)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_responses_uses_text_format() {
    ...
    assert_headless_success(&result, "mull -p --json-schema (responses)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_backend_uses_structured_output_tool() {
    ...
    assert_headless_success(&result, "mull -p --json-schema (messages)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_validates_text_when_tool_not_called() {
    ...
    assert_headless_success(
        &result,
        "mull -p --json-schema (messages, text)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_retries_on_schema_violation() {
    ...
    assert_headless_success(
        &result,
        "mull -p --json-schema (messages, retry)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn invalid_json_schema_disables_structured_output_and_surfaces_error() {
    ...
    assert_headless_success(
        &result,
        "mull -p --json-schema (invalid schema)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
// ============================================================================
// ACP stdio tests (mull agent stdio)
//
// These test the agent as a server: spawn `mull agent stdio`, speak the full
...
async fn test_stdio_full_session_lifecycle() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let client = MullStdioClient::spawn(&server, workdir.path()).await;
...
/// Verify that palmshed.ai/session/close frees the session.
...
async fn test_stdio_prompt_then_immediate_load_session() {
    with_local_set(|| async {
        let server = MockInferenceServer::start().await.expect("start mock server");
        let workdir = git_workdir();
        let mut writer = MullStdioClient::spawn(&server, workdir.path()).await;
        ...
        let reader = MullStdioClient::spawn_with_home(&server, workdir.path(), shared_home).await;
...
async fn test_stdio_xcode_escaped_slash_methods_get_responses() {
    ...
    agent
        .send_line(&format!(
            r#"{{"jsonrpc":"2.0","id":"{auth_id}","method":"authenticate","params":{{"methodId":"api_key","_meta":{{"headless":true}}}}}}"#
        ))
        .await;
    let auth_resp = agent
...
/// Isolated headless run with a custom `~/.mull/`. Clean env (no leaked
/// host credentials). Write config files into `mull_dir()` before `run()`.
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        std::fs::create_dir_all(home.path().join(".mull")).unwrap();
        Self {
            home,
            workdir: git_workdir(),
            env: vec![
                ("MULL_CLI_CHAT_PROXY_BASE_URL".into(), server.url()),
                ("MULL_TELEMETRY_ENABLED".into(), "false".into()),
                ("MULL_FEEDBACK_ENABLED".into(), "false".into()),
                ("MULL_TRACE_UPLOAD".into(), "false".into()),
                ("MULL_INSTRUMENTATION".into(), "disabled".into()),
                ("MULL_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }

    fn mull_dir(&self) -> std::path::PathBuf {
        self.home.path().join(".mull")
    }
    ...
    async fn run(&self) -> HeadlessResult {
        ...
        cmd.args(["-p", "say hello", "--yolo"])
            ...
            // Windows resolves `~` via USERPROFILE, not HOME — pin the mull
            // home explicitly so the sandbox holds on all platforms (see
            // `test_env_cmd_tokio`).
            .env("MULL_HOME", self.mull_dir())
...
/// Enterprise BYOK: managed_config.toml overrides mull-build with a custom
...
async fn test_headless_managed_config_byok_sends_authorized_requests() {
    ...
    std::fs::write(
        h.mull_dir().join("managed_config.toml"),
        format!(
            r#"
[endpoints]
deployment_key = "test-deployment-key"
mull_api_base_url = "{url}"

[model.mull-build]
api_backend = "responses"
base_url = "{url}"
context_window = 500000
env_key = "MULL_TEST_BYOK_TOKEN"
model = "grok-4.5"
...
"#,
    ...
    .unwrap();
    h.env("MULL_TEST_BYOK_TOKEN", "test-byok-secret-token");
...
async fn headless_reasoning_efforts_payload_parses_and_legacy_effort_rides_wire() {
    ...
    assert_headless_success(&result, "mull -p reasoning_efforts list", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_timeout_exit_kills_pending_background_task() {
    ...
    assert_headless_success(
        &result,
        "mull -p with pending background task",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_no_wait_exit_kills_background_task() {
    ...
    assert_headless_success(&result, "mull -p --no-wait-for-background", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_waits_for_short_background_task_and_exits_clean() {
    ...
    assert_headless_success(&result, "mull -p with short background task", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        marker.exists(),
