...
//!
//! Each test spawns a real `mull agent stdio` process against a mock
//! inference server and asserts on the persisted `summary.json`.
//!
//! Run locally:
//! ```bash
//! cargo test -p mull-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.mull/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".mull").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let mull_dir = home.path().join(".mull");
        std::fs::create_dir_all(&mull_dir).expect("create .mull dir");
        std::fs::write(
...
"#,
        ...
        let client = MullStdioClient::spawn_with_home(&server, workdir.path(), home).await;
