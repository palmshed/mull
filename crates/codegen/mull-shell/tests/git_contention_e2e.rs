...
//!
//! With the fs-watch machinery on (`palmshed.ai/hunkTracker` + `palmshed.ai/gitHeadChanged`
...
//!
//! Knobs: MULL_PERF_GIT_FILES (default 300), MULL_PERF_GIT_PICKS (default 6).
...
use mull_test_support::{MockInferenceServer, ScriptedResponse, SseEvent};
...
use mull_test_utils::env::env_usize;

/// Run a git command with deterministic author/committer; assert success.
fn git(dir: &Path, args: &[&str]) -> String {
    mull_test_utils::git::run_git(dir, args)
}

// ── scan counting ─────────────────────────────────────────────────────────

use mull_hunk_tracker::{REFRESH_SCAN_LOG_PREFIX, REFRESH_SKIP_LOG_PREFIX};
use mull_test_utils::tracing_capture::MessagePrefixCounter;
...
fn install_global_scan_counter() -> ScanCounter {
    // MULL_E2E_LOG=<filter> tees shell logs to stderr for local debugging.
    let filter = std::env::var("MULL_E2E_LOG").ok();
    ScanCounter(
        mull_test_utils::tracing_capture::install_prefix_counter_global(
...
fn build_repo(files: usize, picks: usize) -> (TempDir, String) {
    ...
    mull_test_utils::git::write_fanout_tree(wd, files, 100);
    git(wd, &["add", "."]);
    git(wd, &["commit", "-m", "populate tree"]);

    let base = mull_test_utils::git::make_feature_branch(wd, picks);
    (dir, base)
}
...
async fn run_storm(
    ...
    let files = env_usize("MULL_PERF_GIT_FILES", 300);
    let picks = env_usize("MULL_PERF_GIT_PICKS", 6);
    ...
    local
        .run_until(async move {
            ...
            let method = init
                ...
                .unwrap_or_else(|| panic!("{label}: api_key auth method not advertised"));
            client_conn
                .authenticate(
...
fn git_rebase_refresh_storm_e2e() {
    ...
    let server = mock_rt
        ...
        .expect("mock server");
    let mull_home = TempDir::new().expect("mull home");
    ...
    let on = agent_rt.block_on(run_storm(
        &server,
        Some(json!({
            "palmshed.ai/hunkTracker": { "mode": "agent_only" },
