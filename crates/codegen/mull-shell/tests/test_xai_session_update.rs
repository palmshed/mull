//! Integration test for `_palmshed.ai/session/update` notifications.
//!
//! This test verifies that:
//! 1. Palmshed session notifications (e.g., diff_review) can be sent via ext_notification
...
use mull_shell::extensions::notification::{
    DiffContent, SessionNotification, SessionUpdate as MullSessionUpdate,
};
use mull_shell::session::info::Info as SessionInfo;
use mull_shell::session::persistence::default_model_id;
use mull_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};

/// Test that Palmshed session notifications round-trip through storage correctly.
#[tokio::test]
async fn test_mull_session_notification_storage_roundtrip() {
    ...
    let mull_notification = SessionNotification {
        session_id: session_id.clone(),
        update: MullSessionUpdate::DiffReview {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Mull(Box::new(mull_notification.clone())),
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        loaded.updates.len(),
        2,
        "Should have 2 updates (1 Palmshed + 1 ACP)"
    );

    // Verify Palmshed notification
    match &loaded.updates[0] {
        SessionUpdate::Mull(notification) => {
            assert_eq!(notification.session_id, session_id);
            match &notification.update {
                MullSessionUpdate::DiffReview { content } => {
...
/// must survive `updates.jsonl` and reload as a replayable `_palmshed.ai/session/update`.
#[tokio::test]
async fn test_turn_completed_round_trips_through_storage() {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Mull(Box::new(mull_notification)))
        .await
        .unwrap();
...
/// Test that totalTokens can be extracted from both ACP and Palmshed notifications.
#[tokio::test]
async fn test_extract_total_tokens_from_mixed_updates() {
    ...
    // Add Palmshed notification with totalTokens
    let mull_notification = SessionNotification {
        session_id: session_id.clone(),
        update: MullSessionUpdate::DiffReview { content: vec![] },
...
#[test]
fn test_mull_session_notification_serialization() {
