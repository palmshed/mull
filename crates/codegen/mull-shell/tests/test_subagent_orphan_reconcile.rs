...
//!
//! This test spawns a real `mull agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p mull-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // MullStdioClient sets HOME=<temp>; the binary uses <HOME>/.mull as MULL_HOME.
        let mull_home = shared_home.path().join(".mull");
        let session_dir = locate_session_dir(&mull_home, session_id.0.as_ref());
