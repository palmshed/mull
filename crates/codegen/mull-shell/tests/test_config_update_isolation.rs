...
/// All tests share this as MULL_HOME (the `OnceLock` in mull-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = mull_shell::config::load_effective_config().unwrap();
    let effective_cfg = mull_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    mull_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = mull_shell::util::config::load_config_from_toml(&user_toml);
