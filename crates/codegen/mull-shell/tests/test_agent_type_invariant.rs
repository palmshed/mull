...
//! - Session resume
//!
//! Each test spawns a real `mull agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p mull-shell --test test_agent_type_invariant -- --ignored
//! ```
...
/// - `model-a`: no agent_type (→ "mull")
/// - `model-b`: no agent_type (→ "mull")
...
/// `mull-build` harness. The system prompt sent to the LLM should contain
/// the mull-build identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_mull_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Mull")
                || sys_prompt.contains("mull")
                || sys_prompt.contains("Mull")
                || sys_prompt.contains("mull"),
            "default model should use mull-build harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_mull = original_sys_prompt.contains("Mull")
            || original_sys_prompt.contains("mull")
            || original_sys_prompt.contains("Mull")
            || original_sys_prompt.contains("mull");
        let resumed_has_mull = resumed_sys_prompt.contains("Mull")
            || resumed_sys_prompt.contains("mull")
            || resumed_sys_prompt.contains("Mull")
            || resumed_sys_prompt.contains("mull");
        assert_eq!(
            original_has_mull,
            ...
            "resumed session should use the same harness as the original\n\
             original identity markers: mull={original_has_mull}\n\
             resumed identity markers: mull={resumed_has_mull}\n\
...
/// default to `"mull"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_mull_build() {
    with_local_set(|| async {
            ...
            let sys_prompt = server
                ...
                .expect("should have at least one inference request");
            assert!(
                sys_prompt.contains("Mull") || sys_prompt.contains("mull") || sys_prompt.contains("Mull") || sys_prompt.contains("mull"),
                "model without agent_type should default to mull-build harness\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
}
/// The `MULL_AGENT` escape hatch should override the model's agent_type.
/// Setting `MULL_AGENT=mull-build` with an alternate-agent model should use
/// mull-build harness.
#[tokio::test]
#[ignore]
async fn test_mull_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            mull_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("MULL_AGENT", "mull");
            let mut child = cmd.spawn().expect("spawn mull");
            ...
            let incoming = mull_acp_lib::LineBufferedRead::spawn_local(incoming);
            ...
            assert!(
                sys_prompt.contains("Mull") || sys_prompt.contains("mull") || sys_prompt.contains("Mull") || sys_prompt.contains("mull"),
                "MULL_AGENT=mull-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
