...
use mull_shell::sampling::{
    ApiBackend, Client, ConversationItem, ConversationRequest, ConversationToolChoice,
    SamplingError, ToolCall, ToolSpec, rs,
};
use mull_shell::session::storage::JsonlStorageAdapter;
use mull_test_support::sse::responses_api_reasoning_and_text_events;
...
async fn test_chat_completions_streaming_tool_calls() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        ScriptedResponse::sse(chat_completion_tool_call_stream(tool_calls, "mull-test")),
    );
    let client = create_test_client(&server.url(), ApiBackend::ChatCompletions);
...
/// Upgrade path, mull-build / Responses API: a legacy session whose
...
async fn responses_upgrade_roundtrips_reconstructed_reasoning_as_typed_input() {
    // 1. Seed a legacy mull-build chat_history.jsonl (inline reasoning
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy mull-build reasoning","encrypted":"ENC_BLOB_xyz","id":"rs_mullbuild_legacy"},"model_id":"mull"}"#,
            "\n",
        ),
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        Some("legacy mull-build reasoning")
    );
}
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use mull_shell::sampling::rs::ResponseStreamEvent;
        match event {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        if let ResponseStreamEvent::ResponseCompleted(completed) = event {
            for output in completed.response.output {
                use mull_shell::sampling::rs::OutputItem;
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use mull_shell::sampling::rs::OutputMessageContent;
...
async fn test_request_includes_headers() {
    ...
    // x-mull-conv-id / x-mull-req-id are no longer sent after the OpenRouter
    // migration — MullRequestHeaders::apply() is a no-op.
}
...
/// `SamplerConfig::doom_loop_recovery` puts `x-mull-doom-loop-check` on the
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    use mull_sampling_types::doom_loop::{DOOM_LOOP_CHECK_EVENT_TYPE, SAMPLE_CHECK_EVENT_DATA};
    ...
    assert_eq!(logged.header("x-mull-doom-loop-check"), Some("true"));
}
...
async fn test_doom_loop_check_disabled_sends_no_header_and_drops_check_frames() {
    ...
    let logged = server.requests().pop().unwrap();
    assert!(logged.path.contains("/responses"));
    assert_eq!(logged.header("x-mull-doom-loop-check"), None);
}
...
async fn test_responses_backend_hits_responses_endpoint_not_chat_completions() {
    ...
    // The session-level dispatch in `mull-sampler` selects the
