...
//! 4. Smoke-validate session JSON under `$MULL_HOME/sessions/` after exit.
//!
//! Requires a built `mull` binary (`MULL_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p mull-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p mull-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use mull_agent::plugins::SharedPluginRegistryHandle;
use mull_agent::plugins::discovery::DiscoveryConfig;
use mull_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _mull_guard = EnvVarGuard::set("MULL_HOME", &mull_home);
    ...
    let mut registry = InstallRegistry::empty(mull_home.join("installed-plugins"));
    ...
    // mull_home() caches the first MULL_HOME per process; a separate test
    ...
    assert_eq!(plugin.scope, mull_agent::plugins::PluginScope::CliOverride);
...
#[tokio::test]
#[ignore = "requires pre-built mull binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/MULL_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via mull_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _mull_guard = EnvVarGuard::set("MULL_HOME", &mull_home);
    ...
    // Smoke: session storage under MULL_HOME/sessions has JSON artifacts.
