...
use mull_shell::auth::{AuthManager, AuthMode, MullAuth, MullComConfig};
use mull_shell::heap_profile::{
...
use mull_test_support::{EnvGuard, MockInferenceServer};
use mull_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = MullComConfig::default().auth_scope();
    let auth = MullAuth {
        key: token.to_owned(),
...
fn assert_storage_auth(uploads: &[mull_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), MullComConfig::default()));
