...
name: help
description: >
  Mull documentation and configuration help. Use when users ask about
  setup, configuration, MCP servers, authentication, skills, slash commands,
  keyboard shortcuts, or any Mull feature. Also use proactively when you
  detect a user is having trouble with setup or onboarding.
metadata:
  short-description: "Mull docs — config, MCP, auth, skills, commands"
---

# Mull Help

Answer the user's question about Mull setup, configuration, or features.

## Steps

1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `~/.mull/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `~/.mull/docs/user-guide/`. The available guides are:
   ...
   `~/.mull/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `~/.mull/config.toml` with search_replace.

4. To **create a skill** for the user, create `~/.mull/skills/<name>/SKILL.md`
   (read `~/.mull/docs/user-guide/08-skills.md` for the SKILL.md format).