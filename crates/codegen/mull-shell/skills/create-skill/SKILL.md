...
name: create-skill
description: >
  Interactively create a new Mull skill (SKILL.md + optional scripts/references).
  Use when the user wants to create a skill, scaffold a skill, or runs /create-skill.
metadata:
  short-description: "Create a new Mull skill"
---

# Create Skill

Interactively gather requirements from the user and create a fully working Mull skill on disk.
...
2. **Scope** - present the user with two options:
   - **Project** (Recommended): `<repo-root>/.mull/skills/<name>/SKILL.md` - available only in this repo, shareable with teammates
   - **User**: `~/.mull/skills/<name>/SKILL.md` - available in all projects
...
- Trigger phrases and keywords so Mull knows when to auto-invoke it
...
Where `<SKILL_DIR>` is:
- User scope: `~/.mull/skills/<name>`
- Project scope: `<repo-root>/.mull/skills/<name>`
...
2. Tell the user the skill is ready and how to use it:
   ...
   - Automatic: Mull will invoke it when the description matches user intent
