...
- **Using Mull**
  ...
  - [SSH Passthrough](#ssh-passthrough-mull-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`mull inspect`)](#introspection)
...
- [Building with Mull](#building-with-mull) — headless API, ACP SDK integration
...
curl -fsSL https://palmshed.ai/cli/install.sh | bash -s 0.1.42
```
...
On first launch, Mull opens your browser to authenticate with palmshed.ai:

```bash
mull
```

Credentials are stored in `~/.mull/auth.json` and persist across sessions. Tokens expire after 7 days; Mull will prompt you to re-authenticate when needed.
...
For CI/CD, automation, or environments without browser access, use an API key from [console.palmshed.ai](https://console.palmshed.ai):
...
Authenticate developers via your own Identity Provider (Okta, Azure AD, Auth0) instead of `accounts.palmshed.ai`.
...
export MULL_OIDC_ISSUER="https://acme.okta.com"
export MULL_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export MULL_CLI_CHAT_PROXY_BASE_URL="https://mull-proxy.acme.com/v1"
```

**3. Run `mull`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.mull/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Mull is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.

#### How It Works

```
┌──────────────┐     sh -c     ┌────────────────────────┐
│     Mull     │──────────────▶│  your auth binary      │
│              │               │                        │
...
1. Mull runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by Mull and saved to `~/.mull/auth.json` as the access token
5. exit 0 → success; exit non-zero → Mull falls through to interactive login
...
|--------|---------------|-------------|
| **stdout** | The token — nothing else | Mull (parsed and stored in `auth.json`) |
| **stderr** | Login URLs, status messages, errors, progress | The user (displayed in their terminal) |

**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Mull reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want Mull to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Mull when to proactively refresh. Without it, Mull assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Mull captures this)
...
```bash
# Or via environment variables
export MULL_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export MULL_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export MULL_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Mull cannot detect expiry proactively and will only refresh after a 401.

The command is run via `sh -c`, so it can be a binary path, a shell script, or a pipeline.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with palmshed.ai". In headless mode (`mull -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=mull-cli")
...
When Mull needs to refresh an expired token, it re-runs your binary with `MULL_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$MULL_AUTH_EXPIRED" = "1" ]; then
...
`MULL_AUTH_EXPIRED` is optional — if your binary ignores it, Mull still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

Mull supports automatic credential refresh for external auth providers and OIDC. When Mull detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Mull handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Mull re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Mull re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Mull silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Mull refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export MULL_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `mull login` before starting — Mull runs your binary automatically on first launch. You _can_ run `mull login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Mull tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
mull --debug-file /tmp/mull-auth.log -p "hello"
tail -f /tmp/mull-auth.log
```
...
If you've authenticated with `mull login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the mull CLI sends internally:

```bash
curl -s -N -X POST "https://openrouter.ai/api/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.palmshed.ai/sign-in".key' ~/.mull/auth.json)" \
  -H "X-XAI-Token-Auth: mull-cli" \
  -H "x-mull-model-override: mull-build" \
  -d '{
    "model": "mull-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.mull/auth.json` (set by `mull login`)                                                                                                                              |
| `X-XAI-Token-Auth: mull-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-mull-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `mull-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `mull-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `mull login` to refresh.
...
Run Mull non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `mull-build`)               |
...
mull -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
mull -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
mull -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
mull -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
mull -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
mull -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
mull -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
mull -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
mull -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
mull --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
mull -p "What does this project do?"

# Use a specific model
mull -p "Optimize this function" -m mull-build

# Get JSON output for parsing
mull -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
mull -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
mull -p "Remember: the secret number is 42" --output-format json
mull -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
mull -p "Continue where we left off" -c

# Run in a different directory
mull -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
mull -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
mull -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
mull -p "Generate a README" > README.md

# Parse JSON output with jq
mull -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
mull -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | mull -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  mull -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
mull -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run Mull as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `mull-build`)                           |
...
```bash
mull agent headless --mull-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`mull ssh`)

Use `mull ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
mull ssh -t user@host
mull ssh -L 8080:localhost:8080 user@host

# With remote command
mull ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `mull ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Mull can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `mull -p` and parses JSON output.
...
class MullChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["mull", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="mull-build", stream=False):
...
class MullChat {
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("mull", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await MullACPChat(cwd=".").init()
...
class MullACPChat {
  ...
  async init() {
    this.proc = spawn("mull", ["agent", "stdio"]);
...
// Usage
const client = await new MullACPChat(".").init();
...
Mull implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Mull reads configuration from `~/.mull/config.toml`. If the file doesn't exist, Mull uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "mull-build"           # model used for new sessions
...
telemetry = false                      # anonymous usage telemetry (env: MULL_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: MULL_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: MULL_RESPECT_GITIGNORE)
...
allowed_domains = ["docs.rs", "palmshed.ai"]           # override the built-in ~84-domain allowlist
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: MULL_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: MULL_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: MULL_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: MULL_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: MULL_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `MULL_TELEMETRY_BUILD_EVENTS_URL`, `MULL_TELEMETRY_BUILD_EVENTS_API_KEY`, and `MULL_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Mull can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Mull language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Mull can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Mull can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Mull looks for server definitions in:

- project config: `<repo>/.mull/lsp.json`
- user config: `~/.mull/lsp.json`
...
- LSP tools are enabled (`MULL_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
MULL_LSP_TOOLS=1 mull
```
...
If LSP tools are enabled but no usable server config is found, Mull emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Mull does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `MULL_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
[models]
default = "company-mull"

[model.company-mull]
model = "mull-build"
base_url = "https://mull-proxy.acme.com/"
name = "Mull Latest (Proxy)"
...
With this config, `mull` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Mull reads these files and appends their contents to the system prompt.

Mull scans for agent rules in this order:
...
Within each directory, Mull checks for these filenames:
...
Skills are reusable prompt packages that extend Mull with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

Mull discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.mull/skills/`           | Local | Highest  |
| `<repo_root>/.mull/skills/` | Repo  | Medium   |
| `~/.mull/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.mull/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Mull decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Mull invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Mull automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Mull discovers agent definitions from `.mull/agents/` (project), `~/.mull/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `MULL_AGENT` env var
4. Default `mull-build` agent
...
```bash
mull --agent-profile ./my-agent.md
# or
export MULL_AGENT="my-custom-agent"
```
...
```bash
export MULL_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "mull-build"              # route explore to a lighter model
```
...
model = "mull-build"
prompt_file = ".mull/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".mull/personas/concise.md"  # or load from file
```

Both are also discovered from `.mull/roles/*.toml` and `.mull/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Mull with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.mull/plugins/`            | Project |
| `~/.mull/plugins/`          | User    |
...
Mull discovers hooks from `.mull/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → `PALMSHED_API_KEY`. If neither `api_key` nor `env_key` is set, Mull falls back to the global `PALMSHED_API_KEY` environment variable.

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Mull falls back to built-in defaults for known models.
...
**How it works:** When you override a built-in model, Mull starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `MULL_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `mull models` output). You also need a `[model.*]` entry so Mull knows
...
>
> [model.my-custom-model]              # 2. tell Mull how to reach it
...
Point Mull at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `MULL_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `PALMSHED_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `MULL_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export MULL_MODELS_BASE_URL="https://api.acme.com/v1"
export PALMSHED_API_KEY="palmshed-..."
mull
```

Mull fetches the model list from `{MULL_MODELS_BASE_URL}/models` on startup and sends inference requests to `MULL_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `MULL_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.mull/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Mull uses API key auth (`Authorization: Bearer`) instead of session auth. `mull login` is not required — only the API key.

---

## MCP Servers

Extend Mull's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.mull/config.toml`:
...
MCP servers can also be configured per-project in `.mull/config.toml`. Mull walks from the current directory up to the git repo root, loading `.mull/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.mull/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.mull/config.toml` | This repository   | ↑        |
| `<cwd>/.mull/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.mull/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.mull/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.mull/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.mull/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `MULL_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets Mull remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.mull/memory/`:
- **Global** (`~/.mull/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.mull/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.mull/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries

Workspace directories are suffixed with a short hash for uniqueness (e.g. `palmshed-a3f7b2c9/`). The hash is derived from the git remote URL so all clones and worktrees of the same repository share the same memory directory.
...
At the end of each session, Mull saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.mull/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Mull searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
Key options under `[memory]` in `~/.mull/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.mull/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Mull emits the `mull-shell-memory_injection`
...
Mull can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.mull/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.mull/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.mull/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.mull/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.mull/sandbox.toml` (global) or `.mull/sandbox.toml` (per-project):
...
The sandbox is applied to the **entire mull process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Mull logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.mull/sandbox-events.jsonl`
...
Use `mull inspect` to see everything Mull discovers in the current directory:

```bash
mull inspect          # human-readable output
mull inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.mull/skills/`, `~/.mull/skills/`, plugins, and config paths
...
Mull automatically discovers configuration from Claude Code directories alongside native `.mull/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Mull uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.mull/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Mull at runtime.

---

## Built-in Tools

Mull includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `MULL_WEB_FETCH=1`. 

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (Palmshed, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `palmshed.ai/company`).

---

## Session Persistence

Mull automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.mull/sessions/`, organized by URL-encoded working directory:

```
~/.mull/sessions/<encoded-cwd>/<session-id>/
...
mull -p "Remember: X=42" -s my-session
mull -p "What is X?" -s my-session

# Resume existing session (errors if not found)
mull -p "Continue" -r my-session

# Continue most recent session in current directory
mull -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
mull -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.mull/config.toml` | Configuration file                                  |
| `~/.mull/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.mull/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.mull/memory/`     | Cross-session memory files and index                |
| `~/.mull/skills/`     | User-scoped skill definitions                       |
| `~/.mull/plugins/`    | User-scoped plugins                                 |
| `~/.mull/agents/`     | User-scoped agent definitions                       |
| `.mull/config.toml`   | Project-scoped config (MCP servers)                 |
...
| `.mull/hooks/`        | Project-scoped hooks                                |
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `PALMSHED_API_KEY`         | API key from [console.palmshed.ai](https://console.palmshed.ai). Used for custom endpoint auth and API key login      |
| `MULL_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://openrouter.ai/api/v1`)                          |
| `MULL_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `MULL_MODELS_LIST_URL`          | Override the model list URL if it differs from `{MULL_MODELS_BASE_URL}/models`                                              |
| `MULL_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `MULL_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `MULL_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `MULL_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `MULL_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `MULL_HOME`                     | Override config directory (default: `~/.mull`)                                                           |
| `MULL_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `MULL_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `MULL_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `MULL_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `MULL_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `MULL_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `MULL_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `MULL_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `MULL_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `MULL_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.mull/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `MULL_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `mull` commands and flags.
...
mull completions bash > ~/.local/share/bash-completion/completions/mull
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Mull-managed location):

```bash
mkdir -p ~/.mull/completions/bash
mull completions bash > ~/.mull/completions/bash/mull.bash
```
...
mull completions zsh > ~/.zsh/completions/_mull
```
...
```bash
mkdir -p ~/.mull/completions/zsh
mull completions zsh > ~/.mull/completions/zsh/_mull
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.mull/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `mull` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `mull --debug` or `MULL_LOG_FILE` instead:

```bash
# Per-session debug log (~/.mull/debug/<sessionId>.txt)
mull --debug

# Log to a custom path
MULL_LOG_FILE=/tmp/mull-debug.log mull

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.mull/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `MULL_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
MULL_LOG_FILE=/tmp/mull-debug.log RUST_LOG="info,mull_shell::auth=debug" mull
```
...
cat ~/.mull/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.mull/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.mull/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
