...
//!
//! Run: `cargo bench -p mull-shell --bench session_list`
...
use mull_fast_worktree::{ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
use mull_shell::session::info::Info;
use mull_shell::session::persistence::Summary;
use mull_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use mull_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = mull_shell::util::mull_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = mull_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    repo.remote(
        "origin",
        "https://github.com/palmshed/session-list-benchmark.git",
    )
    ...
    let worktree_base = mull_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(mull_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn write_summary(
    ...
    let summary = Summary {
        ...
        git_remotes: vec!["git@github.com:palmshed/benchmark.git".to_owned()],
...
fn bench_session_list(c: &mut Criterion) {
    ...
    unsafe {
        std::env::set_var("MULL_HOME", home.path());
    }
    assert_eq!(mull_shell::util::mull_home::mull_home(), home.path());
