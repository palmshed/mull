...
- **mull inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct mull --minimal --resume command.
...
- **Mull no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **mull setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask Mull above the footer shortcuts.
...
- **palmshed.ai/pr/status** now reports whether an open PR is in the merge queue.
...
- **`mull worktree ls`** now works as a short alias for `mull worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.mull/config.toml`.
...
- **File links and media** now work for worktree sessions under ~/.mull/worktrees/.
...
- **mull inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain mull launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive mull agent stdio without silent parse drops on session/* calls.
...
- **mull --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- **mull --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **mull update** on Windows no longer fails when a previous.old executable is still running.
...
- **mull models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `mull plugin marketplace add`.
...
- **Plan files** now default to.mull/plan.md to match Mull conventions.
...
- **--minimal** flag now shows in `mull --help`.
...
- **Tool result previews** no longer paint opaque panels in `mull --minimal`.
- **mull wrap** now correctly handles quoted strings and shell aliases.
...
- **Mull.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **`mull update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `mull wrap`.
...
- **mull agent stdio** now checks for updates in the background like other modes.
...
- **mull wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Added `mull wrap`** to run any command with local clipboard support.
...
- **Fixed `mull agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or mull-desktop.
...
- You can now pass --json-schema to mull -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `MULL_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **OIDC sessions** with PALMSHED_API_KEY present no longer lose refresh on idle.
...
- **mull -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in mull -p and mull agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `mull` after running `mull update`.
- Headless (`mull -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`mull plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **mull sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **mull quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, mull sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, MULL_HUNK_TRACKER, or config.
...
- **Mull** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`mull dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`mull mcp add`** now accepts positional arguments (e.g. `mull mcp add filesystem -- npx...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`mull update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.mull.
...
- **MCP servers menu** and slash commands now work when starting mull outside a project directory.
...
- **mull --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `mull --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `mull update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`mull --debug`** now produces per-session log files under ~/.mull/debug/ even with a leader process.
...
- **`mull update`** now restarts any older running leader so all clients get the new binary.
...
- **API-key users** can now run `mull agent --leader` without forced interactive login or timeouts.
...
- **Official Palmshed plugin marketplace** now appears automatically in the Marketplace tab on first launch.
- **Image and video generation** now use api.palmshed.ai directly for all users.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the MULL_IMAGE_GEN_HARNESS env var.
...
- **mull inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via MULL_CURSOR_MCPS_ENABLED / MULL_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside Mull and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the mull-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality mull-imagine-image-quality model.
...
- Memory system: /remember command, note modal with raw/enhanced preview, palmshed.ai/memory/rewrite ACP extension, Ctrl+F fullscreen toggle for /memory modal.
...
- **Version output** now shows [alpha] or [stable] in `--version`, banners, `/session-info`, and `mull inspect`.
...
- **New `/export` slash command** and `mull export` CLI command let you save or copy conversation transcripts as Markdown.
...
- **New `mull plugin` commands** let you manage plugins and marketplaces directly from the terminal.
...
- **New `mull plugin` commands** let you manage plugins and marketplaces directly from the terminal.
- New `/export` command and `mull export` let you save or copy conversation transcripts as Markdown.
...
- **Mull** now reminds itself to take action in very long conversations while still obeying instructions like "just plan".
...
- **Mull** now reminds itself to execute plans in long conversations while strictly following instructions like "just plan".
...
- **Added `mull logout` subcommand** to sign out from the terminal without launching the TUI.
...
- **API-key users** no longer see unavailable models such as mull-build that 404 on selection.
- **Image and video generation** now recover better from network issues and large response bodies.
- **Improved error messages** when hitting a subscription wall with an API key set, suggesting `mull logout`.
...
- **Web search and X search** can now execute server-side with full session persistence when MULL_BACKEND_SEARCH is enabled.
...
- **Goal mode** is now opt-in via config.toml or the MULL_GOAL environment variable.
...
- **Mull CLI** now runs natively on Intel Macs via x86_64 builds.
...
- **Paywall** now lifts automatically after subscribing via mull login or external purchase without needing to re-login.
...
- **Model list** no longer gets stuck on only mull-build after waking from sleep on macOS.
...
- **Goal mode** is now opt-in via `[features] goal = true` in config.toml or the MULL_GOAL environment variable.
...
- **New `mull logout` command** clears your cached login session directly from the terminal.
...
- **Fixed model list** getting stuck on only mull-build after waking from sleep on macOS by refreshing the catalog on every auth token update.
...
- **API key users** no longer see unavailable models like mull-build in the model list.
- **Image and video generation** now handles slow or interrupted network responses more reliably without generic decode errors.
- **Fixed error messages** when hitting a subscription wall with an API key set, now suggesting `mull logout`.
- **Fixed a crash** on exit when using MCP servers that require token refresh.
...
- **Model picker** now recovers automatically after sleep/resume or temporary network issues instead of staying stuck on Mull.
...
- **`mull sessions search`** now finds local and remote sessions correctly, and **`mull -r <id>`** restores remote sessions reliably.
...
- **AI shell command suggestions** enabled via MULL_SUGGESTIONS_AI=1 environment variable.
...
- **Fixed git and bash commands** failing in Mull Desktop on Windows.
...
- **`mull -w my-worktree`** creates a custom-named worktree folder.
...
- **`/usage manage`** now opens billing page; **`mull login`** enforces tiers correctly after refresh.
...
- **Legacy web login** no longer treated as first-party Palmshed account for sharing and billing; prompts **`mull login`** upgrade.
...
- **Large tool outputs** shortened when sent to Mull, full text saved to file in session folder.
...
- **Legacy authentication** removed; run **`mull login`** to upgrade to OAuth.
...
- **Legacy web login** no longer treated as premium Palmshed account; run **`mull login`** instead.
...
- **New `mull-build-v2` agent** available in config.toml for prompt/tool testing.
...
- **Fixed `mull sessions list`** auth failures.
...
- **New `mull-build-v2` agent** option in config.toml for testing prompt and tool changes.
...
- **`mull sessions list`** now works reliably after login.
...
- **Added mull memory clear** CLI command with scope selection and confirmation.
...
- **Computer Hub integration** into mull-shell
...
- **mull memory clear command** with scope and confirmation
...
- **Reasoning effort settings** no longer incorrectly apply to unsupported models like mull-build.
...
- **Reasoning effort settings** no longer incorrectly apply to models without support, like mull-build.
...
- **mull-build model** now defaults to Chat Completions API.
...
- **Shell commands now work** in Mull Desktop on Windows.
...
- **New `mull-build-orchestrator` agent** uses a lead model to coordinate subagents for builds.
...
- **New `mull import`** resumes Claude Code sessions with `mull import <id/path>`.
- **New `mull-build-orchestrator`** coordinates subagents for complex builds.
...
- **Fixed `mull share`** failing with backend ZodError due to duplicate request headers.
...
- **New `mull login --devbox`** option for signing in from headless devbox environments without browser access.
...
- **New `mull mcp doctor`** diagnoses MCP servers and suggests fixes.
...
- **New `mull mcp doctor`** diagnoses MCP server configuration and connectivity.
...
- **After plan approval**, Mull suggests `/implement` for structured coding when available.

## Bug Fixes

- **Model lists and pickers** now show only models accessible via your `MULL_CODE_PALMSHED_API_KEY`.
...
- **`mull models`** and TUI picker now show team-specific models when **`MULL_CODE_PALMSHED_API_KEY`** is set.
...
- **Native Windows support** now available experimentally for running Mull without WSL.
...
- **Clearer login error messages** now suggest `mull login`, API key env var, or config.toml.
...
- **Native Windows support** (opt-in) lets you run Mull CLI directly without WSL, with MCP server compatibility.
...
- **Authentication errors** now clearly suggest `mull login`, env var, or config.toml api_key.
- **Fixed false-positive loop warnings** for different shell commands during edit-test cycles.
...
- **Import Claude settings** from `.claude/` into `.mull/config.toml` via welcome screen ('i') or `/import-claude`.
- **Skills reload automatically** when editing/saving SKILL.md files without restarting `mull`.
...
- **Improved `mull login`** with automatic code delivery from browser consent page.
...
- **New `mull trace`** command uploads or exports session data for debugging.
...
- **New `mull trace`** command exports or uploads session data for debugging.
...
- **`mull models`** now lists available models and the default.
...
- **`mull models`** command now lists available models and authentication status.
...
- **X Premium and Premium+** tiers now grant access to Mull.
...
- **Model picker** now displays config key names like `mull-build` for aliased models.
...
- **Welcome screen** shows **SuperMull upsell** without active subscription.
...
- **Welcome screen** restricts actions and upsells SuperMull without subscription.
...
- **SuperMull upsell** shown on welcome screen without subscription.
...
- **`mull sessions search`** now finds sessions by content with scores and snippets.
...
- **`mull sessions search`** now supports **full-text queries** across chat content and tools.
- **Session picker** now combines fuzzy matching with **deep content search** results.
- **Unified logs** now saved to `~/.mull/logs/unified.jsonl` with **Download Logs** in desktop.
...
- **Auth.json hot-reload** fixed for mull login in other terminals.
...
- **Auto-update restart** now launches the correct new version.
- **Auth.json hot-reload** now detects `mull login` from other terminals.
...
- **External `mull login`** now hot-reloads auth without app restart.
...
- **External `mull login`** now hot-reloads auth without restarting.
...
- **New `mull ssh`** command enables clipboard copy from remote sessions in Apple Terminal.
...
- **`mull login --device-auth`** enables login from SSH, Docker, and other headless environments without port forwarding.
...
- **Added** `mull login --device-auth` **flag** for headless environments like SSH and Docker.
...
- **/feedback command** activates dedicated mode for submitting session feedback to palmshed.ai.
...
- **Auth info extension** returns login method ID and email via palmshed.ai/auth/info ACP method.
...
- **Startup announcements** emits palmshed.ai/announcements/refreshed ACP notification on agent init.
...
- **Bundled agents** discovered from ~/.mull/bundled/agents/ with lowest precedence after project/user/built-in.
...
- **Enterprise managed configs** via ~/.mull/managed_config.toml/requirements.toml with `mull inspect` and `mull setup`.
- **web_fetch tool** enabled via [features] web_fetch in config.toml alongside env/remote settings.
- **mull login defaults to OAuth** instead of legacy relay; use --legacy or MULL_OAUTH_ENABLED=0 for old behavior.
...
- **Automatic cleanup** removes old mull/mull-pager binaries post-update, keeping current + previous.
...
- **Automatic cleanup** removes old mull/mull-pager binaries post-update, keeping only current and previous.
...
- **Per-hook enable/disable toggles** via ~/.mull/disabled-hooks file and 'e' key with **multi-line j/k navigation fix**.
- **Bash-mode execute blocks auto-expand** after completion to display output immediately.
- **Video generation tool** supports Palmshed API with async polling, download and sequential MP4 naming.
- **Marketplace plugin installs** route through git_install with provenance tracking.
- **Parallel tool dispatch** via MULL_PARALLEL_TOOL_DISPATCH=1 runs concurrent Phase 2 execution.
...
- **Git stash-all** via palmshed.ai/git/stash ACP extension wired to desktop Changes panel.
...
- **OAuth login option** via --oauth/--legacy flags, MULL_OAUTH_ENABLED env, feature flag (legacy default).
...
- **Adds live PR status display** in desktop changes panel via new ACP palmshed.ai/pr/status extension.
...
- **Restores web search functionality** by reinjecting required proxy headers like x-mull-client-version.
...
- **MullNight default theme** with **runtime color quantization** ensures correct rendering across truecolor, 256-color, and 16-color terminals.
...
- **Legacy relay auth flow** via `mull login --legacy` restores pre-OIDC accounts.palmshed.ai token exchange.

## Bug Fixes

- **Terminal-width-constrained tables** wrap cell text proportionally without overflow or column misalignment.
- **Auth fallback to legacy scope** reads old accounts.palmshed.ai tokens on devboxes provisioned via x setup.
...
- **OIDC OAuth replaces relay auth**; removed `--auth-signin-url`/`exchange-code-url`/`redirect-target` flags, run `mull login` to migrate.

## Features

- **LSP integration** injects diagnostics as reminders and exposes opt-in code intelligence tools via `.mull/lsp.json`.
...
- **Image generation tool** gated behind a server-side flag or `MULL_IMAGE_GEN=1` env var with session auth.
- **Dynamic nested skill discovery** in `.mull/skills/` subdirs with runtime system reminder announcements.
...
- **Configurable permission policies** enable automatic allow/deny rules via ~/.mull/config.toml before existing checks.
...
- **Suppresses config warning** for [desktop] section owned by mull-desktop using opaque serde sink field.
...
- **Skills discovery expands** to.agents/skills directories alongside.mull and.claude.
...
- **OIDC OAuth2 replaces legacy relay login** for `mull login`; add `--legacy` flag for old flow and remove `--auth-signin-url` etc. CLI flags (migrate by deleting auth.json).
- **Adds `mull login --legacy`** fallback; removes `--auth-signin-url` etc flags (use env/config defaults, no migration needed for most users).

## Features

- **ACP git extensions** (`palmshed.ai/git/info`, `/branches`, `/checkout`) enable cloud-local git ops without subprocesses.
...
- **ACP hooks/plugins listing** (`palmshed.ai/hooks/list`, `/plugins/list`) exposes loaded configs for pager modals.
- **Image generation tool** via Palmshed Imagine API saves to session/images/ folder.
- **Management actions for hooks/plugins** via palmshed.ai/{hooks,plugins}/action with live modal refresh.
...
- **Dynamic discovery of nested skills** from.mull/skills/ subdirs at runtime via reminders.
- **PR status extension (palmshed.ai/pr/status)** fetches branch PR state, title, number, and URL via gh CLI.
...
- **Silent [desktop] config handling** consumes mull-desktop section without spurious 'unrecognized key' warnings.
...
- **Question-answer panel** in pager with **MullBuildPlanNoSubagents** mode excluding subagent tools.
...
- **Discover skills from**.claude/skills directories alongside.mull/skills.
...
- **Codex grep_files uses** shared rg_path() matching mull_build pattern.
...
- **ACP palmshed.ai/auth/logout** removes scopes from ~/.mull/auth.json.
- **Custom npm registry** via config.toml or MULL_NPM_REGISTRY respects enterprise.npmrc.
...
- **Help skill reads ~/.mull/config.toml** to answer MCP server and model configuration queries.
- **Deployment keys supported** on all API proxy endpoints including storage and sessions.
- **Public install-mull.sh script** supports deployment keys and channels without VPN.
...
- **Bundled /help skill** extracts README.md to ~/.mull/ for slash command and model self-help.
...
- **Subagent persona support** enables layered instructions from config.toml and.mull/personas/*.toml with fail-closed resolution.
...
- **Question-answer panel** adds pager UI for interactive permissions and subagent-free MullBuildPlan agent.
...
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP logout method** removes scopes from ~/.mull/auth.json.
- **Discovers skills from.claude/skills** alongside.mull/skills across local, repo, and user directories.
...
- **Codex grep_files uses shared rg_path** matching mull_build and opencode patterns.
...
- **Full-text search across past sessions** via palmshed.ai/session/search ACP method with FTS5 indexing and workspace filtering.
- **ACP resume session in worktree** via palmshed.ai/git/worktree/resume_session for programmatic clients.
...
- **Configurable bash tool params** like timeouts injected from config.toml `[toolset.bash]` into MullBuild actors.

## Bug Fixes

- **Precise io::ErrorKind mapping** from ACP file errors enables consistent NotFound and PermissionDenied handling.
- **Silent mull-pager installation** hides internal details from install and auto-update user output.
...
- **SSE stream parsing** handles flat error format from Mull proxy without deserialization failure.
...
- **Post-A/B session hooks** via MULL_AB_POST_SESSION_HOOK capture worktree changes.
...
- **Enhanced announcement UX** adds paging, prev/next commands, and `MULL_DEV_ANNOUNCEMENTS` override.
...
- **Optional auth for feedback** enables unauthenticated submissions with `MULL_USER_METADATA`.
- **Plan mode tools** add `enter_plan_mode`, `exit_plan_mode`, and `ask_user_question` with notifications.
- **Auto-installs mull-pager** alongside mull during internal and GitHub release updates.
...
- **Independent feedback gating** via MULL_FEEDBACK_ENABLED separates it from telemetry controls.
...
- **Mull 4.20 default model** with CLI/env/config/remote-settings overrides for catalog models.
...
- **`mull completions` subcommand** generates bash/zsh completion scripts, with fast-path exit before any network or auth warmup.
...
- **use_tool works in all server contexts** by dispatching through ToolCallContext instead of session-actor interception, fixing silent failures in mull-tools-server.
...
- **Custom skill path discovery** now works end-to-end — paths added via `palmshed.ai/skills/add` or config.toml are included in agent skill lookup.
...
- **`mull memory reindex` and `doctor` CLI commands** for index maintenance, plus access-frequency boost in hybrid search.
...
- **User-defined subagents** via .mull/agents/*.md files with per-subagent config toggles and dynamic Task tool descriptions.
...
- **Session token routing fixed** for default models — `mull login` users no longer get 400 errors on proxy-routed models.
...
- **Restored remote sessions create distinct local children** with parent tracking, preventing identity reuse and duplicate restores on repeated `mull -r`.
...
- **Logged-in ACP clients can use default fast models again** by routing session tokens through the chat API proxy instead of api.palmshed.ai.
...
- **Faster startup before code navigation** by lazily building indexes only for web clients that advertise palmshed.ai/codeNavigation.
...
- **Shell completions** via `mull completions <shell>`, leader CLI, and stdio reconnect replay.
- **User-defined subagents** via.mull/agents/*.md with config toggles.
...
- **Full-text search across sessions** via new `palmshed.ai/session/search` ACP extension with FTS5 indexing.
...
- **Disables web search tool** via `--disable-web-search` flag or `MULL_DISABLE_WEB_SEARCH=1` for benchmarks.
- **hashline_read tool** annotates file output with anchors like `LINE:LOCAL:CONTEXT→CONTENT`.
- **Mull 4.20 default model** with CLI/env/config/remote-settings overrides for web search and summaries.
- **Unauthenticated feedback submission** via optional tokens and `MULL_USER_METADATA` env.
...
- **Hooks system** executes custom scripts for pre/post-tool and session events from ~/.mull/hooks/ with deny-wins trust controls.
- **Independent feedback flag** gates /feedback and popups separately from telemetry via MULL_FEEDBACK_ENABLED.
- **Leader CLI** adds `mull leader list/info/profile/kill/dev` for discovery and CPU profiling.
...
- **ACP `palmshed.ai/subagent/list_running`** queries live progress for all running subagents of parent session.
- **Bash tool params from config.toml** override schema defaults like `timeout_secs` for MullBuild.
- **Live subagent progress** in TUI tasks panel via ACP polling and push notifications.
- **ACP extension resumes sessions in worktrees** via single call matching `mull -w -r` CLI flow.
...
- **Custom skills now discoverable** via config.toml paths and `palmshed.ai/skills/add` in system prompts.
...
- **SSE parsing handles flat errors** from Mull proxy alongside OpenAI-standard nested format.
...
- **Silent mull-pager installation** hides implementation details from install/update output.
...
- **Subagent support** gated behind `--subagents` flag, `MULL_SUBAGENTS` env var, config.toml, or remote settings — disabled by default.
...
- **`mull memory reindex` and `doctor` CLI commands** for index maintenance, plus access-frequency boosting for retrieved memory chunks.
...
- **Cross-devbox session restore** via `mull sessions list/search` CLI subcommands and hardened `mull -r` with upload-ordering safety, cwd-scoped lookups, and staged-vs-unstaged correctness.
...
- **Replay notifications now guaranteed before session/load response** via `forward_with_completion()` drain; removes public `acp_send_fire_and_forget` from mull-acp-lib.
...
- **Kill background tasks by task_id** via new `palmshed.ai/task/kill` ACP extension method.
...
- **On-demand memory flush** via `/flush` command and `palmshed.ai/memory/flush` extension with concurrent-flush prevention.
- **Tool state export/import** via GetToolState RPC and initial_tool_state_json, enabling session warm-start and cloning.
- **`MULL_MEMORY=0` force-disables memory** regardless of config.toml or remote settings; CLI flag still overrides.
...
- **Headless mode (`mull -p`) exit panic** fixed by flushing telemetry before runtime teardown.
...
- **Leader auto-update pre-downloads the binary** before shutdown and resolves the ~/.mull/bin symlink at spawn time.
...
- **Mid-session MCP server toggling** via palmshed.ai/session/update_mcp_servers extension method with optimistic rollback.
- **Sandbox profile configurable via MULL_SANDBOX env variable**, defaulting to workspace profile on devboxes.
- **Agent version exposed in InitializeResponse** metadata for relay and client version discovery.
- **`mull worktree` subcommand** for listing, inspecting, removing, and garbage-collecting session worktrees.
...
- **ACP file operations** were silently bypassing the client filesystem backend, breaking ask-mode write rejection in MullCode.
...
- **Reliable CLI updates** now refresh mull-latest during installs, ensuring symlink-based setups launch the newly installed binary in fresh shells.
...
- **Agent environment metadata becomes queryable** by merging MULL_AGENT_METADATA into initialize responses and persisting the full blob in backend agent records.
...
- **Prompt introspection from the CLI** is available via `mull prompt`, with JSON or section output and persisted session prompt_context snapshots.
...
- **Installation diagnostics** help troubleshoot conflicting mull binaries by listing canonical and invoked paths, versions, update status, and optional JSON output.
...
- **npm installs now use a canonical Mull binary path** via postinstall copy to `~/.mull/bin/mull`, preventing installer conflicts and version confusion.
