...
use mull_tools::types::compat::{CompatConfig, CompatConfigToml};
...
pub use mull_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `mull-config-types` (config
// dependency inversion); re-exported so `crate::util::config::*` paths keep working.
pub use mull_config_types::{McpJsonOAuthBlock, McpServerConfig, McpServerTransportConfig};
...
pub use mull_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use mull_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.mull/config.toml` at each level.
/// Project-scoped `.mull/config.toml` entries override global `~/.mull/config.toml`
...
/// `.mull/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.mull/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.mull/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.mull/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .mull/config.toml"
                );
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.mull/config.toml`.
...
/// Upsert an MCP server entry in `~/.mull/config.toml`.
///
...
/// config file, e.g. a project-scoped `.mull/config.toml`.
...
/// Delete an MCP server entry from `~/.mull/config.toml`.
///
...
/// config file, e.g. a project-scoped `.mull/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = mull_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.mull/config.toml` files.
...
// `.mcp.json` discovery moved to `mull-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use mull_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.mull/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::mull_home::mull_home().join("config.toml")
}

/// Path to the user-level config file (`~/.mull/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.mull/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".mull").join("config.toml")
}
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.mull/skills", "~/.mull/skills/special/SKILL.md"]
ignore = ["~/.mull/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.mull/skills", "~/.mull/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.mull/skills/noisy/SKILL.md"]);
    }
...
"#;
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${MULL_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
    ...
    fn mcp_json_all_toml_names_includes_disabled() {
        ...
        std::fs::create_dir_all(&mull_dir).unwrap();
        std::fs::write(
