...
/// [`mull_config::env_bool`] parser (`MULL_TOOLS_FIND_BFS` /
/// `MULL_TOOLS_GREP_UGREP`, plus the `MULL_FIND_BFS` / `MULL_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.mull/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`mull_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = mull_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = mull_config::env_bool(primary).or_else(|| mull_config::env_bool(alias));
    ...
    (
        resolve("MULL_TOOLS_FIND_BFS", "MULL_FIND_BFS", "find_bfs"),
        resolve("MULL_TOOLS_GREP_UGREP", "MULL_GREP_UGREP", "grep_ugrep"),
    )
}
...
const ENV_PERSISTENT_SHELL: &str = "MULL_PERSISTENT_SHELL";
...
mod persistent_local_shell_tests {
    ...
    // MULL_PERSISTENT_SHELL is process-global (the documented kill-switch a dev
...
/// the shared [`mull_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "MULL_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`MULL_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            mull_tools::implementations::mull_build::ask_user_question::RESPONSE_TIMEOUT.as_secs(),
        )
}

/// Resolve `[toolset.ask_user_question] timeout_secs` (positive seconds).
///
/// Precedence: requirements > env (`MULL_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        mull_tools::implementations::mull_build::ask_user_question::response_timeout_env_secs(),
...
) -> mull_tools::implementations::mull_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use mull_tools::implementations::mull_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
...
mod tests {
    ...
    // Assumes MULL_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
