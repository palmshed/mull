...
pub fn load_relay_sync_enabled_sync() -> bool {
    toml_bool_sync(Some("MULL_RELAY_SYNC_ENABLED"), "relay", "enabled")
}
...
/// 1. Nearest project `.mull/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.mull/config.toml` `[permission]` section
///
...
"#;
            ...
            && let Some(TomlValue::Table(models)) = table.get("models")
        {
            let default = models
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            assert_eq!(default.as_deref(), Some("mull-code-fast-1"));
        } else {
