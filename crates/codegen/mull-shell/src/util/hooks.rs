...
use mull_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .mull/hooks/ directories are still scanned (they hold
    ...
    // user_mull_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .mull.
    ...
    if let Some(ref mull) = mull {
        global.push(mull.join("hooks"));
    }

    let custom_paths: Vec<PathBuf> = mull
        .as_ref()
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".mull").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    mull_hooks::discovery::HookRegistry,
    Vec<mull_hooks::error::HookError>,
) {
