...
pub enum ClientMode {
    /// Headless mode (mull agent, mull agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (mull agent stdio, mull -p) - uses local IPC.
...
pub struct ClientCapabilities {
    ...
    /// Whether this client has advertised `palmshed.ai/codeNavigation.enabled`.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `mull update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `mull update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `mull update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["mull-a".into(), "mull-b".into()],
            pid: 4242,
        };
