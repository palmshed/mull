//! Leader-follower IPC architecture for mull-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.mull/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use mull_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::MullBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = mull_version::VERSION;
...
/// Maximum reconnection attempts for bounded mode (headless/`mull -p`).
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<MullBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(MullBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<MullBuildEnvironment> {
    let environments: &[MullBuildEnvironment] = &[MullBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<MullBuildEnvironment> {
    let environments: &[MullBuildEnvironment] = &[MullBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::mull_home::mull_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : mull_version::VERSION,
                            }
                        )),
                    );
                }
...
impl From<&crate::auth::MullComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::MullComConfig) -> Self {
        Self {
            mull_ws_url: c.mull_ws_url.clone(),
            mull_ws_origin: c.mull_ws_origin.clone(),
        }
    }
}
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`mull -p` where hanging forever is unacceptable.
...
///     "mull-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
///
/// The `env_urls.mull_ws_url` determines which leader instance to connect to.
/// Different WS URLs get different leader processes (via hashed socket paths).
///
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "mull-tui", "vscode")
...
pub async fn connect_or_spawn(
    ...
    let mut lock = LeaderLock::new(&env_urls.mull_ws_url);
...
///
/// For a **managed install** — the running binary lives under `mull_home`
/// (e.g. `~/.mull/...`) — prefer the managed `~/.mull/bin/mull` symlink. After an
/// auto-update or `mull update` atomically swaps that symlink, `current_exe()`
...
/// `mull_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `mull_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.mull/bin/mull` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::mull_home::mull_home())
}
fn resolve_binary_with_home(mull_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(mull_home, std::env::current_exe().ok())
}
/// Binary file name for the managed mull install (`mull` / `mull.exe`).
fn managed_mull_bin_name() -> &'static str {
    if cfg!(windows) { "mull.exe" } else { "mull" }
}
...
fn resolve_binary_impl(
    ...
    let managed_bin = mull_home.join("bin").join(managed_mull_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, mull_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    cmd.arg("--mull-ws-url").arg(&env_urls.mull_ws_url);
    cmd.arg("--mull-ws-origin").arg(&env_urls.mull_ws_origin);
    ...
    let log_path = crate::util::mull_home::mull_home().join("leader.log");
    ...
    let leader_log = std::env::var("MULL_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "mull_shell=info,mull_acp_lib=warn,mull_mcp=warn".into());
...
mod tests {
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            mull_ws_url: "wss://test.invalid".into(),
            mull_ws_origin: "https://test.invalid".into(),
        };
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("mull"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let target_v2 = bin_dir.join("mull-v2");
        std::fs::write(&target_v2, "new-binary").unwrap();
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("mull")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let new_target = bin_dir.join("mull-v2");
        std::fs::write(&new_target, "new-binary").unwrap();
        let managed = bin_dir.join("mull");
        std::os::unix::fs::symlink(&new_target, &managed).unwrap();
        let stale_target = bin_dir.join("mull-v1");
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_mull_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_mull_bin_name());
