...
use fs2::FileExt;
use mull_workspace::util::is_lock_contended;
...
pub fn compute_ws_url_suffix(ws_url: &str) -> String {
    ...
    // `mull_ws_url` is always the *relay* endpoint (see
...
/// `~/.mull/leader.sock` — e.g. run a local branch build's leader without
...
/// bypassed entirely.
pub const LEADER_SOCKET_ENV: &str = "MULL_LEADER_SOCKET";
...
pub fn lock_path_for_ws_url(ws_url: &str) -> PathBuf {
    resolve_lock_path(leader_socket_override(), &mull_home(), ws_url)
}
...
pub fn socket_path_for_ws_url(ws_url: &str) -> PathBuf {
    resolve_socket_path(leader_socket_override(), &mull_home(), ws_url)
}
...
impl LeaderLock {
    /// Create a new LeaderLock using the default paths in mull home.
...
mod tests {
    ...
    fn override_socket_path_wins_over_ws_url_derivation() {
        let root = Path::new("/home/u/.mull");
        let override_sock = PathBuf::from("/home/u/.mull/leader-branch.sock");
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/u/.mull/leader-branch.lock")
        );
    }
