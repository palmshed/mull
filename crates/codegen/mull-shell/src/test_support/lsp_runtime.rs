...
use mull_tools::implementations::mull_build::task::types::{SubagentRequest, SubagentResult};
use std::collections::HashMap;
use std::path::PathBuf;
use std::sync::Arc;
use tokio::sync::{mpsc, oneshot};
pub(crate) type GatewayOut = <acp::AgentSide as mull_acp_lib::AcpSide>::OutMessage;
...
pub(crate) fn ctx_with_toggle(toggle: HashMap<String, bool>) -> SubagentSpawnContext {
    ...
    SubagentSpawnContext {
        ...
        fs: Arc::new(mull_workspace::file_system::LocalFs::new(PathBuf::from(
            "/tmp",
        ))),
        ...
        auth_manager: Arc::new(crate::auth::AuthManager::new(
            std::path::Path::new("/tmp/nonexistent-mull-test"),
        ...
        parent_skills: None,
        parent_skills_config: mull_agent::prompt::skills::SkillsConfig::default(),
        parent_compat: mull_tools::types::compat::CompatConfig::default(),
        auto_wake_delivered: None,
        synthetic_trace_tx: None,
        task_output_tool_name: mull_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL
            .to_string(),
...
impl mull_tools::implementations::lsp::LspBackend for DummyLspDispatch {
    ...
    async fn dispatch(
        &self,
        _input: &mull_tools::implementations::lsp::LspToolInput,
    ) -> mull_tools::implementations::lsp::LspToolResult {
        mull_tools::implementations::lsp::LspToolResult {
            text: String::new(),
            is_error: false,
        }
    ...
    ) -> Option<mull_tools::implementations::lsp::DiagnosticsSummary> {
        None
    }
    ...
    ) -> Vec<mull_tools::implementations::lsp::FileDiagnosticEntry> {
        vec![]
    }
}
