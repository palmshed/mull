...
//!
//! - `palmshed.ai/mcp/list` — list available MCP servers (agent-scoped or session-annotated)
//! - `palmshed.ai/mcp/call` — invoke an MCP tool directly, outside the LLM loop
//! - `palmshed.ai/mcp/servers_updated` — notification pushed when managed configs resolve
//! - `palmshed.ai/mcp/server_status` — per-server delta pushed by the
...
// rmcp is quarantined in mull-mcp; see that crate's docs.
use mull_mcp::rmcp;
...
/// Agent-only `palmshed.ai/mcp/*` ACP method/notification names.
///
/// Unlike [`wire::MCP_CALL`] (the cross-SDK contract, which stays in
/// `mull_mcp::wire`), these methods are private to the agent↔client channel and
...
pub mod mcp_methods {
    ...
    pub const PREFIX: &str = "palmshed.ai/mcp/";

    pub const LIST: &str = "palmshed.ai/mcp/list";
    pub const READ_RESOURCE: &str = "palmshed.ai/mcp/read_resource";
    pub const AUTH_STATUS: &str = "palmshed.ai/mcp/auth_status";
    pub const AUTH_TRIGGER: &str = "palmshed.ai/mcp/auth_trigger";
    pub const TOGGLE: &str = "palmshed.ai/mcp/toggle";
    pub const TOGGLE_TOOL: &str = "palmshed.ai/mcp/toggle_tool";
    pub const UPSERT: &str = "palmshed.ai/mcp/upsert";
    pub const DELETE: &str = "palmshed.ai/mcp/delete";

    pub const SERVERS_UPDATED: &str = "palmshed.ai/mcp/servers_updated";
    pub const TOOLS_CHANGED: &str = "palmshed.ai/mcp/tools_changed";
    pub const INIT_PROGRESS: &str = "palmshed.ai/mcp/init_progress";
}
...
// Re-export the `palmshed.ai/mcp/server_status` schema +
// method constant from the dispatcher module so external callers
// have a single import point alongside the other `palmshed.ai/mcp/*`
// types.
//
...
pub async fn notify_servers_updated(
    gateway: &mull_acp_lib::AcpAgentGatewaySender,
    ...
    if let Ok(params) = serde_json::value::to_raw_value(&payload) {
        ...
        tracing::info!("Sent palmshed.ai/mcp/servers_updated notification to client");
    }
}

// ── Dispatch ────────────────────────────────────────────────────────

/// Inbound `palmshed.ai/mcp/*` methods this agent services, resolved from the wire string.
///
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<mull_tools::bridge::ToolBridge>,
    event_writer: Option<&mull_file_utils::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(mull_file_utils::events::Event::McpHealthCheck {
        ...
        let (status, tools) = if ready {
            ...
            tools.sort_by_key(|a| a.name.clone());
...
pub async fn init_agent_mcp_pool(mcp_state: &Arc<TokioMutex<McpState>>, cwd: &std::path::Path) {
    ...
    let noop = mull_file_utils::events::EventWriter::noop();
...
//
// Implements the `McpResourceProvider` trait from mull-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `mull-mcp` directly.
...
impl mull_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<mull_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(mull_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<mull_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(mull_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(mull_tools::types::resources::McpResourceContent::Text(text)),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                uri: content_uri,
                mime_type,
                ...
                description: None,
                mime_type,
                content: Some(mull_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
mod tests {
    ...
    /// The emit-only reverse method (`palmshed.ai/mcp/sdk_call`) shares the `palmshed.ai/mcp/`
    ...
    fn inbound_sdk_call_has_no_forward_route() {
        ...
        assert_eq!(
            ...
            None,
            "inbound palmshed.ai/mcp/sdk_call must not resolve to a forward handler"
        );
    ...
    fn test_mcp_list_response_serialization() {
        let resp = McpListResponse {
            servers: vec![
                McpServerEntry {
                    name: "mull_com_linear".to_string(),
                    display_name: None,
                    source: McpServerSource::Managed,
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("Mull CLI".to_string()),
                    },
                    source_label: None,
                    session: None,
                },
        ...
        assert_eq!(json["servers"][0]["scopeName"], "Mull CLI");
    ...
    fn disabled_managed_http_rows_keep_non_gateway_placeholder_config() {
        let entry = disabled_server_placeholder_entry("mull_com_slack");
    ...
    fn disabled_placeholders_match_handle_list_catalog_relationships() {
        ...
        append_disabled_like_handle_list(
            &mut servers,
            &["mull_com_slack", "mull_mcp_linear"],
            true,
        );
        assert!(
            servers.is_empty(),
        ...
        let mut servers = build_mcp_catalog_with_gateway_tools(&[], &[], None, &Default::default());
        append_disabled_like_handle_list(&mut servers, &["mull_com_slack"], false);
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0].name, "mull_com_slack");
