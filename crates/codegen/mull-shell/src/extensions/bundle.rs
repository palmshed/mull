...
mod tests {
    ...
    fn test_auth() -> crate::auth::MullAuth {
        crate::auth::MullAuth {
            key: "token".to_string(),
    ...
    fn status_only_reports_bundled_cache_not_higher_priority_sources() {
        ...
        std::fs::create_dir_all(project_root.join(".mull/personas")).unwrap();
        std::fs::create_dir_all(project_root.join(".mull/roles")).unwrap();
        std::fs::write(
            project_root.join(".mull/personas/researcher.toml"),
            "instructions = \"project persona\"\n",
        )
        .unwrap();
        std::fs::write(
            project_root.join(".mull/roles/reviewer.toml"),
