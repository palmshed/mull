//! `palmshed.ai/hooks/*` extension handlers.
//!
...
use mull_hooks::event::{HookEventEnvelope, HookEventName};
use mull_hooks::matcher::HookMatcher;
use mull_hooks_plugins_types::{HookEvent, HookHandlerType, HookInfo};
...
pub fn hook_spec_to_info(spec: &mull_hooks::config::HookSpec) -> HookInfo {
    use mull_hooks::event::HookEventName;
    ...
    HookInfo {
        ...
        disabled: mull_hooks::trust::is_hook_disabled(&spec.name),
    }
}

// Wire types for client-registered hooks (`palmshed.ai/hooks/run`); the gate that uses
...
/// for both the `palmshed.ai/hooks/run` request (gate) and the `palmshed.ai/hooks/event` notification
...
/// Response payload for `palmshed.ai/hooks/run` (client to agent). `Default` (used on
...
/// Parse client hooks from `session/new` `_meta["palmshed.ai/hooks"]`, shaped
...
pub(crate) fn parse_client_hooks(meta: Option<&acp::Meta>) -> ClientHooks {
    ...
    let Some(map) = meta
        .and_then(|m| m.get("palmshed.ai/hooks"))
    ...
    for (event_name, value) in map {
        ...
        let Ok(event) = HookEventName::deserialize(de) else {
            tracing::warn!(event = %event_name, "ignoring unknown palmshed.ai/hooks event");
            continue;
        };
        let Some(array) = value.as_array() else {
            tracing::warn!(event = %event_name, "palmshed.ai/hooks event value is not an array; skipping");
            continue;
        };
...
/// clear) when the request meta carries `palmshed.ai/hooks`, else `None` so a reconnect that
...
pub(crate) fn reconnect_client_hooks(meta: Option<&acp::Meta>) -> Option<ClientHooks> {
    meta.and_then(|m| m.get("palmshed.ai/hooks"))
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let group = WireGroup::deserialize(value)
        .inspect_err(
            |err| tracing::warn!(%event, %err, "ignoring malformed palmshed.ai/hooks group"),
        )
        .ok()?;
    if group.hook_callback_ids.is_empty() {
        tracing::warn!(%event, "ignoring palmshed.ai/hooks group with no hookCallbackIds");
        return None;
    }
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern) => match HookMatcher::new(pattern) {
            ...
            Err(err) => {
                tracing::warn!(%event, pattern, %err, "ignoring palmshed.ai/hooks group with invalid matcher");
                return None;
            }
        },
    };
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "palmshed.ai/hooks/action" => {
            let req: mull_hooks_plugins_types::HooksActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    ...
    fn parse_client_hooks_canonicalizes_subagent_alias() {
        let meta = serde_json::json!({
            "palmshed.ai/hooks": { "SubagentEnd": [{ "hookCallbackIds": ["cb"] }] }
        });
        let hooks = parse_client_hooks(meta.as_object());
    ...
    /// Reconnect refresh applies hooks only when the load meta carries `palmshed.ai/hooks`:
    ...
    fn reconnect_client_hooks_only_when_key_present() {
        ...
        let cleared =
            reconnect_client_hooks(serde_json::json!({ "palmshed.ai/hooks": {} }).as_object());
        assert!(cleared.is_some_and(|h| h.is_empty()));

        let set = reconnect_client_hooks(
            serde_json::json!({
                "palmshed.ai/hooks": { "PreToolUse": [{ "hookCallbackIds": ["cb"] }] }
            })
            .as_object(),
        );
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use mull_hooks::event::{HookEventEnvelope, HookPayload};
