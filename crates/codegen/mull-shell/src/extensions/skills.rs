...
use mull_agent::prompt::skills::{CompatConfig, SkillInfo, SkillsConfig, list_skills_with_plugins};
...
fn discover_auto_sources(cwd: &str, skills: &[SkillInfo]) -> Vec<(String, usize)> {
    ...
    for subdir in &subdirs {
        try_add_source(mull_home.join(subdir), None);
    }
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "palmshed.ai/skills/add" => {
            ...
            .await
            {
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::SkillAdded {
                    added_count: 0,
                    total_skills: 0,
                    success: false,
                });
        ...
        "palmshed.ai/skills/remove" => {
            ...
            .await
            {
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::SkillRemoved {
                    success: false,
                });
