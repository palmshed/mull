...
use mull_tools::types::{KillOutcome, TaskSnapshot};
...
/// Wire DTO for the `palmshed.ai/task/kill` ext request.
///
/// `pub` (with both serde directions) so ACP clients (mull-pager) build
...
/// Wire DTO for the `palmshed.ai/task/kill` ext response payload (nested under
...
/// Wire DTO for the `palmshed.ai/subagent/cancel` ext request.
///
/// `pub` (with both serde directions) so ACP clients (mull-pager) build
...
/// Wire DTO for the `palmshed.ai/subagent/cancel` response payload (under `result` in
...
/// Handle `palmshed.ai/scheduler/*` extension methods.
...
/// Handle `palmshed.ai/subagent/*` extension methods.
pub async fn handle_subagent(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "palmshed.ai/subagent/list_running" => {
...
mod tests {
    ...
    // ── palmshed.ai/subagent/cancel outcome wire DTO ──────────────────────────
