...
/// Palmshed-specific session notification (parallel to acp::SessionNotification)
/// This wraps an MullSessionUpdate with session context for persistence and replay.
...
impl PromptUsage {
    ...
    pub fn project_from_ledger(
        ledger: Option<&mull_chat_state::UsageLedger>,
        incomplete: bool,
    ) -> Option<Self> {
...
impl From<&mull_chat_state::UsageTotals> for PromptUsageModel {
    fn from(t: &mull_chat_state::UsageTotals) -> Self {
        ...
        let mull_chat_state::UsageTotals {
            input_tokens,
            output_tokens,
            cached_read_tokens,
            reasoning_tokens,
...
impl From<&mull_chat_state::UsageLedger> for PromptUsage {
    fn from(ledger: &mull_chat_state::UsageLedger) -> Self {
        let mut usage = Self {
...
pub enum SessionUpdate {
    ...
    HooksChanged {
        hooks: Vec<mull_hooks_plugins_types::HookInfo>,
    ...
    ///
    /// Emitted by the `palmshed.ai/recap` ext method: on demand via the `/recap`
    ...
    /// outcome. Rides the persisted `_palmshed.ai/session/update` rail (unlike the
    /// fire-and-forget `palmshed.ai/session/prompt_complete` notification), so a
...
/// detailed migration guidance (`mull logout` / `mull login`) in the
...
pub struct CompactionSegmentFile {
    pub items: Vec<mull_sampling_types::ConversationItem>,
    /// Curated summary, analysis tags already stripped.
    pub summary: String,
    pub detail: mull_chat_state::CompactionDetail,
...
pub struct CompactionRequestFile {
    ...
    /// Note: this is a per-artifact ID, not the model API's `x_mull_req_id`
    ...
    /// or `"detailed"` (10-section structured prompt for mull-build and similar agents).
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attempt_details: Vec<mull_chat_state::compaction_utils::CompactionAttempt>,
}
...
pub struct RecapRequestFile {
    ...
    /// API's `x_mull_req_id` (also recorded below for proxy correlation).
    pub request_id: String,
    ...
    pub model: String,
    /// Sampling request id sent to the proxy (`palmshed-recap-{uuid}`).
    pub x_mull_req_id: String,
    /// Sampling conversation id (`recap-{uuid}`).
    pub x_mull_conv_id: String,
...
mod tests {
    ...
    fn recap_request_file_roundtrips() {
        let artifact = RecapRequestFile {
            ...
            x_mull_req_id: "palmshed-recap-abc".into(),
            x_mull_conv_id: "recap-abc".into(),
        ...
        assert_eq!(parsed.x_mull_req_id, "palmshed-recap-abc");
        assert_eq!(
    ...
    fn compaction_request_file_v2_roundtrips_attempt_details() {
        use mull_chat_state::compaction_utils::CompactionAttempt;
    ...
    fn memory_flush_completed_with_path_roundtrips() {
        let update = SessionUpdate::MemoryFlushCompleted {
            result: "written".into(),
            path: Some("/home/user/.mull/memory/ws/sessions/log.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_dream_completed_roundtrips() {
        let update = SessionUpdate::MemoryDreamCompleted {
            ...
            path: Some("/home/user/.mull/memory/ws/MEMORY.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_session_saved_roundtrips() {
        let update = SessionUpdate::MemorySessionSaved {
            path: "/home/user/.mull/memory/ws/sessions/2026-01-15-fix-auth-abc12345.md".into(),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_files_variant_roundtrips_through_json() {
        let update = SessionUpdate::MemoryFiles {
            files: vec![
                MemoryFileInfo {
                    path: "/home/user/.mull/memory/MEMORY.md".into(),
                    source: "global".into(),
                    size_bytes: 1024,
                    modified_epoch_secs: Some(1_700_000_000),
                },
                MemoryFileInfo {
                    path: "/project/.mull/memory/MEMORY.md".into(),
