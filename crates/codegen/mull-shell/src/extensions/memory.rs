//! `palmshed.ai/memory/flush`, `palmshed.ai/memory/rewrite`, and `palmshed.ai/compact_conversation`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        m if m.starts_with("palmshed.ai/compact_conversation") => handle_compact(agent, args).await,
        "palmshed.ai/memory/flush" => handle_flush(agent, args).await,
        "palmshed.ai/memory/rewrite" => handle_rewrite(agent, args).await,
