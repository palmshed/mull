//! `palmshed.ai/share_session` extension handler.
//!
...
async fn handle_share_session(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let auth = require_mull_auth_for_share(&agent.auth_manager)?;
...
async fn upload_share_data_to_gcs(
    ...
    if let Err(e) = mull_file_utils::gcs::upload_bytes_signed(
...
fn require_mull_auth_for_share(
    auth_manager: &crate::auth::AuthManager,
) -> Result<crate::auth::MullAuth, acp::Error> {
    super::auth_gate::require_mull_auth(
        auth_manager,
        "Authentication required to share session",
        "Share session is disabled. Run `mull login` to authenticate.",
    )
}
...
mod tests {
    ...
    use crate::auth::{AuthMode, MullAuth};
    ...
    fn make_auth_manager_with_token_expiring_in(
        ...
        // We must explicitly set oidc_issuer to a first-party Palmshed issuer.
        // Only OIDC tokens against https://auth.palmshed.ai (or the local-dev equivalent)
        // return true from is_mull_auth(). This is required for the share tests to
        // exercise the happy path through require_mull_auth_for_share.
        let auth = MullAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some("https://auth.palmshed.ai".to_string()),
            key: "test-key".into(),
    ...
    fn share_works_outside_the_5m_early_invalidation_window() {
        ...
        assert!(require_mull_auth_for_share(&mgr).is_ok());
    }
    ...
    fn share_succeeds_inside_the_5m_early_invalidation_window() {
        ...
        let res = require_mull_auth_for_share(&mgr);
        assert!(
            res.is_ok(),
            "require_mull_auth_for_share must succeed for a still-valid buffered Palmshed token"
        );
    }
    ...
    fn share_fails_with_no_auth_at_all() {
        ...
        assert!(require_mull_auth_for_share(&mgr).is_err());
    }

    #[test]
    fn share_rejects_non_mull_auth_with_actionable_mull_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            MullComConfig::default(),
        ...
        // API key is the simplest non-Palmshed credential (External and enterprise OIDC
        // are also rejected the same way).
        let non_xai = MullAuth {
            auth_mode: AuthMode::ApiKey,
            key: "palmshed-test-key".into(),
            create_time: Utc::now(),
            ..Default::default()
        };
        mgr.hot_swap(non_xai);

        let err = require_mull_auth_for_share(&mgr).expect_err(
            "non-Palmshed accounts (API key, External, enterprise IdP) must be rejected",
        );

        // This is the key assertion the review asked for: we must test the *exact*
        // actionable data string for the non-Palmshed path (distinct from the generic
