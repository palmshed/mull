//! `palmshed.ai/plugins/*` extension handlers.
//!
...
pub fn loaded_plugin_to_info(plugin: &mull_agent::plugins::LoadedPlugin) -> PluginInfo {
    use mull_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &mull_agent::plugins::PluginOrigin) -> PluginOrigin {
    use mull_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
        AgentOrigin::CliOverride => PluginOrigin::CliOverride,
        AgentOrigin::ProjectMull => PluginOrigin::ProjectMull,
        AgentOrigin::ProjectClaude => PluginOrigin::ProjectClaude,
        AgentOrigin::UserMull => PluginOrigin::UserMull,
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "palmshed.ai/plugins/action" => {
            let req: mull_hooks_plugins_types::PluginsActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    use super::*;
    use mull_agent::plugins::PluginOrigin as AgentOrigin;
    use mull_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> mull_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        mull_agent::plugins::LoadedPlugin {
    ...
    fn info_carries_origin_and_marketplace_display_name() {
        let plugin = make_loaded_plugin(AgentOrigin::MarketplaceInstall {
            source_name: Some("Palmshed Official".to_string()),
