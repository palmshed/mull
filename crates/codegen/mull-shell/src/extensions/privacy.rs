//! `palmshed.ai/privacy/setCodingDataRetention` extension handler.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "palmshed.ai/privacy/setCodingDataRetention" => handle_set(agent, args).await,
...
async fn handle_set(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let auth = agent.auth_manager.auth().await.map_err(|e| {
        ...
        acp::Error::auth_required()
            .data("Authentication required. Run `mull login` to re-authenticate.")
    })?;

    let proxy_url = agent.cfg.borrow().endpoints.proxy_url();
    let url = format!("{proxy_url}/privacy/coding-data-retention");
    let token_header = agent.auth_manager.mull_com_config().token_header.clone();
