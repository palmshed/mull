//! Handler for palmshed.ai/git/worktree/* extension methods.
...
const WORKTREE_EXT_LOG: &str = "mull_worktree";
...
impl WorktreeNotificationSender for GatewayWorktreeNotifier {
    async fn send_worktree_status(&self, progress: WorktreeStatus) {
        ...
        let notification =
            acp::ExtNotification::new("palmshed.ai/git/worktree/status", params.into());
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "palmshed.ai/git/worktree/create" => {
        ...
        "palmshed.ai/git/worktree/remove" => {
        ...
        "palmshed.ai/git/worktree/apply" => {
        ...
        "palmshed.ai/git/worktree/create_from_worktree" => {
            let mut req =
            ...
            let result = ops
                .dispatch(
                    &mull_workspace::workspace_ops::PrepareWorktreeFromWorktreeReq {
                        inner: req.clone(),
                    },
                    None,
                )
        ...
        "palmshed.ai/git/worktree/create_from_worktree_sync" => {
            ...
            let resolved_root = ops
                .dispatch(
                    &mull_workspace::workspace_ops::GitResolveRootReq {
            ...
            if let Some(git_root) = resolved_root {
                let vcs_kind = ops
                    .dispatch(
                        &mull_workspace::workspace_ops::DetectVcsKindReq {
                            path: git_root.clone(),
                        },
                        None,
                    )
                    .await
                    .unwrap_or(mull_workspace::session::git::VcsKind::Git);
                if vcs_kind.is_jj() {
            ...
            let result = ops
                .dispatch(
                    &mull_workspace::workspace_ops::CreateWorktreeFromWorktreeSyncReq {
                        inner: req.into_wire(),
                    },
                    None,
                )
        ...
        "palmshed.ai/git/worktree/resume_session" => {
            ...
            let agent_id = mull_telemetry::id::agent_id();
        ...
        "palmshed.ai/git/worktree/list" => {
            let req: mull_workspace::workspace_ops::WorktreeListReq =
        ...
        "palmshed.ai/git/worktree/show" => {
            let req = serde_json::from_str::<ShowWorktreeRequest>(args.params.get())?;
            let op = mull_workspace::workspace_ops::WorktreeShowReq {
        ...
        "palmshed.ai/git/worktree/gc" => {
            ...
            let op = mull_workspace::workspace_ops::WorktreeGcReq {
                dry_run: req.dry_run,
                max_age_secs,
                force: req.force,
            };
        ...
        "palmshed.ai/git/worktree/db/stats" => {
            let result = ops
                .dispatch(&mull_workspace::workspace_ops::WorktreeDbStatsReq {}, None)
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            to_response(Ok(result))
        }
        "palmshed.ai/git/worktree/db/rebuild" => {
            let result = ops
                .dispatch(
                    &mull_workspace::workspace_ops::WorktreeDbRebuildReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
        ...
        "palmshed.ai/git/worktree/db/path" => {
            let result = ops
                .dispatch(&mull_workspace::workspace_ops::WorktreeDbPathReq {}, None)
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            to_response(Ok(result))
        }
...
mod tests {
    ...
    fn list_request_with_filters() {
        let json = r#"{"repo": "mull", "type": ["session", "fork"], "includeAll": true}"#;
        let req: ListWorktreeRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.repo.as_deref(), Some("mull"));
    ...
    fn db_path_response_serializes() {
        let resp = WorktreeDbPathResponse {
            path: "/home/user/.mull/worktrees.db".into(),
        };
        let json = serde_json::to_string(&resp).unwrap();
        assert!(json.contains("\"path\":\"/home/user/.mull/worktrees.db\""));
    }
