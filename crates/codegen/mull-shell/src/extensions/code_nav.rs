...
//! using the mull-codebase-graph index.
//!
//! ## Extension Methods
//!
//! | Method | Description |
//! |--------|-------------|
//! | `palmshed.ai/code/goto-definition` | Definition location(s) for symbol at position |
//! | `palmshed.ai/code/goto-references` | Reference location(s) for symbol at position |
//! | `palmshed.ai/code/find-definitions` | All definitions of a symbol by name |
//! | `palmshed.ai/code/find-references` | All references to a symbol by name |
//! | `palmshed.ai/code/status` | Indexing status |
...
/// Reason string for the `palmshed.ai/code/status` response.
///
...
pub async fn handle(
    ...
    use mull_workspace::workspace_ops::*;

    match args.method.as_ref() {
        "palmshed.ai/code/goto-definition" => {
        ...
        "palmshed.ai/code/goto-references" => {
            let req: GotoRequest = serde_json::from_str(args.params.get())
                .map_err(|e| acp::Error::invalid_params().data(format!("invalid params: {e}")))?;
            let cwd = resolve_cwd(agent, req.cwd.clone(), req.session_id.as_ref())?;
            let was_newly_started =
        ...
        "palmshed.ai/code/find-definitions" => {
        ...
        "palmshed.ai/code/find-references" => {
            let req: FindSymbolRequest = serde_json::from_str(args.params.get())
                .map_err(|e| acp::Error::invalid_params().data(format!("invalid params: {e}")))?;
            let cwd = resolve_cwd(agent, req.cwd.clone(), req.session_id.as_ref())?;
            let was_newly_started =
...
fn to_code_nav_ext_response(resp: mull_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for mull-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
            "client must advertise palmshed.ai/codeNavigation.enabled to use code navigation"
        }
