//! `palmshed.ai/auth/*` and legacy `palmshed.ai/{get,set}ApiKey` extension handlers.
//!
...
//! `auth.json`.
//!
//! OpenRouter migration: API key env var is now `OPENROUTER_API_KEY`.
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "palmshed.ai/auth/getBearerToken" => handle_get_bearer_token(agent).await,
        "palmshed.ai/getApiKey" => handle_get_api_key(),
        "palmshed.ai/setApiKey" => handle_set_api_key(args),
        "palmshed.ai/auth/submit_code" => handle_submit_code(agent, args),
        "palmshed.ai/auth/get_url" => handle_get_url(agent).await,
        "palmshed.ai/auth/logout" => handle_logout(agent, args).await,
        "palmshed.ai/auth/info" => handle_info(agent),
        "palmshed.ai/auth/check_subscription" => handle_check_subscription(agent).await,
...
fn handle_get_api_key() -> ExtResult {
    let key = crate::agent::auth_method::read_mull_api_key_env().ok();
...
fn handle_set_api_key(args: &acp::ExtRequest) -> ExtResult {
    ...
    if let Some(k) = key {
        if k.is_empty() {
            crate::auth::clear_api_key(&mull_home)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::remove_var("OPENROUTER_API_KEY") };
        } else {
            crate::auth::store_api_key(&mull_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `mull-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a mull-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("mull-asset:///{key}")),
        None => None,
    };
