//! `palmshed.ai/billing` extension handler.
//!
//! Fetches the authenticated user's Mull billing configuration
...
/// Current billing configuration for Mull coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `MullBuildBillingConfig` fields
...
/// against both the new `GetMullCreditsConfig` and the legacy
/// `GetMullBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// (`GetMullCreditsConfig.prepaid_balance`); absent in the legacy billing
    /// shape.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prepaid_balance: Option<Cent>,
    /// Whether this user is on unified usage billing (shared weekly/monthly
    /// pool). From `MullCreditsConfig.is_unified_billing_user`, which billing
    /// sets from remote settings `unified_consumer_billing_enabled`. `None` when
    /// absent (legacy `GetMullBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Top-level response (primarily from `GET /rest/mull/credits` + auto-topup-rule).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BillingConfigResponse {
    ...
    /// User-friendly subscription tier name (e.g. "SuperMull Heavy").
...
///
/// Keeps history to a count + the most recent period so `~/.mull/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_mull_auth(
        &agent.auth_manager,
        "Authentication required to fetch billing data",
        "Billing data requires auth with palmshed.ai. Run `mull login` to authenticate.",
    )?;
    ...
    // forwards to the backend `GetMullCreditsConfig`.
    ...
    // Every prompt / /usage / poll path hits `palmshed.ai/billing`; log the fetched
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_mull_auth(
        ...
        "Auto top-up data requires auth with palmshed.ai. Run `mull login` to authenticate.",
    )?;
...
mod tests {
    ...
    fn billing_unified_log_ctx_includes_credits_and_collapses_history() {
        ...
        assert_eq!(ctx["subscriptionTier"], "SuperMull");
    ...
    fn billing_config_deserializes_credits_config_shape() {
        // Newer `GetMullCreditsConfig` response: percentage-based usage,
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_MULL_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
