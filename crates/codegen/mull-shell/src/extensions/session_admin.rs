...
//! lifecycle:
//!
//! - `palmshed.ai/session/rename`                  rename a session locally + remote
//! - `palmshed.ai/session/delete`                  delete a session locally + remote
//! - `palmshed.ai/session/update_mcp_servers`      mid-session MCP server swap
//! - `palmshed.ai/session/fork`                    fork a session into a new one
//! - `palmshed.ai/internal/reload_all_mcp_servers` config hot-reload, all sessions
//! - `palmshed.ai/internal/reload_project_mcp_servers` config hot-reload, cwd-scoped
//! - `palmshed.ai/internal/reload_skills`          skills file watcher fan-out
//! - `palmshed.ai/internal/reload_models`          model list hot-reload from config.toml
//! - `palmshed.ai/internal/reload_models_cache`    model catalog hot-reload from disk cache
//! - `palmshed.ai/internal/auth_cleared`           auth hot-clear cleanup
//! - `palmshed.ai/plugins/reload`                  rebuild shared plugin registry
//! - `palmshed.ai/commands/list`                   list slash commands
...
use mull_telemetry::id::agent_id;

#[tracing::instrument(skip_all, fields(method = %args.method))]
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "palmshed.ai/session/rename" => handle_session_rename(agent, args).await,
        "palmshed.ai/session/delete" => handle_session_delete(agent, args).await,
        "palmshed.ai/session/update_mcp_servers" => handle_update_mcp_servers(agent, args).await,
        "palmshed.ai/session/fork" => handle_session_fork(agent, args).await,
        "palmshed.ai/internal/reload_all_mcp_servers" => handle_reload_all_mcp_servers(agent).await,
        "palmshed.ai/internal/reload_project_mcp_servers" => {
            handle_reload_project_mcp_servers(agent, args).await
        }
        "palmshed.ai/internal/reload_skills" => handle_reload_skills(agent),
        "palmshed.ai/internal/reload_models" => handle_reload_models(agent),
        "palmshed.ai/internal/reload_models_cache" => handle_reload_models_cache(agent),
        "palmshed.ai/internal/auth_cleared" => handle_auth_cleared(agent),
        "palmshed.ai/plugins/reload" => handle_plugins_reload(agent).await,
        "palmshed.ai/commands/list" => handle_commands_list(agent, args).await,
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session rename requires Palmshed OAuth credentials"),
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `mull sessions delete <id>` CLI path.
...
async fn soft_delete_chat_conversation(agent: &MvpAgent, conversation_id: &str) -> ExtResult {
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session delete requires Palmshed OAuth credentials"),
            other => acp::Error::internal_error()
...
/// hot-reload watcher when `<cwd>/.mull/config.toml`,
...
/// Hot-reload the model catalog from `~/.mull/models_cache.json` after an
...
async fn handle_plugins_reload(agent: &MvpAgent) -> ExtResult {
    ...
    // Explicit desktop `palmshed.ai/plugins/reload`: force a full local-install re-copy.
    agent
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `palmshed.ai/commands/list` (the pull used by mull-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .mull/plugins under it, plus
