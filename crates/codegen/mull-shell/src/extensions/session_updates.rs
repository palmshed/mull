//! ACP extension handler for bulk session updates (`palmshed.ai/session/updates`).
//!
...
//! the update type (`"session/update"` for ACP, `"_palmshed.ai/session/update"` for
//! Palmshed extensions) and extract the notification payload from `params`.
...
/// Send updates as chunked `_palmshed.ai/session/updates/chunk` notifications.
    ...
    for (chunk_idx, chunk) in lines.chunks(chunk_size).enumerate() {
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            gateway.forward_fire_and_forget(acp::ExtNotification::new(
                "palmshed.ai/session/updates/chunk",
...
mod tests {
    ...
    fn dummy_gateway() -> mull_acp_lib::AcpAgentGatewaySender {
    ...
    fn make_request(
        ...
        acp::ExtRequest::new("palmshed.ai/session/updates", std::sync::Arc::from(raw))
    }
    ...
    async fn handle_tail_request_with_rewind_returns_only_live_timeline() {
        ...
        std::fs::write(
            ...
            [
                ...
                r#"{"timestamp":5,"method":"_palmshed.ai/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":1}}}"#,
    ...
    fn mull_rewind(target: usize) -> String {
        format!(
            r#"{{"timestamp":0,"method":"_palmshed.ai/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"rewind_marker","target_prompt_index":{target}}}}}}}"#
        )
    }
