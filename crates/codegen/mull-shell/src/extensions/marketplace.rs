//! `palmshed.ai/marketplace/*` extension handlers.
//!
//! Provides marketplace browsing and install endpoints for the pager modal.
//! Delegates to `mull-plugin-marketplace` crate for scanning and install logic.

use agent_client_protocol as acp;
use mull_hooks_plugins_types::{
...
fn load_filtered_marketplace_sources() -> Vec<mull_plugin_marketplace::MarketplaceSource> {
    crate::plugin::load_filtered_marketplace_sources()
}

pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "palmshed.ai/marketplace/list" => handle_list(agent, args).await,
        "palmshed.ai/marketplace/action" => handle_action(agent, args).await,
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                mull_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                mull_plugin_marketplace::SourceKind::Local { path } => path.to_str().unwrap_or("?"),
            };
            format!("{}={}", s.name, url)
        })
        .collect();
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            for source in &sources {
                if let Some(ref filter) = source_url_or_path {
                    let identity = match &source.kind {
                        mull_plugin_marketplace::SourceKind::Local { path } => {
                            path.display().to_string()
                        }
                        mull_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
                    };
                    if &identity != filter {
                        continue;
                    }
                }
                if let mull_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
                    let cache_root = mull_plugin_marketplace::git::default_cache_root();
                    if let Err(e) = mull_plugin_marketplace::git::force_sync_source_cache(
                        url,
                        branch.as_deref(),
                        &cache_root,
                    ) {
            ...
            mull_hooks_plugins_types::ActionOutcome {
                status: mull_hooks_plugins_types::OutcomeStatus::Success,
                message: msg,
...
async fn handle_update(
    ...
    use mull_hooks_plugins_types::{ActionOutcome, OutcomeStatus};
    use mull_plugin_marketplace::installer;

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &mull_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            mull_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            mull_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match mull_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        mull_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = mull_plugin_marketplace::git::default_cache_root();
            match mull_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                mull_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = mull_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = mull_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = mull_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, mull_hooks_plugins_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
        ...
        Err(mull_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            mull_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match mull_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    mull_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
async fn auto_install_defaults(
    agent: &MvpAgent,
    sources: &[mull_plugin_marketplace::MarketplaceSource],
    ...
    for (source, scan) in sources.iter().zip(results.iter()) {
        ...
        let mut reg = mull_agent::plugins::install_registry::InstallRegistry::load();
        ...
        if let Some((existing_key, _)) = existing {
            ...
            let _ = reg.save();
            reg = mull_agent::plugins::install_registry::InstallRegistry::load();
        }
...
fn scan_source(
    source: &mull_plugin_marketplace::MarketplaceSource,
    ...
    let scan = mull_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = mull_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match mull_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: mull_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.mull/config.toml`.
async fn handle_add_source(url: &str) -> mull_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &mull_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if mull_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        mull_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
    ...
    let config_path = mull_config::mull_home().join("config.toml");
    let mull_home = mull_config::mull_home();
    let _save_guard = crate::util::config::lock_config_writes().await;
    let write = {
        let name = name.clone();
        tokio::task::spawn_blocking(move || {
            let _flock = acquire_init_lock(&mull_home).ok();
...
/// Remove a marketplace source from `~/.mull/config.toml` and uninstall all
/// plugins that were installed from it.
async fn handle_remove_source(source_url_or_path: &str) -> mull_hooks_plugins_types::ActionOutcome {
    ...
    match tokio::task::spawn_blocking(move || remove_source_locked(&src)).await {
        Ok(outcome) => outcome,
        Err(e) => mull_hooks_plugins_types::ActionOutcome {
            status: mull_hooks_plugins_types::OutcomeStatus::InternalError,
            message: format!("Config write task failed: {e}"),
            requires_reload: false,
            requires_restart: false,
        },
...
fn remove_source_locked(source_url_or_path: &str) -> mull_hooks_plugins_types::ActionOutcome {
    use crate::plugin;
    use mull_hooks_plugins_types::{ActionOutcome, OutcomeStatus};
    ...
    let config_path = mull_home.join("config.toml");
    let is_official = mull_plugin_marketplace::is_official_source_url(source_url_or_path);
...
/// Acquire an advisory exclusive `flock` on `<mull_home>/.config-init.lock`,
...
fn acquire_init_lock(mull_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(mull_home);
    let lock_path = mull_home.join(".config-init.lock");
...
/// Auto-register the official Palmshed marketplace source on first run.
///
...
pub fn ensure_official_marketplace_source(mull_home: &std::path::Path) {
    let config_path = mull_home.join("config.toml");
    ...
    let _lock = match acquire_init_lock(mull_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %mull_home.join(".config-init.lock").display(),
    ...
    // (settings.json / known_marketplaces.json) under mull_home. Scoped to
    // mull_home only (not ~/.claude) to keep tests hermetic; a user with the URL
    // solely in ~/.claude gets one duplicate entry that the UI dedupes by URL.
    let toml_sources = mull_plugin_marketplace::load_sources(&parsed);
    let json_sources = mull_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&mull_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, mull_plugin_marketplace::SourceKind::Git { url, .. }
            if mull_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            mull_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                mull_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = mull_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
                "auto-registered official Palmshed marketplace source"
            );
        }
        Ok(()) => {}
        Err(e) => {
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<mull_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        mull_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            mull_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                mull_plugin_marketplace::SourceKind::Git { url, .. }
                    if mull_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            mull_plugin_marketplace::SourceKind::Git { url, .. }
                if url == mull_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                mull_plugin_marketplace::OFFICIAL_SOURCE_NAME,
                mull_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"palmshed-official":{{"source":{{"source":"git","url":"{}"}}}}}}"#,
                mull_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_extra_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"extraKnownMarketplaces":{{"palmshed-official":{{"source":{{"source":"git","url":"{}"}}}}}}}}"#,
                mull_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = mull_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
            version: Some("1.0.0".into()),
            description: Some("demo".into()),
            category: Some("dev".into()),
            author: Some("mull".into()),
            tags: vec!["cli".into()],
            ...
            components: Some(mull_hooks_plugins_types::PluginComponents {
                skills: vec![mull_hooks_plugins_types::ComponentItem::new(
                    "code-review",
