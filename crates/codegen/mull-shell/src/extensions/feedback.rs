//! `palmshed.ai/feedback`, `palmshed.ai/feedback/dismiss`, `palmshed.ai/btw`, and `palmshed.ai/review/*`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "palmshed.ai/feedback" | "palmshed.ai/feedback/dismiss" => {
            tracing::info!("handling user feedback");
            handle_feedback(agent, args).await
        }
        m if m.starts_with("palmshed.ai/review") => {
...
/// Handle `palmshed.ai/btw` -- a side question that doesn't interrupt the current turn.
...
async fn handle_feedback(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !agent.cfg.borrow().is_feedback_enabled() {
        return Err(acp::Error::internal_error().data(
            "Feedback is disabled. To enable, set MULL_FEEDBACK_ENABLED=true or \
...
///
/// Methods:
/// - `palmshed.ai/review/comment`: record a new inline code comment to cloud storage
/// - `palmshed.ai/review/comment/delete`: record a tombstone event for a deleted comment
async fn handle_review(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "palmshed.ai/review/comment" => {
        ...
        "palmshed.ai/review/comment/delete" => {
