//! `mull mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = mull_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.mull/config.toml".to_string(),
...
// ── Managed (mull) server discovery ─────────────────────────

const MANAGED_SOURCE_LABEL: &str = "mull";
...
/// Discover managed `mull_com_*` servers if the user has Palmshed auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    let mull_home = mull_tools::util::mull_home::mull_home();
    let mull_com_config = MullComConfig::default();
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&mull_home, mull_com_config));
    ...
    if !snapshot.is_managed_mcp_eligible() {
        return managed_skipped(format!("{:?} auth (not Palmshed OIDC)", snapshot.auth_mode));
    }

    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `mull login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use mull_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `mull mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `mull mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `mull mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `mull mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
