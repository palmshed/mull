...
/// Format: https://palmshed.ai/build/{sessionId}
pub fn build_share_url(session_id: &str) -> String {
    let base_url =
        std::env::var("MULL_CODE_WEB_URL").unwrap_or_else(|_| "https://palmshed.ai".to_string());
...
fn handle_relay_message(
    ...
    match method {
        Some("initialize") => {
            ...
            if let Some(request_id) = id {
                ...
                *initialized = true;
                ...
                let upsert = json!({
                    "jsonrpc": "2.0",
                    "method": "_palmshed.ai/session/upsert",
                    "params": {
                        "sessionId": session_id,
                        "cwd": cwd,
                    }
        ...
        Some("_palmshed.ai/relay/initialized") => {
...
mod tests {
    ...
    fn test_build_share_url_default() {
        ...
        assert_eq!(url, "https://palmshed.ai/build/test-session-123");
    }
    ...
    fn test_build_share_url_with_uuid() {
        ...
        assert_eq!(
            url,
            "https://palmshed.ai/build/01937d8a-1234-7abc-9def-0123456789ab"
        );
    }
