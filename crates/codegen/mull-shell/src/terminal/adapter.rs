//! AcpTerminalAdapter: implements `mull-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates mull-tools' `TerminalBackend` trait into ACP protocol calls:
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (mull-desktop) spawns with `shell: true`
...
/// Wraps mull-shell's ACP gateway to satisfy mull-tools' TerminalBackend.
