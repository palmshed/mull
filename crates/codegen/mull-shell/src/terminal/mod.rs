...
/// Resolved absolute path to bash. On Unix uses the `mull_config` shell
/// resolution cascade (`$MULL_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        mull_config::shell::unix_shell_path(mull_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `mull-tools`.
pub use mull_tools::util::pager_env;
...
/// tool output. Used when the client sets `palmshed.ai/bashOutputNoColor: true` (e.g.
/// mull-pager which renders its own UI and doesn't need raw ANSI codes).
///
