...
const NOTIFICATION_METHOD: &str = "palmshed.ai/terminal/pty/notification";
...
/// `MULL_SHELL`) since `$SHELL` is absent.
fn resolve_pty_shell(shell: Option<&str>) -> (String, Vec<String>) {
    ...
    #[cfg(not(unix))]
    {
        use mull_config::shell::{WindowsShell, detect_windows_shell};
