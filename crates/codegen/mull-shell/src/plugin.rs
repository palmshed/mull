...
use mull_agent::plugins::discovery::PluginScope;
use mull_agent::plugins::git_install::{self, UpdateStatus};
use mull_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use mull_plugin_marketplace::git::{self, SourceCacheLease};
use mull_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.mull/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &mull_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `mull plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `mull plugin install ./{name}`, or add a \
                     source with `mull plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `mull plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `mull plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &mull_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.mull/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user mull via user_mull_home() (None when no home resolves) and
    // home separately, so removal still runs from $MULL_HOME when no home dir
    // exists, and never touches a cwd-relative .mull.
    let home = dirs::home_dir();
    let mull = mull_config::user_mull_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref mull) = mull {
        settings_candidates.push(mull.join("settings.local.json"));
        settings_candidates.push(mull.join("settings.json"));
    }
    ...
    if let Some(ref mull) = mull {
        known_candidates.push(mull.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            registered_source_label(&git_source(
                "Palmshed Official",
                "https://github.com/palmshed/plugins.git"
            )),
            "Palmshed Official (palmshed/plugins)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "Palmshed Official (pin: sentry@palmshed/plugins)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "Palmshed Official (palmshed/plugins)".into(),
        ...
        assert!(
            msg.contains("  - Palmshed Official (palmshed/plugins)"),
            "{msg}"
        );
    ...
    fn ambiguous_qualifier_error_lists_source_names() {
        let err = MarketplaceInstallError::AmbiguousQualifier {
            qualifier: "palmshed/plugins".into(),
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("mull plugin install ./sentry"), "{msg}");
        assert!(msg.contains("mull plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        let err = MarketplaceInstallError::NameAmbiguous {
            name: "sentry".into(),
            candidates: vec!["Palmshed Official (pin: sentry@palmshed/plugins)".into()],
        };
        let msg = err.to_string();
        assert!(
        ...
        assert!(
            msg.contains("  - Palmshed Official (pin: sentry@palmshed/plugins)"),
            "{msg}"
        );
        assert!(
            msg.contains("mull plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    const OFFICIAL_URL: &str = "https://github.com/palmshed/plugins.git";

    #[test]
    fn plan_install_qualifier_unknown_lists_registered_labels() {
        let sources = [
            git_source("Palmshed Official", OFFICIAL_URL),
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "Palmshed Official (palmshed/plugins)".to_string(),
    ...
    fn plan_install_qualifier_ambiguous_lists_source_names() {
        let sources = [
            ...
            git_source("Mirror B", "git@github.com:palmshed/plugins.git"),
        ];
        let err = plan_install(&sources, "sentry", Some("palmshed/plugins"), |_| {
            Ok(Vec::new())
        })
        .expect_err("two sources share the owner/repo");
        match err {
            MarketplaceInstallError::AmbiguousQualifier { qualifier, sources } => {
                assert_eq!(qualifier, "palmshed/plugins");
                assert_eq!(
                    sources,
    ...
    fn plan_install_qualifier_not_found_when_scan_lacks_name() {
        let sources = [git_source("Palmshed Official", OFFICIAL_URL)];
        let err = plan_install(&sources, "sentry", Some("palmshed/plugins"), |_| {
            Ok(vec![mp_entry("other")])
        })
        ...
        match err {
            MarketplaceInstallError::QualifiedNameNotFound {
                ...
                assert_eq!(source_display, "Palmshed Official");
            }
    ...
    fn plan_install_qualifier_ok_selects_source_and_entry() {
        ...
        let plan = plan_install(&sources, "SeNtRy", Some("palmshed/plugins"), |_| {
    ...
    fn plan_install_bare_name_official_match_proceeds_despite_skip() {
        ...
        let plan = plan_install(&sources, "sentry", None, |source| {
            if source.name == "Palmshed Official" {
                Ok(vec![mp_entry("sentry")])
            } else {
                Err("sync failed".to_string())
            }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> mull_workspace::permission::resolution::MarketplaceAllowlist {
        mull_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = mull_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        let name = resolve_qualified_source_name_with(&sources, "palmshed/plugins")
            .expect("qualifier should match the official source");
        assert_eq!(name, "Palmshed Official");
    }
