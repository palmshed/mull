//! Shim — see `mull_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `mull_shell::instrumentation_timer!`). Keeping the macro here
...
pub use mull_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `mull_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `mull_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `mull_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `mull_telemetry::instrumentation`.
#[macro_export]
