//! Data APIs for `mull models`. Clients own display.
...
/// Status for the `mull models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `mull_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    /// [`crate::agent::auth_method::should_advertise_mull_api_key`] so
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.mull_com_config.mull_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_mull_api_key(
            agent_config.mull_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    use super::*;
    use crate::agent::auth_method::{LEGACY_PALMSHED_API_KEY_ENV_VAR, PALMSHED_API_KEY_ENV_VAR};
    ...
    ///
    /// Uses `MULL_AUTH_PATH` (not `MULL_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            EnvGuard::unset(PALMSHED_API_KEY_ENV_VAR),
            EnvGuard::unset(LEGACY_PALMSHED_API_KEY_ENV_VAR),
            EnvGuard::unset("MULL_AUTH"),
            EnvGuard::set("MULL_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("MULL_DEPLOYMENT_KEY"),
            EnvGuard::unset("MULL_WS_ORIGIN"),
            EnvGuard::unset("MULL_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_api_key_env() {
        ...
        let _key = EnvGuard::set(PALMSHED_API_KEY_ENV_VAR, "palmshed-test-key");
    ...
    fn resolve_legacy_api_key_env() {
        let (_dir, _g) = isolate_auth_sources();
        let _key = EnvGuard::set(LEGACY_PALMSHED_API_KEY_ENV_VAR, "legacy-key");
        assert_eq!(AuthStatus::resolve(&Config::default()), AuthStatus::ApiKey);
    }
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("MULL_AUTH", &json);

        assert_eq!(
            AuthStatus::resolve(&Config::default()),
            AuthStatus::LoggedIn("palmshed.ai".to_owned())
        );
    }
