//! `mull inspect` — configuration introspection.
//!
//! Shows everything Mull discovers in the current directory: project
...
use mull_tools::util::truncate::estimate_tokens;
...
/// Enterprise login-hardening policy resolved from `[mull_com_config]`
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `MULL_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .mull/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `mull inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = mull_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = mull_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = mull_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        mull_version: mull_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if has_rules_directory(file_path, ".mull")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = mull_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        mull_agent::prompt::skills::CompatConfig::default(),
    )
    .await;

    let mull_home = Some(crate::util::mull_home::mull_home());
    ...
    // `mull_agent::prompt::agents_md::read_agents_config_with_paths`
...
/// which handles both Mull TOML and vendor settings fallback in one codepath.
async fn list_permissions(cwd: &Path) -> PermissionsReport {
    use mull_workspace::permission::resolution;
...
/// (`[mull_com_config]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let mull_com_config = config
        .map(|c| c.mull_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: mull_com_config.api_key_auth_disabled(),
        disable_api_key_auth: mull_com_config.disable_api_key_auth,
        force_login_team_uuid: mull_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[mull_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = mull_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        mull_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&mull_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &mull_agent::plugins::PluginRegistry,
    skills_config: &mull_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = mull_agent::prompt::skills::list_skills_with_plugins(
    ...
    skills
        .into_iter()
        .map(|s| {
            let source = skill_entry_source(&s, &mull_home);
...
/// `<mull_home>/skills/<name>/SKILL.md` and discovered as user skills, so a
...
/// discovery: deployed pagers parse `palmshed.ai/skills/list` into a typed
...
fn skill_entry_source(s: &mull_agent::prompt::skills::SkillInfo, mull_home: &Path) -> ConfigSource {
    use mull_tools::implementations::skills::types::SkillScope;
    ...
    match s.scope {
        ...
        SkillScope::User => {
            if crate::builtin::is_extracted_bundled_skill(&s.name, &path, mull_home) {
...
fn list_agents(
    ...
    let agents = mull_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[mull_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                mull_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                mull_agent::plugins::PluginScope::Project => Scope::Project,
                mull_agent::plugins::PluginScope::User => Scope::User,
                mull_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: mull_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    mull_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = mull_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `mull mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// `.mull/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_mull_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.mull_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `mull mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use mull_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use mull_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for path in ["/repo/.mull/rules/team.md", r"C:\repo\.mull\rules\team.md"] {
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.mull/requirements.toml";
    ...
    fn skill_entry_source_maps_scopes() {
        let home = Path::new("/home/u/.mull");

        let s = skill_fixture("a", "/repo/.mull/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.mull/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.mull/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::User { .. }
        ));
        ...
        let s = skill_fixture(
            "d",
            "/home/u/.mull/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("e", "/home/u/.mull/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    /// path `<mull_home>/skills/<name>/SKILL.md`; a same-named skill anywhere
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.mull/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("help", "/repo/.mull/skills/help/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        // Bundled name in a user dir outside <mull_home>/skills: stays user.
        let s = skill_fixture(
            "help",
        ...
        // Bundled frontmatter name in a different dir under <mull_home>/skills:
        // not the extracted copy — stays user.
        let s = skill_fixture(
            "help",
            "/home/u/.mull/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Non-bundled name under <mull_home>/skills: stays user.
        let s = skill_fixture(
            "my-skill",
            "/home/u/.mull/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.mull dirs.
        ...
        let registry = mull_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
