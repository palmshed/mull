...
//!       [--api-key <key> | $OPENROUTER_API_KEY | <mull-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use mull_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// API key. Overrides `$OPENROUTER_API_KEY` when set; falls back to
    /// `$OPENROUTER_API_KEY`, then `<mull-home>/auth.json` when absent or empty.
    #[arg(long)]
    ...
    /// shell uses (`$MULL_HOME` or `~/.mull`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    mull_home: Option<PathBuf>,
}
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert_eq!(cli.api_base_url, "https://openrouter.ai/api/v1");
        assert!(cli.output.is_none());
        assert!(cli.api_key.is_none());
        assert!(cli.min_confidence.is_none());
        assert!(cli.include_reasoning.is_none());
        assert!(cli.mull_home.is_none());
    }
    ...
    #[test]
    fn cli_mull_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.mull_home, Some(PathBuf::from("/tmp/scratch-mull")));
    }
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["openrouter/free"]);
        assert_eq!(by_id("api_base_url"), vec!["https://openrouter.ai/api/v1"]);
