...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.mull/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** mull process
    /// sharing the same `~/.mull` (the writer may also be this process;
...
/// Watches `~/.mull/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.mull/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.mull/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let mull_home_buf = mull_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(mull_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(mull_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(mull_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(mull_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %mull_home.display(),
                    error = %e,
                    "failed to watch mull home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.mull/config.toml` so the loop above
        // watches `<cwd>/.mull/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.mull/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.mull/config.toml` trigger a
    ...
    /// `<cwd>/.mull/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.mull/` does not yet
/// exist at session-open time, the `.mull/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.mull/` followed by a
/// write to `<cwd>/.mull/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let mull_dir = cwd.join(".mull");
    if let Err(e) = debouncer
        .watcher()
        .watch(&mull_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .mull directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let mull_dir = cwd.join(".mull");
    if let Err(e) = debouncer.watcher().unwatch(&mull_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .mull directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.mull/` hasn't
...
/// Watches skill directories (`~/.mull/skills/`, `<repo>/.mull/skills/`, etc.)
...
/// recursively: `mull_home` (`~/.mull`, or `$MULL_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.mull` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, mull_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, mull_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".mull", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, mull_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(mull_home) {
        return true;
    }
...
/// `~/.mull` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](mull_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = mull_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &mull_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &mull_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only mull_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_mull_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let mull_home = home.join(".mull");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &mull_home, Some(home));

        // mull_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&mull_home));
        ...
        assert!(!g(&home.join("repo").join(".mull")));
    }

    /// Regression for the ~/.mull inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.mull/worktrees/`) must not
    ...
    /// A write to `<mull_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another mull process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.mull/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let mull_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_mull = cwd.path().join(".mull");
        fs::create_dir_all(&project_mull).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_mull.join("config.toml"), "").unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.mull/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.mull/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_mull = new_cwd.path().join(".mull");
        fs::create_dir_all(&project_mull).unwrap();
        fs::write(project_mull.join("config.toml"), "").unwrap();
