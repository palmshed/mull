...
use crate::auth::{MullAuth, read_auth_json};
...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.mull/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.mull/config.toml`,
    ...
    /// (`<cwd>/.mull/config.toml`, `<cwd>/.mcp.json`, or
    ...
    /// mull process sharing the home dir). The agent should consult the cache
    /// thrash unrelated sessions.
    ProjectMcpServersChanged {
        /// The project root whose `.mull/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.mull/models_cache.json` was rewritten on disk (possibly by another
    ...
    ModelsCacheChanged,
    /// Updated UI settings — agent broadcasts `palmshed.ai/config_changed` to IPC clients.
    Ui {
        theme: Option<String>,
        yolo: bool,
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            if has_auth {
                ...
                match result {
                    Ok(Err(e)) => {
                        ...
                        let path = self.mull_home.join("auth.json");
            ...
            // same tick (e.g. `~/.mull/config.toml` AND
    ...
    fn reload_auth(&mut self) -> anyhow::Result<()> {
        let auth_path = self.mull_home.join("auth.json");
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.mull/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.mull/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.mull/config.toml` | `<cwd>`           |
...
/// for `.mull/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.mull/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
/// `ConfigUpdate::Skills`) and `mull inspect` (via the `crate::config`
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> mull_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> mull_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    async fn reloader_detects_new_auth_key() {
        ...
        assert!(
            matches!(update, ConfigUpdate::Auth(a) if a.key == "new-key"), // a is Box<MullAuth>, Deref coercion
    ...
    /// The hash must reflect ancestor `.mull/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".mull").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .mull/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_empty() {
        ...
        assert_eq!(skills, mull_agent::prompt::skills::SkillsConfig::default());
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.mull/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.mull/skills".to_string()]);
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.mull/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.mull/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.mull/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.mull/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
