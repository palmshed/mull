...
use crate::extensions::notification::SessionUpdate as MullSessionUpdate;
...
use mull_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use mull_agent::prompt::skills::SkillsConfig;
use mull_sampler::SamplerConfig as SamplingConfig;
use mull_sampling_types::truncate_bytes;
...
use mull_tools::types::compat::CompatConfig;
use mull_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use mull_workspace::file_system::CodebaseIndexManager;
use mull_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use mull_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
const SESSION_LOG: &str = "mull_session";
#[path = "compaction.rs"]
mod compaction;
...
pub(crate) use spawn::*;
/// Client-registered hook gates (the `palmshed.ai/hooks/run` reverse request).
mod hooks;
...
impl mull_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        mull_tools::types::resources::ManagedGatewayToolCallResponse,
        mull_tool_runtime::ToolError,
    > {
        let auth_key = self
            .auth_manager
            .get_valid_token()
            ...
            .ok_or_else(|| mull_tool_runtime::ToolError::unauthorized("no auth token available"))?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                mull_tool_runtime::ToolError::unauthorized(detail)
            } else if status == reqwest::StatusCode::FORBIDDEN {
                mull_tool_runtime::ToolError::permission_denied(detail)
            } else {
                let tool_id = mull_tool_protocol::ToolId::new(caller).unwrap_or_else(|_| {
                    mull_tool_protocol::ToolId::new("use_tool").expect("valid")
                });
                mull_tool_runtime::ToolError::execution(tool_id, detail)
            };
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            mull_tool_runtime::ToolError::network_error(format!(
                "Managed MCP gateway tool call failed: {}",
                e.without_url()
            ))
        }
        crate::session::managed_mcp::ManagedMcpFetchError::NoAuth => {
            mull_tool_runtime::ToolError::unauthorized("no auth token available")
        }
    }
}
#[cfg(test)]
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, mull_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(err.kind, mull_tool_runtime::ToolErrorKind::PermissionDenied);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, mull_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, mull_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// `mull-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<mull_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: mull_chat_state::ChatStateHandle,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<mull_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<mull_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-mull-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<mull_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<mull_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<mull_agent::Agent>,
    /// Dedup slot for `palmshed.ai/git_head_changed`, shared with the fs-watch
    /// `GitHead` consumer (see `git_head_dedup_key`).
    pub(crate) last_reported_branch: Arc<parking_lot::Mutex<Option<String>>>,
    /// Client opted into `palmshed.ai/gitHeadChanged`. When false (headless/SDK),
    ...
    /// Kill-switch (`MULL_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<mull_tools::implementations::mull_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, mull_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: mull_agent_lifecycle::LocalExtensionRegistry,
    ...
    /// Safe: session actor is single-threaded (LocalSet), no concurrent access.
    pub(crate) hook_registry: std::cell::RefCell<Option<Arc<mull_hooks::discovery::HookRegistry>>>,
    /// Client hooks from `session/new` `_meta["palmshed.ai/hooks"]`; gated in
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<mull_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<mull_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: mull_computer_hub_sdk::ObservabilityBridge,
    ...
    /// Handle to the per-session `mull-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: mull_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`mull_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`mull_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: mull_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    fn build_command_availability(
        ...
        use mull_tools::implementations::memory::{MEMORY_GET_TOOL_NAME, MEMORY_SEARCH_TOOL_NAME};
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names
                .iter()
                .any(|n| n == mull_tools::implementations::mull_build::SCHEDULER_CREATE_TOOL_NAME),
    ...
    async fn send_feedback_notification(&self, request: crate::session::feedback::FeedbackRequest) {
        self.send_mull_notification(MullSessionUpdate::FeedbackRequest(request.into()))
            .await;
    }
}
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<mull_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `mull prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &mull_agent::PromptContext) {
...
#[expect(dead_code, reason = "API for future viewers/debug tools")]
pub(crate) fn load_prompt_context(session_info: &SessionInfo) -> Option<mull_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
    load_prompt_context_from_dir(&dir)
}
fn load_prompt_context_from_dir(
    session_dir: &std::path::Path,
) -> Option<mull_agent::PromptContext> {
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use mull_tools::types::output::{MCPOutput, ToolOutput};
    use mull_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl mull_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl mull_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> mull_tool_protocol::ToolId {
            mull_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::mull_tool_runtime::ListToolsContext,
        ) -> mull_tool_types::ToolDescription {
            mull_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
...
mod tool_meta_stamp_tests {
    //! Pin the `palmshed.ai/tool` stamps on the harness emission paths: the early
    ...
    use super::*;
    use mull_tools::registry::types::ToolConfig;
    use mull_tools::tool_taxonomy::TOOL_META_KEY;
    use mull_workspace::permission::PermissionCommand;
    use tokio::sync::mpsc;
    ...
    /// The `palmshed.ai/tool` object from an event's `_meta`, if present.
    ...
    async fn prepare_tool_call_stamps_early_tool_call_and_refinement() {
        ...
        local
            .run_until(async {
                ...
                let t = tool_meta(early.as_ref()).expect("early ToolCall carries palmshed.ai/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["namespace"], "mull_build");
                assert!(t.get("input").is_none(), "identity-only before parse");
                let refined = refined.expect("refinement ToolCallUpdate emitted");
                let t = tool_meta(refined.as_ref()).expect("refinement carries palmshed.ai/tool");
        ...
        local
            .run_until(async {
                ...
                let t = tool_meta(update.meta.as_ref())
                    .expect("permission-request ToolCallUpdate carries palmshed.ai/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["input"]["path"], "/tmp/stamp.txt");
            })
...
mod managed_gateway_tool_tests {
    ...
    use mull_tools::types::output::{MCPOutput, ToolOutput};
    use mull_tools::types::tool::{ToolKind, ToolNamespace};
    use mull_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<mull_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
