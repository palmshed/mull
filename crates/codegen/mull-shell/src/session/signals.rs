...
pub use mull_telemetry::enums::PrCreationSource;
...
impl SessionSignalsHandle {
    ...
    pub fn snapshot_gcs_queue(&self, stats: &mull_file_utils::queue::UploadQueueStats) {
        ...
        let _ = self.tx.send(SignalEvent::RecordGcsQueueSnapshot {
            ...
            orphans_cleaned: mull_file_utils::queue::last_orphans_cleaned(),
        });
    }
...
pub(crate) fn merge_tightest_trigger(current: Option<String>, new: &[String]) -> Option<String> {
    mull_sampling_types::doom_loop::DoomLoopSignal::tightest(
        current
            .iter()
            .map(String::as_str)
                ...
                SignalEvent::TakeTurnEndSnapshot(respond_to) => {
                    ...
                    let mut outcomes: Vec<ToolOutcome> =
                        std::mem::take(&mut self.tool_outcomes_this_turn)
                            ...
                            .collect();
                    outcomes.sort_by_key(|a| a.tool_name.clone());
...
mod tests {
    ...
    async fn test_restore_signals_full_round_trip() {
        ...
        // ITL stats must survive the restore (regression test for mull-critique bug)
        assert_eq!(
        ...
        // This is the exact scenario the mull-critique bug describes: the
