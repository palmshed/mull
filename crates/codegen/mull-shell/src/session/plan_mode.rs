...
pub struct PlanModeTracker {
    ...
    /// `~/.mull/sessions/<cwd>/<session_id>/plan.md`
...
impl PlanModeTracker {
    ...
    /// directory (e.g., `~/.mull/sessions/<encoded-cwd>/<session-id>/`).
...
mod tests {
    ...
    fn plan_file_in_session_dir() {
        let t = PlanModeTracker::new(PathBuf::from("/home/user/.mull/sessions/proj/abc-123"));
        assert_eq!(
            t.plan_file_path(),
            Path::new("/home/user/.mull/sessions/proj/abc-123/plan.md")
        );
    }
    #[test]
    ...
    /// Build a test TemplateRenderer with standard Mull tool mappings.
    ...
    fn is_plan_file_write_exact_match() {
        let plan = Path::new("/home/user/.mull/sessions/proj/abc/plan.md");
        let target = Path::new("/home/user/.mull/sessions/proj/abc/plan.md");
