...
//! `mull-file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use mull_file_utils::events::tracker::EventTracker;
pub(crate) use mull_file_utils::events::types::{
...
) -> Option<mull_sampling_types::PriorTurnInterrupt> {
    use mull_sampling_types::PriorTurnInterrupt;
    match category {
...
/// committing it would silently run mull-build flavor. Distinct from
...
/// (`mull-file-utils`). The two enums are intentional mirrors and live in
