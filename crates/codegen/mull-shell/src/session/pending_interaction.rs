...
use crate::extensions::notification::{SessionNotification, SessionUpdate as MullSessionUpdate};
...
pub enum PendingKind {
    ...
    Question,
    /// `palmshed.ai/exit_plan_mode` plan approval.
    PlanApproval,
}

/// Whether a blocking plan-approval reverse-request is parked in `pending`.
///
/// The resume re-park issues `palmshed.ai/exit_plan_mode` from a detached task
...
fn broadcast(gateway: &GatewaySender, session_id: &acp::SessionId, update: MullSessionUpdate) {
...
impl PendingInteractionGuard {
    ...
    pub fn new(
        ...
        broadcast(
            &gateway,
            &session_id,
            MullSessionUpdate::PendingInteraction {
    ...
    fn drop(&mut self) {
        ...
        if removed {
            broadcast(
                &self.gateway,
                &self.session_id,
                MullSessionUpdate::InteractionResolved {
