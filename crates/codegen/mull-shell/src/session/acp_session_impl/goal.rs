...
impl RoleCapability {
    ...
    fn is_satisfied(
        self,
        summary: &mull_tools::implementations::mull_build::task::types::SubagentTypeSummary,
    ) -> bool {
        match self {
...
impl SessionActor {
    ...
    pub(super) async fn drain_goal_updates_with_extra(
        ...
        extra: Vec<mull_tools::implementations::mull_build::update_goal::UpdateGoalEnvelope>,
    ) {
        use mull_tools::implementations::mull_build::update_goal::{RejectReason, UpdateGoalAck};
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        //    silently run mull-build flavor. Non-strict names (mull-build
        //    family) run the default flavor and pass; unresolvable names fall
        //    through to the describe `Unknown` arm below.
        if mull_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    /// agrees with `from_summary` — `search_replace` on the default mull-build
    /// host, not the literal `write`). The `{TOOLSET_TOOLS}` block is empty on
    /// this path (no per-role toolset enumeration); on the default mull-build
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use mull_sampling_types::conversation::SyntheticReason;
