...
impl SessionActor {
            ...
            Err(e) => {
                ...
                if terminal {
                    ...
                    crate::session::telemetry::emit_mcp_connection_span(
                        ...
                        Some(elapsed_ms),
                        None,
                        Some(mull_telemetry::events::McpErrorType::Auth.as_str()),
                    );
    ...
    /// Emit per-server `palmshed.ai/mcp/tools_changed` notifications.
    ///
    ...
    /// Handle explicit auth trigger from the client (palmshed.ai/mcp/auth_trigger).
    ///
    ...
    pub(super) async fn handle_mcp_auth_trigger(&self, server_name: &str) -> Result<(), String> {
        if server_name.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX) {
            return Err("To authenticate, visit palmshed.ai".to_string());
        }
        let client = {
    ...
    pub(super) async fn maybe_inject_mcp_reminder(&self) {
        ...
        let failed_section = {
            ...
            failed.sort_by_key(|a| a.0.clone());
            if failed.is_empty() {
                None
            } else {
    ...
    /// or removed it from `~/.mull/config.toml` while we were
    /// sleeping.
    ///
    ...
    /// which parses `~/.mull/config.toml` + the project
    /// `.mull/config.toml`) on every call. The auto-restart task
    /// calls this at most:
    ...
    /// `MullClientHandler`, and the liveness watcher is armed —
    ...
    ///
    /// The `MullClientHandler` constructed inside `try_handshake`
    ...
    pub(crate) async fn respawn_stdio(&self, server: &str) -> Result<(), String> {
        ...
        if let Some(tx) = event_tx {
            ...
            let _ = tx.send(mull_mcp::servers::McpClientEvent::ToolsChanged {
                server: server.to_string(),
            });
        }
        let arc_client = std::sync::Arc::new(new_client);
        let _ = arc_client
            .arm_liveness_watcher(mull_mcp::liveness::DEFAULT_POLL_INTERVAL)
            .await;
        {
            let mut mcp_state = self.mcp_state.lock().await;
            mcp_state
    ...
    pub(super) async fn ensure_mcp_tools_initialized(&self) {
        ...
        if mcp_server_configs.is_empty() && !has_acp {
            ...
            if mcp_state.generation() == generation {
                ...
                self.events
                    .emit(mull_file_utils::events::Event::McpInitCancelled {
        ...
        }
        {
            ...
            let managed_count = mcp_server_configs
                ...
                .count() as u32;
            self.events
                .emit(mull_file_utils::events::Event::McpManagedConfigResult {
        ...
        let mcp_clients: Vec<_> = mcp_results
            ...
            .filter_map(|result| match result {
                ...
                Err(e) => {
                    ...
                    self.events
                        .emit(mull_file_utils::events::Event::McpServerFailed {
                            server_name: sname,
        ...
        tokio::task::spawn_local(async move {
            ...
            for client in mcp_clients.iter() {
                ...
                futs.push(async move {
                    ...
                    ew.emit(mull_file_utils::events::Event::McpServerStarting {
            ...
            > = std::collections::HashMap::new();
            {
                ...
                if mcp_state.generation() != generation {
                    ...
                    event_writer.emit(mull_file_utils::events::Event::McpInitCancelled {
                        reason: MCP_INIT_CANCELLED_CONFIG_CHANGED.to_string(),
                    });
                    return;
                }
                    ...
                    match result {
                        Ok((server_name, registrations, elapsed, timeout_sec)) => {
                            ...
                            for reg in registrations {
                                ...
                                if reg.model_visible {
                                        ...
                                        .await
                                    {
                                        ...
                                        event_writer
                                            .emit(mull_file_utils::events::Event::McpToolRegistrationFailed {
                                                server_name: server_name.clone(),
                            ...
                            let transport_enum = match server_transport_map
                                ...
                                .unwrap_or("unknown")
                            {
                                "stdio" => mull_telemetry::events::McpTransport::Stdio,
                                "sse" => mull_telemetry::events::McpTransport::Sse,
                                _ => mull_telemetry::events::McpTransport::Http,
                            };
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::McpServerConnected {
                                    server_name: server_name.clone(),
                                    tool_count,
                            ...
                            let transport_str = server_transport_map
                                ...
                                .copied()
                                .unwrap_or("unknown");
                            event_writer.emit(mull_file_utils::events::Event::McpServerConnected {
                                server_name: server_name.clone(),
                        ...
                        Err((server_name, ref e, needs_auth, elapsed, timeout_sec)) => {
                            ...
                            let error_type_label = match error_cat {
                                mull_file_utils::events::McpErrorCategory::AuthRequired => {
                                    mull_telemetry::events::McpErrorType::Auth
                                }
                                mull_file_utils::events::McpErrorCategory::Timeout => {
                                    mull_telemetry::events::McpErrorType::Timeout
                                }
                                _ => mull_telemetry::events::McpErrorType::HandshakeFailed,
                            };
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::McpServerFailed {
                                    server_name: server_name.clone(),
                            ...
                            event_writer.emit(mull_file_utils::events::Event::McpServerFailed {
                                server_name: server_name.clone(),
                            ...
                            let detail = (!needs_auth).then(|| {
                                mull_tools::util::truncate_str_with_marker(&e.to_string(), 200)
                                    .into_owned()
                            });
                ...
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::McpInitCompleted {
                ...
                event_writer.emit(mull_file_utils::events::Event::McpInitCompleted {
    ...
    pub(crate) fn connected_server_summaries(
        &self,
    ) -> Vec<mull_tools::types::tool_index::ServerSummary> {
        use mull_tools::types::tool_index::ToolSearchIndex;
    ...
    pub(super) async fn mcp_announcement_snapshot(&self) -> Option<McpAnnouncementSnapshot> {
        let server_summaries = self.connected_server_summaries();
        let mut text =
            mull_tools::implementations::search_tool::build_server_reminder(&server_summaries)?;
        if let Some(hint) = self.rendered_mcp_hint().await {
            text.push_str(&hint);
        }
