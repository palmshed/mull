...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<mull_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<mull_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            mull_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : mull_telemetry::memory_log::TARGET,
            ...
            mull_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            Some(mull_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
            ...
            Some(mull_chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
            reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
            body_bytes_after, }) => {
            mull_telemetry::unified_log::info("shell.image_budget", Some(session
            ...
            })),); } Some(mull_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
            Some(mull_chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
            ...
            .fire_hook(mull_hooks::event::HookEventName::SessionEnd, None,
            mull_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            mull_hooks::dispatcher::dispatch_non_blocking(& registry,
            mull_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(mull_hooks::event::HookEventName::Stop,
            None, mull_hooks::event::HookPayload::Stop { reason : "channel_closed"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            mull_hooks::dispatcher::dispatch_non_blocking(& registry,
            mull_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            mull_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            session.send_mull_notification(MullSessionUpdate::MemorySessionSaved { path :
            ...
            mull_telemetry::session_ctx::log_event(mull_telemetry::events::SessionEnded
            ...
            mull_file_utils::trace_context::link_current_span_to_meta(& meta); } let
            ...
            .update_credentials(mull_chat_state::Credentials { api_key : r.api_key,
            ...
            .send(mull_hooks_plugins_types::HooksListResponse { hooks, project_trusted,
            ...
            .send_mull_notification(MullSessionUpdate::PluginUpdatesInstalled { updates },)
            ...
            .push_capped(mull_tools::implementations::mull_build::task::types::MonitorEventNotification
            ...
            mull_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .handle_mull_session_notification(notification). await; }
            ...
            .send(mull_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            .emit(mull_file_utils::events::Event::McpServerToggled { server_name :
            ...
            .forward_fire_and_forget(acp::ExtNotification::new("palmshed.ai/mcp/tools_changed",
            ...
            session.fire_hook(mull_hooks::event::HookEventName::SessionStart, None,
            mull_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            mull_hooks::dispatcher::dispatch_non_blocking(& registry,
            mull_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            .fire_hook(mull_hooks::event::HookEventName::SessionEnd, None,
            mull_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            mull_hooks::dispatcher::dispatch_non_blocking(& registry,
            ...
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(mull_hooks::event::HookEventName::Stop,
            None, mull_hooks::event::HookPayload::Stop { reason : "shutdown"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            mull_hooks::dispatcher::dispatch_non_blocking(& registry,
            mull_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[mull_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = mull_chat_state::compaction_utils::extract_user_query(&raw);
