...
// mull-interjection-core crate so the server-side agent loop can adopt the
...
pub(crate) use mull_interjection_core::{InterjectionBuffer, drain_formatted, format_interjection};

/// Shell instantiation of the shared entry type: images are ACP content.
pub(crate) type PendingInterjection =
    mull_interjection_core::PendingInterjection<acp::ImageContent>;
...
/// `palmshed.ai/session/interjection` broadcast, so a live echo would duplicate it.
