...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &mull_tools::implementations::mull_build::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
impl SessionActor {
    ...
    pub(crate) async fn normalize_images_with_notices(
        ...
        if !norm_result.compressed.is_empty() {
            ...
            let images = norm_result
                ...
                .collect();
            self.send_mull_notification(MullSessionUpdate::ImageCompressed { images, message })
                .await;
        }
        if !norm_result.re_encode_fallbacks.is_empty() {
            ...
            self.send_mull_notification(MullSessionUpdate::ImageCompressed {
                images: vec![],
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let turn_start_input = mull_agent_lifecycle::TurnStartInput::new(
        ...
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = mull_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        mull_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    mull_telemetry::session_ctx::log_event(
                        mull_telemetry::events::SlashCommandUsed {
                            command: sk.name.clone(),
                            args_provided: !sk.args.is_empty(),
                        },
                    );
                    mull_telemetry::session_ctx::log_event(
                        mull_telemetry::events::SkillDispatched {
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        mull_telemetry::session_ctx::log_event(
                            mull_telemetry::events::PluginUsed {
        ...
        self.observability_bridge
            .emit(
                mull_tool_protocol::session_event::SessionEvent::TurnStarted {
                    turn_number,
                    model_id: model_id.clone(),
                    yolo_mode,
                },
            ...
            .await;
        self.send_before_turn_event(mull_tool_protocol::turn_hook::BeforeTurnPayload {
            turn_number: self.chat_state_handle.get_prompt_index().await as u64,
            model_id: model_id.clone(),
            yolo_mode: self.permissions.is_yolo_mode(),
            conversation_message_count: msg_count,
            session_relationship: mull_tool_protocol::turn_hook::DEFAULT_SESSION_RELATIONSHIP
                .to_string(),
        ...
        mull_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
            session_id: self.session_info.id.0.to_string(),
            turn_number: turn_idx,
        });
        let current_prompt_index = self.chat_state_handle.get_prompt_index().await;
        mull_telemetry::session_ctx::begin_prompt_id();
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = mull_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || mull_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = mull_telemetry::events::PromptSubmitted {
            ...
            mull_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
        }
        ...
        self.tool_bridge_handle()
            .update_resource(mull_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            mull_hooks::event::HookEventName::UserPromptSubmit,
            mull_hooks::event::HookPayload::UserPromptSubmit {
        ...
        self.observability_bridge
            .emit(mull_tool_protocol::session_event::SessionEvent::TurnEnded {
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                ...
                self.send_after_turn_event(mull_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: mull_tool_protocol::turn_hook::TurnHookOutcome::Completed,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                ...
                .await;
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::TurnCompleted {
                    outcome: mull_telemetry::events::Outcome::Completed,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                self.send_after_turn_event(mull_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: mull_tool_protocol::turn_hook::TurnHookOutcome::Cancelled,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
                ...
                .await;
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::TurnCompleted {
                    outcome: mull_telemetry::events::Outcome::Cancelled,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                self.send_after_turn_event(mull_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: mull_tool_protocol::turn_hook::TurnHookOutcome::Error,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
                ...
                mull_telemetry::session_ctx::log_session_event(mull_telemetry::events::ApiError {
                ...
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::TurnCompleted {
                    outcome: mull_telemetry::events::Outcome::Error,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id,
                    cancellation_category: None,
                ...
                self.dispatch_hook(
                    mull_hooks::event::HookEventName::StopFailure,
                    mull_hooks::event::HookPayload::StopFailure {
                        error: format!("{err}"),
                    },
                    Some(prompt_id),
                    None,
        ...
        mull_telemetry::session_ctx::log_session_event(
        ...
        self.dispatch_hook(
            mull_hooks::event::HookEventName::Stop,
            mull_hooks::event::HookPayload::Stop {
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
                    contributor
                        .on_turn_done(&mull_agent_lifecycle::TurnDoneInput)
                        .await;
                }
            }
            Ok(TurnOutcome::Cancelled { .. }) | Ok(TurnOutcome::MaxTurnsReached { .. }) => {
                let input = mull_agent_lifecycle::TurnAbortInput::new(
                    mull_agent_lifecycle::TurnAbortReason::Interrupted,
                );
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
            ...
            Err(err) => {
                ...
                let input = mull_agent_lifecycle::TurnErrorInput { message: &message };
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
    ...
    /// monitor events only via the structured `palmshed.ai/monitor_event` channel.
    pub(crate) async fn inject_pending_monitor_events(&self) {
        let Some(buffer) = &self.tool_context.monitor_event_buffer else {
            return;
        };
        let mine = mull_tools::implementations::mull_build::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = mull_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = mull_tools::reminders::wrap_reminder(&body);
    ...
    pub(super) async fn process_conversation_turn_with_recovery(
        ...
        loop {
            ...
            if attempt > recovery.max_retries {
                ...
                self.send_mull_notification(MullSessionUpdate::AutoRecoveryExhausted {
                    attempts: attempt,
                    error: error_desc,
                })
                .await;
            ...
            self.send_mull_notification(MullSessionUpdate::AutoRecoveryStarted {
                attempt,
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use mull_tools::types::memory_backend::MemoryBackend as _;
        ...
        tracing::info!(
            target : mull_telemetry::memory_log::TARGET, configured_min_score,
            "MEMORY_INJECT_SEARCH: results={result_count}"
        );
        mull_telemetry::session_ctx::log_event(mull_telemetry::memory_telemetry::MemoryInjection {
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<mull_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            request.x_mull_session_id = Some(self.session_info.id.to_string());
            request.x_mull_turn_idx =
                Some(self.chat_state_handle.get_prompt_index().await.to_string());
            request.x_mull_agent_id = Some(mull_telemetry::id::agent_id());
            if request.x_mull_deployment_id.is_none() {
                request.x_mull_deployment_id = crate::managed_config::resolve_deployment_id(
            ...
            self.observability_bridge
                .emit(
                    mull_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: mull_tool_protocol::session_event::SessionPhase::Sampling,
                    },
                )
                .await;
            mull_telemetry::unified_log::info(
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                mull_telemetry::session_ctx::log_event(
                    mull_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            let turn_refused = stop_reason == Some(mull_sampling_types::StopReason::ContentFilter);
            ...
            for item in response.items {
                match item {
                    mull_sampling_types::ConversationItem::Assistant(_) => {
            ...
            self.observability_bridge
                .emit(
                    mull_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: mull_tool_protocol::session_event::SessionPhase::ToolExecution,
                    },
                )
                .await;
...
mod user_echo_broadcast_tests {
    ...
    /// the text from the `palmshed.ai/session/interjection` broadcast, so a live
