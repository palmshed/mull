...
impl SessionActor {
    ...
    pub(super) async fn get_rewind_points(&self) -> RewindPointsResponse {
        ...
        let file_meta_map: std::collections::HashMap<
            usize,
            &mull_workspace::session::file_state::RewindPointMeta,
    ...
    pub(super) async fn handle_rewind(
        ...
        let mut files_to_revert: std::collections::HashMap<
            mull_workspace::session::file_state::FlexiblePath,
            Option<String>,
        ...
        if wants_conversation_rewind {
            ...
            self.persist_mull_update_only(MullSessionUpdate::RewindMarker {
                target_prompt_index: target_index,
    ...
    /// Out-of-band history repair (`palmshed.ai/session/repair`) for a resident
    /// session: run `mull_chat_state::compaction_utils::repair_history` inside
    ...
    pub(super) async fn handle_repair_history(
        &self,
        dry_run: bool,
    ) -> anyhow::Result<mull_chat_state::compaction_utils::HistoryRepairReport> {
        ...
        if turn_flag.load(std::sync::atomic::Ordering::SeqCst) {
            anyhow::bail!(mull_chat_state::commands::RepairHistoryBlocked);
        }
