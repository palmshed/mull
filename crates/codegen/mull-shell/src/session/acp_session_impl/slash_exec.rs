...
impl SessionActor {
    ...
    pub(super) async fn execute_builtin_slash_command(
        ...
        mull_telemetry::session_ctx::log_event(mull_telemetry::events::SlashCommandUsed {
        ...
        match action {
            ...
            BuiltinAction::SetYolo { enabled } => {
                ...
                if let Some(actual) = yolo_toggle_report(was, actual) {
                    ...
                    mull_telemetry::session_ctx::log_event(mull_telemetry::events::YoloToggled {
                        enabled: actual,
                        previous_state: was,
                        trigger: mull_telemetry::events::YoloTrigger::SlashCommand,
                    });
                    tracing::info_span!(
                ...
                let msg = match Self::do_hooks_trust_project(&self.session_info.cwd) {
                    Ok(root) => {
                        mull_telemetry::session_ctx::log_event(
                            mull_telemetry::events::HookTrusted { success: true },
                        );
                        format!("Trusted: {}.", root.display())
                    }
                    Err(e) => {
                        mull_telemetry::session_ctx::log_event(
                            mull_telemetry::events::HookTrusted { success: false },
                        );
                        e
                    }
                };
            ...
            BuiltinAction::HooksAdd { path } => {
                if path.is_empty() {
                    self.send_slash_command_output(
                        "Usage: /hooks add <path>\nProvide a path to a hook JSON file or directory under ~/.mull/.",
                    )
                    .await;
                } else {
                    // CWE-427: Use shared add_hooks_path() which validates
                    // paths are under ~/.mull/ to prevent hook path injection.
                    match crate::config::add_hooks_path(&path) {
                        Ok(()) => {
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::HookAdded { success: true },
                            );
                        ...
                        Err(e) => {
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::HookAdded { success: false },
                            );
                            self.send_slash_command_output(&format!(
            ...
            BuiltinAction::HooksRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_hooks_path(&path) {
                        Ok(()) => {
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::HookRemoved { success: true },
                            );
                            self.send_slash_command_output(&format!(
                        ...
                        Err(e) => {
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::HookRemoved { success: false },
                            );
                            self.send_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsReload => {
                match &self.plugin_registry_handle {
                    Some(handle) => {
                        ...
                        mull_telemetry::session_ctx::log_event(
                            mull_telemetry::events::PluginReloaded { success: true },
                        );
                        self.send_slash_command_output(&msg).await;
                    }
                    None => {
                        mull_telemetry::session_ctx::log_event(
                            mull_telemetry::events::PluginReloaded { success: false },
                        );
                        self.send_slash_command_output(
            ...
            BuiltinAction::SessionInfo => {
                ...
                let context_pct = mull_token_estimation::usage_percentage(ctx.used, ctx.total);
            ...
            BuiltinAction::PluginsAdd { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::add_plugin_path(&path_str) {
                        Ok(()) => {
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::PluginAdded {
                                    source: mull_telemetry::events::PluginSource::LocalPath,
                                    success: true,
                                },
                            );
            ...
            BuiltinAction::PluginsRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_plugin_path(&path_str) {
                        Ok(()) => {
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::PluginRemoved { success: true },
                            );
                        ...
                        Err(e) => {
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::PluginRemoved { success: false },
                            );
                            self.send_slash_command_output(&format!(
                ...
                if source.is_empty() {
                    ...
                    if !trust {
                        ...
                        let source_desc = match &install_source {
                            mull_agent::plugins::git_install::InstallSource::Git {
                                url, ..
                            } => {
                                format!("remote git repo: {url}")
                            }
                            mull_agent::plugins::git_install::InstallSource::Local {
                                path, ..
                            } => {
                        ...
                        match crate::plugin::install_plugin(&source, cwd) {
                            Ok(outcome) => {
                                ...
                                mull_telemetry::session_ctx::log_event(
                                    mull_telemetry::events::PluginInstalled {
                                        install_kind: kind,
                                        success: true,
                                        trust: true,
                                        error_category: None,
            ...
            BuiltinAction::PluginsUninstall { name, confirm } => {
                if name.is_empty() {
                    ...
                    match crate::plugin::uninstall_plugin(&name, confirm, false) {
                        Ok(outcome) => {
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::PluginUninstalled {
                                    confirmed: true,
                                    success: true,
                                },
                            );
            ...
            BuiltinAction::MemoryBrowse => {
                ...
                self.send_mull_notification(MullSessionUpdate::MemoryFiles { files: file_infos })
                    .await;
                ok_end_turn(0, None)
            }
                        ...
                        if let Err(e) = storage.ensure_initialized() {
                            ...
                            *self.memory.search_counter.borrow_mut() =
                            ...
                            let backend: std::sync::Arc<
                                dyn mull_tools::types::memory_backend::MemoryBackend,
            ...
            BuiltinAction::GoalClear => {
                ...
                self.send_mull_notification(update).await;
