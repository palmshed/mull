...
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(&self, question: &str) -> Result<String, String> {
        ...
        let mut items: Vec<ConversationItem> =
            mull_chat_state::compaction_utils::strip_reasoning_blocks(
        ...
        let request = ConversationRequest {
            ...
            temperature: None,
            x_mull_conv_id: Some(btw_session_id.clone()),
            x_mull_req_id: Some(format!("palmshed-btw-{}", uuid::Uuid::new_v4())),
            x_mull_session_id: Some(parent_session_id.clone()),
    ...
    pub(super) async fn handle_recap(&self, auto: bool) {
        ...
        let x_mull_conv_id = format!("recap-{}", uuid::Uuid::new_v4());
        let x_mull_req_id = format!("palmshed-recap-{}", uuid::Uuid::new_v4());
        ...
        let request = ConversationRequest {
            ...
            model: Some(model.clone()),
            temperature: None,
            x_mull_conv_id: Some(x_mull_conv_id.clone()),
            x_mull_req_id: Some(x_mull_req_id.clone()),
            x_mull_session_id: Some(self.session_info.id.to_string()),
            x_mull_agent_id: Some(mull_telemetry::id::agent_id()),
            ..Default::default()
        };
        ...
        // Applies to manual `/recap` too: spinner-less clients (e.g. Mull
    ...
    fn persist_recap_request_artifact(
        ...
        let artifact = RecapRequestFile {
            ...
            x_mull_req_id: x_mull_req_id.to_owned(),
            x_mull_conv_id: x_mull_conv_id.to_owned(),
            strip_reasoning,
    ...
    /// (`MULL_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// [`prompt_suggest::DEFAULT_SUGGEST_MODEL`] (`mull-0.1`). Every
    ...
    /// `mull-0.1` for OAuth users) the request is **skipped
    ...
    /// a reasoning model's response, and some models (e.g. `mull-build`)
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            temperature: None,
            x_mull_conv_id: Some(format!("promptsuggest-{}", uuid::Uuid::new_v4())),
            x_mull_req_id: Some(format!("palmshed-promptsuggest-{}", uuid::Uuid::new_v4())),
            x_mull_session_id: Some(self.session_info.id.to_string()),
            x_mull_agent_id: Some(mull_telemetry::id::agent_id()),
            ..Default::default()
        };
        ...
        tracing::debug!(
            raw_preview = %mull_tools::util::truncate_str(raw.trim(), 60),
