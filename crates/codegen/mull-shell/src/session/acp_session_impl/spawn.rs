...
fn drop_cli_catchall_allows(
    rules: Vec<mull_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<mull_workspace::permission::types::PermissionRule>,
    Vec<mull_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if mull_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod cli_catchall_drop_tests {
    ...
    use mull_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use mull_workspace::permission::rules::parse_permission_rule;
    use mull_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    credentials: mull_chat_state::Credentials,
    auth_method_id: crate::agent::auth_method::SharedAuthMethodId,
    auth_manager: Option<Arc<AuthManager>>,
    attribution_callback: Option<mull_sampler::SharedAttributionCallback>,
    ...
    compaction_mode: mull_chat_state::CompactionMode,
    ...
    web_search_sampling_config: Option<mull_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: mull_tools::implementations::mull_build::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: mull_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<mull_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<mull_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<mull_workspace::permission::PermissionHandle>,
    api_key_provider: Option<mull_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<mull_hooks::discovery::HookRegistry>>,
    workspace_ops: mull_workspace::WorkspaceOps,
    cli_permission_rules: Vec<mull_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn mull_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        mull_tools::implementations::mull_build::scheduler::types::SchedulerHandle,
    >,
    ...
    mull_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(mull_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            mull_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
            )
            .await;
        let yolo_pin = mull_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("mull: --allow catch-all ignored: {reason}");
            }
        }
        if !cli_permission_rules.is_empty() {
            match &mut permission_config {
                ...
                None => {
                    permission_config =
                        Some(mull_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(mull_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if mull_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    mull_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<dyn mull_workspace::permission::PermissionHookTransport>
                });
        ...
        let (permissions, permission_events_rx) =
            mull_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        mull_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            mull_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("MULL_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "MULL_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = mull_sampling_types::SamplingConfig {
    ...
    let actor_pruning_config = mull_chat_state::PruningConfig {
    ...
    let chat_state_handle = mull_chat_state::ChatStateActor::spawn_with_pruning(
        conversation.clone(),
    ...
    let auto_wake_delivered =
        mull_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        mull_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        mull_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    ...
    let terminal_backend: std::sync::Arc<dyn mull_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn mull_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn mull_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(mull_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(mull_tools::computer::local::LocalFs)
        };
    ...
    let harness_metrics = if telemetry_enabled || mull_telemetry::external::is_active() {
    ...
    let compaction_policy = mull_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        mull_tools::implementations::mull_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn mull_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target : mull_telemetry::memory_log::TARGET, error = % e,
        ...
        }
        {
            ...
            tokio::task::spawn_blocking(move || match gc_storage.gc(gc_max_age) {
                Ok(removed) if removed > 0 => {
                    tracing::info!(
                        target : mull_telemetry::memory_log::TARGET, removed,
        ...
        let params = crate::session::memory::MemoryBackendParams {
            ...
            auth_credentials: auth_manager.as_ref().map(|am| {
                ...
                ) as std::sync::Arc<dyn mull_auth::AuthCredentialProvider>
            }),
        };
        ...
        mull_telemetry::session_ctx::log_event(
            mull_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        mull_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    let mcp_state = {
        ...
        if !acp_mcp_servers.is_empty() {
            ...
            tracing::info!(
                ...
                "Registered in-process SDK MCP servers (palmshed.ai/mcp/sdk_call)"
            );
        }
    ...
    let resolved_task_output =
        mull_tools::reminders::task_completion::resolve_task_output_tool_name(agent.tool_bridge())
            .await;
    let resolved_read =
        mull_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge()).await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        mull_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    let scheduler_handle_for_handle = {
        let toolset = agent.tool_bridge().toolset();
        let res = toolset.resources.lock().await;
        res.get::<mull_tools::implementations::mull_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let feedback_client_type = match client_type {
        ClientType::MullTui => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
        ClientType::MullWeb => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Web,
        ...
        ClientType::MullPager => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
    };
    ...
    let resolved_workspace_root = mull_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = mull_sampler::RetryPolicy {
        max_retries: max_retries.unwrap_or(5),
        rate_limit_retry_threshold: 2,
    };
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<mull_sampler::SamplingEvent>();
    let sampler_handle = mull_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<mull_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<mull_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = mull_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        mull_tools::implementations::mull_build::update_goal::UpdateGoalEnvelope,
    >();
    let obs_bridge = {
        let sid = mull_tool_protocol::SessionId::new(&*session_info.id.0)
            .unwrap_or_else(|_| mull_tool_protocol::SessionId::new("unknown").expect("valid"));
        mull_computer_hub_sdk::ObservabilityBridge::new(None, sid)
    };
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: mull_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match mull_workspace::session::git::discover_git_root(root) {
                mull_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    mull_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => mull_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(mull_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(mull_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            mull_tools::implementations::mull_build::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                tracing::info!(
                    target : mull_telemetry::memory_log::TARGET, files = files.len(),
                ...
                mull_telemetry::session_ctx::log_event(
                    mull_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        use mull_tools::implementations::mull_build::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use mull_tools::implementations::mull_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = mull_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            mull_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        mull_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) =
        tokio::sync::oneshot::channel::<Result<SessionInitResult, mull_agent::AgentBuildError>>();
            ...
            local.block_on(&rt, async move {
                let _trace_span = parent_traceparent.as_ref().map(|tp| {
                    let meta = serde_json::json!({ "traceparent" : tp })
                        ...
                        .cloned()
                        .unwrap_or_default();
                    let span = mull_file_utils::trace_context::span_from_meta_traceparent(&meta);
                    span.entered()
                });
