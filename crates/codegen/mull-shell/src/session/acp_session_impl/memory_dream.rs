...
pub(super) struct MemoryFlushSnapshot {
    counts: mull_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &mull_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                mull_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                mull_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        mull_telemetry::session_ctx::log_event(
            mull_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_inner(
        ...
        self.send_mull_notification(MullSessionUpdate::MemoryDreamCompleted {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            x_mull_req_id: Some(format!("palmshed-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: mull_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        let flush_start = std::time::Instant::now();

        self.send_mull_notification(MullSessionUpdate::MemoryFlushStarted)
            .await;
        ...
        let result = async {
            ...
            mull_telemetry::session_ctx::log_event(
                mull_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                ...
                model: Some(model),
                x_mull_conv_id: Some(session_id.clone()),
                x_mull_req_id: Some(format!("palmshed-flush-{}", uuid::Uuid::new_v4())),
                x_mull_session_id: Some(session_id.clone()),
                x_mull_agent_id: Some(mull_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: mull_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        mull_telemetry::session_ctx::log_event(
            mull_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => mull_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => mull_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => mull_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => mull_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        mull_telemetry::session_ctx::log_event(mull_telemetry::events::MemoryFlushed {
        ...
        self.send_mull_notification(MullSessionUpdate::MemoryFlushCompleted {
            result: outcome,
            path: flush_path,
        })
        .await;
    ...
    pub(super) async fn snapshot_memory_flush_state(&self) -> MemoryFlushSnapshot {
        ...
        let chat_history = crate::sampling::conversation_to_chat_messages(
            mull_chat_state::compaction_utils::prepare_conversation_for_summarization(conversation),
        );
        MemoryFlushSnapshot {
            counts,
            chat_history,
    ...
    /// LLM call using the `mull-build` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    mull_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                mull_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events =
                    mull_sampler::stream_responses(raw, meta, request_id, idle_timeout, doom_loop);
                mull_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = mull_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                mull_sampler::collect_response(events).await
            }
        };
