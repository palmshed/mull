...
use mull_agent_lifecycle::LocalExtensionRegistryBuilder;
use mull_agent_lifecycle::{LocalSessionLifecycleContributor, LocalTurnLifecycleContributor};
use mull_agent_lifecycle::{
...
/// `MULL_IDLE_NOTIFICATION_DELAY_MS` overrides it (used by E2E tests).
fn idle_notification_delay() -> Duration {
    resolve_idle_notification_delay(std::env::var("MULL_IDLE_NOTIFICATION_DELAY_MS").ok())
}
...
/// Covered by the headless E2E via `MULL_IDLE_NOTIFICATION_DELAY_MS`.
...
mod idle_notification_delay_tests {
    ...
    /// Pins the public `MULL_IDLE_NOTIFICATION_DELAY_MS` contract: a valid override is interpreted as milliseconds (the E2E seam depends on this).
    #[test]
