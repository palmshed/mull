...
) -> mull_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> mull_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use mull_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
pub(super) fn notification_hook_for_update(
    ...
    match update {
        MullSessionUpdate::DiffReview { .. } => Some((
            "permission_prompt".into(),
            Some("Diff review requested".into()),
            None,
            Some("info".into()),
        )),
        MullSessionUpdate::AutoRecoveryExhausted { error, .. } => Some((
            "agent_error".into(),
            Some(error.clone()),
            None,
            Some("error".into()),
        )),
        MullSessionUpdate::RetryState(RetryState::Exhausted { reason, .. }) => Some((
            "agent_error".into(),
            Some(reason.clone()),
            None,
            Some("error".into()),
        )),
        MullSessionUpdate::RetryState(RetryState::Failed { message, .. }) => Some((
            "agent_error".into(),
            Some(message.clone()),
            None,
            Some("error".into()),
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> mull_hooks::runner::RunContext<'_> {
        mull_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_annotation(&self, message: &str) {
        self.send_mull_notification(MullSessionUpdate::HookAnnotation {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[mull_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, mull_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
        use crate::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
        use mull_hooks::result::HookRunResult;
        ...
        self.send_mull_notification(MullSessionUpdate::HookExecution {
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: mull_hooks::event::HookEventName,
        payload: mull_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            mull_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx).await;
    ...
    pub(super) async fn emit_hook_executed_telemetry(
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                mull_hooks::result::HookRunResult::Success {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    mull_telemetry::events::HookOutcome::Success,
                ),
                mull_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    mull_telemetry::events::HookOutcome::Error,
                ),
                mull_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            mull_telemetry::session_ctx::log_event(mull_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn hook_execution_does_not_fire_notification_hook() {
        let update = MullSessionUpdate::HookExecution {
    ...
    fn hook_annotation_does_not_fire_notification_hook() {
        let update = MullSessionUpdate::HookAnnotation {
    ...
    fn retry_in_progress_does_not_fire_notification_hook() {
        let update = MullSessionUpdate::RetryState(RetryState::Retrying {
            attempt: 1,
            max_retries: 3,
            reason: "timeout".into(),
        });
    ...
    fn feedback_request_does_not_fire_notification_hook() {
        let update = MullSessionUpdate::FeedbackRequest(FeedbackRequestNotification {
    ...
    fn diff_review_fires_permission_prompt() {
        let update = MullSessionUpdate::DiffReview { content: vec![] };
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = MullSessionUpdate::TaskCompleted {
            task_snapshot: mull_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
