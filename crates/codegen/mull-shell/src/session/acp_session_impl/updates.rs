...
//! its buffered/transient/direct variants, Palmshed-notification handling, and
...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, mull_chat_state::UsageTotals)],
    ...
    /// Producer for the **high-frequency streaming path** with an Palmshed
    /// extension payload. Routes through `event_tx` -> `ReplayBuffer` ->
    /// `emit_buffered` so chunks get merged + debounced + emitted.
    ///
    /// For one-shot Palmshed events (RetryState, ImageCompressed, HookExecution,
    /// AutoCompactCompleted, etc.), use `send_mull_notification` instead.
    ///
    /// The frequency-based split (`send_buffered_mull_update` vs `send_mull_notification`)
    /// mirrors the ACP-side split between `send_update` (high-frequency,
    /// buffered) and `emit_notification_direct` (low-frequency, direct).
    pub(super) async fn send_buffered_mull_update(&self, update: MullSessionUpdate) {
        self.close_rewind_window().await;
    ...
    /// - **Palmshed** (`ToolCallDeltaChunk`) -> inlines a gateway
    ...
    pub(super) async fn emit_buffered(&self, notification: SessionNotification) {
        match notification {
            ...
            SessionNotification::Xai(n) => {
                self.log_outbound_mull_buffered(&n);
                if self
                    .notifications
                    .gateway_enabled
    ...
    /// Tracing log for buffered Palmshed notifications emerging from
    /// emit_buffered. Mirrors `log_outbound_notification` for ACP.
    /// Visible with `RUST_LOG=acp_event=info`.
    fn log_outbound_mull_buffered(&self, notification: &XaiSessionNotification) {
        if !matches!(
            notification.update,
            MullSessionUpdate::ToolCallDeltaChunk { .. }
        ) {
            return;
        }
        tracing::info!(
            target : "acp_event", event = "mull_buffered_notification_sent", session_id =
            % self.session_info.id, "Sending buffered Palmshed session notification"
        );
    }
    ...
    /// Handle Palmshed session notifications - store them in persistence
    ...
    pub(super) async fn handle_mull_session_notification(
        &self,
        ...
        if !matches!(
            ...
            tracing::debug!("storing Palmshed session notification");
        }
        {
        ...
        match &notification.update {
            MullSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    mull_hooks::event::HookEventName::SubagentStart,
                    None,
                    mull_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = mull_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        mull_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
            ...
            MullSessionUpdate::SubagentFinished {
                ...
                let envelope = self.fire_hook(
                    mull_hooks::event::HookEventName::SubagentEnd,
                    None,
                    mull_hooks::event::HookPayload::SubagentStop {
                        subagent_id: subagent_id.clone(),
        ...
        let _ = self
            ...
            .persistence_tx
            .send(PersistenceMsg::Update(
                crate::session::storage::SessionUpdate::Mull(Box::new(notification)),
            ));
    }
    /// Persist an Palmshed extension notification to `updates.jsonl` **without** sending it
    ...
    pub(super) fn persist_mull_update_only(&self, update: MullSessionUpdate) {
        let notification = XaiSessionNotification {
            session_id: self.session_info.id.clone(),
            update,
        ...
        if self
            ...
            .is_err()
        {
            tracing::warn!("Failed to send Palmshed update to persistence channel");
        }
    }
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            mull_hooks::event::HookEventName::Notification,
            None,
            mull_hooks::event::HookPayload::Notification {
    ...
    /// Send an Palmshed extension notification to the client
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_mull_notification(&self, update: MullSessionUpdate) {
        self.send_mull_notification_with_extra_meta(update, None)
            .await;
    }
    /// [`Self::send_mull_notification`] with caller-supplied `_meta` keys merged
    /// into the standard eventId/timestamp meta. Caller keys win on collision.
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_mull_notification_with_extra_meta(
        &self,
...
#[cfg(test)]
mod mull_event_id_stamping_tests {
    use super::support::create_test_actor;
    use super::*;
    fn persisted_mull_event_id(
        prx: &mut tokio::sync::mpsc::UnboundedReceiver<PersistenceMsg>,
    ) -> String {
        loop {
            match prx.try_recv().expect("an Palmshed line must be persisted") {
                PersistenceMsg::Update(crate::session::storage::SessionUpdate::Mull(notif)) => {
                    return notif
                        ...
                        .and_then(|m| m.get("eventId"))
                        .and_then(|v| v.as_str())
                        .expect("persisted Palmshed lines must carry an eventId")
                        .to_string();
                }
                _ => continue,
            }
    ...
    /// `send_mull_notification` (own emission) and
    /// `handle_mull_session_notification` (inbound/forwarded, meta-less) —
    ...
    #[tokio::test]
    async fn actor_persisted_mull_lines_carry_event_id() {
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    tokio::sync::mpsc::unbounded_channel::<mull_acp_lib::AcpClientMessage>();
                let (persistence_tx, mut prx) =
                    tokio::sync::mpsc::unbounded_channel::<PersistenceMsg>();
                let actor = create_test_actor(0, 256_000, 85, gateway_tx, persistence_tx).await;
                actor
                    .send_mull_notification(MullSessionUpdate::HookAnnotation {
                        message: "own emission".into(),
                    })
                    .await;
                let own_id = persisted_mull_event_id(&mut prx);
                assert!(own_id.starts_with("test-actor-"));
                actor
                    .handle_mull_session_notification(XaiSessionNotification {
                        session_id: acp::SessionId::new("test-actor"),
                        update: MullSessionUpdate::HookAnnotation {
                            message: "inbound".into(),
                        },
                        meta: None,
                    })
                    .await;
                let inbound_id = persisted_mull_event_id(&mut prx);
                assert!(inbound_id.starts_with("test-actor-"));
                assert_ne!(own_id, inbound_id);
                actor.persist_mull_update_only(MullSessionUpdate::HookAnnotation {
                    message: "persist-only".into(),
                });
                let persist_only_id = persisted_mull_event_id(&mut prx);
