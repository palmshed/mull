...
impl SessionActor {
    ...
    /// Emit `palmshed.ai/git_head_changed` after an edit/shell command that may have
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            mull_workspace::session::git::get_worktree_info(cwd),
            mull_workspace::session::git::get_branch(cwd),
        );
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            let notification =
                acp::ExtNotification::new("palmshed.ai/git_head_changed", raw.into());
            self.notifications
                .gateway
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<mull_tools::implementations::mull_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) async fn handle_completion(&self, prompt_id: String, result: PromptTurnResult) {
        ...
        // replayed `_palmshed.ai/session/update` rail so a viewer that re-attaches
    ...
    /// persists + forwards via `send_mull_notification`. Retiring
    ...
    pub(super) async fn emit_turn_completed(
        ...
        self.send_mull_notification_with_extra_meta(
    ...
    pub(super) fn classify_install_error(
        err: &mull_agent::plugins::install_registry::InstallError,
    ) -> String {
