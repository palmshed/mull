...
/// `SamplingError::is_auth_error` in mull-sampling-types: 403 is
...
pub(super) fn is_auth_tool_error(err: &mull_tool_runtime::ToolError) -> bool {
...
struct SessionTokenAuthGate {
    ...
    /// BYOK status still refresh against cli-chat-proxy / `*.palmshed.ai` without
...
impl SessionTokenAuthGate {
    ...
    fn new(
        ...
        Self {
            ...
            model_byok,
            endpoint_is_first_party: crate::util::is_first_party_mull_url(base_url),
        }
    }
    fn active(self) -> bool {
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    ...
    tool_name: &str,
    mut call: F,
) -> Result<mull_tools::types::output::ToolRunResult, mull_tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<mull_tools::types::output::ToolRunResult, mull_tool_runtime::ToolError>,
        >,
{
...
impl SessionActor {
    ...
    /// against first-party Palmshed hosts).
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl mull_sampler::HeaderInjector for TraceContextInjector {
            fn inject(&self, headers: &mut reqwest::header::HeaderMap) {
                if let Some(tp) = mull_file_utils::trace_context::current_traceparent()
        ...
        impl mull_sampler::BearerResolver for AuthManagerBearerResolver {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| mull_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
        ...
        SamplingConfig {
            ...
            bearer_resolver: if use_bearer_resolver {
                self.auth_manager
                    .as_ref()
                    .map(|am| -> mull_sampler::SharedBearerResolver {
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<mull_workspace::permission::ClassifierMessage>,
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            mull_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            mull_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            mull_workspace::permission::classifier_output_json_schema(),
                        ),
                        reasoning_effort: classifier_reasoning_effort,
                        x_mull_conv_id: Some(session_id.clone()),
                        x_mull_req_id: Some(format!("palmshed-perm-auto-{}", uuid::Uuid::new_v4())),
                        x_mull_session_id: Some(session_id),
        ...
        let clf =
            mull_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    /// catalog routing (Tier-1 catalog creds / Tier-2 Palmshed-proxy via session token
    /// / `PALMSHED_API_KEY` / deployment key), gathering the session-local auth context
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<mull_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
        ...
        let disable_api_key_auth = self
            .auth_manager
            .as_ref()
            .map(|am| am.mull_com_config().api_key_auth_disabled())
            .unwrap_or(false);
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(mull_sampler::SamplingClient, String)> {
        ...
        let client = mull_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
                tracing::warn!(
                    error = % e,
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<mull_sampler::SamplingClient, acp::Error> {
        self.refresh_token_if_expired().await;
        let mut full_config = self.reconstruct_full_config().await;
        full_config.force_http1 = force_http1;
        let sampling_client =
            mull_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    ...
    /// `mull-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: mull_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use mull_sampler::SamplingErrorKind;
        ...
        let client_version = mull_version::VERSION;
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: run `mull logout` then `mull login` to re-authenticate with OAuth2.\n\n\
        ...
        let error_type = if mull_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    ///    `send_mull_notification(RetryState::Failed)`.
    pub(crate) async fn run_turn_via_sampler(
        ...
        let request_id = mull_sampler::RequestId::random();
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = mull_sampler::SamplingErrorInfo::from(&rich_err);
