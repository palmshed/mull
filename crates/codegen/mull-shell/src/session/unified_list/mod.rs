...
/// Env lane (desktop `MULL_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`MULL_CHAT_MODE`); hard-off in release builds.
...
/// Parse `palmshed.ai/session/list` params and, under process-wide chat mode, force
...
impl ParsedMeta {
    fn parse(meta: Option<&serde_json::Value>) -> Self {
        ...
        let facet_filters = meta
            ...
            .unwrap_or_default();
        let query = meta
            .get("palmshed.ai/query")
            .and_then(|v| v.as_str())
            .map(str::to_owned);
        let limit = meta
            .get("palmshed.ai/limit")
...
pub fn force_kind_chat(req: &mut ListReq) {
    ...
    let mut filters = match meta.remove("palmshed.ai/facetFilters") {
        Some(serde_json::Value::Object(map)) => map,
        _ => serde_json::Map::new(),
    };
    filters.insert(
    ...
    meta.insert(
        "palmshed.ai/facetFilters".to_owned(),
...
pub struct ExtListResponseMeta {
    #[serde(rename = "palmshed.ai/facets")]
    pub facets: FacetSummary,
    #[serde(rename = "palmshed.ai/partial")]
...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            cwd: "/Users/me/mull".into(),
            hostname: Some("devbox".into()),
            source: "local".into(),
            model_id: Some("mull".into()),
            num_messages: 7,
            last_active_at: Some(updated_at.into()),
            branch: Some("main".into()),
            repo_name: Some("mull".into()),
            worktree_label: Some("wt".into()),
            git_root_dir: Some("/Users/me/mull".into()),
            git_remotes: vec!["git@github.com:example/repo.git".into()],
            source_workspace_dir: Some("/Users/me/palmshed-src".into()),
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["_meta"]["palmshed.ai/session"]["kind"], "build");
        assert_eq!(value["gitRootDir"], "/Users/me/mull");
        assert_eq!(value["gitRemotes"][0], "git@github.com:example/repo.git");
        assert_eq!(value["sourceWorkspaceDir"], "/Users/me/palmshed-src");
    ...
    fn facets_carry_kind_and_cwd() {
        ...
        assert!(matches!(r.facets.get(CWD_FACET_KEY),
            Some(FacetValue::One(serde_json::Value::String(c))) if c == "/Users/me/mull"));
    }
    #[test]
    fn bare_session_info_is_minimal_plus_meta() {
        ...
        assert_eq!(value["sessionId"], "s1");
        assert_eq!(value["cwd"], "/Users/me/mull");
        assert_eq!(value["title"], "a summary");
        assert_eq!(value["_meta"]["palmshed.ai/session"]["kind"], "build");
    ...
    fn parsed_meta_reads_facet_filters_query_and_limit() {
        let meta = serde_json::json!(
            { "palmshed.ai/facetFilters" : { "kind" : ["build"], "starred" : true },
            "palmshed.ai/query" : "antelope", "palmshed.ai/limit" : 5, }
        );
    ...
    fn forced_kind_replaces_client_build_filter() {
        let mut req = ListReq {
            meta: Some(serde_json::json!({ "palmshed.ai/facetFilters" : { "kind" : ["build"] }, })),
            ..ListReq::default()
        };
        force_kind_chat(&mut req);
    ...
    fn forced_kind_preserves_other_facets() {
        let mut req = ListReq {
            meta: Some(serde_json::json!(
                { "palmshed.ai/facetFilters" : { "kind" : ["build"], "starred" : [true],
                "workspace" : ["w1"] }, "palmshed.ai/query" : "antelope", "palmshed.ai/limit" : 5,
                }
            )),
            ..ListReq::default()
        };
    ...
    fn mull_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        let am = std::sync::Arc::new(crate::auth::AuthManager::new(
            dir,
            crate::auth::MullComConfig::default(),
        ));
        am.hot_swap(crate::auth::MullAuth {
            auth_mode: crate::auth::AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::mull_oauth2_issuer().to_owned()),
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = mull_test_support::EnvGuard::set(
            "MULL_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
        let home = tempfile::tempdir().expect("tempdir");
        let client = ConversationsClient::new(mull_auth_manager(home.path()));
        let mut req = ListReq {
            meta: Some(serde_json::json!({ "palmshed.ai/facetFilters" : { "kind" : ["build"] }, })),
            ..ListReq::default()
        };
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = mull_test_support::EnvGuard::unset("MULL_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = mull_test_support::EnvGuard::set("MULL_SESSION_LIST_CONVERSATIONS", "1");
            assert!(!conversations_lane_enabled());
        }
        {
            let _off = mull_test_support::EnvGuard::set("MULL_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::MULL_CHAT_MODE_ENV;
        let _chat_off = mull_test_support::EnvGuard::unset(MULL_CHAT_MODE_ENV);
        let _desktop_off = mull_test_support::EnvGuard::unset("MULL_SESSION_LIST_CONVERSATIONS");
        assert!(
            !conversations_lane_active(),
            "no env ⇒ lane off (Build-mode default)"
        );
        {
            let _desktop = mull_test_support::EnvGuard::set("MULL_SESSION_LIST_CONVERSATIONS", "1");
            assert!(!conversations_lane_active());
        }
        {
            let _chat = mull_test_support::EnvGuard::set(MULL_CHAT_MODE_ENV, "1");
            assert!(
                !conversations_lane_active(),
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        let raw = serde_json::json!(
            { "_meta" : { "palmshed.ai/facetFilters" : { "kind" : ["build"], "starred" : [true]
            } }, }
        )
        .to_string();
        {
            let _off = mull_test_support::EnvGuard::unset(MULL_CHAT_MODE_ENV);
        ...
        {
            let _on = mull_test_support::EnvGuard::set(MULL_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
    ...
    /// Wire pin for the cross-crate `palmshed.ai/partial` envelope the pager parses:
    ...
    fn ext_list_response_serializes_partial_reasons() {
        for (reason, wire) in [
            ...
            .expect("serialize");
            assert_eq!(
                value["_meta"]["palmshed.ai/partial"],
        ...
        .expect("serialize");
        assert_eq!(
            healthy["_meta"]["palmshed.ai/partial"],
            serde_json::json!({ "conversations" :
            false })
        );
    }
