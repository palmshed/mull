...
use crate::extensions::notification::{
    SessionNotification as XaiSessionNotification, SessionUpdate as MullSessionUpdate,
};
...
impl GoalNotifySender {
    ...
    /// latch updates that must not run the `send_mull_notification`
    /// rewind-window-close side effect.
    pub(crate) fn send_update(&self, update: MullSessionUpdate) {
    ...
    fn dispatch_update(&self, update: MullSessionUpdate, persist: bool) {
...
/// drop its goal state.
pub(crate) fn build_goal_cleared() -> MullSessionUpdate {
    MullSessionUpdate::GoalUpdated {
        goal_id: String::new(),
...
mod tests {
    ...
    fn planning_field(update: &MullSessionUpdate) -> Option<bool> {
        match update {
            MullSessionUpdate::GoalUpdated { planning, .. } => *planning,
            _ => panic!("expected GoalUpdated"),
        }
    }
    ...
    fn build_goal_updated_verifying_flag_reflects_latch() {
        ...
        let mut o = make_base_orchestration();
        let verifying = |u: &MullSessionUpdate| match u {
            MullSessionUpdate::GoalUpdated {
                verifying_completion,
                ..
            } => *verifying_completion,
