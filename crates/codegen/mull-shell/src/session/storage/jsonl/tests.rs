...
async fn write_compaction_segment_numbers_and_indexes_resume_safely() {
    ...
    let seg = |summary: &str| CompactionSegmentFile {
        ...
        detail: mull_chat_state::CompactionDetail::Verbose,
    ...
    let base = adapter
        .session_dir(&info)
        .join(mull_chat_state::compaction_transcript::COMPACTION_DIR);
...
async fn load_session_without_updates_defers_rewind_points() {
    use mull_workspace::session::file_state::RewindPoint;
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use mull_paths::RelPathBuf;
    use mull_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use mull_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
#[tokio::test]
async fn test_mull_session_update_round_trip() {
    use crate::extensions::notification::{
        DiffContent, SessionNotification as XaiSessionNotification,
        SessionUpdate as MullSessionUpdateType,
    };
    let temp_dir = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let info = create_test_info();
    adapter.init_session(&info, default_model_id()).await.unwrap();
    let mull_notification = XaiSessionNotification {
        session_id: acp::SessionId::new("test-session-123"),
        update: MullSessionUpdateType::DiffReview {
            content: vec![
                DiffContent { diff :
    ...
    adapter
        .append_update(&info, &SessionUpdate::Mull(Box::new(mull_notification.clone())))
        .await
        .unwrap();
    ...
    adapter
        ...
        .unwrap();
    let loaded = adapter.load_session(&info).await.unwrap();
    assert_eq!(loaded.updates.len(), 2, "Should have 2 updates (1 Palmshed + 1 ACP)");
    match &loaded.updates[0] {
        SessionUpdate::Mull(notification) => {
            assert_eq!(notification.session_id.0.as_ref(), "test-session-123");
            match &notification.update {
                MullSessionUpdateType::DiffReview { content } => {
        ...
        _ => panic!("Expected Palmshed update as first item"),
    }
...
async fn test_subagent_notifications_round_trip() {
    ...
    let spawned = XaiSessionNotification {
        ...
        update: MullSessionUpdateType::SubagentSpawned {
    ...
    adapter.append_update(&info, &SessionUpdate::Mull(Box::new(spawned))).await.unwrap();
    let finished = XaiSessionNotification {
        session_id: acp::SessionId::new("parent-session"),
        update: MullSessionUpdateType::SubagentFinished {
            subagent_id: "child-001".to_string(),
            child_session_id: "child-001".to_string(),
    ...
    adapter.append_update(&info, &SessionUpdate::Mull(Box::new(finished))).await.unwrap();
    let loaded = adapter.load_session(&info).await.unwrap();
    assert_eq!(loaded.updates.len(), 2);
    match &loaded.updates[0] {
        SessionUpdate::Mull(notification) => {
            match &notification.update {
                MullSessionUpdateType::SubagentSpawned {
                    subagent_id,
                    child_session_id,
                    description,
                    subagent_type,
    ...
    match &loaded.updates[1] {
        SessionUpdate::Mull(notification) => {
            match &notification.update {
                MullSessionUpdateType::SubagentFinished {
                    subagent_id,
                    status,
                    tool_calls,
                    turns,
    ...
    assert_eq!(spawned_json["method"], "_palmshed.ai/session/update");
    ...
    assert_eq!(finished_json["method"], "_palmshed.ai/session/update");
...
async fn copy_session_data_copies_compaction_segments_when_enabled() {
    ...
    assert!(
        ...
        .join(mull_chat_state::compaction_transcript::COMPACTION_DIR).exists()
    );
}
#[tokio::test]
...
#[tokio::test]
async fn test_copy_session_data_transforms_mull_updates() {
    use crate::extensions::notification::{
        DiffContent, SessionNotification as XaiSessionNotification,
        SessionUpdate as MullSessionUpdateType,
    };
    ...
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let source_info = Info {
        id: acp::SessionId::new("source-mull"),
        cwd: "/source".to_string(),
    };
    adapter.init_session(&source_info, default_model_id()).await.unwrap();
    let mull_notification = XaiSessionNotification {
        session_id: acp::SessionId::new("source-mull"),
        update: MullSessionUpdateType::DiffReview {
            content: vec![
                DiffContent { diff :
    ...
    adapter
        .append_update(&source_info, &SessionUpdate::Mull(Box::new(mull_notification)))
        .await
        .unwrap();
    let target_info = Info {
        id: acp::SessionId::new("fork-source-palmshed-abcd1234"),
    ...
    match &loaded.updates[0] {
        SessionUpdate::Mull(notification) => {
            assert_eq!(notification.session_id.0.as_ref(), "fork-source-palmshed-abcd1234");
        }
        _ => panic!("Expected Palmshed update"),
    }
}
#[tokio::test]
...
async fn test_load_prompts_only_applies_rewind_truncation() {
    use crate::extensions::notification::{
        SessionNotification as MullNotification, SessionUpdate as MullSessionUpdate,
    };
    let temp_dir = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let info = Info {
    ...
    let rewind = MullNotification {
        session_id: info.id.clone(),
        update: MullSessionUpdate::RewindMarker {
    ...
    for update in [
        ...
        SessionUpdate::Mull(Box::new(rewind)),
...
async fn test_copy_session_data_copies_tool_state() {
    ...
    let tool_state_json = serde_json::json!(
        { "state" : { "mull_build.TodoState" : { "todos" : [] } } }
    );
...
fn fork_filter_removes_synthetic_user_messages() {
    use mull_sampling_types::conversation::*;
    let mut items = vec![
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    mull_sandbox::set_configured_profile("workspace");
    let expected = mull_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn fork_filter_keeps_turn_with_reasoning_between_user_and_assistant() {
    ...
    let mut items = vec![
        ...
        ConversationItem::Reasoning(mull_sampling_types::synthesized_reasoning_item("thinking",)),
...
fn fork_filter_keeps_multi_tool_cycle_turn_with_reasoning() {
    ...
    let mut items = vec![
        ConversationItem::system("sys"), ConversationItem::user("q"),
        ConversationItem::Reasoning(mull_sampling_types::synthesized_reasoning_item("plan",)),
        ...
        ConversationItem::Reasoning(mull_sampling_types::synthesized_reasoning_item("reflect",)),
...
fn fork_filter_keeps_multi_tool_turn_with_reasoning_between_results() {
    ...
    let mut items = vec![
        ...
        ConversationItem::Reasoning(mull_sampling_types::synthesized_reasoning_item("mid")),
...
fn scan_session_dirs_finds_all_sessions() {
    ...
    let cwd = crate::util::mull_home::encode_cwd_dirname("/home/user/project");
...
fn scan_session_dirs_filters_by_cwd() {
    let tmp = TempDir::new().unwrap();
    let now = chrono::Utc::now();
    let cwd_a = crate::util::mull_home::encode_cwd_dirname("/home/user/project-a");
    let cwd_b = crate::util::mull_home::encode_cwd_dirname("/home/user/project-b");
...
fn scan_session_dirs_skips_non_directory_entries() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::mull_home::encode_cwd_dirname("/project");
...
async fn list_sessions_recent_returns_most_recent_by_mtime() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::mull_home::encode_cwd_dirname("/workspace");
...
async fn list_sessions_sorts_by_last_active_at_over_updated_at() {
    ...
    let cwd = crate::util::mull_home::encode_cwd_dirname(cwd_path);
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd,
...
async fn list_sessions_recent_sorts_by_updated_at() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::mull_home::encode_cwd_dirname("/ws");
    let now = chrono::Utc::now();
...
async fn list_sessions_recent_spans_multiple_workspaces() {
    let tmp = TempDir::new().unwrap();
    let cwd_a = crate::util::mull_home::encode_cwd_dirname("/project-a");
    let cwd_b = crate::util::mull_home::encode_cwd_dirname("/project-b");
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd_a,
...
/// path end-to-end (loader + serde + mull_sampling_types::
...
/// the legacy mull-build / Opus / chat-completions shape.
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"results...","reasoning":{"text":"the results are about cats","encrypted":"enc-blob","id":"rs_legacy"},"model_id":"mull"}"#,
        ],
    );
    assert_eq!(
        items.len(), 5,
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let mull_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy thinking","encrypted":"enc","id":"rs_legacy"},"model_id":"mull"}"#,
            ...
            r#"{"type":"assistant","content":"a2","model_id":"mull"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
    ...
    assert_eq!(
        legacy_assistant.model_id.as_deref(), Some("mull"),
    ...
    let mull_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
...
fn read_chat_history_is_idempotent_on_post_pr_sessions() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a","model_id":"mull"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
...
fn read_chat_history_skips_merged_line_from_interrupted_append() {
    ...
    let good_2 = r#"{"type":"assistant","content":"after","model_id":"mull"}"#;
    let raw = format!("{good_1}\n{partial}{merged_onto}\n{good_2}\n");
    let temp_dir = TempDir::new().unwrap();
