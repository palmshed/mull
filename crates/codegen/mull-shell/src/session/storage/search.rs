...
//! other concurrently running mull processes (which may wipe or downgrade
...
use super::{
    ContentPeek, PALMSHED_SESSION_UPDATE_METHOD, PromptExtractEvent, RawLinePeek, RawParamsPeek,
...
/// User-configurable overrides via `~/.mull/settings.json` are deferred
/// to a follow-up.
...
///
/// TODO: When multiple mull processes run concurrently, they each have
...
fn search_db_path(root_dir: &Path) -> PathBuf {
    ...
    mull_sqlite_journal::JournalMode::for_db_path(&path).effective_db_path(&path)
}
...
fn collect_all_indexable_content_single_pass(updates_path: &Path) -> io::Result<(String, u64)> {
    ...
    for line_result in reader.lines() {
        ...
        // Control events (rewind markers) come from Palmshed extensions
        // ("_palmshed.ai/session/update"). Dispatch on source first, then tag.
        if !is_mull {
            // ── ACP content events ──────────────────────────────────
            match tag {
...
fn collect_delta_content(updates_path: &Path, offset: u64) -> io::Result<DeltaResult> {
    ...
    for line_result in reader.lines() {
        ...
        match tag {
            Some(t) if is_mull && t == *REWIND_MARKER => {
                return Ok(DeltaResult::NeedsFullReread);
            }
            Some(t) if !is_mull && t == *USER_MESSAGE_CHUNK => {
                flush_assistant(&mut current_assistant, &mut assistant_texts);
                if let Ok(peek) = serde_json::from_str::<UserContentPeek<'_>>(raw_params)
                    && let Some(content) = peek.update.content
                    && content.content_type == Some("text")
            ...
            Some("agent_message_chunk") if !is_mull => {
...
mod tests {
    ...
    fn mull_update(session_update_json: &str) -> String {
        format!(
