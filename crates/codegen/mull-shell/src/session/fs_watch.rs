//! Session-level fs-watch policy over [`mull_fsnotify`].
//!
//! Mechanism (OS watch, coalesce, refcount) lives in `mull_fsnotify`. This module
...
use mull_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
use tokio::sync::mpsc;
...
/// Dedup key for `palmshed.ai/git_head_changed`, shared by the watcher's `GitHead`
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> mull_workspace::file_system::FileIndexDelta {
    use mull_workspace::file_system::FileIndexDelta;
...
pub(crate) struct CapabilityInputs {
    ...
    pub hunk_tracking: bool,
    pub code_nav: bool,
    /// `palmshed.ai/gitHeadChanged`; opt-in (absent => off).
...
impl ClientNotify {
    async fn on_change(&self, paths: &[PathBuf], kind: FsEventKind) {
        ...
        match self.mode {
            ClientFsMode::Events => {
                // Present-tense strings are the `palmshed.ai/fs_notify` wire protocol;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = mull_workspace::session::git::get_branch(&self.cwd).await;
        let commit = mull_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = mull_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = mull_workspace::session::git::GitHeadChanged {
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: mull_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root = mull_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || mull_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = mull_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(mull_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: mull_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {
