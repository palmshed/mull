...
pub enum SessionCommand {
    ...
    SetSessionModel {
        sampling_config: mull_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// the `model` field sent in the `x-mull-model-override` header and merges
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<mull_chat_state::ModelMetadata>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<mull_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.mull/hooks`,
    ...
    /// Out-of-band history repair (`palmshed.ai/session/repair`): fix tool-pairing
    ...
    RepairHistory {
        dry_run: bool,
        respond_to:
            oneshot::Sender<anyhow::Result<mull_chat_state::compaction_utils::HistoryRepairReport>>,
    },
    GetRewindPoints {
    ...
    /// Palmshed extension session notification - client-side events to store in persistence
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, mull_chat_state::UsageTotals)>,
    ...
    /// mid-session via the `palmshed.ai/session/update_mcp_servers` extension method).
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<mull_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<mull_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<mull_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<mull_hooks_plugins_types::HooksListResponse>,
    },
    /// Execute a hooks management action from the pager modal.
    HooksAction {
        action: mull_hooks_plugins_types::HooksAction,
        respond_to: oneshot::Sender<mull_hooks_plugins_types::ActionOutcome>,
    },
    ...
    /// This session's plugin registry, as served by `palmshed.ai/plugins/list`.
    PluginsList {
        respond_to: oneshot::Sender<Option<std::sync::Arc<mull_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// from the notification bridge, which does not go through `send_mull_notification`).
    ...
    /// serialized mailbox; the rebroadcast of `palmshed.ai/queue/changed` is the
    ...
    /// `palmshed.ai/queue/changed` is the truth signal for every attached client.
    ...
    /// tool-free model call (default `mull-0.1` when available via
    ...
    /// `mull-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    Interject {
        ...
        /// `palmshed.ai/session/interjection` so the originating pager can dedup its
    ...
    TakeTurnMessages {
        respond_to: oneshot::Sender<Option<mull_chat_state::TurnCapture>>,
    },
    ...
    TakeHarnessTraceTurns {
        respond_to: oneshot::Sender<Vec<Vec<mull_sampling_types::conversation::ConversationItem>>>,
    },
