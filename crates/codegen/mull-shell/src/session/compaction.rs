...
use mull_chat_state::compaction_utils::{
...
use mull_sampling_types::{ApiBackend, ConversationItem};
use std::sync::Arc;
...
/// Override with `MULL_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("MULL_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    use mull_sampling_types::ConversationItem;
    #[test]
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("MULL_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        mull_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("MULL_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG MULL_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(mull_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
/// (`mull-chat-state` `replace_conversation`, the authority): it matches the reseed's
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                mull_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
        ...
        self.send_mull_notification(MullSessionUpdate::AutoCompactCompleted {
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            mull_telemetry::session_ctx::log_event(mull_telemetry::events::AutoCompactSuppressed {
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    mull_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    mull_chat_state::estimate_conversation_tokens(&full_conv),
                );
                if mull_token_estimation::exceeds_threshold(
                    projected_preserved,
                    context_window,
                    self.compaction.threshold_percent.get(),
                ) {
    ...
    async fn run_compact_inner(
        ...
        trigger: mull_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        ...
        let trigger_str = match trigger {
            mull_telemetry::events::CompactionTrigger::Manual => "manual",
            mull_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = mull_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            mull_hooks::event::HookEventName::PreCompact,
            mull_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            mull_chat_state::compaction_utils::prepare_conversation_for_segment(
        ...
        let simplified_messages = if verbatim_input_enabled {
            mull_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
                summary_strips_reasoning,
            )
        } else {
            mull_chat_state::compaction_utils::prepare_conversation_for_summarization(
                full_conversation,
            )
        };
        if conv_len == 0 {
        ...
        let effective_tool_defs: Vec<mull_sampling_types::ToolDefinition> = self
        ...
        let compaction_tool_tokens =
            mull_chat_state::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<mull_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(mull_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<mull_sampling_types::HostedTool> = if use_backend_search {
        ...
        let estimated_input_tokens =
            mull_chat_state::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, mull_telemetry::events::CompactionTrigger::Auto);
        ...
        let fr_config = mull_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match mull_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(mull_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(mull_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(mull_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            mull_telemetry::session_ctx::log_event(
                                mull_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = mull_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    mull_chat_state::compaction_utils::fit_conversation_to_budget(
                                        verbatim, budget,
                                    )
                                }
                                InputStage::Lossy => {
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                mull_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                mull_tools::computer::types::TaskKind::Bash => {
                        ...
                        self.tool_context.subagent_event_tx
                    {
                        ...
                        use mull_tools::implementations::mull_build::task::types::{
                    ...
                    let todos = {
                        ...
                        use mull_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn mull_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn mull_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : mull_telemetry::memory_log::TARGET, count = n,
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        mull_sampling_types::ContentPart::Text { text } => {
        ...
        self.dispatch_hook(
            mull_hooks::event::HookEventName::PostCompact,
            mull_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) fn should_auto_compact(
        ...
        if mull_token_estimation::exceeds_threshold(
            ...
            let percentage = mull_token_estimation::usage_percentage_u8(total_tokens, cw);
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &mull_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = mull_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
    ...
    pub(crate) async fn run_compact_only(
        ...
        mull_telemetry::session_ctx::log_event(mull_telemetry::events::AutoCompactFired {
            tokens_before: trigger_info.tokens_used,
            percentage: trigger_info.percentage,
        });
        self.signals_handle()
            .record_compaction(trigger_info.tokens_used);
        self.send_mull_notification(MullSessionUpdate::AutoCompactStarted {
        ...
        let result = self
            .run_compact_inner(None, None, mull_telemetry::events::CompactionTrigger::Auto)
            .await;
        ...
        match result {
            ...
            Err(e) => {
                ...
                if self
                    ...
                    == SUPPRESS_NONE
                {
                    self.send_mull_notification(MullSessionUpdate::AutoCompactFailed {
                        error: String::new(),
                    })
                    .await;
                }
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<mull_sampling_types::ToolSpec>,
        user_context: Option<&str>,
    ...
    fn persist_compaction_checkpoint(
        ...
        use crate::extensions::notification::{
            CompactionCheckpointFile, CompactionCheckpointInfo, SessionUpdate as MullSessionUpdate,
        };
        ...
        self.persist_mull_update_only(MullSessionUpdate::CompactionCheckpoint(Box::new(info)));
        tracing::info!(
            prompt_index_at_compaction,
...
mod inline_auto_compact_flow_tests {
    ...
    use mull_workspace::file_system::MockFs;
    use mull_workspace::permission::PermissionHandle;
    ...
    async fn create_test_actor(
        ...
        gateway_tx: mpsc::UnboundedSender<mull_acp_lib::AcpClientMessage>,
        ...
        let hunk_tracker_handle = mull_hunk_tracker::HunkTrackerActor::spawn(
            ...
            cwd.to_path_buf(),
            hunk_tx,
            mull_hunk_tracker::TrackingMode::AgentOnly,
        ...
        let chat_state_handle = mull_chat_state::ChatStateActor::spawn(
            vec![],
            mull_sampling_types::SamplingConfig {
            ...
            Box::new(mull_chat_state::NullChatPersistence),
            chat_event_tx,
            tokio_util::sync::CancellationToken::new(),
        );
        ...
        SessionActor {
            ...
            compaction: crate::session::compaction_config::CompactionConfig {
                ...
                compaction_mode: mull_chat_state::CompactionMode::Transcript,
                verbatim_input: true,
            ...
            extension_registry: mull_agent_lifecycle::LocalExtensionRegistry::default(),
            ...
            vcs_kind: mull_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: mull_sampler::SamplerHandle::noop(),
        ...
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    mpsc::unbounded_channel::<mull_acp_lib::AcpClientMessage>();
                ...
                async fn notification_for(reason: SuppressReason) -> String {
                    ...
                    while let Ok(msg) = persistence_rx.try_recv() {
                        if let PersistenceMsg::Update(
                            crate::session::storage::SessionUpdate::Mull(notif),
                        ) = msg
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use mull_test_support::MockInferenceServer;
        use std::sync::atomic::Ordering::Relaxed;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
    ...
    async fn forked_release_still_over_threshold_suppresses_auto() {
        ...
        local
            .run_until(async {
                ...
                while let Ok(msg) = persistence_rx.try_recv() {
                    if let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Mull(
                        notif,
                    )) = msg
                        && matches!(
                            &notif.update,
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("API error (status 402 Payment Required): Mull usage balance exhausted"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
            classify("Mull usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> mull_sampler::SamplingErrorInfo {
        mull_sampler::SamplingErrorInfo {
            kind: mull_sampler::SamplingErrorKind::Api,
    ...
    async fn test_compact_on_error_triggers_when_tokens_exceed_new_window() {
        ...
        local
            .run_until(async {
                let (gateway_tx, _) = mpsc::unbounded_channel::<mull_acp_lib::AcpClientMessage>();
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = mull_sampler::SamplingErrorInfo {
                    kind: mull_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
    ...
    async fn get_transcript_path_returns_some_when_file_exists() {
        ...
        local
            .run_until(async {
                ...
                let mut actor =
                    create_test_actor(50_000, 200_000, 85, gateway_tx, persistence_tx).await;
                actor.compaction.compaction_mode = mull_chat_state::CompactionMode::Transcript;
                ...
                actor.compaction.compaction_mode = mull_chat_state::CompactionMode::Summary;
