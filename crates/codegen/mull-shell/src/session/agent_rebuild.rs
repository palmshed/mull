...
//! [`mull_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`mull_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`mull_agent::Agent`] owns an [`mull_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`mull_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use mull_agent::config::AgentDefinition;
use mull_agent::error::AgentBuildError;
use mull_agent::prompt::context::PromptAudience;
use mull_agent::prompt::skills::SkillsConfig;
use mull_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use mull_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use mull_tools::implementations::lsp::LspBackend;
use mull_tools::implementations::mull_build::ask_user_question::types::UserQuestionRequest;
...
use mull_tools::types::SharedApiKeyProvider;
use mull_tools::types::compat::CompatConfig;
use mull_tools::types::memory_backend::MemoryBackend;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<mull_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client: Option<mull_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use mull_tools::implementations::mull_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(mull_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(mull_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use mull_tools::implementations::mull_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            mull_tools::computer::local::LocalTerminalBackend::new_local(
                mull_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: mull_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(mull_tools::types::tool::ToolKind::Task)
            .expect("MullBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("MullBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_mull_build())
                    .await
