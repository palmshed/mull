...
impl TokenUsageCategory {
    ...
    pub fn skills_listing(text: &str, skill_count: usize) -> Self {
        Self {
            ...
            tokens: mull_token_estimation::estimate_tokens(text),
...
pub struct ContextInfo {
    ...
    /// always matches the actual trigger (e.g. 65 for mull-build in remote settings).
...
impl ContextInfo {
    ...
    pub fn from_notification(used: u64, total: u64) -> Self {
        Self {
            used,
            total,
            usage_pct: mull_token_estimation::usage_percentage_u8(used, total),
            free_tokens: mull_token_estimation::free_tokens(total, used),
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `mull-build`).
...
pub fn is_coding_model_slug(model: &str) -> bool {
    // OpenRouter migration: accept openrouter/* slugs as the primary coding models.
    model.starts_with("openrouter/") || model == "mull"
}
...
/// Full wire response for `palmshed.ai/session/info`.
///
...
mod tests {
    ...
    fn should_show_model_fingerprint_truth_table() {
        ...
        assert!(should_show_model_fingerprint(false, "mull"));
