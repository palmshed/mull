...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> mull_computer_hub_sdk::ObservabilityBridge {
    mull_computer_hub_sdk::ObservabilityBridge::new(
        None,
        mull_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> mull_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> mull_agent::Agent {
    use mull_tools::implementations::mull_build::update_goal::UpdateGoalTool;
    use mull_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// Mull-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_mull_build_agent_with_todo() -> mull_agent::Agent {
    use mull_tools::implementations::mull_build::todo::TodoWriteTool;
    use mull_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> mull_agent::Agent {
    use mull_tools::implementations::mull_build::enter_plan_mode::EnterPlanModeTool;
    use mull_tools::implementations::mull_build::exit_plan_mode::ExitPlanModeTool;
    use mull_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<mull_tools::registry::types::ToolConfig>,
) -> mull_agent::Agent {
    test_agent_from_config(
        mull_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        mull_agent::AgentDefinition::default_mull_build(),
        std::sync::Arc::new(mull_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: mull_agent::AgentDefinition,
    backend: std::sync::Arc<dyn mull_tools::computer::types::TerminalBackend>,
) -> mull_agent::Agent {
    use mull_tools::computer::local::LocalFs;
    use mull_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("mull-test"),
    ...
    mull_agent::Agent::new(
        definition,
        mull_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        mull_agent::ReminderPolicy::default(),
        mull_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<mull_acp_lib::AcpClientMessage>,
    persistence_tx: tokio::sync::mpsc::UnboundedSender<PersistenceMsg>,
) -> (
    SessionActor,
    tokio::sync::mpsc::UnboundedReceiver<SessionEvent>,
) {
    let cwd = mull_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(mull_workspace::file_system::MockFs::new(cwd.to_path_buf()));
    ...
    let actor = SessionActor {
        ...
        permissions: mull_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
