...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(mull_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            ...
            agent
                .tool_bridge()
                .update_resource(
                    mull_tools::implementations::mull_build::task::types::CurrentPromptIdResource(
                        "running".to_string(),
                    ),
                )
                .await;
...
/// user's next queued prompt) and rebroadcasts `palmshed.ai/queue/changed`. The
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![mull_sampling_types::ToolCall {
                    id: "call-1".into(),
    ...
    local
        .run_until(async {
            ...
            let cfg = mull_sampler::SamplerConfig {
                ...
                temperature: None,
                top_p: None,
                api_backend: mull_sampler::ApiBackend::Responses,
                auth_scheme: Default::default(),
                extra_headers: Default::default(),
                context_window: 100_000,
                client_version: None,
            ...
            let sampler_handle = mull_sampler::SamplerActor::spawn(
                cfg,
                mull_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) =
                tokio::sync::mpsc::unbounded_channel::<mull_acp_lib::AcpClientMessage>();
            ...
            let request = ConversationRequest {
                items: vec![ConversationItem::User(mull_sampling_types::UserItem {
                    content: vec![mull_sampling_types::ContentPart::Text { text: "hi".into() }],
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use mull_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(
                    ...
                    matches!(p, mull_sampling_types::ContentPart::Text { text } if
