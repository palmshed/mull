...
use super::support::*;
use super::*;
/// Build an actor whose toolset parses mull `search_replace` plus the plan
...
async fn build_gate_actor() -> SessionActor {
    ...
    *actor.agent.borrow_mut() = test_agent_with_tools(vec![
        ToolConfig::from_id("MullBuild:read_file"),
        ToolConfig::from_id("MullBuild:search_replace"),
...
async fn tool_result_text(actor: &SessionActor, call_id: &str) -> String {
    ...
    conv.iter()
        ...
        .find_map(|item| match item {
            mull_sampling_types::ConversationItem::ToolResult(tr) if tr.tool_call_id == call_id => {
...
/// worst case) still rejects a mull edit outside the plan file, without ever
/// reaching the permission layer, and steers the model to `exit_plan_mode`.
#[tokio::test(flavor = "current_thread")]
async fn plan_mode_rejects_mull_edit_outside_plan_file_despite_allow_all_permissions() {
