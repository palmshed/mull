...
//! notification the real `send_mull_notification` persists — not a
...
fn is_turn_completed(m: &PersistenceMsg) -> bool {
    matches!(
        m,
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Mull(n))
            if matches!(n.update, MullSessionUpdate::TurnCompleted { .. })
    )
}
            ...
            *actor
            ...
            while let Ok(msg) = gateway_rx.try_recv() {
                if let mull_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                    && args.request.method.as_ref() == "palmshed.ai/session_notification"
                    && let Ok(v) =
