...
mod interjection_broadcast_tests {
    ...
    /// `handle_interjection` depends on: method `palmshed.ai/session/interjection`
            ...
            .run_until(async {
                ...
                while let Ok(msg) = gateway_rx.try_recv() {
                    if let mull_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                        && args.request.method.as_ref() == "palmshed.ai/session/interjection"
                    {
                        payload =
                ...
                let payload = payload.expect("an palmshed.ai/session/interjection broadcast");
                assert_eq!(
