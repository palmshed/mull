    ...
    local
        .run_until(async {
            ...
            *actor.goal_update_rx.borrow_mut() = Some(rx);
            tx.send(
                mull_tools::implementations::mull_build::update_goal::envelope_for_test(
                    mull_tools::implementations::mull_build::update_goal::UpdateGoalInput {
                        completed: Some(true),
                        message: None,
                        blocked_reason: None,
                    },
...
fn spawn_gateway_notification_capture(
    mut gateway_rx: tokio::sync::mpsc::UnboundedReceiver<mull_acp_lib::AcpClientMessage>,
    ...
    local
        .run_until(async {
            ...
            let cancelled: PromptTurnResult = Ok(crate::session::commands::PromptTurnOk {
                ...
                completion_kind: crate::session::commands::PromptCompletionKind::Cancelled {
                    category: Some(
                        mull_file_utils::events::types::CancellationCategory::MidTurnAbort,
                    ),
                    context: None,
                },
                structured_output: None,
...
async fn drain_goal_updates_completes_after_blocked_does_not_leak_pause_message() {
    ...
    local
            .run_until(async {
                ...
                let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
                *actor.goal_update_rx.borrow_mut() = Some(rx);
                tx.send(mull_tools::implementations::mull_build::update_goal::envelope_for_test(mull_tools::implementations::mull_build::update_goal::UpdateGoalInput {
                        completed: None,
                        message: None,
...
async fn drain_goal_updates_harness_disabled_does_not_drop_ack() {
    use mull_tools::implementations::mull_build::update_goal::{
...
fn make_completed_cmd() -> mull_tools::implementations::mull_build::update_goal::UpdateGoalEnvelope
{
    let input = mull_tools::implementations::mull_build::update_goal::UpdateGoalInput {
        completed: Some(true),
        message: None,
        blocked_reason: None,
    };
...
fn spawn_notif_with_model(
    ...
    if let MullSessionUpdate::SubagentSpawned { model: m, .. } = &mut notif.update {
...
fn spawn_notif(subagent_id: &str, resumed_from: Option<&str>) -> XaiSessionNotification {
    XaiSessionNotification {
        ...
        update: MullSessionUpdate::SubagentSpawned {
...
fn finish_notif(subagent_id: &str, tokens_used: u64) -> XaiSessionNotification {
    XaiSessionNotification {
        session_id: acp::SessionId::new("test-actor"),
        update: MullSessionUpdate::SubagentFinished {
            subagent_id: subagent_id.into(),
            child_session_id: subagent_id.into(),
...
async fn goal_tokens_used_cross_session_resume_anchors_at_zero() {
    ...
    local
        .run_until(async {
            ...
            actor
                .handle_mull_session_notification(spawn_notif("x", Some("from-prior-session")))
                .await;
            let anchor = actor
...
async fn goal_tokens_used_monotonic_under_out_of_order_events() {
    ...
    local
        .run_until(async {
            let actor = make_test_actor_with_active_goal().await;
            actor
                .handle_mull_session_notification(spawn_notif("a", None))
                .await;
            actor
                .handle_mull_session_notification(finish_notif("a", 50_000))
                .await;
            // Stale event with smaller cumulative arrives second.
            actor
                .handle_mull_session_notification(finish_notif("a", 40_000))
                .await;
            assert_eq!(
                actor
                    .subagent_token_records
    ...
    local
        .run_until(async {
            ...
            actor
                ...
                .await;
            actor
                .handle_mull_session_notification(spawn_notif("a1", Some("a")))
                .await;
            let rec = actor
                .subagent_token_records
                .lock()
            ...
            actor
                .handle_mull_session_notification(finish_notif("a1", 80_000))
                .await;
...
async fn subagent_spawn_captures_effective_model_id() {
    ...
    local
        .run_until(async {
            let actor = make_test_actor_with_active_goal().await;
            actor
                .handle_mull_session_notification(spawn_notif_with_model(
                    "a",
                    None,
                    Some("grok-4.5"),
                ))
...
async fn subagent_spawn_absent_model_captured_as_none() {
    ...
    local
        .run_until(async {
            let actor = make_test_actor_with_active_goal().await;
            actor
                .handle_mull_session_notification(spawn_notif_with_model("a", None, None))
                .await;
            let model = actor
                .subagent_token_records
                .lock()
...
fn progress_notif(subagent_id: &str, tokens_used: u64) -> XaiSessionNotification {
    XaiSessionNotification {
        session_id: acp::SessionId::new("test-actor"),
        update: MullSessionUpdate::SubagentProgress {
            subagent_id: subagent_id.into(),
            parent_session_id: "test-actor".into(),
            child_session_id: subagent_id.into(),
            duration_ms: 2_000,
...
async fn subagent_progress_advances_goal_tokens_live_without_double_count() {
    ...
    local
        .run_until(async {
            ...
            let count_gateway_goal_updated =
                |rx: &mut tokio::sync::mpsc::UnboundedReceiver<mull_acp_lib::AcpClientMessage>| {
                    ...
                    while let Ok(msg) = rx.try_recv() {
                        ...
                        if args.request.method.as_ref() != "palmshed.ai/session_notification" {
                            continue;
                        }
                        let Ok(v) =
            ...
            actor
                .handle_mull_session_notification(progress_notif("a", 30_000))
                .await;
            assert_eq!(
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Mull(n)) = msg
                {
                    assert!(
                        !matches!(n.update, MullSessionUpdate::SubagentProgress { .. }),
...
async fn subagent_progress_live_tokens_monotonic_across_child_compaction() {
    ...
    local
        .run_until(async {
            ...
            actor
                .handle_mull_session_notification(progress_notif("a", 100_000))
                .await;
            assert_eq!(
                actor
                    .goal_tracker
            ...
            actor
                .handle_mull_session_notification(progress_notif("a", 60_000))
                .await;
            assert_eq!(
                actor
                    .goal_tracker
...
async fn subagent_progress_edge_ticks_cannot_corrupt_token_records() {
    ...
    local
        .run_until(async {
            ...
            actor
                .handle_mull_session_notification(progress_notif("a", 10_000))
                .await;
            assert_eq!(actor.goal_tokens_used(0), 30_000, "ratchet must hold");
            // Unknown subagent id: dropped, nothing recorded.
            actor
                .handle_mull_session_notification(progress_notif("ghost", 99_000))
                .await;
            assert_eq!(actor.goal_tokens_used(0), 30_000);
            ...
            actor
                .handle_mull_session_notification(progress_notif("foreign", 70_000))
                .await;
            assert_eq!(
                actor
                    .subagent_token_records
            ...
            actor
                .handle_mull_session_notification(finish_notif("a", 35_000))
                .await;
            actor
                .handle_mull_session_notification(progress_notif("a", 80_000))
                .await;
            assert_eq!(
                actor.goal_tokens_used(0),
                35_000,
