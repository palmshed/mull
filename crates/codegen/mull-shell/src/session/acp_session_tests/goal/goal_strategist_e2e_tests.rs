...
//!
//! Tests mutate `MULL_GOAL_CLASSIFIER` so they carry `serial`.
...
use mull_tools::implementations::mull_build::update_goal::UpdateGoalInput;
...
async fn answer_strategist(
    ...
    req: Box<mull_tools::implementations::mull_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
async fn make_actor(
    ...
    actor.tool_context.cwd =
        mull_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
