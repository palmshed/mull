        ...
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    ...
                    assert!(
                        artifact.x_mull_req_id.starts_with("palmshed-recap-"),
                        "req id: {}",
                        artifact.x_mull_req_id
                    );
...
async fn manual_recap_over_budget_trims_persisted_request_and_is_display_only() {
    ...
    local
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    let est = mull_chat_state::estimate_conversation_tokens(&artifact.chat_history);
                    assert!(
                        est <= PROMPT_BUDGET,
...
/// shape, so it also covers the laxer mull ChatCompletions/Responses shapes.)
#[test]
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    use crate::session::helpers::session_recap;
    use mull_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use mull_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    // mull backend => strip_reasoning=false; the over-budget branch strips anyway.
    let items = session_recap::budget_recap_items(conv, "system-reminder", false, 8_000);
    let req = ConversationRequest::from_items(items);
    let msg = mull_sampling_types::build_messages_request(&req);
