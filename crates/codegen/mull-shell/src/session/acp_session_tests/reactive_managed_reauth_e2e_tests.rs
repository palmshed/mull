...
//! `palmshed.ai/mcp/server_status` pushes that clients consuming only `server_status`
...
//! handshake against rmcp 2.1 in `mull-mcp/tests/repro_sse_flood.rs`): a
...
use chrono::Utc;
use mull_mcp::servers::{ClientStateKind, HttpConfig, McpClient};
use std::collections::HashMap;
use std::sync::Arc;
use std::sync::atomic::{AtomicBool, AtomicUsize, Ordering};

const MANAGED: &str = "mull_com_testconnector";
...
async fn actor_with_proxy(
    proxy_base: &str,
    gw_tx: tokio::sync::mpsc::UnboundedSender<mull_acp_lib::AcpClientMessage>,
...
/// Drain all `palmshed.ai/mcp/server_status` pushes currently queued on `gw_rx`.
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use mull_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

    // Managed-prefix gate: only `mull_com_*` tools enter entry-B.
