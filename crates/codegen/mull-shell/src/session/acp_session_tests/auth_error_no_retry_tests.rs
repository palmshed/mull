...
fn auth_manager_with_refresher(
    ...
    let am = Arc::new(AuthManager::new(dir.path(), MullComConfig::default()));
    am.hot_swap(MullAuth {
...
/// attribution.
fn auth_error() -> mull_sampler::SamplingErrorInfo {
    mull_sampler::SamplingErrorInfo {
        kind: mull_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: mull_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        mull_chat_state::AuthType::SessionToken => "cached_token",
        mull_chat_state::AuthType::ApiKey => "api_key",
    };
...
/// `ApiKey` (session-token cache miss + `PALMSHED_API_KEY`).
async fn make_actor_with_method_and_credentials(
    ...
    actor
        .chat_state_handle
        .update_credentials(mull_chat_state::Credentials {
...
/// `PALMSHED_API_KEY`) must NOT attempt an OIDC session-token refresh. The
...
fn model_not_found_error() -> mull_sampler::SamplingErrorInfo {
    mull_sampler::SamplingErrorInfo {
            kind: mull_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model mull-build does not exist or your team does not have access".into(),
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must mention `mull logout`, got: {msg}"
            );
            assert!(
                msg.contains("mull login"),
                "hint must mention `mull login`, got: {msg}"
            );
            assert!(
...
fn unauthorized_401_error() -> mull_sampler::SamplingErrorInfo {
    mull_sampler::SamplingErrorInfo {
            kind: mull_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://openrouter.ai/api/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_mull_token_auth=mull-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
...
// transiently collapsed to `ApiKey` (session-token cache miss + `PALMSHED_API_KEY`)
...
fn session_token_auth_gate_truth_table() {
    ...
    // Session method + Unknown BYOK: refresh only against a first-party Palmshed
...
/// Negative: a genuine `api_key` method keeps its configured key on the
...
/// End-to-end for the frozen-gate bug: a session born on `api_key` (gate
...
async fn set_session_model_invalidates_byok_memo_for_same_model_id() {
    ...
    local
        .run_until(async {
            ...
            let cfg = mull_sampler::SamplerConfig {
