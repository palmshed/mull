...
use mull_tools::types::output::{ToolOutput, ToolRunResult};
...
fn ok_result(text: &str) -> Result<ToolRunResult, mull_tool_runtime::ToolError> {
    Ok(ToolRunResult {
...
fn err(msg: &str) -> Result<ToolRunResult, mull_tool_runtime::ToolError> {
...
fn http_err(status: u16, msg: &str) -> Result<ToolRunResult, mull_tool_runtime::ToolError> {
    Err(
        mull_tool_runtime::ToolError::new(mull_tool_runtime::ToolErrorKind::Custom, msg.to_owned())
            .with_details(
...
fn is_auth_tool_error_classification() {
    ...
    let cases: Vec<(bool, mull_tool_runtime::ToolError)> = vec![
        ...
        (
            true,
            mull_tool_runtime::ToolError::new(
        ...
        // the inference path's gate in mull-sampling-types/src/error.rs:
        ...
        (
            true,
            mull_tool_runtime::ToolError::invalid_arguments(r#"{"error":"invalid_token"}"#),
        ),
        // Fallback path: case-insensitive "unauthorized" anywhere in
        // the message body.
        (
            true,
            mull_tool_runtime::ToolError::invalid_arguments("UNAUTHORIZED"),
        ),
        // Negative: transport failure must not trigger a token refresh.
        (
            false,
            mull_tool_runtime::ToolError::invalid_arguments("Image generation timed out after 60s"),
        ),
        // Negative: structural not-found error; not a network response.
        (
            false,
            mull_tool_runtime::ToolError::not_found(
                mull_tool_protocol::ToolId::new("image_gen").expect("valid"),
        ...
        (
            false,
            mull_tool_runtime::ToolError::invalid_arguments("request id req_401abc failed"),
        ),
    ];
