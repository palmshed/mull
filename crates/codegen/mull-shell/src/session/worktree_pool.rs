...
//!   under `~/.mull/worktree_pool/<instance_id>/` with a `.pid` liveness
...
use mull_fast_worktree::{WorktreeBuilder, WorktreeSync};
use tokio::sync::Notify;
use tokio_util::sync::CancellationToken;

const WORKTREE_POOL_LOG: &str = "mull_worktree_pool";
use crate::util::config::PoolConfig;
use crate::util::mull_home::mull_home;

use mull_tty_utils::git_command;
...
/// `~/.mull/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.mull/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.mull/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.mull/worktree_pool/`, reads each `.pid` file, and checks
...
/// `CleanAll` mode. Placed under `~/.mull/worktree_pool/<instance_id>/<uuid>/`.
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(mull_fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(mull_fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                mull_fast_worktree::remove_worktree(&path_clone)
            })
            .await;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || mull_fast_worktree::remove_worktree(&path_owned)).await;
