...
//!
//! Receives [`mull_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`mull_mcp::liveness`]),
//! - the [`mull_mcp::servers::MullClientHandler`] (server-pushed
...
//! `palmshed.ai/mcp/server_status` and the payload schema defined by
...
pub const SERVER_STATUS_METHOD: &str = "palmshed.ai/mcp/server_status";
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`mull_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `mull_com_` prefix)
    /// or `local` (user `.mull/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`mull_mcp::servers::ClientStateKind::Ready`]
...
/// [`mull_mcp::servers::McpClientEventKind`]). Used as the
...
pub fn build_payload(
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        // NeedsAuth ("visit palmshed.ai"), not a generic Unavailable, so a
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && mull_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///   `ConfigRemoved` keys,
/// - emit one ACP `palmshed.ai/mcp/server_status` push per surviving
///   buffer entry, via the provided gateway.
///
/// `gateway` is a [`mull_acp_lib::AcpAgentGatewaySender`] (forwarded
...
/// 3. `flush_window` — emit ACP `palmshed.ai/mcp/server_status` per
///    surviving entry.
...
mod tests {
    ...
    fn managed_handshake_non_auth_stays_unavailable() {
        for reason in ["403 Forbidden", "cli-chat-proxy returned 502"] {
            ...
            let ev = McpClientEvent::HandshakeFailed {
                server: "mull_com_slack".to_string(),
    ...
    /// Contract: managed server names (starting with `mull_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(classify_source("mull_com_linear"), McpServerSource::Managed);
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        use mull_mcp::servers::{McpClient, McpState};
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("mull_com_slack"), // managed
    ...
    fn discard_gateway() -> mull_acp_lib::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        mull_acp_lib::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = mull_acp_lib::AcpAgentGatewaySender::new(gw_tx);
