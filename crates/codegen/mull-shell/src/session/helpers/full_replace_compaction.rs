//! mull-build's L5 wiring onto the shared full-replace engine
//! (`mull_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](mull_compaction::sample_full_replace_summary);
//! this module adapts mull-build's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](mull_compaction::FullReplaceError).
...
use mull_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use mull_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for mull-build's full-replace pass.
///
...
/// else the structured mull-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the mull-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured mull-build), ignoring the shared
...
/// Map mull-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces mull-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    mull_telemetry::session_ctx::log_event(CompactionRetryDegraded {
