...
//! shell-specific types (`mull_tools::MemoryBackend`, memory context).
//!
//! The core [`CompactionStateContext`] struct and its builder live in
//! `mull_chat_state::compaction_utils`. This module adds system-reminder
//! rendering that requires dependencies not available in `mull-chat-state`.
//!
//! The three **common** active-agent sections (background tasks, TODO list,
//! running subagents) are formatted by
//! [`mull_compaction::reminder`] so mull-chat and mull-build stay in lockstep.
...
pub fn to_system_reminder_sync(
    ...
    skills: &[mull_tools::implementations::skills::types::SkillInfo],
...
pub async fn to_system_reminder(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[mull_tools::implementations::skills::types::SkillInfo],
    memory_backend: Option<&dyn mull_tools::types::memory_backend::MemoryBackend>,
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
...
fn to_system_reminder_inner(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[mull_tools::implementations::skills::types::SkillInfo],
    memory_results: &[mull_tools::types::memory_backend::MemorySearchResult],
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
    ...
    if let Some(listing) =
        mull_tools::types::skill_discovery_tracker::format_compaction_skill_listing(skills)
    {
    ...
    if !ctx.connected_mcp_servers.is_empty() {
        use mull_tools::implementations::search_tool::format_compaction_server_line;
