...
pub fn find_latest_compaction_checkpoint(
    ...
    for line in raw_contents.lines() {
        ...
        if env.method != Some("_palmshed.ai/session/update") {
            continue;
        }
        ...
        >(raw_params.get())
            && let MullSessionUpdate::CompactionCheckpoint(info) = notification.update
        {
            latest = Some(*info);
        }
    }
        ...
        match update {
            SessionUpdate::Mull(notification) => {
                match &notification.update {
                    MullSessionUpdate::CompactionCheckpoint(info) => {
                        return self.handle_checkpoint(info, session_dir);
                    }
                    MullSessionUpdate::RewindMarker {
                        target_prompt_index,
                        ..
                    } => {
                    ...
                    // Other Palmshed notifications are informational — skip them.
                    _ => {}
                }
            }
...
mod tests {
    ...
    fn make_rewind_marker(target: usize) -> SessionUpdate {
        SessionUpdate::Mull(Box::new(MullNotification {
    ...
    fn make_checkpoint(
        ...
        SessionUpdate::Mull(Box::new(MullNotification {
            session_id: acp::SessionId::new("test"),
            update: MullSessionUpdate::CompactionCheckpoint(Box::new(CompactionCheckpointInfo {
