...
use mull_tools::implementations::skills::skill::format_skill_name;
...
// ── palmshed.ai/commands/list ext method ────────────────────────────────
...
///   matching today's Mull flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use mull_tools::implementations::skills::skill::{
...
/// `mull-tools` so it stays identical to the pager's `LoopCommand` and the
...
fn build_loop_prompt_blocks(args: &str) -> Vec<acp::ContentBlock> {
    use mull_tools::implementations::mull_build::{loop_schedule_instruction, loop_usage_message};
