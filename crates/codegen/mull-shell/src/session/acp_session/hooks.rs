...
//!
//! Hooks registered at `session/new` (`_meta["palmshed.ai/hooks"]`) come in two flavors,
//! both matched by the agent ([`mull_hooks::matcher::HookMatcher`], shared with
//! file hooks):
//! - **`PreToolUse` gate**: an awaited reverse *request* `palmshed.ai/hooks/run`; a `deny`
//!   blocks the tool.
//! - **All other events**: fire-and-forget *notifications* `palmshed.ai/hooks/event`,
...
use mull_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use mull_telemetry::events::ClientHookGateOutcome;
...
const HOOK_EVENT_METHOD: &str = "palmshed.ai/hooks/event";
const HOOK_RUN_METHOD: &str = "palmshed.ai/hooks/run";

/// Default per-callback bound for a client's `palmshed.ai/hooks/run` reply; on timeout the gate
...
/// Outcome of the `palmshed.ai/hooks/run` reverse request, before interpreting it as a
    ...
    match outcome {
        ReverseOutcome::Responded(raw) => {
            match serde_json::from_str::<ClientHookResponse>(raw.get()) {
                Ok(resp) => {
                    ...
                    let label = match resp.decision {
                        ...
                        ClientHookDecision::Other => {
                            tracing::warn!(
                                "palmshed.ai/hooks/run returned an unknown decision value; failing open"
                            );
                ...
                Err(err) => {
                    tracing::warn!(%err, "malformed palmshed.ai/hooks/run response; failing open");
                    (
        ...
        ReverseOutcome::Transport(err) => {
            tracing::warn!(%err, "palmshed.ai/hooks/run transport error (no client wired?); failing open");
            (
                ClientHookResponse::default(),
        ...
        ReverseOutcome::Timeout => {
            tracing::warn!("palmshed.ai/hooks/run timed out; failing open");
            (
                ClientHookResponse::default(),
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        mull_telemetry::session_ctx::log_event(mull_telemetry::events::HookBlocked {
    ...
    /// `palmshed.ai/hooks/run` once per matching callback with the shared `envelope` (the
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = mull_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    mull_telemetry::session_ctx::log_event(
                        mull_telemetry::events::ClientHookGate {
    ...
    /// Issue one `palmshed.ai/hooks/run` reverse request, bounded by a per-callback `timeout`.
    ...
    /// `palmshed.ai/hooks/event` notification to each matching registered callback.
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    mull_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
