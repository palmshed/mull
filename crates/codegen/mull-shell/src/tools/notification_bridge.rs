//! Notification bridge: translates `mull-tools` `ToolNotification` events
//! into `mull-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use mull_tools::notification::types::{ToolNotification, ToolNotificationHandle};
...
use mull_workspace::session::file_state::FileStateTracker;
...
pub struct NotificationBridgeConfig {
    ...
    /// `palmshed.ai/incrementalBashOutput` capability.
    ...
    pub auto_wake_delivered: mull_tools::reminders::task_completion::AutoWakeDeliveredIds,
    ...
    /// next-turn exit reminder. Mull-build leaves this `false` — its
...
async fn emit_current_mode_update(
    ...
    mode: mull_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::BashExecutionBackgrounded(bg) => {
            ...
            // Forward as palmshed.ai/task_backgrounded ExtNotification so the TUI can
            ...
            let _ = config.persistence_tx.send(PersistenceMsg::Update(
                crate::session::storage::SessionUpdate::Mull(Box::new(notification.clone())),
            ));
            ...
            if let Some(params) = params {
                let ext_notification =
                    acp::ExtNotification::new("palmshed.ai/task_backgrounded", params.into());
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor = task_snapshot.kind == mull_tools::computer::types::TaskKind::Monitor;
            ...
            // stdout. The pager still receives palmshed.ai/task_completed below for UI.
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    mull_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    mull_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = mull_tools::reminders::wrap_reminder(&body);
            ...
            if let Some(params) = params {
                let notification: acp::ExtNotification =
                    acp::ExtNotification::new("palmshed.ai/task_completed", params.into());
                config.gateway.forward_fire_and_forget(notification);
            }
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, mull_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
        ...
        ToolNotification::PlanModeExited(exited) => {
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                emit_current_mode_update(config, mull_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
        ...
        ToolNotification::ScheduledTaskFired(fired) => {
                ...
                serde_json::to_value(&fired_notif).and_then(|v| serde_json::value::to_raw_value(&v))
            {
                config
                    .gateway
                    .forward_fire_and_forget(acp::ExtNotification::new(
                        "palmshed.ai/scheduled_task_fired",
                        params.into(),
                    ));
            }
        }
        ...
        ToolNotification::ScheduledTaskRemoved(removed) => {
                ...
                .and_then(|v| serde_json::value::to_raw_value(&v))
            {
                config
                    .gateway
                    .forward_fire_and_forget(acp::ExtNotification::new(
                        "palmshed.ai/scheduled_task_deleted",
                        params.into(),
                    ));
            }
        }
        ...
        ToolNotification::ScheduledTaskCreated(created) => {
                ...
                .and_then(|v| serde_json::value::to_raw_value(&v))
            {
                config
                    .gateway
                    .forward_fire_and_forget(acp::ExtNotification::new(
                        "palmshed.ai/scheduled_task_created",
                        params.into(),
                    ));
            }
        }
...
mod tests {
    use super::*;
    use mull_tools::computer::types::TaskKind;
    ...
    fn make_test_config_full() -> (
        NotificationBridgeConfig,
        mpsc::UnboundedReceiver<mull_acp_lib::AcpClientMessage>,
        mpsc::UnboundedReceiver<PersistenceMsg>,
        mpsc::UnboundedReceiver<SessionCommand>,
    ) {
        let (gateway_tx, gateway_rx) = mpsc::unbounded_channel();
        let gateway = mull_acp_lib::AcpAgentGatewaySender::new(gateway_tx);
    ...
    /// drain it). The pager's `palmshed.ai/task_completed` notification still fires.
    #[tokio::test]
    async fn bash_task_completed_suppresses_auto_wake_during_goal_loop() {
        ...
        while let Ok(msg) = gateway_rx.try_recv() {
            if let mull_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "palmshed.ai/task_completed"
            {
                found_ext = true;
            }
        }
        assert!(
            found_ext,
            "palmshed.ai/task_completed ExtNotification must still be sent for UI"
        );
    }
    ...
    /// `will_wake` off the emitted `palmshed.ai/task_completed` params.
    fn task_completed_will_wake(
        gateway_rx: &mut mpsc::UnboundedReceiver<mull_acp_lib::AcpClientMessage>,
    ) -> Option<bool> {
        while let Ok(msg) = gateway_rx.try_recv() {
            if let mull_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "palmshed.ai/task_completed"
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(mull_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            mull_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
        ...
        match msg {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Mull(notif)) => {
                assert!(matches!(
                    &notif.update,
                ...
                assert!(
                    ...
                    "persisted Palmshed bridge lines must carry an eventId"
                );
            }
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification =
            ToolNotification::BashOutputChunk(mull_tools::notification::types::BashOutputChunk {
                base: mull_tools::notification::types::BashNotificationBase {
        ...
        let broadcast_id = match gateway_rx.try_recv().expect("chunk must be broadcast") {
            mull_acp_lib::AcpClientMessage::SessionNotification(args) => args
                .request
                .meta
                .as_ref()
                .and_then(|m| m.get("eventId"))
    ...
    async fn scheduled_task_removed_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskRemoved(
            mull_tools::notification::types::ScheduledTaskRemoved {
                task_id: "loop-1".into(),
            },
        );
        let mut offsets = HashMap::new();
        ...
        match msg {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Mull(notif)) => {
                ...
                assert!(
                    mull_persisted_event_id(&notif).is_some(),
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            mull_tools::notification::types::BashExecutionBackgrounded {
                base: mull_tools::notification::types::BashNotificationBase {
        ...
        match persistence_rx.try_recv().expect("must persist") {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Mull(notif)) => {
                assert!(mull_persisted_event_id(&notif).is_some());
            }
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            mull_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
    ...
    async fn legacy_monitor_event_without_owner_is_injected() {
        // Legacy / non-mull-build backends record no owner; such events must
    ...
    async fn block_waited_task_skips_auto_wake_prompt() {
        ...
        // The palmshed.ai/task_completed ExtNotification for UI updates must still be sent.
        let mut found_ext = false;
        while let Ok(msg) = gateway_rx.try_recv() {
            if let mull_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "palmshed.ai/task_completed"
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        ...
        let notification =
            ToolNotification::PlanModeExited(mull_tools::notification::types::PlanModeExited {
        ...
        while let Ok(msg) = gateway_rx.try_recv() {
            if let mull_acp_lib::AcpClientMessage::SessionNotification(args) = msg
    ...
    /// Default (mull) polarity: the exit_plan_mode tool result is the model's
    ...
    async fn plan_mode_exited_does_not_arm_exit_reminder_by_default() {
        ...
        let notification =
            ToolNotification::PlanModeExited(mull_tools::notification::types::PlanModeExited {
                tool_call_id: "tc-exit-mull".into(),
                plan_content: Some("- step 1".into()),
                plan_file_path: "/tmp/test-session/plan.md".into(),
            });
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification =
            ToolNotification::PlanModeEntered(mull_tools::notification::types::PlanModeEntered {
