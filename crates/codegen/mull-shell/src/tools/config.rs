use crate::models;
use mull_sampler::SamplerConfig;
use mull_tools::implementations::mull_build;
use mull_tools::registry::types::ToolConfig;
use serde::{Deserialize, Serialize};

/// Production mull-build foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production mull-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `MULL_WEB_FETCH_PROXY` env > remote settings > None.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> mull_tools::implementations::mull_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("MULL_WEB_FETCH_PROXY"))
...
/// from `mull_tools::registry::types::ToolsetConfig` which holds
...
impl ShellToolsetConfig {
    ...
    pub fn new(base: Option<Self>, sampling_config: Option<SamplerConfig>) -> Self {
        let default_base = SamplerConfig {
            api_key: None,
            base_url: "https://api.palmshed.ai/v1".to_string(),
            model: String::new(),
...
///
/// Selects between the standard `MullBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `MullBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "MullBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "MullBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"MullBuild:read_file"));
        assert!(ids.contains(&"MullBuild:search_replace"));
        assert!(ids.contains(&"MullBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"MullBuildHashline:hashline_read"));
        assert!(ids.contains(&"MullBuildHashline:hashline_edit"));
        assert!(ids.contains(&"MullBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            mull_agent::config::AgentDefinition::plan(),
            mull_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "MullBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"MullBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"MullBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"MullBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production mull-build must set the 10h foreground ceiling"
        );
    }
