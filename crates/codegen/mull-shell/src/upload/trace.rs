...
use mull_file_utils::queue::{EnqueueOutcome, TraceExportSource, UploadQueue, UploadRetryPolicy};
use mull_workspace::permission::PermissionEvent;
use serde::Serialize;
use std::path::Path;
use std::sync::Arc;
...
pub(crate) fn spawn_trace_upload<T: Serialize + Send + 'static>(
    ...
    tokio::spawn(async move {
        ...
        let ok = mull_file_utils::gcs::upload_bytes(
            &gcs_config,
            &object_path,
            &bytes,
            "application/json",
...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        mull_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        mull_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = mull_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: mull_sandbox::is_active(),
    })
}
...
fn classify_workspace(cwd: &str) -> String {
    ...
    if path.ancestors().any(|p| p.join(".git").exists()) {
        "git".to_owned()
    } else if mull_file_utils::workspace_classifier::is_project_dir(path) {
        "project".to_owned()
    } else {
...
pub(crate) async fn upload_subagent_metadata(
    ...
    if let Err(e) =
        mull_file_utils::gcs::upload_bytes(&config, &gcs_path, &json, "application/json").await
    {
        tracing::warn!(
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&mull_agent::plugins::PluginRegistry>,
) {
...
use mull_file_utils::gcs::upload_bytes;
...
pub(crate) async fn upload_artifact_to_gcs(
    ...
    match upload_bytes(&config, gcs_path, content, content_type).await {
        ...
        Err(e) => {
            let status_code = e
                .downcast_ref::<mull_file_utils::storage_client::HttpUploadError>()
                .map(|e| e.status_code);
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        mull_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(mull_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
pub(crate) async fn upload_turn_messages(
    ctx: &PromptTraceContext,
    _capture: mull_chat_state::TurnCapture,
    _wait: UploadWait,
) -> bool {
    super::manifest::skip_artifact(
        &ctx.artifact_tracker,
...
pub(crate) fn build_chat_history_session_state(
    _messages: &[mull_sampling_types::conversation::ConversationItem],
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_attribution(
        &self,
    ) -> Option<Arc<dyn mull_file_utils::storage_client::Auth401AttributionCallback>> {
        mull_file_utils::gcs::StorageConfig::proxy_attribution(&self.with_auth())
    }
    fn proxy_credentials(&self) -> Option<Arc<dyn mull_auth::AuthCredentialProvider>> {
        mull_file_utils::gcs::StorageConfig::proxy_credentials(&self.with_auth())
    }
    fn proxy_http_client(&self) -> Option<reqwest::Client> {
        mull_file_utils::gcs::StorageConfig::proxy_http_client(&self.with_auth())
    }
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    mull_workspace::recovery::run_startup_recovery(&mull_home, &queue).await;
                tracing::info!(?report, "startup spill recovery complete");
                queue.cleanup_orphans(mull_file_utils::queue::DEFAULT_MAX_AGE);
            }
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    mull_workspace::recovery::purge_spilled_items(&mull_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
...
pub(crate) fn spawn_purge_stale_upload_scratch() {
    {
        ...
        let dir = crate::util::mull_home::mull_home()
            .join("upload_queue")
            .join("scratch");
...
pub(crate) fn spawn_upload_queue(
    ...
    let queue = UploadQueue::spawn(mull_home, resolver, UploadRetryPolicy::default());
...
pub(crate) async fn upload_trace_artifact_blocking(
    ...
    let queue_result = if let Some(queue) = &ctx.upload_queue {
            ...
            .await
        {
            ...
            Err(e)
                if e.downcast_ref::<mull_file_utils::queue::QueueClosed>()
                    .is_some() =>
            {
                tracing::debug!(
...
mod tests {
    ...
    fn dynamic_resolver_refreshes_proxy_token() {
        ...
        let mull_com_config = MullComConfig::default();
        let scope = mull_com_config.auth_scope();
        let initial_auth = MullAuth {
    ...
    async fn resolve_async_falls_back_when_no_refresher() {
        ...
        let auth_json = serde_json::to_string_pretty(&store).unwrap();
        std::fs::write(dir.path().join("auth.json"), &auth_json).unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), mull_com_config));
        let resolver = DynamicResolver {
            auth_manager,
            base_config: TraceExportConfig {
                bucket_url: None,
    ...
    async fn resolve_async_drives_refresh_chain_when_token_expired() {
        ...
        impl crate::auth::refresh::TokenRefresher for FreshRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(crate::auth::MullAuth {
    ...
    async fn proactive_refresh_makes_trace_resolve_a_cache_hit() {
        ...
        let mull_com_config = MullComConfig::default();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), mull_com_config));
        auth_manager.hot_swap(MullAuth {
            key: "expired-oidc".into(),
    ...
    fn dynamic_resolver_supplies_proxy_credentials_and_attribution() {
        use crate::session::repo_changes::UploadMethod;
        use mull_file_utils::queue::TraceExportSource;
        let dir = tempfile::tempdir().unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
    ...
    async fn dynamic_resolver_auth_recovery_wakes_on_already_rotated_token() {
        ...
        auth_manager.hot_swap(crate::auth::MullAuth {
    ...
    fn home_project_dir() -> Option<tempfile::TempDir> {
        ...
        if !mull_file_utils::workspace_classifier::is_project_dir(&home.join("probe")) {
            return None;
        }
