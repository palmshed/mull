...
//! `StorageClient` constructed inside `mull_file_utils::gcs::*` helpers.
//!
...
//! to an `mull_file_utils::gcs::*` helper.
...
/// shell-side adapter that lets `mull_file_utils::gcs::*` helpers wire
...
///
/// ```ignore
/// mull_file_utils::gcs::upload_bytes(
...
/// `MULL_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("MULL_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match mull_file_utils::gcs::upload_bytes(
