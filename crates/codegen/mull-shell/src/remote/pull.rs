...
pub(crate) mod hydrate {
    ...
    fn write_summary(
        ...
        let summary = Summary {
            ...
            head_branch: None,
            request_id: None,
            // Record the *local* mull_home (where this hydrated copy lives),
            ...
            sandbox_profile: mull_sandbox::configured_profile_name().map(String::from),
    ...
    impl ChatReducer {
        ...
        fn process(
            ...
            match update {
                ...
                SessionUpdate::Mull(n) => self.handle_xai(&n.update),
            }
        }
    ...
    const REPLAYABLE_METHODS: &[&str] = &["session/update", "_palmshed.ai/session/update"];
