...
//!
//! Run with: `cargo test -p mull-shell -- pull_smoke --ignored --nocapture`
...
mod tests {
    ...
    fn load_prod_auth() -> Option<MullAuth> {
        let path = crate::util::mull_home::mull_home().join("auth.json");
        let contents = std::fs::read_to_string(&path).ok()?;
        let store: BTreeMap<String, MullAuth> = serde_json::from_str(&contents).ok()?;
    ...
    async fn smoke_push_pull_round_trip() {
        ...
        let auth = load_prod_auth().expect("No auth.json — run `mull login`");
