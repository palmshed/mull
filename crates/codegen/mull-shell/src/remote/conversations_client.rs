...
/// Body for `PUT /rest/app-chat/conversations/{id}` (mull-web `chatUpdateConversation`).
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("MULL_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
    ...
    async fn require_mull_auth(&self) -> Result<MullAuth, ConvError> {
    ...
    pub async fn list_conversations(
        ...
        let auth = self.require_mull_auth().await?;
