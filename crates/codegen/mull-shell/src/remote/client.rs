...
use crate::auth::{MullAuth, MullComConfig};
...
const MULL_CODE_BACKEND_URL: &str = "https://code.palmshed.ai";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const MULL_CODE_WEB_URL: &str = "https://palmshed.ai";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("MULL_CODE_WEB_URL").unwrap_or_else(|_| MULL_CODE_WEB_URL.to_string());
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::mull_auth_credentials::MullAuthCredentials::new(
    ...
    let mut builder = credentials
        .apply(builder, url)
        .header("x-mull-client-version", mull_version::VERSION);
    ...
    mull_file_utils::trace_context::inject_trace_context_into_request(builder)
}
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn mull_auth::AuthCredentialProvider> = std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("MULL_CODE_BACKEND_URL")
                .unwrap_or_else(|_| MULL_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&MullAuth`.
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn mull_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<MullAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// OpenRouter migration: X-XAI-Token-Auth removed (not needed for OpenRouter).
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-mull-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            "x-mull-client-version",
            HeaderValue::from_static(mull_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = mull_file_utils::trace_context::inject_trace_context_into_request(
...
/// Fetch `mull_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-mull-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(mull_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header("x-mull-agent-id", agent_id)
        .header("x-mull-client-version", mull_version::VERSION)
        .header(
            "x-mull-client-identifier",
            crate::http::process_client_identifier(),
...
impl ListModelsEndpoint {
    fn from_endpoints(
        ...
        if endpoints.has_custom_endpoint() {
            ...
            Self {
                url: format!("{}/models", endpoints.mull_api_base_url),
                auth: EndpointAuth::ApiKey,
            }
        } else {
            Self {
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_mull_api_key_env()
                .or_else(|_| {
                ...
                .map_err(|_| {
                    BackendError::Auth(
                        "No API key for custom models endpoint. Set PALMSHED_API_KEY.".into(),
                    )
                })?;
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| mull_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(mull_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-mull-agent-id"),
                            client_identifier: header_str(&headers, "x-mull-client-identifier"),
                            client_version: header_str(&headers, "x-mull-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-mull-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-mull-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-mull-client-version"
        );
    ...
    fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
        ...
        let mgr = crate::auth::AuthManager::new(dir.path(), crate::auth::MullComConfig::default());
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("mull-cli"));
    ...
    fn parse_openai_format_uses_id_field() {
        let value = serde_json::json!(
            { "id" : "grok-3", "object" : "model", "owned_by" : "mull", "context_window" :
            131072 }
        );
        let result = parse_remote_model_value(&value, "https://api.palmshed.ai/v1").unwrap();
        assert_eq!(result.model, "grok-3");
        assert_eq!(result.base_url, "https://api.palmshed.ai/v1");
    ...
    fn parse_reads_show_model_fingerprint_field() {
        let value = serde_json::json!(
            { "model" : "mull", "context_window" : 256_000,
            "show_model_fingerprint" : true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "mull", "contextWindow" : 256_000, "showModelFingerprint" :
            true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "mull", "context_window" : 256_000, "_meta" : {
    ...
    fn list_url_derived_from_base_url() {
        ...
        assert_eq!(
            ep.resolve_models_list_url(),
            "https://api.palmshed.ai/v1/models"
        );
    }
    #[test]
    ...
    /// ApiKey → `mull_api_base_url` (ApiKey, public default when unset); a custom
    ...
    fn models_fetch_endpoint_matches_auth_mode() {
        ...
        assert_eq!(session.url, "https://openrouter.ai/api/v1/models");
        ...
        assert_eq!(deployment.url, "https://openrouter.ai/api/v1/models");
    ...
    /// REGRESSION: `mull setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("MULL_DEPLOYMENT_KEY", "palmshed-token-ENTERPRISE") };
        ...
        assert_eq!(url, "https://openrouter.ai/api/v1/deployment/config");
        assert!(
        ...
        unsafe { std::env::remove_var("MULL_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
