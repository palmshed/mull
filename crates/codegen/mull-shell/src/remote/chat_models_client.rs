//! palmshed.ai chat-product model catalog (`POST /rest/modes`) — the models
//! mull-web's chat picker shows, distinct from the CLI `/v1/models` build
...
const MULL_WEB_URL: &str = "https://palmshed.ai";
...
pub enum ChatModelsError {
    #[error("no palmshed.ai credentials")]
    NoAuth,
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("MULL_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("MULL_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("MULL_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| MULL_WEB_URL.to_string());
        Self {
    ...
    /// Gated only on a valid palmshed.ai bearer — deliberately NOT `is_mull_auth()`
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let builder = mull_file_utils::trace_context::inject_trace_context_into_request(builder);
