...
//!
//! The canonical error types now live in `mull_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a mull-shell dependency).

// Re-export everything from the standalone crate.
pub use mull_sampling_types::error::*;
...
/// API key / team rate-limit copy. Personal palmshed.ai upgrades do not raise API
/// team limits; admins purchase credits or a higher spend-based tier.
/// See https://docs.palmshed.ai/developers/rate-limits#rate-limit-tiers
pub const RATE_LIMITED_USER_MESSAGE_API_KEY: &str = "You\u{2019}ve hit your team\u{2019}s API rate limit. Ask a team admin to purchase more credits for higher limits, or try again later. See https://docs.palmshed.ai/developers/rate-limits#rate-limit-tiers";
...
/// for the selected method). Do **not** decide from `has_mull_api_key_env()` alone:
...
/// This stays in mull-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::Api {
            ...
            StatusCode::FORBIDDEN => {
                let message = if message.contains("requires a Mull subscription")
                    && crate::agent::auth_method::has_mull_api_key_env()
                {
                    format!(
                        "{message}\n\nYou have an API key set (PALMSHED_API_KEY). \
                         Your cached OAuth session is being used instead. \
                         To use your API key, run `mull logout` or type /logout in the TUI."
                    )
                } else {
                    message
                };
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                mull_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: mull_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != mull_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == mull_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = mull_chat_state::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &mull_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn rate_limited_user_message_oauth_vs_api_key() {
        ...
        assert!(
            RATE_LIMITED_USER_MESSAGE_API_KEY
                .contains("https://docs.palmshed.ai/developers/rate-limits#rate-limit-tiers")
        );
    ...
    /// Helper: run a closure with PALMSHED_API_KEY temporarily set (or cleared).
    ...
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        ...
        let prev_legacy = std::env::var("MULL_CODE_PALMSHED_API_KEY").ok();
        // SAFETY: serial_test ensures no concurrent env mutation.
        unsafe {
            std::env::remove_var("PALMSHED_API_KEY");
            std::env::remove_var("MULL_CODE_PALMSHED_API_KEY");
            if let Some(k) = key {
                std::env::set_var("PALMSHED_API_KEY", k);
            }
        }
        ...
        unsafe {
            ...
            std::env::remove_var("MULL_CODE_PALMSHED_API_KEY");
            if let Some(v) = prev {
                std::env::set_var("PALMSHED_API_KEY", v);
            }
            if let Some(v) = prev_legacy {
                std::env::set_var("MULL_CODE_PALMSHED_API_KEY", v);
            }
        }
        if let Err(e) = result {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("palmshed-test"), || {
            let err = SamplingError::Api {
                status: StatusCode::FORBIDDEN,
                message: "The model 'mull-build' requires a Mull subscription.".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry: None,
            };
            ...
            assert!(
                msg.contains("mull logout"),
                "should suggest mull logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),
