...
use crate::util::mull_auth_credentials::MullAuthCredentials;
use mull_auth::{
...
fn api_key_id_for(auth: Option<&crate::auth::MullAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| mull_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: MullAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = MullAuthCredentials::new(None);
...
///
/// - `"mull-shell"`   — classic Mull CLI / TUI (mull-shell)
/// - `"mull-pager"`   — new Mull Pager / TUI (mull-pager)
/// - `"mull-desktop"` — Mull Desktop app
/// - `"mull-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-mull-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
/// - `x-mull-client-identifier`
///
/// to the underlying `mull-file-utils::StorageClient` via
...
pub fn build_storage_client_for_proxy(
...
) -> mull_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn mull_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        mull_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(mull_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = MullAuthCredentials::new(user_token);
...
/// Bridge that lets `StorageClient` (which lives in mull-file-utils)
...
impl mull_file_utils::storage_client::Auth401AttributionCallback
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = MullAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        mull_telemetry::external::set_identity(
            mull_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `mull-pager` and `mull-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> mull_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let mull_com_config = crate::auth::MullComConfig::default();
    let exporter = mull_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = mull_com_config.token_header.clone();
    let mull_home = crate::util::mull_home::mull_home();
    let bootstrap = Arc::new(AuthManager::new(&mull_home, mull_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    mull_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::MullComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use mull_auth::AuthCredentialProvider;
    use std::sync::Mutex;
    /// Serializes tests that pin `MULL_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `MULL_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("MULL_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("MULL_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("MULL_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("MULL_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> MullAuth {
        MullAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<MullAuth>) -> Arc<AuthManager> {
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::MullComConfig::default(),
        ));
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use mull_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let dep = ShellAuthCredentialProvider::new(
            make_manager(&dir, None),
            Some("palmshed-token-EX".into()),
            None,
        )
        .snapshot();
        assert_eq!(
            dep.deployment_id.as_deref(),
            Some(deployment_id_from_key("palmshed-token-EX").as_str())
        );
        assert!(dep.api_key_id.is_none());
        let api_auth = MullAuth {
    ...
    /// user token, matching the precedence in `MullAuthCredentials::apply`.
