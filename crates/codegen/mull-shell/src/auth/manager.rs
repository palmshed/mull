...
use crate::auth::config::MullComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use mull_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, MullAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
    token_suffix,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<MullAuth>>>,
    path: PathBuf,
    scope: String,
    ...
    power_listener: parking_lot::Mutex<Option<mull_system_power::SystemPowerListener>>,
...
enum LockOutcome {
    Held(AuthFileLock),
    Adopted(Box<MullAuth>),
}

// ── Construction + builders ──────────────────────────────────────────

impl AuthManager {
    pub fn new(mull_home: &Path, mull_com_config: MullComConfig) -> Self {
        let scope = mull_com_config.auth_scope();
        let proxy_base_url =
        ...
        mull_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "mull_home": mull_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "MULL_HOME": std::env::var("MULL_HOME").unwrap_or_else(|_| "(unset)".into()),
                "MULL_AUTH_PATH": std::env::var("MULL_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "MULL_AUTH": std::env::var("MULL_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // MULL_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("MULL_AUTH") {
            if let Ok(auth) = serde_json::from_str::<MullAuth>(&inline_json) {
            ...
            tracing::warn!("MULL_AUTH set but failed to parse as JSON, falling back to file");
        }

        // MULL_AUTH_PATH: custom file path (overrides default $MULL_HOME/auth.json).
        let path = std::env::var("MULL_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| mull_home.join("auth.json"));
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &MullAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.mull_com_config)?;
    ...
    fn vet_cached(&self, auth: MullAuth) -> Option<MullAuth> {
    ...
    pub(crate) fn current(&self) -> Option<MullAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<MullAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&MullAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<MullAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<MullAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    pub(crate) fn expired_auth(&self) -> Option<MullAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &MullAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &MullAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &MullAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.mull_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `MullAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: MullAuth) -> std::io::Result<MullAuth> {
        ...
        match &write_result {
            Ok(()) => mull_telemetry::unified_log::info(
            ...
            Err(e) => mull_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: MullAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut MullAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    pub(crate) fn mull_com_config(&self) -> &MullComConfig {
        &self.mull_com_config
    }
    ...
    pub(crate) fn run_external_refresh_command(&self, command: &str) -> Option<MullAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: MullAuth) {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<MullAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> MullAuth {
        self.inner.read().clone().unwrap_or_else(|| MullAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: MullAuth) -> Option<MullAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<MullAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<MullAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<MullAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in MULL_AUTH mode).
            DiskAuthState::Ok => {
                mull_telemetry::unified_log::info(
            ...
            | DiskAuthState::EntryMissing
            | DiskAuthState::Unreadable => {
                mull_telemetry::unified_log::warn("auth disk state: entry lost", None, Some(ctx));
            }
        }
    }
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<MullAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<MullAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<MullAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.mull_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`mull login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.mull_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<MullAuth, AuthError> {
        if self.mull_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        mull_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `mull_system_power::PowerState` for the canonical
    ...
    pub(crate) fn is_different_token(&self, candidate: &MullAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<mull_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[mull_com] auth_token_ttl` to drive a real schedule.
...
///
/// Static fallthrough (`PALMSHED_API_KEY` / `auth.json` `mull::api_key`) makes voice
...
impl mull_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.mull_com_config.preferred_method,
        Some(super::config::PreferredAuthMethod::ApiKey)
    )
}

/// Env → `auth.json` `mull::api_key`. Off under kill-switch or `preferred_method = oidc`.
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.mull_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.mull_com_config.preferred_method,
    ...
    non_empty_key(crate::agent::auth_method::read_mull_api_key_env().ok())
...
impl AuthManager {
    /// `mull::api_key` from this manager's auth file, memoized on
...
) -> mull_tools::types::SharedApiKeyProvider {
