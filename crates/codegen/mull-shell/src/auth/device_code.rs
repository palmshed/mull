...
use crate::auth::{AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode, MullAuth};
...
pub enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `mull login` or set PALMSHED_API_KEY instead."
    )]
    NotEnabled,
    #[error(transparent)]
...
/// `x-mull-client-surface` header so device-flow metrics can separate logins a
...
pub async fn request_device_code(
    ...
    let resp = with_alpha_test_key(
        client
            ...
            .header("x-mull-client-surface", surface.as_str())
            .form(&[
                ("client_id", client_id),
...
///
/// On success, persists credentials to `~/.mull/auth.json` and returns
/// the authenticated `MullAuth`.
///
...
pub async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `mull login --device-auth` again.");
        }
...
pub async fn run_device_code_login_channels(
    ...
    // `palmshed.ai/auth/get_url` isn't blocked on a slow/hanging browser launch
...
pub async fn request_github_device_code(
    ...
    let client = crate::http::shared_client();
    let url = crate::auth::config::GITHUB_DEVICE_CODE_URL;
    let scope_str = scopes.join(" ");
    ...
    let resp = client
        ...
        .form(&[("client_id", client_id), ("scope", scope_str.as_str())])
    ...
    if !resp.status().is_success() {
        ...
        let body = resp.text().await.unwrap_or_default();
        return Err(
            anyhow::anyhow!("GitHub device code request failed (HTTP {status}): {body}").into(),
        );
    }

    let server_resp: GitHubDeviceCodeResponse = resp.json().await?;
...
/// Poll GitHub's token endpoint until the user approves.
pub async fn complete_github_device_code_login(
    client_id: &str,
    device_code: DeviceCode,
    auth_manager: &Arc<AuthManager>,
) -> anyhow::Result<(MullAuth, bool)> {
    let client = crate::http::shared_client();
    let token_url = crate::auth::config::GITHUB_TOKEN_URL;
    let mut poll_interval = std::time::Duration::from_secs(device_code.interval.max(1) as u64);
    let deadline = tokio::time::Instant::now()
        + std::time::Duration::from_secs(
            device_code
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `mull login` again.");
        }
        ...
        if let Some(err_val) = body.get("error") {
            let err_str = err_val.as_str().unwrap_or("");
            match err_str {
                "authorization_pending" => continue,
                "slow_down" => {
                    poll_interval +=
                        std::time::Duration::from_secs(DEVICE_SLOW_DOWN_INCREMENT_SECS);
                    continue;
                }
                "access_denied" => {
                    anyhow::bail!("Authorization denied. The user rejected the request.");
                ...
                other => {
                    let detail = body
                        ...
                        .and_then(|v| v.as_str())
                        .unwrap_or(other);
                    anyhow::bail!("GitHub token exchange error: {detail}");
                }
            }
        }

        let tokens: GitHubTokenResponse = serde_json::from_value(body)?;
        return build_github_auth(&tokens, client_id, auth_manager).await;
    }
}
...
async fn build_github_auth(
    tokens: &GitHubTokenResponse,
    client_id: &str,
    auth_manager: &Arc<AuthManager>,
) -> anyhow::Result<(MullAuth, bool)> {
    let user_info = crate::auth::oidc::fetch_github_user_info(&tokens.access_token).await?;
    ...
    let mut auth = MullAuth {
        ...
        refresh_token: None,
        expires_at: None,
        oidc_issuer: Some(crate::auth::config::GITHUB_ISSUER.to_owned()),
        oidc_client_id: Some(client_id.to_owned()),
    };
...
/// Run GitHub device code login: request code → prompt → poll → persist.
pub async fn run_github_device_code_login(
    client_id: &str,
    scopes: &[String],
    auth_manager: &Arc<AuthManager>,
    mut channels: Option<AuthChannels>,
) -> anyhow::Result<(MullAuth, bool)> {
    let device_code = request_github_device_code(client_id, scopes).await?;

    tracing::info!(
        verification_uri = %device_code.verification_uri,
        user_code = %device_code.user_code,
        "GitHub device code login: prompt user"
    );

    let url_tx = channels.as_mut().and_then(|ch| ch.url_tx.take());
    let code_rx = channels.as_mut().map(|ch| &mut ch.code_rx);
    ...
    complete_github_device_code_login(client_id, device_code, auth_manager).await
}
...
pub(crate) mod tests {
    ...
    use crate::auth::{AuthMode, MullComConfig};
    ...
    fn auth_manager_with_mull_home(
        mull_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(mull_home, MullComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let mull_home = temp_dir.path().join(".mull");
        std::fs::create_dir_all(&mull_home).unwrap();
        let auth_manager = auth_manager_with_mull_home(&mull_home, "http://127.0.0.1:9");
        let tokens = super::TokenOk {
            access_token: "access-token".to_string(),
            refresh_token: Some("refresh-token".to_string()),
            expires_in: Some(900),
            scope: Some("openid email offline_access mull-cli:access".to_string()),
            id_token: Some(
    ...
    fn build_auth_seeds_team_metadata_from_access_token() {
        ...
        let claims = serde_json::json!({
            "sub": "user-42",
            "iss": "https://auth.palmshed.ai",
            "aud": "client-id",
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access mull-cli:access team:read",
            "principal_type": "Team",
        ...
        let tokens = super::TokenOk {
            ...
            expires_in: Some(900),
            scope: Some("offline_access mull-cli:access team:read".to_owned()),
            id_token: None,
        };
    ...
    /// persist nothing.
    fn assert_build_auth_rejected(cfg: MullComConfig, token_principal: &str, expected_err: &str) {
        ensure_crypto_provider();
        let temp_dir = tempfile::tempdir().unwrap();
        let mull_home = temp_dir.path().join(".mull");
        std::fs::create_dir_all(&mull_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&mull_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
        ...
        assert!(
            !mull_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::MullAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_mull_home(temp_dir.path(), "http://127.0.0.1:9");
