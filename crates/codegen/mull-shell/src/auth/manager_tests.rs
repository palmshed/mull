...
fn make_auth(expires_at: Option<DateTime<Utc>>, create_time: DateTime<Utc>) -> MullAuth {
    MullAuth {
...
fn auth_scope_uses_oauth2_when_present() {
    let cfg = MullComConfig::default();
    // Default config always has oauth2 set to the Palmshed defaults.
    assert_eq!(
        cfg.auth_scope(),
        format!(
            "{}::{}",
            crate::auth::config::PALMSHED_OAUTH2_ISSUER,
...
fn legacy_scope_fallback_reads_old_auth_json() {
    ...
    // a machine that was authenticated with an older mull version).
...
/// legacy `https://accounts.palmshed.ai/sign-in` entry from auth.json.
#[test]
...
fn manager_collection_predicates_fail_directions() {
    ...
    mgr.hot_swap(MullAuth::test_default());
    assert!(!mgr.is_data_collection_disabled());
...
fn try_use_disk_token_accepts_different_key_on_server_rejected() {
    ...
    let mem_auth = MullAuth {
        key: "old-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    mgr.hot_swap(mem_auth);

    let disk_auth = MullAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    // funnel the user back through `mull login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `mull logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `mull login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
/// `has_mull_code_access`. The `&mut MullAuth` signature already
...
fn apply_user_info_enrichment_preserves_token_fields() {
    let mut disk = MullAuth {
        ...
        has_mull_code_access: Some(true),
    ...
    assert_eq!(disk.has_mull_code_access, snapshot.has_mull_code_access);
...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    use mull_tools::types::ApiKeyProvider;

    let _xai = EnvGuard::unset("PALMSHED_API_KEY");
    let _legacy = EnvGuard::unset("MULL_CODE_PALMSHED_API_KEY");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), MullComConfig::default()));
    mgr.hot_swap(MullAuth {
...
async fn refresh_chain_server_rejected_bypasses_valid_token_double_check() {
    ...
    let valid_but_rejected = MullAuth {
...
async fn enrich_auth_inline_populates_zdr_flags() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), MullComConfig::default()).with_proxy_base_url(&base);

    let mut auth = MullAuth {
        key: "tok".into(),
        ..MullAuth::test_default()
    };
...
fn oidc_session_for_team(principal_id: &str) -> MullAuth {
    MullAuth {
        key: team_jwt(principal_id),
        auth_mode: AuthMode::Oidc,
        refresh_token: Some("rt".into()),
        expires_at: Some(Utc::now() + Duration::hours(1)),
        oidc_issuer: Some(crate::auth::config::PALMSHED_OAUTH2_ISSUER.to_string()),
        oidc_client_id: Some("client".into()),
        ..MullAuth::test_default()
    }
}
...
async fn auth_rejects_token_refreshed_into_wrong_team() {
    ...
    impl TokenRefresher for WrongTeamRefresher {
        async fn refresh(&self, _reason: RefreshReason) -> crate::auth::refresh::RefreshOutcome {
            crate::auth::refresh::RefreshOutcome::Success(Box::new(MullAuth {
                key: self.jwt.clone(),
                auth_mode: AuthMode::Oidc,
                refresh_token: Some("rt-new".into()),
                expires_at: Some(Utc::now() + Duration::hours(1)),
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    let api_key_session = || MullAuth {
        key: "palmshed-cached-key".into(),
        auth_mode: AuthMode::ApiKey,
        expires_at: Some(Utc::now() + Duration::hours(1)),
        ..MullAuth::test_default()
    };
    ...
    let mgr2 = Arc::new(AuthManager::new(dir2.path(), MullComConfig::default()));
    mgr2.hot_swap(api_key_session());
    assert_eq!(
        mgr2.current().map(|a| a.key),
        Some("palmshed-cached-key".to_string()),
...
/// No OAuth session → env or auth.json `mull::api_key` for voice/tools.
#[tokio::test]
#[serial_test::serial]
async fn shared_api_key_provider_static_fallthrough() {
    ...
    {
        let _legacy = EnvGuard::unset("MULL_CODE_PALMSHED_API_KEY");
        let _key = EnvGuard::set("PALMSHED_API_KEY", "env-only-key");
        assert_eq!(
    ...
    {
        let _key = EnvGuard::set("PALMSHED_API_KEY", "env-should-lose");
        mgr.hot_swap(MullAuth {
...
async fn shared_api_key_provider_kill_switch_blocks_static() {
    ...
    let _key = EnvGuard::set("PALMSHED_API_KEY", "blocked");
    let dir = tempfile::tempdir().unwrap();
...
async fn shared_api_key_provider_oidc_preferred_blocks_static() {
    ...
    let _key = EnvGuard::set("PALMSHED_API_KEY", "should-not-use");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(
        dir.path(),
        MullComConfig {
...
async fn shared_api_key_provider_api_key_preferred_skips_session() {
    ...
    let _legacy = EnvGuard::unset("MULL_CODE_PALMSHED_API_KEY");
    let _key = EnvGuard::set("PALMSHED_API_KEY", "static-preferred");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(
        dir.path(),
        MullComConfig {
...
async fn shared_api_key_provider_sync_falls_through_when_session_expired() {
    ...
    let _legacy = EnvGuard::unset("MULL_CODE_PALMSHED_API_KEY");
    let _key = EnvGuard::set("PALMSHED_API_KEY", "static-after-expiry");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), MullComConfig::default()));
    mgr.hot_swap(MullAuth {
        key: "expired-oidc".into(),
...
async fn shared_api_key_provider_sync_buffered_session_beats_static() {
    ...
    let _legacy = EnvGuard::unset("MULL_CODE_PALMSHED_API_KEY");
    let _key = EnvGuard::set("PALMSHED_API_KEY", "leftover-static");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), MullComConfig::default()));
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use mull_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use mull_telemetry::events::{AuthTokenKind, ManualAuthSurface};
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    fn mgr_with(dir: &std::path::Path, key: &str, mode: AuthMode) -> Arc<AuthManager> {
        let mgr = Arc::new(AuthManager::new(dir, MullComConfig::default()));
    ...
    use mull_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
    assert_eq!(
