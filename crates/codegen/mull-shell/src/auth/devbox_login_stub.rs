...
//! points that can still be reached directly (`mull login --devbox`) return a
//! descriptive error.

use super::manager::AuthManager;
use super::model::MullAuth;
...
pub(crate) async fn mint_devbox_auth(_auth_manager: &AuthManager) -> anyhow::Result<MullAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// Unreachable in practice (guarded by [`is_devbox_environment`]); errors
/// defensively if called.
pub(super) async fn mint_devbox_auth_raw() -> anyhow::Result<MullAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// `mull login --devbox` entry point: always errors in this build.
pub async fn run_devbox_login(_config: &crate::agent::config::Config) -> anyhow::Result<MullAuth> {
    anyhow::bail!(UNAVAILABLE)
}
