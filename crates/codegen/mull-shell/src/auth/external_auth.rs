...
pub(crate) struct ExternalAuthOutput {
    ...
    /// Token issuer. An Palmshed issuer marks the credential as first-party;
    /// see [`MullAuth::is_mull_auth`].
    #[serde(default)]
    pub issuer: Option<String>,
}

/// Parse process output (stdout) into a `MullAuth`. Accepts bare token or JSON.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<MullAuth> {
...
pub(crate) fn run_external_auth_sync(command: &str, is_refresh: bool) -> Option<MullAuth> {
...
pub(crate) fn refresh_with_command(command: &str, prev_auth: &MullAuth) -> Option<MullAuth> {
...
mod tests {
    ...
    #[test]
    fn parse_output_issuer_claim_enables_mull_auth() {
        ...
        // palmshed.ai issuer claim → first-party session (relay-eligible).
        let auth = parse_output(&ok(
            r#"{"access_token":"t","expires_in":900,"issuer":"https://auth.palmshed.ai"}"#,
        ))
        .unwrap();
        assert_eq!(
            auth.oidc_issuer.as_deref(),
            Some("https://auth.palmshed.ai")
        );
        assert!(auth.is_mull_auth());

        // Non-palmshed.ai issuer is stored but stays third-party.
        let auth = parse_output(&ok(
    ...
    #[test]
    fn sync_sets_mull_auth_expired_env_on_refresh() {
        let auth = run_external_auth_sync("echo $MULL_AUTH_EXPIRED", true).unwrap();
