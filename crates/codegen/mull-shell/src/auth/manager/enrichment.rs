...
use crate::auth::model::{MullAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: MullAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.mull_com_config.token_header;
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut MullAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: MullAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut MullAuth, user_info: UserInfo) {
