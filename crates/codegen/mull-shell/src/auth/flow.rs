...
use crate::auth::{AuthManager, MullAuth, MullComConfig, parse_output};
use crate::util::mull_home;
...
/// session.
fn is_cached_credential_compatible(auth: &MullAuth, mull_com_config: &MullComConfig) -> bool {
    let expected_issuer = mull_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| mull_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(mull_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("MULL_LOGIN_DEVICE_FLOW")
...
/// Palmshed OAuth2 provider supports it. Enterprise OIDC (`oidc=Some`) always uses
/// the loopback flow, mirroring `run_auth_flow_inner`'s precedence.
async fn cli_should_use_device(
    config: &MullComConfig,
    login_override: LoginTransportOverride,
) -> bool {
    !crate::auth::oidc::is_configured(config) && should_use_device_flow(login_override).await
}

/// Whether interactive Palmshed OAuth2 login uses the RFC 8628 device flow (vs loopback).
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `MULL_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `mull_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("MULL_LOGIN_DEVICE_FLOW");
...
/// How login presents itself; surfaced to the TUI via `palmshed.ai/auth/get_url`.
...
impl AuthUrlMode {
    /// Wire string for the `palmshed.ai/auth/get_url` ACP response.
...
async fn run_external_auth_provider(
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.mull_com_config());
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = mull_com_config.oidc.is_some(),
        has_oauth2 = mull_com_config.oauth2.is_some(),
        has_external_auth = mull_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if reauth {
        auth_manager.clear()?;
        // Also remove the legacy accounts.palmshed.ai scope so stale tokens
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, mull_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, mull_com_config)
        }) {
            mull_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = mull_com_config.auth_provider_command {
    ...
    // `mull login --devbox` uses run_devbox_login and is not gated here.
    if !mull_com_config.blocks_automatic_oidc()
    ...
    // Palmshed OAuth2 also defaults to loopback; the device flow (robust on
    // remote/SSH where the loopback redirect can't reach the CLI) is opt-in via
    // --device-auth / MULL_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(mull_com_config) {
        return crate::auth::oidc::run_login_flow(mull_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = mull_com_config.oauth2 {
        if crate::auth::config::is_github_issuer(&oauth2_cfg.issuer) {
            // GitHub OAuth2 is configured alongside OpenRouter API-key auth.
            // Offer a choice: default to the API key (the reliable OpenRouter
            // path); GitHub device flow is the alternative. In a non-TTY
            // context (e.g. the TUI) we can't prompt, so default to API key
            // and let `run_api_key_login` surface a clear error if needed.
            if prompt_github_or_api_key().await == AuthChoice::Github {
                return crate::auth::device_code::run_github_device_code_login(
    ...
    run_api_key_login(auth_manager).await
}

/// Which sign-in method the user picked when both GitHub OAuth and OpenRouter
/// API-key auth are available.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum AuthChoice {
    ApiKey,
    Github,
...
/// Prompt the user to choose between GitHub OAuth and OpenRouter API key.
///
/// Defaults to the API key on empty input or whenever stdin is not a terminal
/// (so the TUI / piped contexts never block on a prompt).
async fn prompt_github_or_api_key() -> AuthChoice {
    use std::io::IsTerminal;

    if !std::io::stdin().is_terminal() {
        return AuthChoice::ApiKey;
    }

    eprintln!();
    eprintln!("Choose a sign-in method:");
    eprintln!("  1) OpenRouter API key  (recommended)");
    eprintln!("  2) GitHub");
    eprint!("> ");
    ...
    match input {
        ...
        "2" => AuthChoice::Github,
        other => {
            eprintln!("Unrecognized choice '{other}', defaulting to OpenRouter API key.");
            AuthChoice::ApiKey
        }
    }
}

/// Interactive API key login: open OpenRouter keys page, prompt for key, save.
///
/// Only works from a real terminal (CLI `mull login`).  When stdin is not a
/// TTY (e.g. inside the TUI), returns an error directing the user to run
/// `mull login` in a separate terminal.
async fn run_api_key_login(auth_manager: &Arc<AuthManager>) -> anyhow::Result<(MullAuth, bool)> {
    use std::io::IsTerminal;

    if !std::io::stdin().is_terminal() {
        anyhow::bail!(
            "API key login requires a terminal. \
             Run `mull login` in a separate terminal window, \
             or set the OPENROUTER_API_KEY environment variable."
        );
    }

    let keys_url = "https://openrouter.ai/keys";
    let _ = webbrowser::open(keys_url);

    eprintln!();
    eprintln!("OpenRouter API Key Login");
    eprintln!();
    eprintln!("A browser window has opened to {keys_url}");
    eprintln!("If it didn't open, visit the URL manually.");
    eprintln!();
    eprintln!("Paste your API key (starts with sk-or-):");
    eprint!("> ");
    use std::io::Write as _;
    let _ = std::io::stderr().flush();
    ...
    if key.is_empty() {
        anyhow::bail!("No API key provided. Run `mull login` to try again.");
    }

    let mull_home = mull_home::mull_home();
    crate::auth::storage::store_api_key(&mull_home, &key)
    ...
    let is_new = auth_manager.current().is_none();
    auth_manager
        .update(auth.clone())
        .await
    ...
    eprintln!();
    eprintln!("API key saved. You're logged in.");
...
pub async fn try_ensure_fresh_auth(mull_com_config: &MullComConfig) -> Option<MullAuth> {
    let mull_home = mull_home::mull_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&mull_home, mull_com_config.clone()));

    // auth() handles cached-valid (fast path), OIDC refresh, external
    // binary -- all through refresh_chain (single mutation point).
    auth_manager.configure_refresher(mull_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    if let Some(auth) = try_ensure_fresh_auth(mull_com_config).await {
        return Some(auth);
    }
    let mull_home = mull_home::mull_home();
    let auth_manager = Arc::new(AuthManager::new(&mull_home, mull_com_config.clone()));
    ...
    mint_session_noninteractive(&auth_manager, mull_com_config).await
}
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<MullAuth> {
    auth_manager
        .current_or_expired()
        .filter(|a| a.is_mull_auth() && a.refresh_token.is_some())
}
...
async fn mint_session_noninteractive(
    ...
    if let Some(cmd) = mull_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: MullAuth) -> MullAuth {
...
fn report_signed_in(auth: &MullAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<MullAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(mull_com_config).await)
    } else {
        ensure_authenticated(mull_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `mull login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `MULL_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // oauth2=None) always uses the loopback flow; only the Palmshed OAuth2 provider
    // supports the device flow. Without this guard, `mull login` on an
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.mull_com_config, login_override).await {
        if config.mull_com_config.oauth2.is_none() {
            // No OIDC and no oauth2 here, so `--oauth` can't help.
            anyhow::bail!(
                "Sign-in is not available for this deployment. Set PALMSHED_API_KEY instead."
            );
        }
        let mull_home = mull_home::mull_home();
        let auth_manager = Arc::new(AuthManager::new(&mull_home, config.mull_com_config.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.mull_com_config) {
            eprintln!(
    ...
    // config was actually applied; `mull setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `PALMSHED_API_KEY` / `MULL_CODE_PALMSHED_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        mull_telemetry::external::set_identity(mull_telemetry::external::IdentityAttrs::default());
        mull_telemetry::external::flush();
    ...
    Ok(LogoutResult {
        was_logged_in,
        email,
        api_key_still_set: crate::agent::auth_method::has_mull_api_key_env(),
    })
}

/// `mull logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let mull_home = mull_home::mull_home();
    let auth_manager = AuthManager::new(&mull_home, config.mull_com_config.clone());
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!("You are authenticated via PALMSHED_API_KEY (environment variable).");
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("PALMSHED_API_KEY is still set and will be used for authentication.");
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::auth::config::PALMSHED_OAUTH2_ISSUER;
    use crate::env::EnvVarGuard;
    use chrono::Utc;

    /// Run `f` with `MULL_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("MULL_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("MULL_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("MULL_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A palmshed.ai first-party (palmshed.ai-issuer) OIDC session — `is_mull_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> MullAuth {
        MullAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(PALMSHED_OAUTH2_ISSUER.to_string()),
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), MullComConfig::default());
        ...
        // Third-party external (no palmshed.ai issuer) stays excluded.
        mgr.hot_swap(MullAuth {
            oidc_issuer: None,
            auth_mode: AuthMode::External,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only mull_home: reading a missing auth.json succeeds (empty), but
    ...
    async fn mint_session_noninteractive_uses_external_provider() {
        let dir = tempfile::tempdir().unwrap();
        let cfg = MullComConfig {
            auth_provider_command: Some("printf '%s' palmshed-ext-token".to_string()),
        ...
        assert_eq!(auth.map(|a| a.key), Some("palmshed-ext-token".to_string()));
    }
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), MullComConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("MULL_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("MULL_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `mull login` must use loopback, not device —
        ...
        // The Palmshed OAuth2 provider (oidc=None, oauth2=Some) does use device.
        let mull = MullComConfig::default();
        assert!(mull.oauth2.is_some() && mull.oidc.is_none());
        assert!(cli_should_use_device(&mull, LoginTransportOverride::ForceDevice).await);
    }
    ...
    fn oidc_auth(issuer: &str) -> MullAuth {
        MullAuth {
    ...
    fn oidc_cred_with_matching_issuer_is_compatible() {
        ...
        assert!(is_cached_credential_compatible(
            &oidc_auth(PALMSHED_OAUTH2_ISSUER),
            &cfg,
        ));
    }
    ...
    fn pinned_cfg(team: &str) -> MullComConfig {
        MullComConfig {
    ...
    async fn run_auth_flow_uses_valid_disk_token_when_expired() {
        ...
        let valid_disk = MullAuth {
            key: "fresh-token-from-disk".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() + chrono::Duration::hours(1)),
            refresh_token: Some("new-rt".into()),
            oidc_issuer: Some(PALMSHED_OAUTH2_ISSUER.into()),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let mut cfg = MullComConfig::default();
        cfg.oauth2.as_mut().unwrap().issuer = "http://127.0.0.1:1".into();

        let writer = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_no_rt = MullAuth {
        ...
        // MULL_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into Mull: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `mull login` passes `on_stderr=None`; stderr must be inherited so
