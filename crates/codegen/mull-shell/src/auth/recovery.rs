...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&MullAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        mull_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<MullAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<MullAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<MullAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.mull_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<MullAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            mull_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<MullAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<MullAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &MullAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, MullAuth};
    ...
    fn rejected_cred() -> Option<MullAuth> {
        Some(MullAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), MullComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(m: &Arc<AuthManager>) -> (Result<MullAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.mull_com_config().auth_scope();
        let fresh = MullAuth {
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.mull_com_config().auth_scope();
        let expired_different = MullAuth {
