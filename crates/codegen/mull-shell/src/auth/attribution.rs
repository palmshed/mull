...
//!
//! 1. [`mull_telemetry::unified_log::warn`] for the local
//!    `~/.mull/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`mull_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`mull_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`mull_sampler::SamplerConfig::attribution_callback`] at every
...
use mull_sampler::{Auth401AttributionCallback, SamplingConsumer};
use mull_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`mull_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `mull_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn mull_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `mull_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `mull-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `mull-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.mull/logs/unified.jsonl) + scrubbed
    ...
    mull_telemetry::unified_log::warn("auth 401 attribution", session_id, Some(payload.clone()));
...
/// and the mint/expiry fields from the resulting `MullAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `MullAuth` (or
    ...
    // `mull_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.mull/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> MullAuth {
        MullAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    /// `ShellAttribution` implements `mull_tools::Auth401AttributionCallback`
