...
use super::super::config::{MullComConfig, OidcAuthConfig};
use super::super::{AuthManager, MullAuth};
use super::protocol::{
    OidcError, build_authorize_url, build_mull_auth, discover, enforce_login_principal,
...
fn callback_response(result: &CallbackResult) -> (StatusCode, Html<String>) {
    let (title, message) = match result {
        Ok(_) => ("Signed in", "You can close this window and return to Mull."),
...
pub async fn run_login_flow_with_config(
    ...
    let callback_port: u16 = if super::super::config::use_local_auth() {
        56121
    } else if super::super::config::is_github_issuer(&oidc.issuer) {
        56122
    } else {
        0
    };
    ...
    let oauth2 = auth_manager.mull_com_config().oauth2.as_ref();
    ...
    if has_client_ui {
        ...
        eprintln!();
        let provider_label = if oidc.issuer == super::super::config::PALMSHED_OAUTH2_ISSUER {
            "Mull".to_owned()
        } else {
            oidc.issuer.clone()
        };
    ...
    let principal_policy = login_principal_policy(auth_manager.mull_com_config());
    ...
    let mut auth = build_mull_auth(tokens, user_info, &oidc.issuer, &oidc.client_id);
...
mod tests {
    ...
    async fn full_login_flow_via_race() {
        ...
        let auth_manager = Arc::new(
            AuthManager::new(temp_dir.path(), MullComConfig::default())
        ...
        .await
        .unwrap();
        let auth = build_mull_auth(tokens, user_info, &oidc_cfg.issuer, &oidc_cfg.client_id);
