...
use super::super::config::{ForceLoginTeam, MullComConfig, OAuth2ProviderConfig, OidcAuthConfig};
use super::super::{AuthMode, MullAuth};
use base64::Engine;
...
pub fn is_configured(config: &MullComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &MullComConfig) -> Option<ForceLoginTeam> {
...
pub(super) async fn discover(issuer: &str) -> anyhow::Result<Discovery> {
    ...
    if super::super::config::is_github_issuer(&issuer_key) {
        let doc = Discovery {
            authorization_endpoint: super::super::config::GITHUB_AUTHORIZE_URL.to_owned(),
            token_endpoint: super::super::config::GITHUB_TOKEN_URL.to_owned(),
            jwks_uri: None,
        ...
        tracing::debug!("OIDC: using hardcoded GitHub OAuth endpoints");
        return Ok(doc);
    }
...
pub(super) fn build_authorize_url(
    ...
    let is_github = super::super::config::is_github_issuer(&config.issuer);
    let scopes = config.scopes.join(" ");
    if is_github {
        // GitHub OAuth: simpler authorize URL, no nonce/audience/referrer
        let mut url = format!(
...
pub(super) async fn exchange_code(
    ...
    let is_github = token_endpoint == super::super::config::GITHUB_TOKEN_URL;
    let client_secret = if is_github {
        super::super::config::github_client_secret()
    } else {
        None
    };
    tracing::debug!(
        token_endpoint = % token_endpoint, is_github = is_github, has_secret = client_secret.is_some(), "OIDC: exchanging code for tokens"
    );
    ...
    if !is_github {
        params.push(("code_verifier", code_verifier.to_owned()));
    }
    let resp = with_alpha_test_key(
        req.form(&params)
...
pub(super) async fn extract_user_info(
    ...
    // GitHub OAuth: no id_token, fetch user info from GitHub API
    let is_github = super::super::config::is_github_issuer(expected_issuer);
    if is_github && id_token.is_none() {
        return fetch_github_user_info(expected_client_id)
            .await
            .map(|mut user_info| {
...
pub async fn fetch_github_user_info(access_token: &str) -> anyhow::Result<OidcUserInfo> {
    let resp = crate::http::shared_client()
        .get(super::super::config::GITHUB_USER_API_URL)
        .header("accept", "application/json")
        .header("user-agent", "mull")
    ...
    if !resp.status().is_success() {
        ...
        return Err(anyhow::anyhow!(
            "GitHub user info fetch failed: HTTP {status} — {body}"
        ));
    }
    let user: GitHubUser = resp.json().await?;
    let (first_name, last_name) = match user.name {
        Some(name) => {
...
mod tests {
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "mull-cli:access".into()],
            audience: None,
        };
        ...
        let discovery = Discovery {
            authorization_endpoint: "https://auth.palmshed.ai/authorize".into(),
            token_endpoint: "https://auth.palmshed.ai/token".into(),
            jwks_uri: None,
            id_token_signing_alg_values_supported: None,
        };
        let pkce = Pkce {
        ...
        assert!(url.contains("referrer=mull"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("mull-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.palmshed.ai/authorize".into(),
            token_endpoint: "https://auth.palmshed.ai/token".into(),
        ...
        assert!(url.contains("referrer=mull-desktop"));
        assert!(!url.contains("referrer=mull-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!(
            { "sub" : "user-42", "iss" : "https://auth.palmshed.ai", "aud" : "test-client",
            "exp" : 9999999999u64, "iat" : 1000000000u64, "scope" :
            "offline_access mull-cli:access api:access", "principal_type" : "Team",
