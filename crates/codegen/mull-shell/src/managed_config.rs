...
pub const MANAGED_ARTIFACT_FILES: [&str; 3] = [
    mull_config::MANAGED_CONFIG_FILENAME,
    mull_config::REQUIREMENTS_FILENAME,
    mull_config::signed_policy::SIGNATURE_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            mull_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", mull_config::MANAGED_CONFIG_CACHE_FILE),
        format!("{}.", mull_config::signed_policy::SIGNATURE_SIDECAR_FILE),
    ];
...
/// would just 401).
fn eligible_team_principal(auth: MullAuth) -> Option<MullAuth> {
...
/// config is a palmshed.ai feature with one palmshed.ai auth.
fn read_active_team_auth() -> Option<MullAuth> {
    let home = crate::util::mull_home::mull_home();
...
/// files (TUI tick vs `mull login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `mull setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `MULL_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("MULL_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `mull-http` classifier.
...
/// Override with `MULL_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("MULL_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** MullBuildDeployment UUID (the id
...
/// Resolve deployment key from `MULL_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (mull setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("MULL_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.mull/`, trying the
...
/// read-only `mull setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if mull_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(mull_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        mull_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
    }
    ...
    clear_squatting_dir(&home.join(mull_config::MANAGED_CONFIG_CACHE_FILE));
...
/// Post-login hook for `mull login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<MullAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `mull setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(MullAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// are the non-forgeable layers. Recovery: reconnect / `mull setup`; ceasing to serve
...
/// Outcome of the `mull setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.mull`.
    Installed,
...
/// Result of `mull setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `mull setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `mull setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `mull setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `mull setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && mull_config::signed_policy::verification_active()
...
/// Run the `mull setup` sync for the current principal. The caller must check
