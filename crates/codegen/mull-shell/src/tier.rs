...
//! (SuperMull, SuperMull Heavy/Lite, X Premium/+, and any unknown future name)
...
mod tests {
    ...
    fn unrestricted_names() {
        assert!(!is_restricted_tier_name("SuperMull"));
        assert!(!is_restricted_tier_name("SuperMull Heavy"));
        assert!(!is_restricted_tier_name("supermull_lite"));
