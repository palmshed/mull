...
async fn subagent_inherits_session_cli_overrides() {
    use mull_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    ...
    const PIN: &str = mull_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
async fn emit_subagent_notification_stamps_one_event_id_on_both_paths() {
    ...
    let broadcast_id = match gateway_rx.try_recv().expect("broadcast must fire") {
        mull_acp_lib::AcpClientMessage::ExtNotification(args) => {
...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
        resume_worktree_action(true, Some("refs/mull/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
    assert_eq!(
        resume_worktree_action(false, Some("refs/mull/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn mull_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let auto_wake = mull_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
...
fn dummy_tracker(
    ...
    let cwd = mull_paths::AbsPathBuf::new(PathBuf::from("/tmp")).unwrap();
    let fs: Arc<dyn mull_workspace::file_system::AsyncFileSystem> = Arc::new(
        mull_workspace::file_system::LocalFs::new(PathBuf::from("/tmp")),
    );
    ...
    let handle = SessionHandle {
        ...
        max_turns: None,
        hunk_tracker_handle: mull_hunk_tracker::HunkTrackerHandle::noop(),
...
fn explicit_override_takes_precedence_over_role() {
    let overrides = SubagentRuntimeOverrides {
        ...
        capability_mode: Some(mull_tool_types::SubagentCapabilityMode::All),
        ..Default::default()
    };
    let role = mull_subagent_resolution::config::SubagentRole {
    ...
    assert_eq!(
        resolved.capability_mode, Some(mull_tool_types::SubagentCapabilityMode::All)
    );
}
#[test]
fn role_default_used_when_no_explicit_override() {
    ...
    assert_eq!(
        resolved.capability_mode, Some(mull_tool_types::SubagentCapabilityMode::ReadOnly)
    );
}
#[test]
...
fn partial_override_fills_from_role() {
    ...
    assert_eq!(resolved.model.as_deref(), Some("explicit-model"));
    assert_eq!(
        resolved.capability_mode, Some(mull_tool_types::SubagentCapabilityMode::Execute)
    );
}
#[test]
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = mull_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = mull_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(mull_sampling_types::synthesized_reasoning_item("deliberating",)),
...
fn resume_vs_fork_helper_shapes_differ() {
    ...
    assert!(
        ...
        mull_sampling_types::conversation::ContentPart::Text { text } if text
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use mull_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
        ConversationItem::system("parent system"),
...
async fn handle_subagent_request_rejects_file_as_cwd() {
    ...
    let tmp_file = tmp_dir.path().join("mull-test-cwd-file");
...
fn subagent_await_budget_default_and_override() {
    unsafe { std::env::remove_var("MULL_SUBAGENT_AWAIT_BUDGET_MS") };
    assert_eq!(SUBAGENT_AWAIT_BUDGET, std::time::Duration::from_secs(600));
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("MULL_SUBAGENT_AWAIT_BUDGET_MS", "1500") };
    assert_eq!(subagent_await_budget(), std::time::Duration::from_millis(1500));
    unsafe { std::env::set_var("MULL_SUBAGENT_AWAIT_BUDGET_MS", "0") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("MULL_SUBAGENT_AWAIT_BUDGET_MS", "not-a-number") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::remove_var("MULL_SUBAGENT_AWAIT_BUDGET_MS") };
}
#[test]
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("MullBuild:read_file");
...
/// Regression: on the DEFAULT mull-build host —
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    use mull_agent::config::BuiltinAgentName;
    let mut ctx = ctx_with_toggle(HashMap::new());
    ctx.parent_agent_name = Some("ai-oncall-bot".to_string());
    ctx.parent_model_agent_type = Some(
        BuiltinAgentName::MullBuildPlan.as_ref().to_string(),
    );
...
async fn background_unknown_type_emits_subagent_finished_notification() {
    ...
    while let Ok(msg) = gateway_rx.try_recv() {
        if let mull_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            ...
            assert_eq!(req.method.as_ref(), "palmshed.ai/session_notification");
...
async fn cancel_pending_at_promote_removes_fresh_worktree_preserves_resumed() {
    ...
    mull_fast_worktree::WorktreeBuilder::new(&repo, &fresh)
        .standalone(true)
        .create()
        .unwrap();
    assert!(fresh.exists());
    ...
    mull_fast_worktree::WorktreeBuilder::new(&repo, &resumed)
        .standalone(true)
        .create()
        .unwrap();
...
fn subagent_auth_type_rule() {
    use crate::agent::auth_method::{CACHED_TOKEN_AUTH_METHOD_ID, PALMSHED_API_KEY_METHOD_ID};
    use mull_chat_state::AuthType;
    let session = acp::AuthMethodId::new(CACHED_TOKEN_AUTH_METHOD_ID);
    let api_key = acp::AuthMethodId::new(PALMSHED_API_KEY_METHOD_ID);
    let byok = byok_model_entry("mull-byok");
    let plain = test_model_entry("mull-plain");
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(! error.contains("mull models"));
    }
    assert!(
...
fn test_sampling_config(model_slug: &str) -> mull_sampling_types::SamplingConfig {
...
fn spawn_test_parent_chat_state(model_slug: &str) -> mull_chat_state::ChatStateHandle {
    let (mock, _persistence_rx) = mull_chat_state::MockChatPersistence::new();
    let (event_tx, _event_rx) = mpsc::unbounded_channel();
    let token = tokio_util::sync::CancellationToken::new();
    mull_chat_state::ChatStateActor::spawn(
        vec![],
