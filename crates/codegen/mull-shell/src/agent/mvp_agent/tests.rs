...
fn auth_with_mode(mode: crate::auth::AuthMode, key: &str) -> crate::auth::MullAuth {
    crate::auth::MullAuth {
        key: key.into(),
        auth_mode: mode,
...
fn resolve_subscription_tier_prefers_display_then_api_key_then_jwt() {
    ...
    let api = auth_with_mode(crate::auth::AuthMode::ApiKey, "palmshed-not-a-jwt");
    assert_eq!(
...
fn jwt_claim_matches_user_subscription_tier_known_pairs() {
    let cases = [
        ...
        ("supermull_heavy", "SuperMullPro"),
        ("supermull_lite", "SuperMullLite"),
    ];
...
fn jwt_claim_matches_user_subscription_tier_rejects_stale_and_unknown() {
    ...
    assert!(!jwt_claim_matches_user_subscription_tier("free", "MullPro"));
...
mod hunk_tracking_mode {
    ...
    use mull_hunk_tracker::TrackingMode;
    #[test]
...
fn harness_pair(id: &str) -> Vec<mull_sampling_types::conversation::ConversationItem> {
    use mull_sampling_types::ToolCall;
    use mull_sampling_types::conversation::ConversationItem;
    vec![
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_mull_build() {
    let prev = std::env::var("MULL_AGENT").ok();
    unsafe {
        std::env::remove_var("MULL_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("MULL_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return mull-build.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = mull_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `mull-build-plan`, models in the catalog that still declare
/// `agent_type = "mull"` explicitly must NOT preempt an ACP
/// profile. Any value in the `mull-build*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_mull_build_family() {
    let prev = std::env::var("MULL_AGENT").ok();
    unsafe {
        std::env::remove_var("MULL_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["mull", "mull-plan", "mull-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for mull-build family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("MULL_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use mull_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".mull").join("agents");
    ...
    assert_eq!(
        def.model,
        mull_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("MULL_AGENT", v) },
        None => unsafe { std::env::remove_var("MULL_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `mull-build` (a client profile name) to
/// `mull-build-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("mull", "mull-plan"));
    assert!(harnesses_are_compatible("mull-plan", "mull"));
    assert!(harnesses_are_compatible("mull", "mull"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("mull-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "mull-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_mull_build_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "mull-plan"),
        "mull-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_mull_build() {
    assert_eq!(
        resolve_required_agent_type(None, "mull"),
        "mull"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "mull-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "mull-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "mull-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use mull_tools::computer::local::{LocalFs, LocalTerminalBackend};
    use mull_tools::notification::ToolNotificationHandle;
    use mull_tools::registry::types::SessionContext;
    let tmp = tempfile::tempdir().unwrap();
    ...
    let builder = mull_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: mull_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: mull_tools::implementations::mull_build::image_gen::ImageGenConfig::default(),
        video_gen_config: mull_tools::implementations::mull_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: mull_tools::implementations::mull_build::deploy_app::AppBuilderDeployerConfig::default(),
        api_key_provider: None,
        auth_provider: None,
        attribution_callback: None,
        system_reminder_tag: mull_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = mull_hunk_tracker::HunkTrackerActor::spawn(
        "test".to_string(),
        std::path::PathBuf::from("/tmp"),
        hunk_event_tx,
        mull_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        max_turns: None,
        hunk_tracker_handle,
        chat_state_handle: mull_chat_state::ChatStateHandle::noop(),
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            mull_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(mull_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: mull_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "mull".to_string(),
        managed_mcp_proxy_base_url: String::new(),
        session_default_agent_profile: None,
        allowed_subagent_types: None,
        hook_registry: None,
        workspace_ops: mull_workspace::WorkspaceOps::for_test(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use mull_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use mull_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("grok-3", false, Some("mull-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("grok-3", false, Some("mull-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("mull-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("grok-3", true, Some("mull-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("grok-3", true, Some("mull-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("mull-tui"), false);
...
fn parse_code_nav_capability_present_and_true() {
    ...
    meta.insert(
        "palmshed.ai/codeNavigation".to_string(),
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("mull-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("mull-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::MullWeb) {
...
async fn ext_method_routes_auth_cleared_and_refreshes_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::MullAuth {
                key: "eligible".into(),
                auth_mode: crate::auth::AuthMode::WebLogin,
                ..crate::auth::MullAuth::test_default()
            });
            use acp::Agent as _;
            ...
            agent
                .ext_method(acp::ExtRequest::new(
                    "palmshed.ai/internal/auth_cleared",
...
fn build_minimal_agent_for_tests() -> MvpAgent {
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(temp_dir.path(), MullComConfig::default()));
...
fn build_agent_with_auth(auth: crate::auth::MullAuth) -> MvpAgent {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, MullComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let mull_home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("MULL_HOME", mull_home.path());
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(auth_home.path(), MullComConfig::default()));
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
    let gateway = GatewaySender::new(tx);
...
async fn wait_for_in_flight_load_returns_immediately_when_idle() {
    let agent = build_minimal_agent_for_tests();
    let sid = acp::SessionId::new("sess-mullone");
    tokio::time::timeout(
...
/// Drain the agent gateway, returning the first `palmshed.ai/sessions/changed`
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<mull_acp_lib::AcpClientMessage>,
) -> Option<crate::agent::roster::RosterChanged> {
    let mut found = None;
    while let Ok(msg) = rx.try_recv() {
        if let mull_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            if found.is_none()
...
/// an `palmshed.ai/sessions/changed` upsert carrying the *overridden* activity, so
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("mull-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("mull-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("mull-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("mull-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("mull-web"));
...
mod eligibility_gates {
    ...
    fn check_gates(
        ...
        if !matches!(client_type, ClientType::MullWeb) {
            return Err(CodeNavEligibility::ClientNotWeb);
        }
        if !code_nav_enabled {
            return Err(CodeNavEligibility::CapabilityNotAdvertised);
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::MullTui, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
    fn web_client_no_capability_rejected() {
        assert_eq!(
            check_gates(ClientType::MullWeb, false, true),
            Err(CodeNavEligibility::CapabilityNotAdvertised)
        );
    }
    #[test]
...
fn bg_line(task_id: &str) -> String {
    format!(
        r#"{{"timestamp":1,"method":"_palmshed.ai/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_backgrounded","task_id":"{task_id}","command":"sleep 99","cwd":"/tmp"}}}}}}"#
    )
}
fn completed_line(task_id: &str) -> String {
    format!(
        r#"{{"timestamp":2,"method":"_palmshed.ai/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_completed","task_snapshot":{{"task_id":"{task_id}","completed":true}}}}}}}}"#
    )
}
...
fn orphaned_tasks_ignores_unrelated_updates() {
    let tmp = tempfile::tempdir().unwrap();
    let bg = bg_line("t1");
    let unrelated = r#"{"timestamp":1,"method":"_palmshed.ai/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"auto_compact_started","percentage":80}}}"#;
...
fn orphaned_tasks_filters_rewind_dead_branches() {
    ...
    let rewind = r#"{"timestamp":3,"method":"_palmshed.ai/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":0,"created_at":"2025-01-01T00:00:00Z"}}}"#;
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::MULL_COM_METHOD_ID,
...
/// BYOK guard. Users with `api_key` must continue to report `ApiKey`
...
#[tokio::test(flavor = "current_thread")]
async fn auth_type_mull_api_key_no_current_returns_api_key() {
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::PALMSHED_API_KEY_METHOD_ID,
    ));
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(
        agent.auth_type(),
        mull_chat_state::AuthType::ApiKey,
        "api_key auth must report ApiKey -- BYOK has no session-token \
...
async fn auth_type_session_based_with_current_returns_session_token() {
    use crate::auth::MullAuth;
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::OIDC_METHOD_ID,
    ));
    agent.auth_manager.hot_swap(MullAuth::test_default());
    assert!(agent.auth_manager.current().is_some());
    assert_eq!(agent.auth_type(), mull_chat_state::AuthType::SessionToken,);
}
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), mull_chat_state::AuthType::ApiKey,);
}
...
/// Minimal agent whose `mull_com_config` engages the api-key kill switch
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.mull_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
/// Deployment-key / managed-config user: `PALMSHED_API_KEY` resolves and the kill
/// switch is off, so a dead `cached_token` MUST fall through to `api_key`
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    use crate::agent::auth_method::{PALMSHED_API_KEY_ENV_VAR, PALMSHED_API_KEY_METHOD_ID};
    use mull_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("MULL_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(PALMSHED_API_KEY_ENV_VAR, "test-deployment-key");
    let agent = build_minimal_agent_for_tests();
    assert_eq!(
        agent
        ...
        "deployment-key user (PALMSHED_API_KEY set, no kill switch) must fall \
         through to api_key on a dead cached_token -- not interactive login",
    );
}
/// Forced-IdP deployment: even with `PALMSHED_API_KEY` present, the admin kill
/// switch keeps the fallthrough on interactive `palmshed.ai` (api-key auth is
/// neither advertised nor an eligible fallthrough).
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{MULL_COM_METHOD_ID, PALMSHED_API_KEY_ENV_VAR};
    use mull_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("MULL_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(PALMSHED_API_KEY_ENV_VAR, "test-deployment-key");
    ...
    assert_eq!(
        ...
        "disable_api_key_auth must keep the cached_token fallthrough on \
         interactive palmshed.ai so PALMSHED_API_KEY can't bypass forced IdP login",
    );
}
/// No advertiseable credentials at all (no env key, no kill switch): the user
/// genuinely needs to log in, so the fallthrough is interactive `palmshed.ai`.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_mull_com_without_credentials() {
    use crate::agent::auth_method::{
        MULL_COM_METHOD_ID, LEGACY_PALMSHED_API_KEY_ENV_VAR, PALMSHED_API_KEY_ENV_VAR,
    };
    use mull_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("MULL_DISABLE_API_KEY_AUTH");
    let _new = EnvGuard::unset(PALMSHED_API_KEY_ENV_VAR);
    ...
    assert_eq!(
        agent
            ...
            .map(|id| id.0.as_ref()),
        Some(MULL_COM_METHOD_ID),
        "no API-key creds and no kill switch -> interactive palmshed.ai login",
    );
}
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use mull_tools::implementations::mull_build::video_gen::{
...
async fn prepare_image_gen_config_fails_open_without_auth() {
    use mull_tools::implementations::mull_build::image_gen::ImageGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
async fn data_collection_enabled_for_normal_user() {
    let agent = build_agent_with_auth(crate::auth::MullAuth::test_default());
    assert!(
...
fn parse_session_kind_matrix() {
    ...
    let cases: &[(&str, serde_json::Value, SessionKind)] = &[
        (
            "chat",
            json!({ "palmshed.ai/session" : { "kind" : "chat" } }),
            SessionKind::Chat,
        ),
        (
            "build",
            json!({ "palmshed.ai/session" : { "kind" : "build" } }),
            SessionKind::Build,
        ),
        (
            "chat_malformed_sibling",
            json!({ "palmshed.ai/session" : { "kind" : "chat", "facets" : "not-a-map" } }),
            SessionKind::Chat,
        ),
        (
            "unknown_kind",
            json!({ "palmshed.ai/session" : { "kind" : "frob" } }),
            SessionKind::Build,
        ),
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = mull_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = mull_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(mull_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<mull_workspace::permission::PermissionEvent>();
    agent
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::MULL_DISABLE_CUSTOM_BRIDGE_ENV);
    run_local_for_bridge_test(|| async {
        let agent = build_minimal_agent_for_tests();
        let params = serde_json::json!({ "sessionId" : "sess-local" });
        let err = agent
            .ext_method(acp::ExtRequest::new(
                "palmshed.ai/rewind/points",
                std::sync::Arc::from(serde_json::value::to_raw_value(&params).unwrap()),
            ))
            .await
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("mull-tui"));
...
/// Drive `palmshed.ai/internal/evict_sessions` through the real `ext_notification`
...
/// `palmshed.ai/internal/evict_sessions` notification — the realistic shape of a
...
async fn drive_disconnect_many(agent: &MvpAgent, sids: &[&acp::SessionId]) {
    ...
    agent
        .ext_notification(acp::ExtNotification::new(
            "palmshed.ai/internal/evict_sessions",
            raw.into(),
        ))
        .await
        .expect("evict_sessions notification must be handled");
}
/// Drive `palmshed.ai/session/close` through the real `ext_method` dispatch
...
/// Mixed batch in a *single* `palmshed.ai/internal/evict_sessions` notification —
...
/// `palmshed.ai/session/close` dispatch (`ext_method` → `handle_session_close`),
...
/// `palmshed.ai/folder_trust/request` round-trip.
fn build_agent_with_gateway_rx() -> (
    MvpAgent,
    tokio::sync::mpsc::UnboundedReceiver<mull_acp_lib::AcpClientMessage>,
) {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, MullComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
...
/// Pull the next `palmshed.ai/folder_trust/request` reverse-request off the gateway and
/// answer it with `outcome`. Returns the request's decoded params.
async fn answer_folder_trust_request(
    gw_rx: &mut tokio::sync::mpsc::UnboundedReceiver<mull_acp_lib::AcpClientMessage>,
    outcome: &str,
) -> serde_json::Value {
    ...
    let mull_acp_lib::AcpClientMessage::ExtMethod(args) = msg else {
        panic!("expected an ext_method reverse-request, got a different message");
    };
    assert_eq!(args.request.method.as_ref(), "palmshed.ai/folder_trust/request");
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use mull_test_support::EnvGuard;
    use mull_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("MULL_HOME", home.path());
    let _sim = EnvGuard::set(mull_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("MULL_FOLDER_TRUST");
...
fn interactive_trust_prompt_no_request_without_capability() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let sid = acp::SessionId::new("sess-mullocap");
        let (mut handle, _tx, _cmd_rx) = make_live_session_handle(&sid, None);
        handle.info.cwd = repo_path.to_string_lossy().to_string();
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        agent.maybe_spawn_interactive_trust_prompt(&sid, &repo_path, Some(&remote));
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, mull_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(mull_acp_lib::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use mull_test_support::EnvGuard;
    use mull_hooks_plugins_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("MULL_HOME", home.path());
    let _sim = EnvGuard::set(mull_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("MULL_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(mull_acp_lib::AcpClientMessage::ExtMethod(_)))
            ),
...
fn ann(id: &str) -> mull_announcements::RemoteAnnouncement {
    mull_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<mull_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = mull_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = mull_announcements::RemoteAnnouncement {
...
async fn emit_announcements_gate_emits_updates_baseline_and_bumps_gen() {
    ...
    let recv_gen =
        |rx: &mut tokio::sync::mpsc::UnboundedReceiver<mull_acp_lib::AcpClientMessage>| {
            ...
            let mull_acp_lib::AcpClientMessage::ExtNotification(args) = msg else {
                panic!("expected ExtNotification, got another message kind");
            };
            assert_eq!(args.request.method.as_ref(), "palmshed.ai/announcements/update");
...
mod direct_hub_cloud_removed {
    ...
    fn cloud_server_id_meta_is_hard_error() {
        let meta = serde_json::json!({ "palmshed.ai/cloud_server_id" : "srv-123" });
    ...
    fn cloud_server_id_null_still_present_is_hard_error() {
        let meta = serde_json::json!({ "palmshed.ai/cloud_server_id" : null });
        let err = reject_direct_hub_cloud_meta(meta.as_object()).expect_err("must reject");
        assert_direct_hub_error(err);
    }
    #[test]
    fn cloud_server_id_with_gateway_meta_still_hard_error() {
        let meta = serde_json::json!(
            { "palmshed.ai/cloud_server_id" : "srv-legacy", "envId" : "env-1",
            "palmshed.ai/cloud_existing_workspace" : { "server_id" : "ws-1", "cwd" :
            "/workspace" } }
        );
    ...
    async fn emit_settings_update_carries_permission_mode_from_cfg() {
        ...
        local
            .run_until(async {
                ...
                assert_eq!(args.request.method.as_ref(), "palmshed.ai/settings/update");
