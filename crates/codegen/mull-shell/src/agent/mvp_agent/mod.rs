...
use mull_acp_lib::AcpAgentGatewaySender as GatewaySender;
...
use mull_telemetry::id::{agent_id, agent_instance_id};
use mull_telemetry::session_ctx::log_event;
use mull_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use mull_workspace::permission::{ClientType, PermissionEvent};
...
use mull_sampler::SamplerConfig as SamplingConfig;
...
use mull_paths::AbsPathBuf;
use mull_workspace::session::git::GitDiscoveryResult;
use mull_hunk_tracker::HunkTrackerActor;
/// Hard-error message for legacy Direct hub-bind sessions (`palmshed.ai/cloud_server_id`).
...
pub(crate) fn reject_direct_hub_cloud_meta(
    ...
    if session_meta.and_then(|m| m.get("palmshed.ai/cloud_server_id")).is_some() {
...
/// If `persist_data` is provided, it will be included in the meta under `palmshed.ai/persist`.
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&crate::auth::MullAuth>,
) -> Option<String> {
...
/// `/user` already reports a newly qualifying tier (e.g. `SuperMullPro`). In
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "MullPro" => jwt_claim == "supermull",
        "XBasic" => jwt_claim == "x_basic",
        "XPremium" => jwt_claim == "x_premium",
        "XPremiumPlus" => jwt_claim == "x_premium_plus",
        "SuperMullPro" => jwt_claim == "supermull_heavy",
        "SuperMullLite" => jwt_claim == "supermull_lite",
        _ => false,
    }
}
...
/// `_meta["palmshed.ai/session"].kind` → [`SessionKind`]; absent/unknown/malformed → `Build`.
fn parse_session_kind(
    meta: Option<&acp::Meta>,
) -> crate::session::unified_list::SessionKind {
    use crate::session::unified_list::SessionKind;
    use serde::Deserialize;
    meta.and_then(|m| m.get("palmshed.ai/session"))
...
pub(crate) const SESSION_PLUGIN_DIRS_CAPABILITY_KEY: &str = "palmshed.ai/pluginDirs";
...
/// Used to stamp `palmshed.ai/leaderClientId` onto replay notifications so the leader can
...
fn mark_as_replay(
    ...
    if let Some(persist) = persist_data {
        obj.insert("palmshed.ai/persist".to_string(), persist.clone());
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a mull_sampling_types::TokenUsage>,
...
/// Typed payload for the `palmshed.ai/settings/update` notification sent to pager
...
fn announcements_push_payload(
    stored: Option<&[mull_announcements::RemoteAnnouncement]>,
    last_emitted: &[mull_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<mull_announcements::RemoteAnnouncement>> {
    let current = mull_announcements::filter_expired_at(
...
/// Override with `MULL_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("MULL_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
/// gates fails.  Used in `palmshed.ai/code/status` responses and to generate
...
pub enum CodeNavEligibility {
    ...
    ClientNotWeb,
    /// Client did not advertise `palmshed.ai/codeNavigation.enabled`.
...
pub struct MvpAgent {
    ...
    /// palmshed.ai chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// Whether the current client advertised `palmshed.ai/codeNavigation.enabled`.
    ...
    /// Whether the current client advertised `palmshed.ai/folderTrust.interactive` (it
    ...
    /// list. Set by `enforce_mull_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.mull/`).
    ///
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<mull_codebase_graph::IndexManagerHandle>>,
    >,
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        mull_tools::implementations::mull_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: mull_tools::implementations::mull_build::task::types::MonitorEventBuffer,
    ...
    subagent_roles: std::collections::HashMap<
        String,
        mull_subagent_resolution::config::SubagentRole,
    >,
    subagent_personas: std::collections::HashMap<
        String,
        mull_subagent_resolution::config::SubagentPersona,
    >,
    ...
    pub(crate) plugin_registry_handle: mull_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled mull-desktop's first
    ...
    workspace_ops: RefCell<Option<mull_workspace::WorkspaceOps>>,
    ...
    /// Announcements list last actually emitted via `palmshed.ai/announcements/update`
    ...
    last_emitted_announcements: RefCell<Vec<mull_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &mull_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<mull_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !mull_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = mull_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `mull-build` →
/// `mull-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `mull-build` →
/// `mull-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use mull_agent::config::is_strict_harness_agent_type;
    match (
...
use mull_chat_state::conversation_util::replace_or_insert_system_head;
...
///
/// Headers injected:
///  - `x-mull-client-version` -- kept for internal tracking.
///
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-mull-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
                .unwrap_or_else(|| mull_version::VERSION.to_string())
        });
    // OpenRouter migration: X-XAI-Token-Auth and x-authenticateresponse
    // headers removed. OpenRouter does not use these proxy auth headers.
...
/// Parse the client-advertised `palmshed.ai/hunkTracker.mode` string. Case-insensitive
/// and trimmed. Absent/blank/`off`/`disabled` => `None`; unknown => `AllDirty`.
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<mull_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(mull_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<mull_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    /// - Palmshed updates (`"_palmshed.ai/session/update"`) → `ExtNotification`.
    ///
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<mull_acp_lib::AcpResult<()>>,
        >,
        mark_replay: bool,
        ...
        let is_mull = method == "_palmshed.ai/session/update";
        if is_mull {
            if target_client_id.is_none() && !mark_replay {
                ...
                >(raw_params.get()) else {
                    tracing::debug!(
                        "replay: skipping Palmshed update with unparseable params"
                    );
                    return;
                };
                if let Some(obj) = params.as_object_mut() {
                    ...
                    if let Some(m) = meta.as_object_mut() {
                        ...
                        if let Some(pd) = persist_data {
                            m.insert("palmshed.ai/persist".to_string(), pd.clone());
                        }
                        if let Some(tid) = target_client_id {
                            m.insert("palmshed.ai/leaderClientId".to_string(), tid.clone());
                        }
                    }
                }
            ...
            if let Some(tid) = target_client_id {
                stamp_meta_value(&mut notification.meta, "palmshed.ai/leaderClientId", tid);
            }
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<mull_acp_lib::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = mull_tools::types::TaskSnapshot {
    ...
    ///
    /// Non-Palmshed auth (API keys, enterprise) always passes. For Palmshed OAuth2
    /// users, reads `allow_access` from remote settings. Defaults to
    /// `false` (blocked) when remote settings are unavailable.
    pub(super) async fn enforce_mull_code_access(&self, auth: &crate::auth::MullAuth) {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings mull_build_access_gate)"
            );
            self.retry_subscription_check().await;
        }
    }
    /// Single-shot subscription check called by the pager's "Check
    /// subscription" button (`palmshed.ai/auth/check_subscription`). The pager
    ...
    pub(crate) fn auth_response_with_meta(&self) -> AuthenticateResponse {
        ...
        let meta = self
            ...
            .current()
            .map(|auth| {
                let gate = if !self.tier_allowed.get()
                    && gate.is_none()
                    && auth.auth_mode != crate::auth::AuthMode::ApiKey
                {
                    let message = "A subscription is required.".to_string();
                    Some(crate::auth::GateInfo {
                        message,
                        url: Some(
                            "https://palmshed.ai/supergrok?referrer=mull-build".to_string(),
                        ),
    ...
    pub(super) async fn maybe_fetch_post_auth_settings(&self) {
        ...
        let is_mull_auth = auth.is_mull_auth();
        ...
        tracing::info!("post-auth remote_settings fetch succeeded");
        {
                ...
                && cfg.mode != crate::agent::config::AgentMode::Generic
            {
                ...
                if cfg.storage_mode == StorageMode::Writeback && !is_mull_auth {
    ...
    /// Fire-and-forget `palmshed.ai/settings/update` from the current remote snapshot.
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&payload) {
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new("palmshed.ai/settings/update", params.into()),
                );
        }
    }
    ...
    /// cached_token, palmshed.ai/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.mull/bundled/` and the manifest.
...
/// absent — matching the `mull_build_access_gate` flag's server-side default.
///
/// Used by both `enforce_mull_code_access` (initial login gate) and
