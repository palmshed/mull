...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = mull_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                mull_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    mull_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                mull_workspace::permission::AccessKind::Read(Some(".env".into())),
                mull_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        mull_workspace::permission::Decision::PolicyDeny(_)
                    ),
