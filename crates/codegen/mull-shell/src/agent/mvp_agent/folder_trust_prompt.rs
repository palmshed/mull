...
//! (`palmshed.ai/folder_trust/request`) that asks a GUI client (mull-desktop) to decide
...
//! `palmshed.ai/folderTrust.interactive` AND the folder-trust feature flag is on AND the
//! verdict is [`mull_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.mull/lsp.json` servers start on the NEXT session open (the durable grant
...
/// ACP `palmshed.ai/folder_trust/request` payload (agent → GUI client). Serialized as
...
/// ACP `palmshed.ai/folder_trust/request` response (GUI client → agent).
...
impl MvpAgent {
    /// Parse the `palmshed.ai/folderTrust.interactive` capability from an initialize
    ...
    pub(crate) fn parse_interactive_trust_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("palmshed.ai/folderTrust"))
    ...
    /// `palmshed.ai/folderTrust.interactive` AND [`folder_trust::prompt_warranted`]
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = mull_workspace::trust::workspace_key(cwd);
        ...
        let targets: Vec<ReloadTarget> = self
            ...
            .values()
            .filter(|h| {
                mull_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key
            })
            .map(|h| ReloadTarget {
        ...
        tokio::task::spawn_local(async move {
            ...
            let ext_request = acp::ExtRequest::new("palmshed.ai/folder_trust/request", raw_params.into());
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a mull_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a mull_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.mull/hooks`, `.cursor/hooks.json`),
...
mod tests {
    ...
    fn parse_interactive_trust_capability_present_and_true() {
        ...
        meta.insert(
            "palmshed.ai/folderTrust".to_string(),
