...
impl MvpAgent {
    ...
    /// session end — a terminal/explicit close (`palmshed.ai/session/close`). It must
    ...
    /// (`palmshed.ai/session/close`). Finalizes the cloud replica (genuine session
    ...
    /// `palmshed.ai/sessions/changed` notification with the session in `removed` so
    ...
    /// `palmshed.ai/sessions/changed` notification with the current entry in
    ...
    /// Emit an `palmshed.ai/sessions/changed` upsert for a resident session with an
    ...
    /// Fan an `palmshed.ai/sessions/changed` delta out to every attached client.
    ///
    ...
    /// the `palmshed.ai/sessions/changed` special-case in `leader/server.rs`).
