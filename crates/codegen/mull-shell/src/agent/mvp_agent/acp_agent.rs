...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        mull_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::mull_home::mull_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    mull_workspace::permission::cleanup_stale_permission_state(
        ...
        mull_workspace::trust::migrate_legacy_hook_trust();
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_mull_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("mull-web") => client_type = ClientType::MullWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("mull-code-extension") => client_type = ClientType::Extension,
                Some("mull-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        *self.client_type.borrow_mut() = client_type;
        ...
        tracing::info!(
            ...
            "code-nav capability initialized from initialize request; \
             index will start lazily on first palmshed.ai/code/* request if eligible"
        );
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        if !self.cfg.borrow().mull_com_config.api_key_auth_disabled()
            && auth_method::read_mull_api_key_env().is_err()
            && let Some(api_key) = crate::auth::read_api_key(
                &crate::util::mull_home::mull_home(),
            )
        {
            unsafe { std::env::set_var("OPENROUTER_API_KEY", &api_key) };
            tracing::info!("auth: loaded API key from auth.json (api_key scope)");
            mull_telemetry::unified_log::info(
                "auth: loaded API key from auth.json (api_key scope)",
                None,
                None,
            );
        }
        ...
        let has_external_api_key = auth_method::should_advertise_mull_api_key(
            disable_api_key_auth,
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.mull_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.mull_com_config.auth_provider_label.clone(),
                cfg.mull_com_config.auth_provider_command.is_some(),
                cfg.mull_com_config.oidc.is_some(),
                issuer,
            )
        };
        if has_enterprise_oidc {
            ...
            tracing::info!(
                ...
                "auth: advertising palmshed.ai auth method",
            );
        }
        let preferred_method = self.cfg.borrow().mull_com_config.preferred_method;
        ...
        mull_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!(
                    { "mull_home" : crate ::util::mull_home::mull_home().display()
        ...
        debug_assert!(
            ...
            "BYOK invariant violated: api_key MUST be auth_methods.first() \
            ...
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                .agent_capabilities(
                    acp::AgentCapabilities::new()
                        ...
                        .meta(
                            serde_json::json!(
                                { "palmshed.ai/fs_notify" : true, "palmshed.ai/hooks" : { "blockingEvents"
                                : [mull_hooks::event::HookEventName::PreToolUse],
                                "decisions" : ["deny"], }, }
                            )
                                .as_object()
                                .cloned(),
                ...
                .meta({
                    let metadata = parse_json_object_env("MULL_AGENT_METADATA");
                    serde_json::json!(
                        { "mullShell" : true, "defaultAuthMethodId" :
                        default_auth_method_id_wire, (mull_mcp::wire::MCP_SDK) :
                        ...
                        mull_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().mull_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::PALMSHED_API_KEY_METHOD_ID => {
                if self.cfg.borrow().mull_com_config.api_key_auth_disabled() {
                ...
                if sampling_config.api_key.is_none() {
                    if let Ok(api_key) = auth_method::read_mull_api_key_env() {
                        ...
                        .any(|m| m.has_own_credentials())
                    {
                        ...
                        return Err(
                            acp::Error::auth_required()
                                .data(
                                    "Set PALMSHED_API_KEY or add api_key/env_key to config.toml.",
                                ),
                        );
                    }
                }
                ...
                log_event(mull_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().mull_com_config.preferred_method, Some(crate
            ...
            auth_method::MULL_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let mull_ctx = self.auth_manager.mull_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_mull_oauth(cli_oauth);
                tracing::debug!(
                    ...
                    })?;
                {
                    ...
                    tracing::debug!(
                        "auth: palmshed.ai/oidc handler set api_key (SessionToken)"
                    );
                    mull_telemetry::unified_log::debug(
                        "auth: palmshed.ai/oidc handler set api_key (SessionToken)",
                        None,
    ...
    async fn new_session(
        ...
        mull_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || mull_telemetry::external::is_active() {
            ...
            let perm = if session_yolo_mode {
                mull_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                mull_telemetry::enums::PermissionMode::Auto
            } else {
                mull_telemetry::enums::PermissionMode::Ask
            };
            tokio::spawn(async move {
                let git = mull_telemetry::context::collect_git_context(&cwd_str);
                let ev = mull_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                mull_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match mull_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
        ...
        if let Some(obj) = meta.as_object_mut() {
            obj.insert("palmshed.ai/sessionConfig".to_string(), session_config_value);
            obj.insert("palmshed.ai/sessionDetail".to_string(), session_detail_value);
        }
        Ok(
    ...
    async fn load_session(
        ...
        let persist_data = arguments
            .meta
            .as_ref()
            .and_then(|m| m.get("palmshed.ai/persist"))
            .cloned();
        ...
        let git_root = mull_workspace::session::git::find_git_root_from_path(
                cwd.as_path(),
            )
            .ok();
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        mull_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        mull_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        mull_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => mull_telemetry::events::PlanModeState::Inactive,
        };
        ...
        let restore_code_requested = request_meta
            .as_ref()
            .and_then(|m| m.get("palmshed.ai/restore_code"))
            .and_then(|v| v.as_bool())
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            mull_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = mull_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : mull_workspace::session::git::RESTORE_CODE_LOG, session_id =
                % session_id.0, supplied_cwd = % cwd.as_str(), persisted_cwd = % summary
                .info.cwd, target_sha = % target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a mull worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            mull_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use mull_workspace::session::git::RestoreKind;
            let outcome = mull_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let load_envrc = {
            let skip_envrc = request_meta
                .as_ref()
                .and_then(|m| m.get("palmshed.ai/skip_envrc"))
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            if skip_envrc {
                false
        ...
        let preloaded_envrc = mull_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let prompt_display_cwd = request_meta
            .as_ref()
            .and_then(|m| m.get("palmshed.ai/display_cwd"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
        ...
        let persisted_str: &str = &persisted_model.0;
        let is_openrouter = persisted_str.starts_with("openrouter/") || persisted_str == "mull";
        let same_family_fallback = if is_openrouter {
            available.keys().find(|id| id.0.starts_with("openrouter/") || id.0.as_ref() == "mull").cloned()
        } else {
            available.keys().find(|id| !id.0.starts_with("openrouter/") && id.0.as_ref() != "mull").cloned()
        };
        ...
        if let Some(persist) = persist_data {
            response_meta_map.insert("palmshed.ai/persist".to_string(), persist);
        }
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    mull_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = mull_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = mull_workspace::session::git::detect_head_divergence(
            ...
            .and_then(|h| h.current_prompt_id.lock().ok().and_then(|g| g.clone()))
        {
            response_meta_map
                .insert(
                    "palmshed.ai/runningPromptId".to_string(),
        ...
        response_meta_map.insert("palmshed.ai/sessionConfig".to_string(), session_config_value);
        response_meta_map.insert("palmshed.ai/sessionDetail".to_string(), session_detail_value);
        ...
        if self.product_analytics_enabled() {
            log_event(mull_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            mull_file_utils::trace_context::link_current_span_to_meta(
        ...
        if let Some(ctx) = trace_context.clone() {
            ...
            let mut prompt_metadata = PromptMetadata {
                ...
                shell_version: Some(mull_version::VERSION.to_string()),
                workspace_type: None,
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: mull_file_utils::trace_context::current_traceparent(),
                json_schema,
                send_now,
                respond_to: tx,
                persist_ack: None,
            ...
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new(
                        "palmshed.ai/session/prompt_complete",
                        params.into(),
                    ),
                );
        }
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<mull_chat_state::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = mull_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = mull_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = mull_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            mull_file_utils::trace_context::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            "palmshed.ai/getApiKey" | "palmshed.ai/setApiKey" => {
                crate::extensions::auth::handle(self, &args).await
            }
            "palmshed.ai/session/info" | "palmshed.ai/session/close" | "palmshed.ai/session/list"
            | "palmshed.ai/sessions/list" => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            "palmshed.ai/workspaces/list" => {
                crate::agent::handlers::workspaces::handle(self, &args).await
            }
            "palmshed.ai/session/updates" => {
                crate::extensions::session_updates::handle(&args, &self.gateway).await
            }
            "palmshed.ai/session/load_history" => {
                crate::extensions::chat_conversation_history::handle(self, &args).await
            }
            "palmshed.ai/session/search" => {
                crate::extensions::session_search::handle(&args).await
            }
            "palmshed.ai/session/resolve_local_for_worktree_resume"
            | "palmshed.ai/session/rehydrate" => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            "palmshed.ai/session/rename" | "palmshed.ai/session/delete"
            | "palmshed.ai/session/update_mcp_servers" | "palmshed.ai/session/fork"
            | "palmshed.ai/internal/reload_all_mcp_servers"
            | "palmshed.ai/internal/reload_project_mcp_servers" | "palmshed.ai/internal/reload_skills"
            | "palmshed.ai/internal/reload_models" | "palmshed.ai/internal/reload_models_cache"
            | "palmshed.ai/internal/auth_cleared" | "palmshed.ai/plugins/reload"
            | "palmshed.ai/commands/list" => {
                crate::extensions::session_admin::handle(self, &args).await
            }
            "palmshed.ai/session/repair" => crate::extensions::repair::handle(self, &args).await,
            "palmshed.ai/memory/flush" | "palmshed.ai/memory/rewrite" => {
                crate::extensions::memory::handle(self, &args).await
            }
            "palmshed.ai/skills/refresh-baseline" => {
                self.refresh_skill_baseline_for_all_sessions();
                crate::extensions::to_ext_response(
                    Ok(serde_json::json!({ "ok" : true })),
                )
            }
            "palmshed.ai/interject" => crate::extensions::interject::handle(self, &args).await,
            "palmshed.ai/feedback" | "palmshed.ai/feedback/dismiss" | "palmshed.ai/btw" => {
                crate::extensions::feedback::handle(self, &args).await
            }
            "palmshed.ai/recap" => crate::extensions::recap::handle(self, &args).await,
            "palmshed.ai/cloud/terminate" => {
                crate::extensions::auth_gate::require_mull_auth(
                    &self.auth_manager,
                    "Authentication required",
                    "Run `mull login` to authenticate.",
                )?;
            ...
            "palmshed.ai/billing" => crate::extensions::billing::handle(self, &args).await,
            "palmshed.ai/auto-topup-rule" => {
                crate::extensions::billing::handle(self, &args).await
            }
            "palmshed.ai/share_session" => crate::extensions::share::handle(self, &args).await,
            "palmshed.ai/privacy/setCodingDataRetention" => {
                crate::extensions::privacy::handle(self, &args).await
            }
            "palmshed.ai/rollout/survey" => {
                crate::extensions::rollout::handle(self, &args).await
            }
            "palmshed.ai/prompt_history" => {
                crate::extensions::prompt_history::handle(self, &args).await
            }
            "palmshed.ai/suggest" => crate::extensions::suggest::handle(self, &args).await,
            "palmshed.ai/suggestPrompt" => crate::extensions::suggest::handle(self, &args).await,
            s if s.starts_with("palmshed.ai/auth/") => {
                crate::extensions::auth::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/session_summaries/") => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/git/worktree/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            s if s.starts_with("palmshed.ai/git/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::git::handle(self, &ops, &args).await
            }
            s if s.starts_with("palmshed.ai/compact_conversation") => {
                crate::extensions::memory::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/plugins/") => {
                crate::extensions::plugins::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/marketplace/") => {
                crate::extensions::marketplace::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/hooks/") => {
                crate::extensions::hooks::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/hunk-tracker/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::hunk_tracker::handle(self, &ops, &args).await
            }
            s if s.starts_with("palmshed.ai/pr/") => {
            ...
            s if s.starts_with("palmshed.ai/task/") => {
                crate::extensions::task::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/scheduler/") => {
                crate::extensions::task::handle_scheduler(self, &args).await
            }
            s if s.starts_with("palmshed.ai/subagent/") => {
                crate::extensions::task::handle_subagent(self, &args).await
            }
            s if s.starts_with("palmshed.ai/terminal/") => {
            ...
            s if s.starts_with("palmshed.ai/search/") => {
                crate::extensions::search::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/bundle/") => {
                crate::extensions::bundle::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/code/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::code_nav::handle(self, &ops, &args).await
            }
            s if s.starts_with("palmshed.ai/skills/") => {
            ...
            s if s.starts_with("palmshed.ai/review") => {
                crate::extensions::feedback::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/debug/") => {
                crate::extensions::debug::handle(self, &args).await
            }
            s if s.starts_with("palmshed.ai/rewind") => {
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "palmshed.ai/yolo_mode_changed"
        ...
        if args.method.as_ref() == "palmshed.ai/permissions/reset" {
        ...
        if args.method.as_ref() == "palmshed.ai/internal/evict_sessions" {
            self.handle_evict_sessions(&args.params).await;
        }
        if args.method.as_ref() == "palmshed.ai/toggle_plan_mode"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if matches!(
            args.method.as_ref(), "palmshed.ai/queue/remove" | "palmshed.ai/queue/reorder" |
            "palmshed.ai/queue/clear" | "palmshed.ai/queue/edit" | "palmshed.ai/queue/interject"
        )
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        ...
        if args.method.as_ref() == "palmshed.ai/terminal/pty/input"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if args.method.as_ref() == "_palmshed.ai/session/update" {
            if let Ok(notification) = serde_json::from_str::<
                SessionNotification,
            >(args.params.get()) {
                tracing::info!(
                    "Storing Palmshed session notification: session_id={}", notification
                    .session_id.0
                );
                    ...
                    .get(&notification.session_id)
                {
                    ...
                    tracing::warn!(
                        "Received Palmshed session notification for unknown session: {}",
                        notification.session_id.0
                    );
                }
            } else {
                tracing::warn!("Failed to parse Palmshed session notification params");
            }
        }
        if args.method.as_ref() == "palmshed.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "palmshed.ai/telemetry/multi_agent_followup" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| mull_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "palmshed.ai/telemetry/multi_agent_apply" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "palmshed.ai/telemetry/multi_agent_discard" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                mull_telemetry::session_ctx::log_event(mull_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| mull_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == mull_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                mull_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            mull_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
