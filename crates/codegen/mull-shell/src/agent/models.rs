...
use crate::auth::{AuthManager, MullAuth, MullComConfig};
use crate::remote::{FetchModelsResult, fetch_models_blocking};
use crate::sampling::SamplerConfig as SamplingConfig;
use globset::{Glob, GlobSet, GlobSetBuilder};
use mull_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl ModelFetchAuth {
    ...
    ///
    /// A `deployment_key` outranks an ambient `PALMSHED_API_KEY` so a stray env key
    ...
    pub(crate) fn resolve(endpoints: &config::EndpointsConfig, has_cached_session: bool) -> Self {
        if endpoints.has_custom_endpoint() {
            ...
            Self::Deployment
        } else if crate::agent::auth_method::has_mull_api_key_env() {
            Self::ApiKey
        } else {
            Self::Session
        }
...
struct Inner {
    ...
    gateway: RwLock<Option<mull_acp_lib::AcpAgentGatewaySender>>,
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&mull_home, MullComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: mull_acp_lib::AcpAgentGatewaySender) {
    ...
    pub fn apply_config(&self, new_config: config::Config) {
        ...
        *self.inner.models.write() = new_catalog;
        ...
        // Push the new catalog to connected clients (`palmshed.ai/models/update`).
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<mull_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<mull_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    /// differ from the slug for custom/enterprise ids (e.g. key `enterprise-mull-build`
    ...
    /// Hot-reload the catalog from `~/.mull/models_cache.json` after an
    ...
    /// When another mull process sharing `~/.mull` (a `--no-leader` run, a
    /// newer client, mull-desktop) fetches a fresher `/v1/models` catalog and
    ...
    /// `ModelsCacheManager` path is fixed to `mull_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::mull_home::mull_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.mull_version.as_deref() != Some(mull_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            mull_version: Some(mull_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "Mull" both route to `mull-build`)
...
pub(crate) fn prefetch_models_and_settings_blocking(
    ...
    // Settings need a palmshed.ai session; skip for BYOK.
    ...
    match fetch_models_blocking(endpoints, auth, fetch_auth) {
        Ok(FetchModelsResult { models, etag }) if !models.is_empty() => {
            let api_base_url_override = match fetch_auth {
                ModelFetchAuth::ApiKey => Some(endpoints.mull_api_base_url.clone()),
                _ => None,
            };
...
fn resolve_prefetch_env_with_auth(auth: Option<MullAuth>) -> Option<PrefetchEnv> {
    let _timer = crate::instrumentation_timer!("startup.early_prefetch_launch");
    // Config-aware (not env-only) so the prefetch can't leak the bearer to api.palmshed.ai.
...
/// explicit off switch must hold even when a stray login, `PALMSHED_API_KEY`, or
...
fn resolve_prefetch_env(mull_com_config: Option<MullComConfig>) -> Option<PrefetchEnv> {
    let mull_home = crate::util::mull_home::mull_home();
    let auth_manager = AuthManager::new(&mull_home, mull_com_config.unwrap_or_default());
...
pub fn start_early_prefetch_with_auth(auth: Option<MullAuth>) -> Option<EarlyPrefetchHandle> {
...
pub fn start_early_prefetch(mull_com_config: Option<MullComConfig>) -> Option<EarlyPrefetchHandle> {
    let env = resolve_prefetch_env(mull_com_config)?;
    Some(spawn_prefetch_thread(env))
}
...
/// the catalog and `/model` picker use config keys (e.g. `enterprise-mull-build`).
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let tmp = std::env::temp_dir().join("mull-test-models-manager");
        let auth_manager = Arc::new(AuthManager::new(&tmp, MullComConfig::default()));
        ModelsManager::new(
            None,
            IndexMap::new(),
            acp::ModelId::new("default"),
    ...
    fn current_reasoning_effort_seeded_from_config() {
        let tmp = std::env::temp_dir().join("mull-test-models-manager-seed");
        let auth_manager = Arc::new(AuthManager::new(&tmp, MullComConfig::default()));
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use mull_sampling_types::ReasoningEffortOption;
    ...
    fn config_menu_only_model_derives_support_and_default() {
        ...
        let tmp = std::env::temp_dir().join("mull-test-models-manager-menu-only");
        let auth_manager = Arc::new(AuthManager::new(&tmp, MullComConfig::default()));
        let mgr = ModelsManager::new(
            None,
            catalog,
    ...
    fn apply_config_falls_back_when_preferred_not_in_catalog() {
        ...
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("mull-nonexistent".to_string());
        mgr.apply_config(new_cfg);
    ...
    fn reload_from_disk_cache_ignores_stale_cache() {
        ...
        let stale = ModelsCache {
            ...
            models: make_prefetched(&["mull-stale"]),
        };
        cache.atomic_write(&stale);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("mull-stale"));
    ...
    fn reload_from_disk_cache_ignores_auth_method_mismatch() {
        ...
        cache.persist(
            &make_prefetched(&["mull-other-auth"]),
            Some("etag-x"),
            other,
            &mgr.cache_origin(),
        );
        ...
        assert!(!mgr.models().contains_key("mull-other-auth"));
    }
    ...
    fn reload_from_disk_cache_ignores_origin_mismatch() {
        ...
        let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
        cache.persist(
            &make_prefetched(&["mull-other-origin"]),
            Some("etag-y"),
            auth_method,
        ...
        assert!(!mgr.models().contains_key("mull-other-origin"));
        assert!(mgr.inner.etag.read().is_none());
    }
    ...
    fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
        ...
        let legacy = ModelsCache {
            ...
            models: make_prefetched(&["mull-legacy"]),
        };
        cache.atomic_write(&legacy);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("mull-legacy"));
    }
    ...
    fn resolve_custom_endpoint_always_wins() {
        let _key = EnvGuard::set("PALMSHED_API_KEY", "test-key");
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        let _unset = EnvGuard::unset("PALMSHED_API_KEY");
        let _unset_legacy = EnvGuard::unset("MULL_CODE_PALMSHED_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    /// `deployment_key` outranks a stray `PALMSHED_API_KEY`, but session wins over both.
    #[test]
    #[serial]
    fn resolve_deployment_key_outranks_ambient_api_key() {
        let _key = EnvGuard::set("PALMSHED_API_KEY", "stray-env-key");
        let endpoints = config::EndpointsConfig {
            deployment_key: Some("deploy-key".to_owned()),
            ..config::EndpointsConfig::default()
        };
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Deployment,
            "managed deployment_key should outrank an ambient PALMSHED_API_KEY",
        );
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, true),
            ModelFetchAuth::Session,
    ...
    /// once — session auth, ambient `PALMSHED_API_KEY`, `deployment_key`, AND a
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "mull", Some("Auto")),
            make_entry_config_with_id(Some("mull"), "mull", Some("Mull")),
            make_entry_config_with_id(
                Some("mull-composer-2.5-fast"),
                "mull-composer-2.5-fast",
                Some("Mull Fast"),
            ),
        ];
        ...
        assert!(map.contains_key("mull"));
        assert!(map.contains_key("mull-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "mull",
            "auto entry should still route to mull-build"
        );
        assert_eq!(map["mull"].info.model, "mull");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("mull"), "mull", Some("First")),
            make_entry_config_with_id(Some("mull"), "mull", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["mull"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        catalog.insert("auto-mull-build".to_string(), make_model_entry("mull"));
        catalog.insert("mull".to_string(), make_model_entry("mull"));

        let mut cfg = config::Config::default();
        cfg.models.default = Some("mull".to_string());

        let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
        assert_eq!(key, "mull", "must match id, not first slug hit");
    }

    /// No id field — falls back to slug as key.
    #[test]
    fn build_prefetched_map_none_id_falls_back_to_slug() {
        let entries = vec![make_entry_config_with_id(None, "mull", Some("Mull"))];
        let map = build_prefetched_map(entries, None);
    ...
    fn resolve_catalog_key_maps_routing_slug_to_config_key() {
        ...
        models.insert(
            "enterprise-mull-build".to_string(),
        ...
        assert_eq!(key.0.as_ref(), "enterprise-mull-build");
    }
    ...
    fn resolve_catalog_key_last_slug_match_wins() {
        let mut models = IndexMap::new();
        models.insert(
            "default-mull-build".to_string(),
            make_model_entry("grok-4.5"),
        );
        models.insert("user-mull-build".to_string(), make_model_entry("grok-4.5"));

        let persisted = acp::ModelId::new("grok-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "user-mull-build");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        let mut models = IndexMap::new();
        models.insert("mull".to_string(), make_model_entry("mull"));
        models.insert(
            "enterprise-mull-build".to_string(),
            make_model_entry("mull"),
        );
        models.insert("grok-4.3".to_string(), make_model_entry("grok-4.3"));

        let available = test_available_keys(&["enterprise-mull-build", "grok-4.3"]);

        let persisted = acp::ModelId::new("mull");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        let mut models = IndexMap::new();
        models.insert("mull".to_string(), make_model_entry("grok-4.5"));
        models.insert("other".to_string(), make_model_entry("mull"));

        let available = test_available_keys(&["mull", "other"]);

        let persisted = acp::ModelId::new("mull");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "mull");
    }
