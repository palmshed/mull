...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    if cfg.remote_settings.is_none()
        && let Some(handle) =
            crate::agent::models::start_early_prefetch(Some(cfg.mull_com_config.clone()))
    {
        match handle.join() {
            Ok(result) => {
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (mull -p, subagents).
    ...
    // Writeback talks to the code backend; requires palmshed.ai auth.
    if cfg.storage_mode == StorageMode::Writeback
        && !auth_manager.current().is_some_and(|a| a.is_mull_auth())
    {
        tracing::info!("Writeback is disabled: requires auth with palmshed.ai");
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        crate::builtin::extract_bundled_files(&mull_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&mull_home);
        }
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    let mull_auth = auth_manager.current().filter(|a| a.is_mull_auth());
    let user_id = mull_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = mull_auth.as_ref().and_then(|a| a.team_id.clone());
