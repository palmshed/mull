//! palmshed.ai chat-product model catalog: caches `/rest/modes` and maps modes to
...
/// ~54 min, matching mull-web's refetch cadence.
...
pub const MULL_CHAT_MODE_ENV: &str = "MULL_CHAT_MODE";
...
impl ChatModesManager {
    ...
    /// The active palmshed.ai identity, or `None` when unauthenticated. Modes are
...
/// Maps palmshed.ai modes → `SessionModelState`: keeps only `available` modes,
