...
use crate::auth::{AuthManager, MullComConfig, OidcAuthConfig};
use crate::remote::DEFAULT_CONTEXT_WINDOW;
use crate::{config::StorageMode, sampling::ApiBackend, tools::config::ShellToolsetConfig};
use agent_client_protocol as acp;
use indexmap::IndexMap;
use mull_agent::prompt::skills::SkillsConfig;
use mull_sampler::{AuthScheme, SamplerConfig};
use mull_sampling_types::{
...
pub const DEFAULT_AGENT_TYPE: &str = "mull-plan";
...
/// Default base URL for the cli chat proxy (now routed through OpenRouter).
pub const CLI_CHAT_PROXY_BASE_URL_DEFAULT: &str = "https://openrouter.ai/api/v1";
/// Default base URL for the public Palmshed API (now routed through OpenRouter).
pub const PALMSHED_API_BASE_URL_DEFAULT: &str = "https://openrouter.ai/api/v1";
/// Default base URL for the asset server (profile images, etc.).
pub const ASSET_SERVER_URL_DEFAULT: &str = "https://assets.palmshed.ai";
...
pub struct EndpointsConfig {
    ...
    /// Base URL for the public Palmshed API.
    pub mull_api_base_url: String,
    ...
    /// Env: `MULL_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `MULL_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `MULL_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `MULL_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `MULL_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `MULL_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `MULL_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `MULL_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `MULL_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `MULL_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `MULL_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `MULL_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `MULL_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub mull_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `MULL_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    /// internal export (debug). Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub mull_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via
    /// [`external_otel_master_switch_resolved`] — the same layered resolution
    /// (requirement pin > `MULL_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
pub(crate) fn default_asset_server_url() -> String {
    std::env::var("MULL_ASSET_SERVER_URL").unwrap_or_else(|_| ASSET_SERVER_URL_DEFAULT.to_owned())
}
...
/// format, shared with `MULL_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    /// the public default. NEVER falls back to `mull_api_base_url` — that is the
    ...
    /// cli-chat-proxy, never `mull_api_base_url`.
    ...
    /// Managed deployment-config URL (`mull setup`): explicit `managed_config_url`,
    /// else `proxy_url` + `/deployment/config`. Never `mull_api_base_url`, so the
    ...
    /// 1. `mull_internal_otlp_traces_endpoint` (verbatim)
    ...
    /// 3. `proxy_url` + `/traces`.
    /// Uses the proxy default (not the `mull_api_base_url` fallback) so
    /// telemetry reports to Palmshed even when inference is overridden. When the
    ...
    pub fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.mull_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 MULL_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// Extra headers for the INTERNAL export: `mull_internal_otlp_headers`
    ...
    pub fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.mull_internal_otlp_headers) {
    ...
    /// `mull_internal_otlp_*` override shadowed that half.
    ///
    ...
    pub fn internal_otlp_consumed_standard_vars(&self) -> bool {
        ...
        let endpoint_consumed = blank_as_unset(&self.mull_internal_otlp_traces_endpoint).is_none()
            && self.legacy_internal_otlp_traces_endpoint().is_some();
        let headers_consumed = blank_as_unset(&self.mull_internal_otlp_headers).is_none()
    ...
    /// still honored by the internal pipeline even with `MULL_EXTERNAL_OTEL`
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("MULL_CLI_CHAT_PROXY_BASE_URL").ok(),
            mull_api_base_url: std::env::var("MULL_PALMSHED_API_BASE_URL")
                .unwrap_or_else(|_| PALMSHED_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("MULL_MODELS_BASE_URL"),
            models_list_url: env_string("MULL_MODELS_LIST_URL"),
            feedback_base_url: env_string("MULL_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("MULL_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("MULL_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("MULL_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("MULL_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("MULL_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("MULL_DEPLOYMENT_KEY"),
            managed_config_url: env_string("MULL_MANAGED_CONFIG_URL"),
            ...
            mull_internal_otlp_traces_endpoint: env_string("MULL_INTERNAL_OTLP_TRACES_ENDPOINT"),
            mull_internal_otlp_headers: env_string("MULL_INTERNAL_OTLP_HEADERS"),
...
pub use mull_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
fn resolve_compaction_mode_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> mull_chat_state::CompactionMode {
    use mull_chat_state::CompactionMode;
...
fn resolve_compaction_detail_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> mull_chat_state::CompactionDetail {
    use mull_chat_state::CompactionDetail;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(mull_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: mull_tools::types::compat::CompatCell,
...
pub(crate) use mull_telemetry::config::env_telemetry_mode;
pub use mull_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.mull/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                mull_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> mull_agent::plugins::discovery::DiscoveryConfig {
        mull_agent::plugins::discovery::DiscoveryConfig {
...
pub struct ModelsConfig {
    ...
    /// Unset = remote pin, then the client hint / built-in `mull-0.1`
...
/// (`mull workspace` / leader `with_default_hub_url`). Does **not** enable
/// agent-side harness/client connections or alter local session behavior.
///
/// ```toml
/// [hub]
/// url = "wss://hub.palmshed.ai/ws"
/// ```
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct HubConfig {
    /// Hub WebSocket URL (`ws://` or `wss://`) used as the leader default for
    /// `mull workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "MULL_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("MULL_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("MULL_SUGGESTIONS_AI")
    ...
    pub fn resolve_ai_model(&self) -> String {
        resolve_string_flag(
            None,
            "MULL_SUGGESTIONS_AI_MODEL",
            self.ai_model.as_deref(),
            None,
        )
        .map(|r| r.value)
        .unwrap_or_else(|| "mull".to_owned())
    }
}
...
///
/// These supplement the built-in scan locations (`.mull/skills/`,
/// `.agents/skills/`, `~/.mull/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    /// Typed as `MullComConfig` (same schema) so sub-field typos are caught.
    #[serde(default, skip_serializing)]
    pub auth: Option<MullComConfig>,
    /// `[desktop]` section — owned by mull-desktop (Electron app), opaque to the CLI agent.
    #[serde(default, skip_serializing)]
    pub desktop: Option<toml::Value>,
    /// Top-level `announcements` array — consumed by `resolve_announcements`.
    #[serde(default, skip_serializing)]
    pub announcements: Vec<mull_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `mull_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from MULL_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<mull_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `MULL_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    /// and `.mull/roles/*.toml` file discovery.
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, mull_subagent_resolution::config::SubagentRole>,
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, mull_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<mull_tools::implementations::mull_build::video_gen::ZdrVideoOutputS3Config>,
    ...
    /// Image describe model (`mull-build` default via `ModelOverrideConfig::resolve`).
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<mull_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<mull_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut mull_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut mull_agent::config::AgentDefinition) {
...
pub use mull_agent::config::AgentDefinition;
pub use mull_agent::config::Effort;
pub use mull_agent::config::PermissionMode;
pub use mull_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .mull/agents/, ~/.mull/agents/, built-ins)
...
/// 4. `MULL_AGENT` env var
/// 5. Default `mull-build` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `mull_agent::discovery::by_name_in_cwd()`.
    /// Examples: "mull", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// When set, the agent is loaded from this file.
    /// Supports environment variable expansion (e.g., `$HOME/.mull/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            mull_com_config: MullComConfig::default(),
            shortcuts: None,
            hints: None,
            ui: UiConfig::default(),
            ...
            client_version: Some(mull_version::VERSION.to_string()),
...
impl Config {
    ...
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new(
            &crate::util::mull_home::mull_home(),
            self.mull_com_config.clone(),
        )
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        if config.mull_com_config.oidc.is_none() {
            config.mull_com_config.oidc = OidcAuthConfig::from_env();
        }
        if config.mull_com_config.oidc.is_none() && config.mull_com_config.oauth2.is_none() {
            config.mull_com_config.oauth2 = crate::auth::OAuth2ProviderConfig::from_env();
        }
        if let Some(ref mut oauth2) = config.mull_com_config.oauth2
            && crate::auth::config::is_github_issuer(&oauth2.issuer)
            && oauth2.scopes == crate::auth::config::default_oauth2_scopes()
        {
            oauth2.scopes = crate::auth::config::default_github_scopes();
        }
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("MULL_AUTO_WAKE")
    ...
    /// If the TOML contains `[auth]`, copy its contents under `[mull_com_config]`.
    /// `[mull_com_config]` takes precedence if both are present (explicit wins).
    ///
    /// This lets customers write the shorter `[auth.oidc]` instead of `[mull_com_config.oidc]`.
    fn expand_auth_alias(raw_config: &toml::Value) -> toml::Value {
        ...
        {
            if let Some(gcc) = table.get_mut("mull_com_config") {
                ...
                table.insert("mull_com_config".to_owned(), auth);
            }
        }
        config
    }
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("MULL_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `MULL_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MULL_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!(
            ...
            "in_env_trace_upload" : std::env::var("MULL_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled" : std::env::var("MULL_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MULL_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MULL_TWO_PASS_COMPACTION")
            .config(self.features.two_pass_compaction)
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    /// Server-side doom-loop check policy (the `x-mull-doom-loop-check`
    ...
    /// `MULL_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<mull_sampling_types::DoomLoopRecoveryPolicy> {
        use mull_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("MULL_DOOM_LOOP_RECOVERY")
    ...
    /// Gate first-run auto-registration of the official Palmshed marketplace source.
    /// Precedence: env `MULL_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MULL_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MULL_ASK_USER_QUESTION")
    ...
    ///
    /// Precedence: requirements > `MULL_VOICE_MODE` > config/managed
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use mull_tools::implementations::mull_build::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("MULL_IMAGE_EDIT").default(true).resolve()
    }
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("MULL_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("MULL_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("MULL_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("MULL_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("MULL_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `MULL_COMPACTION_MODE` > config > remote settings >
    /// default, unrecognized falling through) and, for `Segments`, attach the
    /// separately-resolved detail level.
    pub(crate) fn resolve_compaction_mode(&self) -> mull_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("MULL_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `MULL_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("MULL_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `MULL_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> mull_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("MULL_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("MULL_CANCEL_REWIND")
            .config(self.features.cancel_rewind)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Resolve whether to use mull's default OAuth2 (Palmshed auth.palmshed.ai).
    ///
    /// Enterprise OIDC (`oidc` in config.toml) always wins — this only gates
    /// the default Palmshed OAuth2 fallback when no enterprise OIDC is configured.
    ///
    /// Priority: `--oauth` > MULL_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_mull_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("MULL_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// `palmshed.ai/mcp/server_status` push.
    ///
    /// Thin delegate to the canonical
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.mull/`.
    ///
    /// Thin delegate to the canonical
...
///
/// `requirement > cli > env (MULL_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("MULL_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (MULL_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `MULL_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("MULL_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (MULL_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
/// `palmshed.ai/mcp/server_status` is wired default-on, with this
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("MULL_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (MULL_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `MULL_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("MULL_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `MULL_*`). Returns
...
pub fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(mull_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("MULL_ERROR_REPORTING"))
...
/// `MULL_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn mull_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("MULL_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.mull/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::mull_home::mull_home().join("requirements.toml");
    let content = std::fs::read_to_string(&path).ok()?;
    toml::from_str(&content).ok()
}
/// Resolve the external-OTEL master switch exactly the way the external
/// stream's activation does: **requirement pin > `MULL_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        mull_config::load_merged_requirements().as_ref(),
        env_bool("MULL_EXTERNAL_OTEL"),
...
/// by `mull setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: mull_telemetry::external::config::ExternalClientInfo,
) -> Option<mull_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<mull_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| mull_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            mull_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = mull_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = mull_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        mull_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = mull_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    mull_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
    ...
    entries
        ...
        .map(|m| {
            ...
            let config = ModelEntryConfig {
                ...
                api_base_url: Some(endpoints.mull_api_base_url.clone()),
                name: m.name,
...
pub struct ModelEntryConfig {
    ...
    pub model: String,
    /// The base URL of the model. e.g. "https://openrouter.ai/api/v1"
    pub base_url: String,
    ...
    /// If not set, falls back to env_key, then OPENROUTER_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Can also be set via the `MULL_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
    ...
    /// Without this set, Palmshed API models send args as one delta
...
pub struct ModelInfo {
    ...
    /// The routing slug sent in API requests.
    pub model: String,
    /// The base URL of the model (session endpoint). e.g. "https://openrouter.ai/api/v1"
    pub base_url: String,
    ...
    /// Always has a value; defaults to `"mull-plan"` when the server
...
/// codebase_indexing = ["/Users/*/mull*", "!/Users/*/old-*"]           # globs, ! to exclude
/// ```
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(untagged)]
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<mull_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when Mull starts outside a Git repository.
    /// Default: false. Used as the local fallback when the `non_git_warning` remote settings
    /// flag in `mull_build_settings` is absent. When the remote flag is present it takes
    ...
    /// (`image_gen_model_override`) / env / default (`mull-imagine-image-quality`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// env (`MULL_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`MULL_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// termination and pushes `palmshed.ai/mcp/server_status` updates to
    ...
    /// `MULL_MCP_AUTO_RESTART` to opt out).
    ///
    /// Resolved via [`Config::resolve_mcp_auto_restart`].
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mcp_auto_restart: Option<bool>,
    /// Pager-side subscription to the `palmshed.ai/mcp/server_status` push.
    ///
    ...
    /// back to the legacy `palmshed.ai/mcp/tools_changed` debounced refetch
...
pub struct ResolvedCredentials {
    pub api_key: Option<String>,
    pub base_url: String,
    pub auth_type: mull_chat_state::AuthType,
    pub auth_scheme: AuthScheme,
}
...
/// Priority: model api_key/env_key > session token > PALMSHED_API_KEY.
///
...
pub fn resolve_credentials(model: &ModelEntry, session_key: Option<&str>) -> ResolvedCredentials {
    ...
    let (api_key, base_url, auth_type) = if let Some(key) = model.own_credential() {
        (
            Some(key),
            info.base_url.clone(),
            mull_chat_state::AuthType::ApiKey,
        )
    } else if let Some(key) = session_key {
        (
            Some(key.to_owned()),
            info.base_url.clone(),
            mull_chat_state::AuthType::SessionToken,
        )
    } else if let Ok(key) = crate::agent::auth_method::read_mull_api_key_env() {
        let url = model
            .api_base_url
            .clone()
            .unwrap_or_else(|| info.base_url.clone());
        (Some(key), url, mull_chat_state::AuthType::ApiKey)
    } else {
...
/// `disable_api_key_auth` at the credential seam: swap a first-party Palmshed API
/// key for the IdP session (absent => request fails => forces login). BYOK
/// (non-Palmshed `base_url`) is untouched; no-op when the switch is off.
pub fn enforce_disable_api_key_auth(
    ...
    if disable_api_key_auth
        && creds.auth_type == mull_chat_state::AuthType::ApiKey
        && crate::util::is_first_party_mull_url(&creds.base_url)
    {
        creds.auth_type = mull_chat_state::AuthType::SessionToken;
        creds.api_key = session_key.map(str::to_owned);
        mull_telemetry::unified_log::debug(
...
pub use mull_telemetry::config::deployment_id_from_key;
...
pub fn try_resolve_model_credentials(
    ...
    enforce_disable_api_key_auth(
        &mut credentials,
        cfg.mull_com_config.api_key_auth_disabled(),
        session_key,
    );
    Some(credentials)
}
...
pub fn resolve_aux_model_sampling_config(
    ...
    let mull_bearer = session_key
        .map(|s| s.to_owned())
        .or_else(|| crate::agent::auth_method::read_mull_api_key_env().ok())
        .or_else(|| endpoints.deployment_key.clone());
    if let Some(bearer) = mull_bearer {
...
/// on BYOK / non-proxy routes for internal slugs like `mull-build`).
...
pub fn resolve_chat_state_auth_type(
    model_id: &str,
    session_key: Option<&str>,
    fallback: mull_chat_state::AuthType,
) -> mull_chat_state::AuthType {
...
pub fn inject_url_derived_headers(
    ...
    // OpenRouter migration: xAI-specific proxy headers (X-XAI-Token-Auth,
    // x-authenticateresponse, x-mull-client-mode) are no longer injected.
    // OpenRouter uses standard Bearer token auth via Authorization header.
...
mod tests {
    ...
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_mull_build(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use mull_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
...
"#;
    ...
    fn inject_url_derived_headers_adds_proxy_headers_for_cli_chat_proxy_url() {
        ...
        assert!(
            ...
            "OpenRouter migration: xAI-specific proxy headers are no longer injected"
        );
        assert!(
    ...
    fn inject_url_derived_headers_skips_proxy_headers_for_external_url() {
        let mut headers = IndexMap::new();
        inject_url_derived_headers(&mut headers, None, "https://api.palmshed.ai/v1");
    ...
    fn finalize_image_describe_sampler_some_stamps_session_fields() {
        ...
        assert_eq!(model, "mull");
        assert_eq!(cfg.model, "mull");
        assert_eq!(cfg.client_identifier.as_deref(), Some("cli"));
        assert_eq!(cfg.max_retries, Some(7));
    }
    #[test]
    fn resolve_aux_model_honors_mull_build_override() {
        let endpoints = EndpointsConfig::default();
        ...
        let resolved = resolve_aux_model_sampling_config(
            "mull", &catalog, &endpoints, None, false, None, None,
        )
    ...
    fn sampling_config_uses_fallback_when_no_model_api_key() {
        ...
        let sampling_config = sampling_config_for_model(
            ...
            ResolvedCredentials {
                ...
                auth_type: mull_chat_state::AuthType::ApiKey,
    ...
    fn default_models_dual_endpoint_routing() {
        ...
        for (model_id, entry) in default_model_entries(&endpoints) {
            ...
            assert_eq!(
                api_key_creds.base_url, endpoints.mull_api_base_url,
                "{model_id}: ExternalApiKey must route to api.palmshed.ai"
            );
        }
    }
    #[test]
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new(["MULL_TEST_ENV_KEY_PRIMARY", "MULL_TEST_ENV_KEY_FALLBACK"]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "MULL_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "MULL_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "MULL_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "MULL_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "MULL_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["MULL_TEST_WS_PRIMARY", "MULL_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "MULL_TEST_WS_PRIMARY" => Some("   ".into()),
                "MULL_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("MULL_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("MULL_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use mull_test_support::EnvGuard;
        let var = "MULL_TEST_FIRST_OWN_CRED_ENV";
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        use mull_chat_state::AuthType;
        let primary = "MULL_TEST_MULTI_ENV_PRIMARY";
        let alias = "MULL_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use mull_chat_state::AuthType;
        use mull_test_support::EnvGuard;
        let primary = "MULL_TEST_EMPTY_ENV_PRIMARY";
        let alias = "MULL_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        use crate::agent::auth_method::{
            LEGACY_PALMSHED_API_KEY_ENV_VAR, PALMSHED_API_KEY_ENV_VAR,
        };
        use mull_chat_state::AuthType;
        use mull_test_support::EnvGuard;
        let sentinel = "palmshed-global-sentinel-key";
        let primary = "MULL_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "MULL_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
        let _global = EnvGuard::set(PALMSHED_API_KEY_ENV_VAR, sentinel);
        let _legacy = EnvGuard::unset(LEGACY_PALMSHED_API_KEY_ENV_VAR);
        let mut model = test_model_entry("m", "https://inference.example/v1", None, None, None);
        model.env_key = Some(EnvKeys::new([primary, alias]));
        assert!(!model.has_own_credentials());
        let creds = resolve_credentials(&model, None);
    ...
    fn resolve_credentials_no_session_key_returns_api_key() {
        let model = test_model_entry("m", "https://example.com/v1", None, None, None);
        let creds = resolve_credentials(&model, None);
        assert_eq!(creds.auth_type, mull_chat_state::AuthType::ApiKey);
    }
    fn api_key_creds(base_url: &str) -> ResolvedCredentials {
        ResolvedCredentials {
            api_key: Some("palmshed-secret".to_string()),
            base_url: base_url.to_string(),
            auth_type: mull_chat_state::AuthType::ApiKey,
            auth_scheme: Default::default(),
        }
    ...
    fn enforce_disable_api_key_auth_blocks_first_party_only() {
        use mull_chat_state::AuthType;
        let mut creds = api_key_creds("https://api.palmshed.ai/v1");
        enforce_disable_api_key_auth(&mut creds, false, Some("session-jwt"));
        assert_eq!(creds.auth_type, AuthType::ApiKey);
        assert_eq!(creds.api_key.as_deref(), Some("palmshed-secret"));
        let mut creds = api_key_creds("https://api.palmshed.ai/v1");
        ...
        let mut creds = ResolvedCredentials {
            ...
            ..api_key_creds("https://api.palmshed.ai/v1")
        };
        enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
        assert_eq!(creds.auth_type, AuthType::SessionToken);
    }
    ...
    /// (non-palmshed.ai) own keys are preserved. (`try_resolve_model_credentials`
    ...
    fn try_resolve_model_credentials_swaps_first_party_own_key_under_kill_switch() {
        ...
        assert_eq!(creds.api_key.as_deref(), Some("palmshed-model-key"));
        enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
        assert_eq!(
            creds.auth_type,
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = mull_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn byok_from_lookup_classifies_all_states() {
        ...
        let session = test_model_entry("m", "https://api.palmshed.ai/v1", None, None, None);
        assert_eq!(
    ...
    fn user_override_adds_api_key_to_default_model() {
        ...
        assert_eq!(
            model.info.base_url, "https://openrouter.ai/api/v1",
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use mull_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use mull_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
        ...
        assert_eq!(
            ...
            CompactionMode::Segments(mull_chat_state::CompactionDetail::default())
        );
        assert_eq!(
    ...
    fn sampling_config_context_window_from_entry_or_default() {
        let model = test_model_entry("any-model", "https://api.palmshed.ai/v1", None, None, None);
        let config = sampling_config_for_model(
            &model,
            resolve_credentials(&model, None),
            None,
        ...
        let mut model =
            test_model_entry("any-model", "https://api.palmshed.ai/v1", None, None, None);
    ...
    /// mull-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_mull_build() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    fn hidden_model_excluded_from_acp_but_kept_in_catalog() {
        ...
        let raw_config: toml::Value = toml::from_str(
            ...
            [model.visible-model]
            model = "visible-model"
            base_url = "https://api.palmshed.ai/v1"
            context_window = 200000
    ...
    fn invalid_glob_is_rejected_by_validation() {
        ...
        assert!(ModelGlobSet::compile(Some(&vec!["mull[".to_string()])).is_err());
        let raw: toml::Value = toml::from_str(
            r#"
            [models]
            allowed_models = ["mull["]
    ...
    #[test]
    fn auth_alias_maps_to_mull_com_config() {
        let raw: toml::Value = toml::from_str(
            r#"
            [auth.oidc]
        ...
        let oidc = cfg.mull_com_config.oidc.expect("oidc should be set");
        assert_eq!(oidc.issuer, "https://example.okta.com");
        assert_eq!(oidc.client_id, "test-id");
    }
    #[test]
    fn mull_com_config_still_works() {
        let raw: toml::Value = toml::from_str(
            r#"
            [mull_com_config.oidc]
            issuer = "https://example.okta.com"
            client_id = "test-id"
            "#,
        )
    ...
    fn disable_api_key_auth_parses_from_auth_alias() {
        ...
        assert_eq!(absent.mull_com_config.disable_api_key_auth, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(cfg.mull_com_config.disable_api_key_auth, Some(true));
    }
    ...
    fn force_login_team_uuid_parses_string_and_array() {
        ...
        let absent = Config::new_from_toml_cfg(&toml::from_str("").unwrap()).unwrap();
        assert_eq!(absent.mull_com_config.force_login_team_uuid, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(
            cfg.mull_com_config.force_login_team_uuid,
    ...
    fn force_login_team_uuid_implies_api_key_auth_disabled() {
        use crate::auth::{ForceLoginTeam, MullComConfig};
        let base = MullComConfig {
            disable_api_key_auth: None,
            force_login_team_uuid: None,
            ..MullComConfig::default()
        };
    ...
    fn e2e_user_overrides_default_model_key_with_custom_endpoint() {
        ...
        assert_eq!(
            ...
            "should route to the user's custom endpoint, not api.palmshed.ai"
        );
    ...
    fn e2e_config_toml_model_overrides_default() {
        ...
        unsafe { std::env::set_var("PALMSHED_API_KEY", "palmshed-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
        unsafe { std::env::remove_var("PALMSHED_API_KEY") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
    }
    #[test]
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "MULL_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn config_models_default_custom_model_is_in_resolved_model_list() {
        ...
        assert!(
            models.contains_key("acme-mull"),
            "user-defined model must be in the resolved model list"
        );
        let model = models.get("acme-mull").unwrap();
        assert_eq!(model.info.model, "grok-4.5");
        assert_eq!(model.info.base_url, "https://inference.example.com/v1");
    }
    #[test]
    fn e2e_default_model_with_session_routes_to_proxy() {
        ...
        assert_eq!(
            sampling.base_url, "https://openrouter.ai/api/v1",
            "session auth should route to cli-chat-proxy, not api.palmshed.ai"
        );
    }
    #[test]
    #[serial]
    fn e2e_default_model_with_external_api_key_routes_to_api_xai() {
        ...
        let model = models
            .get(crate::models::default_model())
            .expect("default model should exist");
        unsafe { std::env::set_var("PALMSHED_API_KEY", "palmshed-external-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.api_key.as_deref(), Some("palmshed-external-key"));
        assert_eq!(
            sampling.base_url, "https://openrouter.ai/api/v1",
            "external API key should route to api via api_base_url"
        );
        unsafe { std::env::remove_var("PALMSHED_API_KEY") };
    }
    #[test]
    fn e2e_user_config_overrides_prefetched_model() {
        ...
        prefetched.insert(
            dm.to_string(),
            test_model_entry(dm, "https://openrouter.ai/api/v1", None, None, None),
        );
        let (_, models) = resolve_models_from_toml(
            &format!(
                r#"
    ...
    fn e2e_credential_priority_model_key_beats_session_beats_env() {
        ...
        unsafe { std::env::set_var("PALMSHED_API_KEY", "env-key") };
        ...
        assert_eq!(
            sampling.base_url, "https://api.palmshed.ai/v1",
    ...
    fn e2e_duplicate_model_field_both_entries_survive() {
        ...
        let user = models.get("acme-mull").unwrap();
        ...
        assert_eq!(sampling.base_url, "https://openrouter.ai/api/v1");
    }
    #[test]
    fn e2e_enterprise_custom_endpoint_skips_mull_defaults() {
        ...
        assert!(
            ...
            "Palmshed default must not leak into enterprise model list"
        );
    ...
    fn e2e_acp_model_info_no_dedup_on_model_field() {
        ...
        assert!(
            acp_models.contains_key(&acp::ModelId::new("default-mull")),
            "default entry should be addressable by map key"
        );
        assert!(
            acp_models.contains_key(&acp::ModelId::new("acme-mull")),
    ...
    fn e2e_enterprise_endpoints_plus_partial_model_override() {
        ...
        let (_, models) = resolve_models_from_toml(
            ...
            mull_api_base_url = "https://enterprise-api.acme.com/v1"
    ...
    fn e2e_enterprise_endpoints_only_no_model_override() {
        ...
        assert_eq!(
            model.api_base_url.as_deref(),
            Some("https://enterprise-api.acme.com/v1"),
            "default model should use enterprise mull_api_base_url"
        );
    }
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            "MULL_CLI_CHAT_PROXY_BASE_URL",
            "MULL_PALMSHED_API_BASE_URL",
            "MULL_FEEDBACK_BASE_URL",
            "MULL_TRACE_UPLOAD_URL",
            "MULL_MANAGED_CONFIG_URL",
            "MULL_MODELS_BASE_URL",
            "MULL_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "MULL_INTERNAL_OTLP_TRACES_ENDPOINT",
            "MULL_INTERNAL_OTLP_HEADERS",
            "MULL_EXTERNAL_OTEL",
        ] {
    ...
    /// `mull_api_base_url` — overriding ONLY inference keeps every aux endpoint on
    ...
    fn aux_endpoints_resolve_to_proxy_never_inference() {
        ...
        let cfg = EndpointsConfig {
            mull_api_base_url: inference.to_string(),
        ...
        assert_eq!(cfg.mull_api_base_url, inference);
    ...
    /// REGRESSION: the managed-config URL never follows `mull_api_base_url`
    ...
    fn loader_managed_config_url_never_follows_inference_endpoint() {
        ...
        let cfg = Config::new_from_toml_cfg(
            &toml::from_str(
                r#"[endpoints]
                mull_api_base_url = "https://inference.acme-corp.example/xai/v1""#,
            )
            .unwrap(),
        )
    ...
    fn resolve_feedback_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("MULL_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("MULL_TELEMETRY_ENABLED") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("MULL_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("MULL_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("MULL_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("MULL_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("MULL_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        assert!(
        ...
        unsafe { std::env::set_var("MULL_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("MULL_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("MULL_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("MULL_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("MULL_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config() {
        unsafe { std::env::set_var("MULL_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("MULL_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("MULL_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("MULL_IMAGE_GEN_MODEL_OVERRIDE") };
        ...
        assert_eq!(
            with(None, Some("mull-imagine-image")).resolve_image_gen_model_override(),
            Some("mull-imagine-image".to_owned())
        );
        assert_eq!(
            with(Some("mull-imagine-image-pro"), Some("mull-imagine-image"))
                .resolve_image_gen_model_override(),
            Some("mull-imagine-image-pro".to_owned())
        );
    }
    #[test]
    #[serial]
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("MULL_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("MULL_IMAGE_EDIT", "1") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("MULL_GOAL");
            std::env::remove_var("MULL_GOAL_CLASSIFIER");
            std::env::remove_var("MULL_GOAL_PLANNER");
            std::env::remove_var("MULL_GOAL_SUMMARY");
            std::env::remove_var("MULL_GOAL_VERIFIER_N");
            std::env::remove_var("MULL_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("MULL_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("MULL_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("MULL_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("MULL_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_planner_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("MULL_GOAL_PLANNER", "0") };
        let r =
            cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("MULL_GOAL_PLANNER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("MULL_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "MULL_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "mull", agent_type = "mull-plan" }

[goal.strategist_model]
model = "mull-composer-2.5-fast"
agent_type = "cursor"

[[goal.skeptic_models]]
model = "mull"
agent_type = "mull-plan"
...
"#;
        ...
        let cfg = Config::new_from_toml_cfg(&raw).unwrap();
        assert_eq!(cfg.goal.planner_model.as_ref().unwrap().model, "mull");
        assert_eq!(
            cfg.goal.strategist_model.as_ref().unwrap().agent_type,
            "cursor"
        );
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "mull");
        assert_eq!(
...
planner_model = { agent_type = "mull-plan" }
"#;
        let raw: toml::Value = toml::from_str(toml_str).unwrap();
        let cfg = Config::new_from_toml_cfg(&raw)
...
"#;
        ...
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "mull");
        assert_eq!(cfg.goal.skeptic_models[1].model, "mull-composer-2.5-fast");
    }
...
classifier_max_runs = 6
planner_model = { model = "mull", agent_type = "mull-plan" }
strategist_model = { model = "mull-composer-2.5-fast", agent_type = "cursor" }
...
"#,
        ...
        let mull_build = crate::util::config::GoalRoleModel {
            model: "mull".into(),
            agent_type: "mull-plan".into(),
        };
        let composer = crate::util::config::GoalRoleModel {
            model: "mull-composer-2.5-fast".into(),
        ...
        assert_eq!(
            planner.value,
            GoalRoleModelChoice::Explicit(mull_build.clone())
        );
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(mull_build),
    ...
    fn config_warns_on_section_typo() {
        let raw: toml::Value = toml::from_str(
            r#"
            [endpoint]
            deployment_key = "palmshed-token-test"
        "#,
        )
        .unwrap();
    ...
    fn internal_otlp_test_config() -> EndpointsConfig {
        EndpointsConfig {
            ...
            mull_internal_otlp_traces_endpoint: None,
            mull_internal_otlp_headers: None,
            external_otel_master_switch: false,
            ..Default::default()
        }
    }
    /// `mull_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
    /// fields regardless of the master switch.
    #[test]
    fn internal_otlp_endpoint_mull_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                mull_internal_otlp_traces_endpoint: Some(
            ...
            assert_eq!(
                ...
                "switch={switch}: MULL_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn internal_otlp_consumed_standard_vars_cases() {
        ...
        for case in cases {
            let cfg = EndpointsConfig {
                ...
                mull_internal_otlp_traces_endpoint: case
                    .internal_ep
                    .then(|| "https://internal.example/traces".to_string()),
                mull_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
                ..internal_otlp_test_config()
            };
            assert_eq!(
    ...
    /// Headers precedence: `mull_internal_otlp_headers` wins; legacy
    /// `otel_exporter_otlp_headers` only when the master switch is unset.
    #[test]
    fn internal_otlp_headers_precedence() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                mull_internal_otlp_headers: Some("x-debug=1".to_string()),
    ...
    fn ext_client() -> mull_telemetry::external::config::ExternalClientInfo {
        mull_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    ...
    fn external_otel_requirements_pin_wins_over_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                Some(&req),
                ext_env(&[("MULL_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `MULL_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("MULL_SUBAGENTS");
            std::env::remove_var("MULL_RESPECT_GITIGNORE");
            std::env::remove_var("MULL_WEB_SEARCH_MODEL");
            std::env::remove_var("MULL_SESSION_SUMMARY_MODEL");
            std::env::remove_var("MULL_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("MULL_CURSOR_RULES_ENABLED");
            std::env::remove_var("MULL_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("MULL_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("MULL_CLAUDE_RULES_ENABLED");
            std::env::remove_var("MULL_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("MULL_MANAGED_MCPS_ENABLED");
            std::env::remove_var("MULL_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("MULL_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "MULL_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "MULL_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "MULL_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("MULL_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("MULL_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("MULL_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("MULL_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("MULL_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("MULL_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("MULL_TELEMETRY_ENABLED") };
...
"#,
        ...
        mull_config::apply_version_overrides(&mut value, &v).unwrap();
        let cfg = Config::new_from_toml_cfg(&value).unwrap();
        assert_eq!(cfg.models.default.as_deref(), Some("grok-4.5"));
    }
    /// Reproduce the enterprise managed config bug: [model.mull-build] sets
    ...
    fn slug_propagation_enterprise_managed_config_key_mismatch() {
        ...
        let by_key = resolved
            .get("mull-build")
            .expect("mull-build key must exist");
        ...
        assert_eq!(
            ...
            "BUG: prefetched 'grok-4.5' should inherit 500k from \
             sibling 'mull-build' (same model slug), not stay at {default_cw}"
        );
    }
    ...
    fn resolve_model_list_config_reasoning_efforts_beats_remote() {
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
        let mut entry = prefetch_model_entry("mull-x", 200_000, ApiBackend::default());
        entry.info.reasoning_efforts = vec![ReasoningEffortOption {
            id: "high".to_string(),
            value: ReasoningEffort::High,
            label: "High".to_string(),
        ...
        let mut prefetched = IndexMap::new();
        prefetched.insert("mull-x".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
    ...
    fn resolve_model_list_inherits_context_window_from_default_when_prefetched_has_fallback() {
        let cfg = Config::default();
        let default_cw = DEFAULT_CONTEXT_WINDOW;
        let entry = prefetch_model_entry("openrouter/free", default_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("openrouter/free".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("openrouter/free").expect("model must exist");
        assert_ne!(
    ...
    fn resolve_model_list_does_not_override_explicitly_set_context_window() {
        ...
        let entry = prefetch_model_entry("mull", explicit_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("mull".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("mull").expect("model must exist");
        assert_eq!(
            entry.info.context_window.get(),
            explicit_cw,
    ...
    fn resolve_model_list_inherits_agent_type_and_api_backend() {
        let cfg = Config::default();
        let default_cw = DEFAULT_CONTEXT_WINDOW;
        let entry = prefetch_model_entry("mull", default_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("mull".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("mull").expect("model must exist");
        let defaults = default_model_entries(&EndpointsConfig::default());
        if let Some(default) = defaults.get("mull") {
    ...
    fn resolve_model_list_prunes_bundled_entries_not_in_prefetch() {
        ...
        if let Some(e) = defs.shift_remove("openrouter/free") {
            p.insert("openrouter/free".to_string(), e);
        }
        let resolved = resolve_model_list(&cfg, Some(p));
        assert!(resolved.contains_key("openrouter/free"));
        let no_p = resolve_model_list(&cfg, None);
        assert!(no_p.contains_key("openrouter/free"));
    }
    #[test]
    ...
    fn resolve_model_list_keeps_prefetch_only_entries_and_prunes_defaults() {
        ...
        assert!(!resolved.contains_key("mull"));
    }
    #[test]
    ...
    /// Regression: enterprise managed config aliases mull-build to their own
    /// endpoint with env_key. The bundled mull-build has supported_in_api=false.
    ...
    fn byok_config_overlay_visible_to_api_key_users() {
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        let resolved = resolve_model_list(&cfg, None);
        let entry = resolved.get("mull-build").expect("mull-build must exist");
        assert!(
    ...
    fn plain_config_overlay_preserves_bundled_visibility() {
        ...
        assert!(
            entry.visible_for_auth(false),
            "non-BYOK config overlay without a matching bundled entry defaults to supported_in_api=true"
        );
    }
    #[test]
    #[serial]
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("MULL_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("MULL_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("MULL_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("MULL_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("MULL_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("MULL_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("MULL_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("MULL_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("MULL_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("MULL_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("MULL_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("MULL_MCP_RECURSIVE_CONFIG_WATCH", "false") };
