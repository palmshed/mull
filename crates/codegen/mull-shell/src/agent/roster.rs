...
//! two ways:
//!
//!   - request/response `palmshed.ai/sessions/list` → `{ "sessions": [RosterEntry, …] }`
//!   - broadcast notification `palmshed.ai/sessions/changed` →
...
/// Response payload for `palmshed.ai/sessions/list`.
#[derive(Serialize, Deserialize, Clone, Debug, Default)]
pub struct RosterListResponse {
    pub sessions: Vec<RosterEntry>,
}

/// Params payload for the `palmshed.ai/sessions/changed` broadcast notification.
#[derive(Serialize, Deserialize, Clone, Debug, Default)]
...
pub const SESSIONS_LIST_METHOD: &str = "palmshed.ai/sessions/list";
pub const SESSIONS_CHANGED_METHOD: &str = "palmshed.ai/sessions/changed";
...
pub(crate) fn merge_roster(
    ...
    entries.sort_by_key(|a| std::cmp::Reverse(a.last_change_unix_ms));
    entries
}
