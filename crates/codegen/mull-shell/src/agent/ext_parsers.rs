...
/// Parse a `palmshed.ai/queue/{remove,reorder,clear,edit,interject}` ext-notification's
...
pub(super) fn parse_queue_edit_command(
    ...
    match method {
        ...
        "palmshed.ai/queue/clear" => Some(SessionCommand::ClearQueue { owner }),
...
mod tests {
    ...
    /// Each `palmshed.ai/queue/*` ext-notification maps to the
    ...
    fn parse_queue_edit_command_maps_each_method() {
        ...
        match parse_queue_edit_command("palmshed.ai/queue/remove", &p, Some("mull-tui".into())) {
            Some(SessionCommand::RemoveQueuedPrompt {
                id,
                expected_version,
                owner,
                ...
                assert_eq!(owner.as_deref(), Some("mull-tui"));
            }
        ...
        match parse_queue_edit_command("palmshed.ai/queue/remove", &p, None) {
            Some(SessionCommand::RemoveQueuedPrompt {
                expected_version, ..
        ...
        match parse_queue_edit_command("palmshed.ai/queue/reorder", &p, None) {
            Some(SessionCommand::ReorderQueue { ordered_ids }) => {
        ...
        match parse_queue_edit_command("palmshed.ai/queue/edit", &p, Some("mull-vscode".into())) {
            Some(SessionCommand::EditQueuedPrompt {
                id,
                new_text,
                editor,
                ...
                assert_eq!(editor.as_deref(), Some("mull-vscode"));
            }
        ...
        match parse_queue_edit_command("palmshed.ai/queue/interject", &p, Some("mull-tui".into())) {
            Some(SessionCommand::InterjectQueuedPrompt {
                id,
                expected_version,
                owner,
        ...
        match parse_queue_edit_command("palmshed.ai/queue/interject", &p, None) {
        ...
        assert!(
            parse_queue_edit_command("palmshed.ai/queue/interject", &serde_json::json!({}), None)
                .is_none()
        );

        // unknown method → None.
        assert!(
            parse_queue_edit_command("palmshed.ai/queue/bogus", &serde_json::json!({}), None)
                .is_none()
        );
        // remove without id → None (can't target an entry).
        assert!(
            parse_queue_edit_command("palmshed.ai/queue/remove", &serde_json::json!({}), None)
                .is_none()
        );
    }
}
