...
const CLIENT_VERSION_HEADER: &str = "x-mull-client-version";

// ============================================================================
// Turn delta wire types (local to mull-shell until cli-chat-proxy catches up)
// ============================================================================
...
pub struct FeedbackClient {
    ...
    base_url: String,
    credentials: crate::util::mull_auth_credentials::MullAuthCredentials,
...
impl FeedbackClient {
    pub fn new(base_url: impl Into<String>, user_token: Option<String>) -> Self {
        ...
        let credentials = crate::util::mull_auth_credentials::MullAuthCredentials::new(user_token);
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(mull_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::mull_auth_credentials::MullAuthCredentials,
    ) -> Arc<dyn mull_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(mull_auth::StaticAuthCredentialProvider::new(
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        // User-token auth: OpenRouter migration removes X-XAI-Token-Auth.
        builder
            .header(CLIENT_VERSION_HEADER, mull_version::VERSION)
            .header(
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let request = mull_file_utils::trace_context::inject_trace_context_into_request(request);
