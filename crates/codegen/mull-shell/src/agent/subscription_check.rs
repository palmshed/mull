...
//! lift the gate.
//!
//! The pager drives the polling via `palmshed.ai/auth/check_subscription`: the 5s
...
/// Subscription tiers that qualify for Mull access.
...
/// the paywall is shown (`palmshed.ai/auth/check_subscription`).
///
...
mod tests {
    ...
    fn partial_tier_name_is_not_qualifying() {
        ...
        assert!(!QUALIFYING_TIERS.contains(&"Mull"));
