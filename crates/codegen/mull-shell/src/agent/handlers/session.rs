...
/// Router for palmshed.ai/session/* and palmshed.ai/session_summaries/* methods.
pub async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
) -> Result<acp::ExtResponse, acp::Error> {
    match args.method.as_ref() {
        "palmshed.ai/session/info" => handle_session_info(agent, args).await,
        "palmshed.ai/session/close" => handle_session_close(agent, args).await,
        "palmshed.ai/session/list" => handle_session_list(agent, args).await,
        "palmshed.ai/sessions/list" => handle_roster_list(agent, args).await,
        m if m.starts_with("palmshed.ai/session_summaries/") => {
...
/// `palmshed.ai/sessions/list` — the FleetView roster. Returns every
...
/// `palmshed.ai/sessions/changed` broadcast.
...
async fn handle_session_close(
    ...
    if existed {
        ...
        tracing::info!(session_id = %req.session_id, "session closed via palmshed.ai/session/close");
    } else {
...
async fn handle_session_summaries(
    ...
    match args.method.as_ref() {
        "palmshed.ai/session_summaries/session_list" => {
        ...
        "palmshed.ai/session_summaries/workspace_list" => {
            tracing::debug!("/mull/session_summaries/workspace_list is working");
        ...
        "palmshed.ai/session_summaries/workspace_list_recent" => {
