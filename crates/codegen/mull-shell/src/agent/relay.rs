...
//! connection to the palmshed.ai relay server with automatic reconnection.
...
/// Config for the palmshed.ai WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a palmshed.ai first-party session
    /// (`is_mull_auth`: palmshed.ai-issuer OIDC or external credential) with a
    /// non-empty bearer. BYOK/ApiKey, non-palmshed.ai issuers (enterprise OIDC,
    ...
    pub(crate) fn for_session(
        ...
        if !session.is_mull_auth() || session.key.is_empty() {
            return None;
        }
        let _ = alpha_test_key;
        Some(Self {
            ws_url: ctx.mull_ws_url.clone(),
            ws_origin: ctx.mull_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to palmshed.ai with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `mull login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from Mull WebSocket server");
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                mull_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    /// Helper to create a test MullAuth with the given key.
    fn test_auth(key: &str) -> MullAuth {
        MullAuth {
            key: key.to_string(),
    ...
    #[test]
    fn for_session_builds_only_for_mull_issuer() {
        use crate::auth::PALMSHED_OAUTH2_ISSUER;
        let cfg = MullComConfig::default();
        let builds = |a: &MullAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let mull = MullAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(PALMSHED_OAUTH2_ISSUER.to_string()),
            ..test_auth("palmshed-bearer")
        };
        assert!(mull.is_mull_auth(), "precondition: is_mull_auth");
        assert!(builds(&mull));
        let external_mull = MullAuth {
            auth_mode: AuthMode::External,
            oidc_issuer: Some(PALMSHED_OAUTH2_ISSUER.to_string()),
            ..test_auth("ext-bearer")
        };
        assert!(external_mull.is_mull_auth(), "precondition: is_mull_auth");
        assert!(builds(&external_mull));
        assert!(!builds(&MullAuth {
            key: String::new(),
            ..mull.clone()
        }));
        assert!(!builds(&MullAuth {
    ...
    /// Helper: write a MullAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &MullAuth) {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        impl TokenRefresher for CountingRefresher {
            async fn refresh(
                ...
                RefreshOutcome::Success(Box::new(MullAuth {
        ...
        let mut config = RelayConfig::for_session(&expired_session, &cfg, None, Some(am.clone()))
            .expect("palmshed.ai OIDC session is relay-eligible");
        let cancel = CancellationToken::new();
