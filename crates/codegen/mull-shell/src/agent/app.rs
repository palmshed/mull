...
use crate::auth::{AuthManager, AuthMode, MullAuth, run_auth_flow};
...
/// or live subagent). The second signal covers relay-driven (palmshed.ai
...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(mull_file_utils::trace_context::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
...
/// `palmshed.ai/...` extension method, for injection into the agent's inbound ACP
/// stream by the leader's own watcher tasks (config hot-reload, skills).
///
/// The wire method is written **`_`-prefixed** (`_palmshed.ai/internal/...`):
...
/// Start a skills file watcher and wire it to inject `palmshed.ai/internal/reload_skills`
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = mull_agent::prompt::workspace_user::optional_workspace_user_dir();
    ...
    tokio::spawn(async move {
        while skills_rx.recv().await.is_some() {
            ...
            let line = internal_reload_request_line(
                "skills-reload",
                "palmshed.ai/internal/reload_skills",
                serde_json::json!({}),
            );
...
/// ([`mull_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    mull_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
...
pub async fn run_stdio_agent(
    ...
    mull_telemetry::unified_log::set_version(mull_version::VERSION);
    ...
    mull_file_utils::queue::cleanup_orphaned_uploads(
        &mull_home::mull_home(),
        mull_file_utils::queue::DEFAULT_MAX_AGE,
    );

    // Log the client that launched us (set by mull-desktop when spawning `mull agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("MULL_CLIENT_VERSION") {
    ...
    // blocking stdin reads (see `mull_acp_lib::spawn_stdin_line_reader`): on
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `mull agent stdio` is a local/interactive entrypoint (spawned by
            // mull-desktop), so it needs the gate like the leader and pager paths;
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    mull_telemetry::unified_log::set_version(mull_version::VERSION);
    // `mull agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `mull agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a palmshed.ai session. \
        Run `mull login` to sign in, or use `mull agent stdio` for API-key access.";
    ...
    let ctx = &agent_config.mull_com_config;
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'mull login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `mull login`."),
        }
    } else if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&mull_home::mull_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
        ...
        let auth_manager = Arc::new(AuthManager::new(&mull_home::mull_home(), ctx.clone()));
        if crate::agent::auth_method::has_mull_api_key_env()
    ...
    // Capture the mull build URL for the first-connection callback
    let mull_code_url = format!("{}/build", ctx.mull_ws_origin);
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open Mull: {} (press Enter to open in browser)",
                mull_code_url
            );
            eprintln!();
            let url_for_open = mull_code_url.clone();
...
fn should_seed_shared_session(existing: Option<&MullAuth>, session: &MullAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
/// Start the leader's palmshed.ai relay connection according to the start policy,
/// returning the slot where the [`RelayHandle`](crate::agent::relay::RelayHandle)
/// is parked once the connection task is running.
///
/// * `relay_on_demand == false` (default — explicit `mull agent leader`
...
///   duplication of mirroring every agent message to palmshed.ai.
///
...
/// When a palmshed.ai session is present, the leader connects to the websocket relay
...
/// * `relay_on_demand` - If true, defer the palmshed.ai relay WebSocket until the
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: mull_version::VERSION.to_string(),
    })
    ...
    let auth: Option<MullAuth> = crate::auth::try_ensure_session_noninteractive(ctx).await;

    // ── Phase 6b: Legacy devbox auth migration ─────────────────────────────
    let auth: Option<MullAuth> = migrate_devbox_auth_if_legacy(auth, &agent_config).await;

    let auth_for_prefetch: Option<MullAuth> = auth.clone();
    let endpoints_for_prefetch = agent_config.endpoints.clone();
    ...
    local_set
        .run_until(async move {
            ...
            // Bridge websocket messages to agent (from palmshed.ai relay)
            ...
            // Start (or arm) the palmshed.ai relay. Eager by default — a bare
            // `mull agent leader` (devbox / systemd) has no local IPC clients
            ...
            let relay_handle_slot = if let Some(relay_config) = relay_config {
                ...
                info!("Relay disabled: no palmshed.ai session token (BYOK / local-only leader)");
                Rc::new(std::cell::RefCell::new(None))
            };
            ...
            let auth_scope = agent_config.mull_com_config.auth_scope();
            // Gated on user_mull_home() so a cwd-relative .mull/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = mull_config::user_mull_home()
                    ...
                    match update {
                        ...
                        ConfigUpdate::ProjectMcpServersChanged { cwd } => {
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "palmshed.ai/internal/reload_project_mcp_servers",
                        ...
                        ConfigUpdate::ModelsChanged => {
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "palmshed.ai/internal/reload_models",
                                serde_json::json!({}),
                            );
                            let mut tx = acp_tx_for_config.lock().await;
                            if let Err(e) = tx.write_all(line.as_bytes()).await {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.mull/models_cache.json
                            // (another mull process fetched a fresher /v1/models
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "palmshed.ai/internal/reload_models_cache",
                                serde_json::json!({}),
                            );
                            let mut tx = acp_tx_for_config.lock().await;
                            if let Err(e) = tx.write_all(line.as_bytes()).await {
                        ...
                        ConfigUpdate::Ui {
                            ...
                            let notification = serde_json::json!({
                                "jsonrpc": "2.0",
                                "method": "palmshed.ai/config_changed",
                                "params": {
                                    "section": "ui",
                                    "changes": {
                                        "theme": theme,
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> MullAuth {
        MullAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::PALMSHED_OAUTH2_ISSUER.to_string()),
    ...
    /// constructor (`for_session`) with a relay-eligible palmshed.ai OIDC session.
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::MullComConfig {
            mull_ws_url: format!("ws://{addr}"),
            mull_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
        crate::agent::relay::RelayConfig::for_session(&auth, &cfg, None, None)
            .expect("palmshed.ai OIDC session must be relay-eligible")
    }
    ...
    /// `mull agent leader` (devbox/systemd — no local IPC clients,
    /// `relay_on_demand == false`) must connect the palmshed.ai relay eagerly.
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `mull agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
    ...
    fn internal_reload_request_line_uses_wire_ext_prefix() {
        ...
        assert_eq!(
            msg["method"], "_palmshed.ai/internal/reload_models",
        ...
        let msg: serde_json::Value = serde_json::from_str(line.trim_end()).unwrap();
        assert_eq!(msg["method"], "_palmshed.ai/internal/auth_cleared");
    }
