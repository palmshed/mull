...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [model("mull", "Mull"), model("grok-4.5", "Mull 4.5")];
        let current = acp::ModelId::from("mull");
        ...
        assert_eq!(selected_models[0].id, "mull");
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("mull", "")];
        let current = acp::ModelId::from("mull");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "mull");
    }
    ...
    fn session_config_option_serializes_camel_case() {
        ...
        assert_eq!(v["label"], "Mull");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn mull_session_detail_serializes_camel_case() {
        let detail = MullSessionDetail::build(
            "sess-1".to_string(),
            "/Users/me/mull".to_string(),
            "mull".to_string(),
            None,
        );
        ...
        assert_eq!(v["cwd"], "/Users/me/mull");
        assert_eq!(v["currentModelId"], "mull");
