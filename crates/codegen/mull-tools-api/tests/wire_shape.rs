...
//! field rename/retype in `mull-tools.proto` cannot silently break those
//! wire contracts (the producer and consumer live in separate services).

use mull_tools_api::ToolConfigEntry;

fn full_entry() -> ToolConfigEntry {
    ToolConfigEntry {
        id: "MullBuild:grep".to_owned(),
...
fn minimal_entry_deserializes_from_id_only() {
    ...
    let back: ToolConfigEntry =
        serde_json::from_value(serde_json::json!({"id": "MullBuild:read_file"}))
            .expect("deserialize minimal");
    assert_eq!(back.id, "MullBuild:read_file");
