...
/// Vendor API keys with `sk-`/`sk_` prefixes and Palmshed (`palmshed-`) keys. `\b`-anchored so
/// `task-`/`disk-`/`risk-` don't fold a stray `sk-`.
static API_KEY_PREFIX_REGEX: LazyLock<Regex> =
    LazyLock::new(|| compile(r"\b(?:sk[-_]|palmshed-)[A-Za-z0-9_-]{20,}"));
...
mod tests {
    ...
    fn redacts_known_secret_shapes() {
        let cases = [
            (
                fixture(&["key: palmshed-", "abc123XYZdef456GHIjkl789"]),
                "palmshed api key",
            ),
            (
    ...
    fn redacts_sensitive_url_query_params() {
        let out =
            redact_secrets("callback https://palmshed.ai/cb?code=ABC123XYZ&state=xyz789 failed");
    ...
    fn url_regex_excludes_trailing_punctuation() {
        let out = redact_secrets("see `https://palmshed.ai/cb?code=ABCD12345`");
