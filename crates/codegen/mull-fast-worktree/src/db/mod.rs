...
use mull_sqlite_journal::JournalMode;
...
impl WorktreeDb {
    /// Open (or create) the DB at `mull_home/worktrees.db`.
    pub fn open(mull_home: &Path) -> Result<Self> {
        Self::open_at(&mull_home.join("worktrees.db"))
    }
    ...
    /// Open the default DB at `~/.mull/worktrees.db`.
    ///
    /// Discovers mull home via `$MULL_HOME`, falling back to the canonicalized
    /// `$HOME/.mull` (matching `mull_config::mull_home`).
...
pub fn resolve_mull_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("MULL_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home = PathBuf::from(std::env::var("HOME").context("neither $MULL_HOME nor $HOME is set")?);
    // Canonicalize the home dir so worktree paths share the same physical .mull
    // tree as trust/hooks even when it is symlinked. The dunce canonicalization
    // must stay in sync with mull_config::default_mull_home();
    // home resolution deliberately differs ($HOME here vs std::env::home_dir()).
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".mull"))
}

/// Serializes tests that mutate the process-global `MULL_HOME` env var so they
...
#[cfg(test)]
static MULL_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`MULL_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `MULL_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `MULL_HOME` before `_lock` releases, so the env is correct
/// before another waiting setter proceeds.
#[cfg(test)]
pub(crate) struct MullHomeFixture {
    _lock: std::sync::MutexGuard<'static, ()>,
    prev: Option<std::ffi::OsString>,
    /// The isolated mull home; pass to `WorktreeDb::open` to read the same DB
...
#[cfg(test)]
impl MullHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = MULL_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("mull-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via MULL_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until MULL_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("MULL_HOME");
        unsafe { std::env::set_var("MULL_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
...
#[cfg(test)]
impl Drop for MullHomeFixture {
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("MULL_HOME", p),
                None => std::env::remove_var("MULL_HOME"),
            }
        }
    }
}
