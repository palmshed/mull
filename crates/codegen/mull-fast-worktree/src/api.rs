...
fn remove_worktree_inner(
    ...
    // Canonicalize before disk removal so unregister_worktree can match the DB
    // record (which stores the canonical path from register_worktree) even after
    // the directory is gone (dunce::canonicalize fails on deleted paths).
    let canonical_path = canonical_for_db(worktree_path);

    let report = remove_worktree_from_disk(&canonical_path, delegate)?;
    ...
    //
    // Pass both the canonical form (for records registered via register_worktree)
    // and the original path (for records inserted with raw paths in tests/GC).
    #[cfg(feature = "metadata")]
    {
        unregister_worktree(&canonical_path);
        if canonical_path != worktree_path {
...
/// handle `~/.mull/worktrees/<repo>/<session>/`) and calls `remove_worktree()`
...
fn try_btrfs_remove(
    ...
    if worktree_path.is_symlink() {
        ...
        if let Ok(Some(_)) = btrfs::is_btrfs_subvolume(&resolved) {
            ...
            // `btrfs` refuses non-subvolumes, the snapshot dir is mull-owned, and
    ...
    if btrfs_info.bind_mount_source.is_some() {
        ...
        mull_tty_utils::detach_std_command(&mut umount_cmd);
...
fn unregister_worktree(worktree_path: &std::path::Path) {
    if let Ok(db) = crate::db::WorktreeDb::open_default() {
        // Try the raw path first (handles records registered with non-canonical
        // paths and post-deletion fallback when canonicalization fails).
        let _ = db.unregister_by_path(worktree_path);
        // Also try the canonical path (matches records written by
        // register_worktree which dunce::canonicalizes before storing).
        if let Ok(canonical) = dunce::canonicalize(worktree_path) {
            let _ = db.unregister_by_path(&canonical);
        }
    }
}

/// Compute the canonical form of a path for DB lookups (matching
/// `register_worktree`'s `dunce::canonicalize`). Falls back to the original
/// path when canonicalization fails (e.g. the path doesn't exist yet).
#[cfg(feature = "metadata")]
fn canonical_for_db(path: &std::path::Path) -> std::path::PathBuf {
...
pub mod gc {
    ...
    /// symlinked `$MULL_HOME` or custom worktree path would otherwise never match.
...
mod tests {
    ...
    fn test_cleanup_worktrees_in_with_nested_dirs() {
        ...
        // Create ~/.mull/worktrees/<repo>/<session>/ structure.
        let worktrees_dir = tmp.path().join("worktrees");
    ...
    fn test_cleanup_worktrees_in_removes_nested_dangling_symlink() {
        // Dangling symlink one level deeper (~/.mull/worktrees/<repo>/<session>):
    ...
    fn remove_with_delegate_deregisters_plain_worktree_without_calling_delegate() {
        mull_test_utils::require_git!();
        use mull_test_utils::git::{git_commit_all, init_git_repo};
        // Isolate MULL_HOME so the post-removal unregister writes to a private DB.
        #[cfg(feature = "metadata")]
        let _fx = crate::db::MullHomeFixture::new();
    ...
    fn delete_fallback_invokes_delegate_when_direct_delete_fails() {
        ...
        let report = delete_snapshot_with_delegate_fallback(
            ...
            Path::new("/home/u/.mull/worktrees/repo/wt"),
            Some(&delegate),
    ...
    fn test_cleanup_orphaned_btrfs_skips_when_mount_target_parent_missing() {
        ...
        let mount_target = unrestored_home.join(".mull/worktrees/x/wt-live");
        assert!(
    ...
    fn test_cleanup_orphaned_btrfs_skips_active() {
        ...
        let mount_target = std::path::PathBuf::from("/home/user/.mull/worktrees/active-wt");
    ...
    mod metadata_integration {
        ...
        fn register_worktree_writes_correct_fields() {
            // Isolate MULL_HOME so register_worktree's open_default write lands
            // in our own DB (lock + private tmp + restore via the fixture).
            let fx = crate::db::MullHomeFixture::new();

            // Unique basename → unique id, so a concurrent open_default writer
            // (MULL_HOME is process-global) can't INSERT-OR-REPLACE our row.
            ...
            // Canonicalize to match the dunce::canonicalize path stored by register_worktree.
            let canonical_wt_path = dunce::canonicalize(&wt_path).unwrap_or(wt_path.clone());
            let db = WorktreeDb::open(&fx.home).unwrap();
            let mine: Vec<_> = db
                .list(&ListFilter::default())
                .unwrap()
                .into_iter()
                .filter(|r| r.path == canonical_wt_path)
                .collect();
        ...
        fn db_record_survives_failed_removal() {
            ...
            let canonical_wt_path = dunce::canonicalize(&wt_path).unwrap_or(wt_path.clone());
            assert!(
                record_present(&db, &canonical_wt_path),
        ...
        fn gc_with_delegate_removes_expired_and_unregisters() {
            ...
            // MULL_HOME == the gc DB dir so remove_worktree's open_default
