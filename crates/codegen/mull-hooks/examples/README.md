...
Sample hooks for Mull. Copy to `~/.mull/hooks/` to enable globally, or to `<project>/.mull/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.mull/hooks/bin
cp examples/hooks/safe-shell.json ~/.mull/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.mull/hooks/bin/
chmod +x ~/.mull/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.mull/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.mull/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.mull/hooks/bin/
chmod +x ~/.mull/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.mull/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.mull/hooks/bin
cp examples/hooks/session-log.json ~/.mull/hooks/
cp examples/hooks/bin/session-log.sh ~/.mull/hooks/bin/
chmod +x ~/.mull/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.mull/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.mull/hooks/bin
cp examples/hooks/tool-logger.json ~/.mull/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.mull/hooks/bin/
chmod +x ~/.mull/hooks/bin/tool-logger.sh
```
...
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Mull names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.mull/hooks/`. The hook stops running on the next session.