...
pub struct HooksMap {
    ...
    /// Event names present in the JSON but not recognized by Mull.
...
pub struct RawHandler {
    ...
    /// keys (`MULL_HOOK_EVENT`, `MULL_HOOK_NAME`, `MULL_SESSION_ID`,
    /// `MULL_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at load
...
pub struct HookSpec {
    ...
    ///    (`MULL_HOOK_EVENT`, `MULL_HOOK_NAME`, `MULL_SESSION_ID`,
    ///    `MULL_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at
    ///    load time and a tracing warning is emitted.
    /// 2. Plugin-injected vars merged in by the plugin adapter
    ///    (`mull-agent::plugins::hooks_adapter`). The adapter sets
    ///    `MULL_PLUGIN_ROOT`, `CLAUDE_PLUGIN_ROOT`, `MULL_PLUGIN_DATA`,
    ...
    ///    (`MULL_HOOK_EVENT`, `MULL_HOOK_NAME`, `MULL_SESSION_ID`,
    ///    `MULL_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`). These are
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.mull/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.mull/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "MULL_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "MULL_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "MULL_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "MULL_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "MULL_HOOKS_PARSE_TEST_MATCHER_VAR";
    ...
    fn parse_hook_file_matcher_braced_var_is_not_env_expanded() {
        ...
        let key = "MULL_HOOKS_PARSE_TEST_MATCHER_BRACED";
    ...
    /// User attempts to set runner-reserved keys (MULL_HOOK_*,
    /// MULL_SESSION_ID, MULL_WORKSPACE_ROOT, CLAUDE_PROJECT_DIR) via
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "MULL_HOOK_EVENT": "spoofed",
                                    "MULL_HOOK_NAME": "spoofed",
                                    "MULL_SESSION_ID": "spoofed",
                                    "MULL_WORKSPACE_ROOT": "/etc",
