use mull_tools::types::{claude_names_for, mull_names_for};
use regex::Regex;
...
///   **exact** match against each name (after external→Mull alias expansion), NOT a regex;
/// - anything else is an **unanchored** regex (also tested against the tool's external
///   alias names, so e.g. `^Bash$` matches the Mull tool `run_terminal_command`).
///
...
/// plus any Mull tool names that term aliases (so `"Bash"` also matches
/// `run_terminal_command`), per the shared external-name to Mull registry in
/// `mull-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for mull_name in mull_names_for(term) {
            push(mull_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_mull_tool() {
        let m = HookMatcher::new("Bash").unwrap();
        assert!(m.is_match("Bash")); // external alias name
        assert!(m.is_match("run_terminal_command")); // Mull name
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_edit_write_matches_mull_tool_exactly() {
        let m = HookMatcher::new("Edit|Write").unwrap();
        assert!(m.is_match("Edit"));
        assert!(m.is_match("Write"));
        assert!(m.is_match("search_replace")); // Mull equivalent
        assert!(m.is_match("hashline_edit")); // second Mull alias
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_read_matches_mull_tool() {
    ...
    #[test]
    fn regex_against_claude_alias_matches_mull_tool() {
        // A regex written against an external alias still matches the Mull tool
