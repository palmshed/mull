...
// (`~/.mull/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_mull_home>/trusted-hook-projects`), or `None` when no user mull home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(mull_config::user_mull_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user mull home (set $MULL_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$MULL_HOME/disabled-hooks`, or `None` when no user mull
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(mull_config::user_mull_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let mull_dir = dir.join(".mull");
        std::fs::create_dir_all(&mull_dir).unwrap();
        mull_dir.join("trusted-hook-projects")
    }
