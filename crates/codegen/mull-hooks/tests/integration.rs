//! Integration tests for mull-hooks.
//!
...
use mull_hooks::discovery::load_hooks;
use mull_hooks::dispatcher;
use mull_hooks::event::*;
use mull_hooks::result::HookDecision;
use mull_hooks::runner::RunContext;
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$MULL_HOOK_EVENT" > {f}; echo "NAME=$MULL_HOOK_NAME" >> {f}; echo "SESSION=$MULL_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`MULL_HOOK_EVENT`, `MULL_HOOK_NAME`,
/// `MULL_SESSION_ID`, `MULL_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$MULL_HOOK_EVENT" > {f}; echo "NAME=$MULL_HOOK_NAME" >> {f}; echo "SESSION=$MULL_SESSION_ID" >> {f}; echo "ROOT=$MULL_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "MULL_HOOK_EVENT": "spoofed_event",
                                "MULL_HOOK_NAME": "spoofed_name",
                                "MULL_SESSION_ID": "spoofed_session",
                                "MULL_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "MULL_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed MULL_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "MULL_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed MULL_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "MULL_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed MULL_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
