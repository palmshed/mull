...
This crate extracts a first-class `Agent` type from `mull-shell`.
...
`mull-shell`, another in-process host, or a headless batch runner.
...
in `.mull/agents/` (project-level) or `~/.mull/agents/` (user-level).

```rust
use mull_agent::{AgentDefinition, AgentBuilder};
use mull_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".mull/agents/code-reviewer.md")?;
...
```rust
use mull_agent::discovery;

// Find all .md files in .mull/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  mull-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`mull-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `mull-agent` depends on it for tool setup.
- **`mull-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `mull-agent` (AGENTS.md
...
|---|---|---|
| `mull-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p mull-agent

# Test
cargo test -p mull-agent

# Clippy
cargo clippy -p mull-agent --fix --allow-dirty
