...
//! - `Default` -- the legacy Mull prefix (built by the shell layer).
...
use mull_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Mull prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .mull/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `mull_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.mull/, ~/.claude/).
    ...
    /// (`~/.mull/projects/<encoded-cwd>/mcps`). Surfaced in
