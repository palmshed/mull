...
//!
//! Searches from cwd to repo root, plus `~/.mull/`. Also discovers
//! `*.md` files in `.mull/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.mull/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.mull/, git repo root, and session cwd.
...
async fn read_agents_config_with_options(
    ...
    let global_dir = mull_tools::util::mull_home::mull_home();
    ...
    // Always include mull_home (~/.mull/) first, then ~/.claude/ and ~/.cursor/
...
/// Used by `mull-shell` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .mull/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.mull/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            mull_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
