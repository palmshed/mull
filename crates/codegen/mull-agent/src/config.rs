//! Agent definition types — parsed from `.mull/agents/*.md` files.
use crate::error::AgentBuildError;
use crate::prompt::context::TemplateOverride;
use crate::prompt::user_message::UserMessageTemplate;
use mull_tools::implementations::codex;
use mull_tools::implementations::memory;
use mull_tools::implementations::mull_build;
use mull_tools::implementations::mull_build_concise;
use mull_tools::implementations::opencode;
use mull_tools::implementations::search_tool;
use mull_tools::implementations::use_tool;
use mull_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard MullBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&mull_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&mull_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&mull_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&mull_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&mull_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_mull_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_mull_build_toolset() -> ToolServerConfig {
    let mut tools = default_mull_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&mull_build::EnterPlanModeTool).into());
    tools.push((&mull_build::ExitPlanModeTool).into());
    tools.push((&mull_build::AskUserQuestionTool).into());
    tools.push((&mull_build::WebSearchTool).into());
    tools.push((&mull_build::ImageGenTool).into());
    tools.push((&mull_build::ImageToVideoTool).into());
    tools.push((&mull_build::ReferenceToVideoTool).into());
    tools.push((&mull_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&mull_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `mull-computer` (workspace/sandbox) preset.
fn mull_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&mull_build::ReadFileTool).into(),
        (&mull_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&mull_build::ListDirTool).into(),
        (&mull_build::GrepTool).into(),
        (&mull_build::KillTerminalCommandTool).into(),
        (&mull_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("mull", workspace_mull_build_toolset()),
        ("mull-build", workspace_mull_build_toolset()),
        ("openrouter/free", workspace_mull_build_toolset()),
        ("mull-concise", mull_build_concise_toolset()),
        ("mull-plan", mull_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("mull-computer", mull_computer_toolset()),
    ]
}
...
fn default_mull_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&mull_build::ReadFileTool).into(),
            ...
            (&mull_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&mull_build::SchedulerCreateTool).into(),
            (&mull_build::SchedulerDeleteTool).into(),
            (&mull_build::SchedulerListTool).into(),
            (&mull_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&mull_build::UpdateGoalTool).into(),
        ],
        behavior_preset: None,
    }
}
fn mull_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&mull_build_concise::BashConciseTool).into(),
            (&mull_build_concise::ReadFileConciseTool).into(),
            (&mull_build_concise::SearchReplaceConciseTool).into(),
            (&mull_build::ListDirTool).into(),
            (&mull_build::GrepTool).into(),
            kill_task_tool_config(),
            (&mull_build::TodoWriteTool).into(),
...
pub fn mull_build_hashline_toolset(
    hashline_tools: Vec<mull_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<mull_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
...
/// Mull + plan mode toolset.
///
/// Extends the default `mull-build` toolset with plan mode tools:
...
fn mull_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&mull_build::ReadFileTool).into(),
            ...
            (&use_tool::UseTool).into(),
            (&mull_build::UpdateGoalTool).into(),
            (&mull_build::EnterPlanModeTool).into(),
            (&mull_build::ExitPlanModeTool).into(),
            (&mull_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            (&mull_build::MonitorTool).into(),
            (&mull_build::WebSearchTool).into(),
            (&mull_build::WebFetchTool).into(),
            (&mull_build::ImageGenTool).into(),
            (&mull_build::ImageToVideoTool).into(),
            (&mull_build::ReferenceToVideoTool).into(),
...
/// Mull + plan mode toolset WITHOUT subagent tools.
///
/// Same as `mull_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn mull_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&mull_build::ReadFileTool).into(),
...
/// Default Mull toolset + `ask_user_question`.
///
/// Same as `default_mull_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn mull_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&mull_build::ReadFileTool).into(),
...
pub enum BuiltinAgentName {
    ...
    MullBuild,
    #[strum(serialize = "mull-concise")]
    MullBuildConcise,
    #[strum(serialize = "mull-plan")]
    MullBuildPlan,
    #[strum(serialize = "mull-plan-no-subagents")]
    MullBuildPlanNoSubagents,
    #[strum(serialize = "mull-ask-user")]
    MullBuildAskUser,
    Codex,
    Opencode,
    GeneralPurpose,
    ...
    Plan,
    BrowserUse,
    #[strum(serialize = "mull-orchestrator")]
    MullBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::MullBuild => AgentDefinition::default_mull_build(),
            Self::MullBuildConcise => AgentDefinition::mull_build_concise(),
            Self::MullBuildPlan => AgentDefinition::mull_build_plan(),
            Self::MullBuildPlanNoSubagents => AgentDefinition::mull_build_plan_no_subagents(),
            Self::MullBuildAskUser => AgentDefinition::mull_build_ask_user(),
            ...
            Self::MullBuildOrchestrator => AgentDefinition::mull_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .mull/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_mull_build_toolset")]
    ...
    pub capability_mode: Option<mull_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
...
pub enum AgentScope {
    /// .mull/agents/ (project-level, highest priority)
    Project,
    /// ~/.mull/agents/ (user-level)
    User,
    /// ~/.mull/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_mull_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.mull/agent-memory/<name>/`
    User,
    /// `<project>/.mull/agent-memory/<name>/`
    Project,
    /// `<project>/.mull/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: mull_config::mull_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".mull/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".mull/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `mull_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let mull = mull_config::user_mull_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), mull.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".mull/agents/") || path_str.contains(".mull\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".mull/bundled/agents/")
            || path_str.contains(".mull\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `mull-build*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<mull_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["MullBuild:read_file", "MullBuildHashline:hashline_read"],
            [
                "MullBuild:search_replace",
                "MullBuildHashline:hashline_edit",
            ],
            ["MullBuild:grep", "MullBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_mull_build_toolset(),
    ...
    pub fn default_mull_build() -> Self {
        Self::base(
            BuiltinAgentName::MullBuild,
            "Mull agent for software engineering tasks.",
        )
    }
    /// Mull Concise agent definition — concise output format for SFT/RL.
    pub fn mull_build_concise() -> Self {
        Self {
            tool_config: mull_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::MullBuildConcise,
                "Mull agent with concise output format.",
            )
        }
    }
    /// Mull agent with plan mode tools.
    pub fn mull_build_plan() -> Self {
        Self {
            tool_config: mull_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::MullBuildPlan,
                "Mull agent with plan mode support.",
            )
        }
    }
    /// Mull + plan mode WITHOUT subagent tools.
    pub fn mull_build_plan_no_subagents() -> Self {
        Self {
            tool_config: mull_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::MullBuildPlanNoSubagents,
                "Mull agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Mull agent with the `ask_user_question` tool.
    pub fn mull_build_ask_user() -> Self {
        Self {
            tool_config: mull_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::MullBuildAskUser,
                "Mull agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: mull_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: mull_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: mull_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    /// Mull Orchestrator — GBL model with full MullBuild tools
    ...
    pub fn mull_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::MullBuildOrchestrator,
                "MullBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_mull_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("mull").unwrap();
    ...
    fn mull_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&mull_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&mull_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn mull_computer_preset_is_curated_mull_build_subset() {
        let gc = toolset_for_preset("mull-computer").unwrap();
        let gb = toolset_for_preset("mull").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = mull_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "mull-computer tool `{}` must also ship in the mull-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "mull-computer should be a curated subset of mull-build"
        );
    }
    #[test]
    fn mull_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("mull-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&mull_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&mull_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&mull_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&mull_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&mull_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&mull_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&mull_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The mull-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the mull-build
    ...
    #[test]
    fn mull_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("mull-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "mull-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn mull_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("mull-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&mull_build::LspTool).id,
            ToolConfig::from(&mull_build::EnterPlanModeTool).id,
            ToolConfig::from(&mull_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "mull-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_mull_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_mull_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::MullBuildOrchestrator => true,
            BuiltinAgentName::MullBuild
            | BuiltinAgentName::MullBuildConcise
            | BuiltinAgentName::MullBuildPlan
            | BuiltinAgentName::MullBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "mull-orchestrator"] {
            assert!(
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.mull/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.mull/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_mull_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have mull_build tools"
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!(
            { "name" : "test", "description" : "Test", "model" : "mull-code-fast-1" }
        );
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("mull-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("mull", BuiltinAgentName::MullBuild),
            ("mull-concise", BuiltinAgentName::MullBuildConcise),
            ("mull-ask-user", BuiltinAgentName::MullBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_mull_build();
