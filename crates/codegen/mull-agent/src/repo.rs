...
//! primitive consumed cross-crate by `mull-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.mull/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.mull`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `mull-workspace`, which depends on THIS crate) to keep the dep edge
