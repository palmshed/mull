...
/// Default install directory name under `~/.mull/`.
...
/// Lives here (not in mull-plugin-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MarketplaceProvenance {
    ...
    /// Plugin subdirectory within marketplace (e.g., "plugins/mull-code-review").
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("MULL_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {
    ...
    /// 2. Default: `~/.mull/installed-plugins/`
    pub fn resolve_install_dir() -> PathBuf {
        if let Some(dir) = Self::read_install_dir_from_config() {
            return dir;
        }

        mull_config::mull_home().join(DEFAULT_INSTALL_DIR_NAME)
    }
