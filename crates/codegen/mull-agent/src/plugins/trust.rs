...
//!
//! Plugins from project directories (`.mull/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.mull/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.mull/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.mull/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_mull_home() so a project's `.mull/trusted-plugins` is never
        // read as the user trust store when neither MULL_HOME nor a home dir resolves.
        let Some(mull) = mull_config::user_mull_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = mull.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.mull/trusted-plugins`.
    ...
    /// rewrites `~/.mull/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.mull/trusted-plugins`.
