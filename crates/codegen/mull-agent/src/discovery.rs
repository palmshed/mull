...
//!
//! Searches `.mull/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.mull/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.mull/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".mull/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.mull/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.mull/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), mull.as_deref())
}
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, mull_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.mull/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.mull/agents/` walking from `cwd` up to repo root
/// 2. `~/.mull/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.mull/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user mull agents, `.claude`
/// compat agents, then bundled. `.mull` dirs resolve from `mull_home`
/// (MULL_HOME-aware) plus the legacy literal `~/.mull` when MULL_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    mull_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.mull, included only when it differs from mull_home
    // (i.e. MULL_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_mull = home
        .map(|h| h.join(".mull"))
        .filter(|legacy| mull_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = mull_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_mull {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let mull = mull_config::user_mull_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), mull.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, mull_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let mull = mull_config::user_mull_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), mull.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, mull_home);
...
///
/// Project-level `.mull/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let mull = mull_config::user_mull_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), mull.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, mull_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, mull_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the Mull
...
/// Load project agent definitions from every `.mull/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectMull,
            PluginScope::User => PluginOrigin::UserMull,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_mull_when_mull_home_differs() {
        let home = Path::new("/home/u");
        let mull = Path::new("/custom/mullhome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(mull))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&mull.join("agents")));
        assert!(paths.contains(&home.join(".mull").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&mull.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".mull").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_mull_home_is_dot_mull() {
        let home = Path::new("/home/u");
        let mull = home.join(".mull");
        let count = user_agent_dirs(Some(home), Some(&mull))
            .into_iter()
            .filter(|(p, _)| *p == mull.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.mull/agents when mull_home == ~/.mull"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_mull_build() {
        let def = by_name("mull");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "mull");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".mull").join("agents");
        let agents_dir_2 = inner_dir.join(".mull").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".mull").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".mull")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".mull")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".mull").join("agents");
        let bundled_dir = home.join(".mull").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".mull")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".mull")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".mull").join("agents");
        let bundled_dir = home.join(".mull").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "mull" that shadows the built-in
        write_agent_file(&agents_dir, "mull.md", "mull", "Custom mull-build");

        let def = by_name_in_cwd("mull", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "mull");
        assert_eq!(def.description, "Custom mull-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .mull/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("mull-orchestrator")
            .expect("from_str must resolve mull-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::MullBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "mull-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("mull-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find mull-build-orchestrator");
        assert_eq!(def.name, "mull-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.mull/agents/) agent named "explore" should NOT shadow
