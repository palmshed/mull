...
use mull_tools::bridge::ToolBridge;
use mull_tools::computer::types::{AsyncFileSystem, TerminalBackend};
...
use std::sync::Arc;
/// The Mull [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-Mull
/// tool registry in `mull-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    mull_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.mull/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<mull_tools::implementations::mull_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn mull_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: mull_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: mull_tools::implementations::mull_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn mull_tools::implementations::lsp::LspBackend>>,
    image_gen_config: mull_tools::implementations::mull_build::image_gen::ImageGenConfig,
    video_gen_config: mull_tools::implementations::mull_build::video_gen::VideoGenConfig,
    ...
    compat: mull_tools::types::compat::CompatConfig,
    ...
    api_key_provider: Option<mull_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<mull_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in mull-shell).
    ...
    preloaded_skills: Option<Vec<mull_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut mull_tools::registry::types::ToolServerConfig) {
    use mull_tools::implementations::mull_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("MullBuild:enter_plan_mode");
    let missing_exit = !existing.contains("MullBuild:exit_plan_mode");
    let missing_ask = !existing.contains("MullBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&mull_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&mull_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&mull_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut mull_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(mull_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<mull_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn mull_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: mull_tools::implementations::mull_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: mull_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `MULL_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: mull_tools::implementations::mull_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn mull_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    /// allowing image generation via the Palmshed Imagine API with session
    /// credentials. When `Disabled` (default), the tool is not registered.
    pub fn with_image_gen_config(
        mut self,
        config: mull_tools::implementations::mull_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    /// allowing video generation via the Palmshed Video Generation API with
    /// session credentials. When `Disabled` (default), the tool is not
    /// registered.
    pub fn with_video_gen_config(
        mut self,
        config: mull_tools::implementations::mull_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: mull_tools::implementations::mull_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: mull_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the MullBuild Task description.
    ...
    ///
    /// When disabled, `MullBuild:ask_user_question` is stripped from the
    ...
    pub fn with_compat_config(mut self, compat: mull_tools::types::compat::CompatConfig) -> Self {
        self.compat = compat;
        self
    }
    /// Set the skills config (custom paths, ignore globs) from config.toml.
    /// Without this, only auto-discovered skills (cwd/.mull/skills, ~/.mull/skills)
    /// are included — custom paths added via `palmshed.ai/skills/add` would be ignored.
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use mull_tools::implementations::mull_build;
                tool_config.tools.push((&mull_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use mull_tools::implementations::mull_build;
                tool_config.tools.push((&mull_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&mull_tools::implementations::mull_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&mull_tools::implementations::mull_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&mull_tools::implementations::mull_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&mull_tools::implementations::mull_build::ImageToVideoTool).into());
                tool_config
                    .tools
                    .push((&mull_tools::implementations::mull_build::ReferenceToVideoTool).into());
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&mull_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let mull_build_ns = mull_tools::types::tool::ToolNamespace::MullBuild.to_string();
            let mem_search_id = format!(
                "{mull_build_ns}:{}",
                mull_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{mull_build_ns}:{}",
                mull_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                mull_tools::types::tool::ToolNamespace::MullBuild,
            );
        ...
        if task_stripped {
            use mull_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::MullBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::MullBuildConcise, "run_terminal_cmd", true)
        ...
        if let mull_tools::implementations::mull_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["MullBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "MullBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    definition.tools,
                    "tools allowlist had unmappable entries; keeping full mull toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                mull_tools::types::resources::TruncationCfg(
                    mull_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(mull_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(mull_sampling_types::HostedTool::XSearch);
            }
        }
...
/// CLI naming for the shared [`mull_tool_types::build_task_description`] builder.
const TASK_TOOL_NAMING: mull_tool_types::TaskToolNaming<'static> =
    mull_tool_types::TaskToolNaming {
...
/// CLI [`mull_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: mull_tool_types::SubagentToolNaming<'static> =
    mull_tool_types::SubagentToolNaming {
...
/// shared [`mull_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => mull_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => mull_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => mull_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`mull_tool_types::SubagentDescriptor`] and defers to
/// [`mull_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<mull_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            mull_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description = mull_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        mull_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: mull_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(mull_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(mull_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: mull_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("mull", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use mull_tools::types::template_renderer::TemplateRenderer;
        use mull_tools::types::tool::ToolKind;
    ...
    async fn build_pager_agent(
        ...
        use mull_tools::computer::local::LocalTerminalBackend;
        use mull_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "mull / subagents+ask_user",
                profile: AgentDefinition::default_mull_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mull / subagents / no-ask-user",
                profile: AgentDefinition::default_mull_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "mull / no-subagents / ask_user",
                profile: AgentDefinition::default_mull_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mull / no-subagents / no-ask-user",
                profile: AgentDefinition::default_mull_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "mull-ask-user / subagents",
                profile: AgentDefinition::mull_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mull-ask-user / no-subagents",
                profile: AgentDefinition::mull_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mull-plan",
                profile: AgentDefinition::mull_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mull-plan / no-ask-user",
                profile: AgentDefinition::mull_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "mull-plan-no-subagents",
                profile: AgentDefinition::mull_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "mull-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::mull_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use mull_tools::computer::local::LocalTerminalBackend;
        use mull_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_mull_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use mull_tools::computer::local::LocalTerminalBackend;
        use mull_tools::implementations::mull_build::ask_user_question::AskUserQuestionParams;
        use mull_tools::notification::ToolNotificationHandle;
        use mull_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_mull_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "MullBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use mull_tools::computer::local::LocalTerminalBackend;
        use mull_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_mull_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("MullBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use mull_tools::computer::local::LocalTerminalBackend;
        use mull_tools::implementations::mull_build::bash::BashParams;
        use mull_tools::notification::ToolNotificationHandle;
        use mull_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_mull_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their Mull
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_mull_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their mull
    ...
    /// treat `mcp__*` as always-on, so mull keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        ...
        use mull_tools::implementations::web_search::WebSearchConfig;
        use mull_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_mull_build();
        ...
        .with_web_search_config(WebSearchConfig::Enabled {
            ...
            base_url: "https://api.palmshed.ai/v1".into(),
    ...
    /// mull-build toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to mull's `search_tool` (MCP
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, mull_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, mull_sampling_types::HostedTool::XSearch)),
